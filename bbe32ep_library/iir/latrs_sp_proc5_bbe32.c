...
void latrs_sp_proc5( int16_t * restrict r,
                     int16_t * restrict d,
               const int16_t *          x,
               const int16_t *          coef,
                     int16_t            gain,
                     int N, int L )
{
  xb_vecNx16 X, g, D, D0, D1, D2, D3, Y, Y0, Coef, Coef1, Coef2, Coef3, Coef4;
  ...
  for ( l=0; l<(L>>LOG2_BBE_SIMD_WIDTH); l++)
  {
    ...
    _D = (      xb_vecNx16 *)(d + 5*BBE_SIMD_WIDTH*l);
    ...
    for ( j=0; j<N; j++ )
    {
      ...
      BBE_SVNX16_X(D, _D, sz_i16*4*BBE_SIMD_WIDTH);
