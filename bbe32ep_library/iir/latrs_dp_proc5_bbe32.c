...
void latrs_dp_proc5( int16_t * restrict r,
                     int16_t * restrict d,
               const int16_t *          x,
               const int16_t *          coef,
                     int16_t            gain,
...
{
  xb_vecNx16 d0, d1;
  xb_vecNx16 X, Y, g, Coef0, Coef1, Coef2, Coef3, Coef4;
  xb_vecNx16 D_l0, D_h0, D_l1, D_h1, D_l2, D_l3, D_h3, D_h2, D_l4, D_h4;
  int32_t Coef34;
  ...
  Coef34 = (((int32_t)(-coef[3]))<<16);
  Coef34 |= (uint16_t)(-coef[4]);
  ...
  for ( l=0; l<(L>>LOG2_BBE_SIMD_WIDTH); l++ )
  {
    ...
    _D = (      xb_vecNx16 *)(d + 2*5*BBE_SIMD_WIDTH*l);
    ...
    D_l4 = BBE_LVNX16_I(_D, sz_i16*8*BBE_SIMD_WIDTH); 
    D_h4 = BBE_LVNX16_I(_D, sz_i16*9*BBE_SIMD_WIDTH);
    ...
    for ( j=0; j<N; j++ )
    {
      ...
      BBE_MULANX16PR(T, D_h3, D_h4, Coef34);
      BBE_MULANX16PR(T, D_h3, D_h4, Coef34);

      Z = BBE_MULUSRNX16(D_l4, Coef4, rnd15);
      BBE_MULUSANX16(Z,  D_l3, Coef3);
      Z = BBE_SRAINX40(Z, 15);
    ...
    BBE_SVNX16_I(D_l2, _D, sz_i16*0*BBE_SIMD_WIDTH);
    BBE_SVNX16_I(D_h2, _D, sz_i16*1*BBE_SIMD_WIDTH);

    BBE_SVNX16_I(D_l3, _D, sz_i16*2*BBE_SIMD_WIDTH);
    BBE_SVNX16_I(D_h3, _D, sz_i16*3*BBE_SIMD_WIDTH);

    BBE_SVNX16_I(D_l4, _D, sz_i16*4*BBE_SIMD_WIDTH);
    BBE_SVNX16_I(D_h4, _D, sz_i16*5*BBE_SIMD_WIDTH);
  }
} // latr_dp_proc5()
