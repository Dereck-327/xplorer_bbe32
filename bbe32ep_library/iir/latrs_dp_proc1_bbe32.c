...
void latrs_dp_proc1( int16_t * restrict r,
                     int16_t * restrict d,
               const int16_t *          x,
               const int16_t *          coef,
                     int16_t            gain,
                     int N, int L )
{
  ...
  for ( l=0; l<(L>>LOG2_BBE_SIMD_WIDTH); l++ )
  {
    ...
    D_s = BBE_LVNX16_I(_D, 0*sz_i16*BBE_SIMD_WIDTH); 
    D_b = BBE_LVNX16_I(_D, 1*sz_i16*BBE_SIMD_WIDTH);
    ...
    for ( j=0; j<N; j++ )
    {
      ...
      BBE_MULSNX16(T, D_b, Coef);
      BBE_MULSNX16(T, D_b, Coef);
      
      Z = BBE_MULUSRNX16(D_s, Coef, rnd15);
      Z = BBE_SRAINX40(Z, 15);

      T = BBE_SUBNX40(T, Z);

      D_s = BBE_PACKLNX40(T); D_b = BBE_PACKVNX40(T, rnd16);
    ...
    BBE_SVNX16_IP(D_s, _D, sz_i16*BBE_SIMD_WIDTH);
    BBE_SVNX16_IP(D_b, _D, sz_i16*BBE_SIMD_WIDTH);
  }
} // latr_dp_proc1()
