...
void latrs_sp_proc8( int16_t * restrict r,
                     int16_t * restrict d,
               const int16_t *          x,
               const int16_t *          coef,
                     int16_t            gain,
                     int N, int L )
{
  xb_vecNx16 X, g, D, D0, D1, D2, Y, Y0, Coef;
  xb_vecNx16 Coef1, Coef2, Coef3, Coef5;
  ...
  Coef1 = BBE_LSNX16_I(coef, 2*1);
  Coef2 = BBE_LSNX16_I(coef, 2*2);
  Coef3 = BBE_LSNX16_I(coef, 2*3);
  Coef5 = BBE_LSNX16_I(coef, 2*5);
  ...
  for ( l=0; l<(L>>LOG2_BBE_SIMD_WIDTH); l++)
  {
    ...
    _D = (      xb_vecNx16 *)(d + 8*BBE_SIMD_WIDTH*l);
    ...
    for ( j=0; j<N; j++ )
    {
      ...
      Coef = BBE_LSNX16_I(coef, sz_i16*7);
      Coef = BBE_REPNX16(Coef, 0);

      D = BBE_LVNX16_I(_D, sz_i16*7*BBE_SIMD_WIDTH);

      // Q29 <- Q29 - Q14*Q15
      BBE_MULSNX16(T, D, Coef); 
    
      Coef = BBE_LSNX16_I(coef, sz_i16*6);
      Coef = BBE_REPNX16(Coef, 0);
      ...
      // Q29 <- (Q29 - 15 w/ rounding)*Q15
      Y = BBE_MULNX16PACKQ(Y, Coef);

      D = BBE_ADDNX16(D, Y);

      BBE_SVNX16_I(D, _D, sz_i16*7*BBE_SIMD_WIDTH);
      ...
      Coef = BBE_LSNX16_I(coef, sz_i16*4);
      Coef = BBE_REPNX16(Coef, 0);
