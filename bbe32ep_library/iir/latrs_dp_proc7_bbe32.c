...
void latrs_dp_proc7( int16_t * restrict r,
                     int16_t * restrict d,
               const int16_t *          x,
               const int16_t *          coef,
                     int16_t            gain,
                     int N, int L )
{
  ...
  xb_vecNx16 D_l0, D_h0, D_l1, D_h1, D_l2, D_h2, D_l3, D_h3, D_l4, D_h4, D_l5, D_h5, D_l6, D_h6;
  int32_t Coef56, Coef33, Coef44;

  xb_vecNx40 T, Z;

  const xb_vecNx16 *_X;
        xb_vecNx16 *_R;
        xb_vecNx16 * restrict _D;
  ...
  Coef33 = (((int32_t)(-coef[3]))<<16);
  Coef33 |= (uint16_t)(-coef[3]);
  Coef44 = (((int32_t)(-coef[4]))<<16);
  Coef44 |= (uint16_t)(-coef[4]);
  Coef56 = (((int32_t)(-coef[5]))<<16);
  Coef56 |= (uint16_t)(-coef[6]);
  ...
  for ( l=0; l<(L>>LOG2_BBE_SIMD_WIDTH); l++ )
  {
    ...
    _D = (      xb_vecNx16 *)(d + 2*7*BBE_SIMD_WIDTH*l);
    _R = (      xb_vecNx16 *)(r + 1*BBE_SIMD_WIDTH*l);
   
    BBE_LVNX16_IP(D_l6, _D, sz_i16*BBE_SIMD_WIDTH); 
    BBE_LVNX16_IP(D_h6, _D, sz_i16*BBE_SIMD_WIDTH);  
    ...
    for ( j=0; j<N; j++ )
    {
      ...
      D_l5 = BBE_LVNX16_I(_D, sz_i16*0*BBE_SIMD_WIDTH); 
      D_h5 = BBE_LVNX16_I(_D, sz_i16*1*BBE_SIMD_WIDTH);
    
      // Q29 <- Q29 - ( Q29*Q15 - 15 w/ rounding )
      BBE_MULANX16PR(T, D_h5, D_h6, Coef56);
      BBE_MULANX16PR(T, D_h5, D_h6, Coef56);
         
      Z = BBE_MULUSRNX16(D_l6, Coef6, rnd15);
      BBE_MULUSANX16(Z,  D_l5, Coef5);
      ...
      D_l4 = BBE_LVNX16_I(_D, sz_i16*2*BBE_SIMD_WIDTH); 
      D_h4 = BBE_LVNX16_I(_D, sz_i16*3*BBE_SIMD_WIDTH);

      Coef4 = BBE_LSNX16_I(coef, sz_i16*4);
      Coef4 = BBE_REPNX16(Coef4, 0);
      
      // Q29 <- Q29 - ( Q29*Q15 - 15 w/ rounding )
      BBE_MULANX16PR(T, D_h4, D_h4, Coef44);
      ...
      // Q29 <- Q29 - ( Q29*Q15 - 15 w/ rounding )
      BBE_MULANX16PR(T, D_h3, D_h3, Coef33);
      ...
      BBE_SVNX16_XP(D_h1, _D, -(int)sz_i16*9*BBE_SIMD_WIDTH);
    }
    
    BBE_SVNX16_I(D_l6, _D, -(int)sz_i16*2*BBE_SIMD_WIDTH);
    BBE_SVNX16_I(D_h6, _D, -(int)sz_i16*1*BBE_SIMD_WIDTH);
  }
} // latr_dp_proc7()
