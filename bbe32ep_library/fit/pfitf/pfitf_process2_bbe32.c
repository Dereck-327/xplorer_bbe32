...
/*-----------------------------
    ...
    f[M]     
-----------------------------*/
static void pfitf_discr3(      float32_t* restrict f,
...
#else
{
  const int Astride = (M+BBE_SIMD_WIDTH/2-1)&~(BBE_SIMD_WIDTH/2-1);
    ...
    const xb_vecN_2xf32 * restrict pA=(const xb_vecN_2xf32 *)(A+2*Astride);
          xb_vecN_2xf32 * restrict pf=(      xb_vecN_2xf32 *)f;
    xb_vecN_2xf32 x0,x1,x2;
...
)
{
    ...
    NASSERT(M>=2);
    pfitf_process(pScr,p,V,R,y,M,2,maxIter,pfitf_discr3);
}

#else
DISCARD_FUN(void,pfitf_process2,
(
  void * restrict             pScr,
  float32_t * restrict        p,
  const float32_t * restrict  V,
