...
/*-----------------------------------------------
    ...
    M'=((M + 7)& ~7)
-----------------------------------------------*/
static void makeVf4(float32_t* restrict t,float32_t* restrict V,const float32_t* restrict x,int M,int N)
#if 0
{
  int m;
  ...
  for (m=0; m<M; m++)
  {
    V[4*Vstride+m]=1.f;
    V[3*Vstride+m]=x[m];
    V[2*Vstride+m]=x[m]*x[m];
    V[1*Vstride+m]=(x[m]*x[m])*x[m];
    V[          m]=((x[m]*x[m])*x[m])*x[m];
  }
  for (;m<Vstride;m++)
  {
...
#else
{
    ...
    const   xb_vecN_2xf32 *restrict px=(const xb_vecN_2xf32 *)x;
            xb_vecN_2xf32 *restrict pV=(      xb_vecN_2xf32 *)(V+4*Vstride);
    valign ax;
    int m;
    ...
    for (m=0; m<((M + 7)& ~7)>>(LOG2_BBE_SIMD_WIDTH-1); m++)
    {
      ...
      BBE_SVN_2XF32_XP(X4 ,pV,4*Vstride*sizeof(float32_t)+sizeof(xb_vecN_2xf32));
    }
}
#endif
/*-------------------------------------------------------------------------
...
)
{
  ...
  makeVf4((float32_t *)pScr,V,x,M,4); /* construct Vandermonde matrix */
  pfitf_chol((float32_t *)pScr,R,V,(float32_t)1.e-6,M,4+1); /* make Cholesky decomposition */
}

#else
DISCARD_FUN(void,pfitf_grid4,
(
  void *                pScr,
  float32_t * restrict  V, 
  float32_t * restrict  R, 
