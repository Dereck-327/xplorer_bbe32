...
/*-----------------------------
    ...
    f[M]     
-----------------------------*/
static void pfitf_discr7(      float32_t* restrict f,
...
#else
{
    ...
    const xb_vecN_2xf32 * restrict pA=(const xb_vecN_2xf32 *)(A+6*Astride);
          xb_vecN_2xf32 * restrict pf=(      xb_vecN_2xf32 *)f;
    xb_vecN_2xf32 xx,x0,x1,x2,x3,x4,x5,x6;
    ...
    BBE_LAVN_2XF32_XP(xx,ax,px,7*sizeof(float32_t));
    ...
    x6=BBE_REPN_2XF32(xx,6);
    for(m=0; m<(Astride>>(LOG2_BBE_SIMD_WIDTH-1)); m++)
    {
        xb_vecN_2xf32 Acc,a4,a5,a6;

        BBE_LAVN_2XF32_XP(Acc,ab,pb,nbytes); nbytes-=sizeof(xb_vecN_2xf32);
        BBE_LVN_2XF32_XP(a6,pA,-Astride*sizeof(float32_t));
        BBE_LVN_2XF32_XP(a5,pA,-Astride*sizeof(float32_t));
        BBE_LVN_2XF32_XP(a4,pA,2*Astride*sizeof(float32_t)+sizeof(xb_vecN_2xf32));

        BBE_MULSN_2XF32(Acc,x6,a6);
    ...
    pA=(const xb_vecN_2xf32 *)(A+3*Astride);
    pf=(      xb_vecN_2xf32 *)f;
    for(m=0; m<(Astride>>(LOG2_BBE_SIMD_WIDTH-1)); m++)
    {
...
)
{
    ...
    NASSERT(M>=6);
    pfitf_process(pScr,p,V,R,y,M,6,maxIter,pfitf_discr7);
}
#else
DISCARD_FUN(void,pfitf_process6,
(
  void * restrict             pScr,
  float32_t * restrict        p,
  const float32_t * restrict  V,
