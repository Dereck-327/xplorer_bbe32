...
/*-----------------------------
    ...
    f[M]     
-----------------------------*/
static void pfitf_discr6(      float32_t* restrict f,
...
#elif 1
{
    ...
    const xb_vecN_2xf32 * restrict pA=(const xb_vecN_2xf32 *)(A+5*Astride);
          xb_vecN_2xf32 * restrict pf=(      xb_vecN_2xf32 *)f;
    xb_vecN_2xf32 xx,x0,x1,x2,x3,x4,x5;
...
#else
{
    ...
    for(m=0; m<(Astride>>(LOG2_BBE_SIMD_WIDTH-1)); m++)
    {
        xb_vecN_2xf32 Acc,Acc1,a0,a1,a2,a3,a4,a5;
        ...
        BBE_LVN_2XF32_XP(a2,pA,-Astride*sizeof(float32_t));
        BBE_LVN_2XF32_XP(a1,pA,-Astride*sizeof(float32_t));
        BBE_LVN_2XF32_XP(a0,pA,5*Astride*sizeof(float32_t)+sizeof(xb_vecN_2xf32));
...
)
{
    ...
    NASSERT(M>=5);
    pfitf_process(pScr,p,V,R,y,M,5,maxIter,pfitf_discr6);
}
#else
DISCARD_FUN(void,pfitf_process5,
(
  void * restrict             pScr,
  float32_t * restrict        p,
  const float32_t * restrict  V,
