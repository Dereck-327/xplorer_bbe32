...
#if 0
{
    float32_t p0=poly[0],p1=poly[1],p2=poly[2],p3=poly[3];
    int p;
    for (p=0; p<P; p++)
    {
        float32_t xp=xi[p];
...
#else
{
    xb_vecN_2xf32 pp,y,x,t,p0,p1,p2,p3;
    ...
    aY=BBE_ZALIGN();
    BBE_LAVN_2XF32_XP(pp,aP,pP,4*sizeof(float32_t));
...
#endif
#else
DISCARD_FUN(void,pfitf_eval3,(float32_t * yi,const float32_t * xi,const float32_t * poly,int P))
#endif
