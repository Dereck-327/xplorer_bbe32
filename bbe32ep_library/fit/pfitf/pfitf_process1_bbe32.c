...
/*-----------------------------
    ...
    f[M]     
-----------------------------*/
static void pfitf_discr2(      float32_t* restrict f,
...
#else
{
  ...
  {
      xb_vecN_2xf32 Acc,a0,a1;

      BBE_LAVN_2XF32_XP(Acc,ab,pb,nbytes); nbytes-=sizeof(xb_vecN_2xf32);
      a1=BBE_LVN_2XF32_X(pA,1*Astride*sizeof(float32_t));
      BBE_LVN_2XF32_IP(a0,pA,sizeof(xb_vecN_2xf32));
...
)
{
    ...
    NASSERT(M>=1);
    pfitf_process(pScr,p,V,R,y,M,1,maxIter,pfitf_discr2);
}

#else
DISCARD_FUN(void,pfitf_process1,
(
  void * restrict             pScr,
  float32_t * restrict        p,
  const float32_t * restrict  V,
