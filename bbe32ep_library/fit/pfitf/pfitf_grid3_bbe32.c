...
/*-----------------------------------------------
    ...
    M'=((M + 7)& ~7)
-----------------------------------------------*/
static void makeVf3(float32_t* restrict t,float32_t* restrict V,const float32_t* restrict x,int M,int N)
#if 0
{
  int m;
...
#else
{
    ...
    const   xb_vecN_2xf32 *restrict px=(const xb_vecN_2xf32 *)x;
            xb_vecN_2xf32 *restrict pV=(      xb_vecN_2xf32 *)(V+3*Vstride);
    valign ax;
    int m;
    ...
    for (m=0; m<((M + 7)& ~7)>>(LOG2_BBE_SIMD_WIDTH-1); m++)
    {
      ...
      BBE_SVN_2XF32_XP(X3 ,pV,3*Vstride*sizeof(float32_t)+sizeof(xb_vecN_2xf32));
    }
}
#endif
/*-------------------------------------------------------------------------
...
)
{
  ...
  makeVf3((float32_t *)pScr,V,x,M,3); /* construct Vandermonde matrix */
  pfitf_chol((float32_t *)pScr,R,V,(float32_t)1.e-6,M,3+1); /* make Cholesky decomposition */
}
#else
DISCARD_FUN(void,pfitf_grid3,
(
  void *                pScr,
  float32_t * restrict  V, 
  float32_t * restrict  R, 
