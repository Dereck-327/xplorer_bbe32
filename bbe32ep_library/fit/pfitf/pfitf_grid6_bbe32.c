...
/*-----------------------------------------------
    ...
    M'=((M + 7)& ~7)
-----------------------------------------------*/
static void makeVf6(float32_t* restrict t,float32_t* restrict V,const float32_t* restrict x,int M,int N)
#if 0
{
  int m;
  ...
  for (m=0; m<M; m++)
  {
    V[6*Vstride+m]=1.f;
    V[5*Vstride+m]=x[m];
    V[4*Vstride+m]=x[m]*x[m];
    V[3*Vstride+m]=(x[m]*x[m])*x[m];
    V[2*Vstride+m]=((x[m]*x[m])*x[m])*x[m];
    V[1*Vstride+m]=(((x[m]*x[m])*x[m])*x[m])*x[m];
    V[          m]=((((x[m]*x[m])*x[m])*x[m])*x[m])*x[m];
  }
  for (;m<Vstride;m++)
  {
    V[6*Vstride+m]=0.f;
...
#else
{
    ...
    const   xb_vecN_2xf32 *restrict px=(const xb_vecN_2xf32 *)x;
            xb_vecN_2xf32 *restrict pV=(      xb_vecN_2xf32 *)(V+6*Vstride);
    valign ax;
    int m;
    ...
    {
      xb_vecN_2xf32 X,X2,X3,X4,X5,X6;
      BBE_LAVN_2XF32_XP(X,ax,px,nBytes);
      X2=BBE_MULN_2XF32(X,X);
      X3=BBE_MULN_2XF32(X2,X);
      X4=BBE_MULN_2XF32(X2,X2);
      X5=BBE_MULN_2XF32(X3,X2);
      X6=BBE_MULN_2XF32(X3,X3);
      BBE_SVN_2XF32_XP(BBE_CONSTN_2XF32(1),pV,-Vstride*sizeof(float32_t));
      nBytes-=sizeof(X);
      BBE_SVN_2XF32_XP(X  ,pV,-Vstride*sizeof(float32_t));
      BBE_SVN_2XF32_XP(X2 ,pV,-Vstride*sizeof(float32_t));
      BBE_SVN_2XF32_XP(X3 ,pV,-Vstride*sizeof(float32_t));
      BBE_SVN_2XF32_XP(X4 ,pV,-Vstride*sizeof(float32_t));
      BBE_SVN_2XF32_XP(X5 ,pV,-Vstride*sizeof(float32_t));
      BBE_SVN_2XF32_XP(X6 ,pV,6*Vstride*sizeof(float32_t)+sizeof(xb_vecN_2xf32));
    }
}
#endif
/*-------------------------------------------------------------------------
...
)
{
  ...
  makeVf6((float32_t *)pScr,V,x,M,6); /* construct Vandermonde matrix */
  pfitf_chol((float32_t *)pScr,R,V,(float32_t)1.e-6,M,6+1); /* make Cholesky decomposition */
}

#else
DISCARD_FUN(void,pfitf_grid6,
(
  void *                pScr,
  float32_t * restrict  V, 
  float32_t * restrict  R, 
