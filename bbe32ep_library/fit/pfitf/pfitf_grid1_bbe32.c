...
/*-----------------------------------------------
    ...
    M'=((M + 7)& ~7)
-----------------------------------------------*/
static void makeVf1(float32_t* restrict t,float32_t* restrict V,const float32_t* restrict x,int M,int N)
#if 0
{
  int m;
...
#else
{
    ...
    for (m=0; m<((M + 7)& ~7)>>(LOG2_BBE_SIMD_WIDTH-1); m++)
    {
      ...
      BBE_SVN_2XF32_X(BBE_CONSTN_2XF32(1),pV,Vstride*sizeof(float32_t));
      nBytes-=sizeof(X);
      BBE_SVN_2XF32_IP(X,pV,sizeof(xb_vecN_2xf32));
    }
}
#endif
...
)
{
  ...
  makeVf1((float32_t *)pScr,V,x,M,1); /* construct Vandermonde matrix */
  pfitf_chol((float32_t *)pScr,R,V,(float32_t)1.e-6,M,1+1); /* make Cholesky decomposition */
}

#else
DISCARD_FUN(void, pfitf_grid1,
(
  void *                pScr,
  float32_t * restrict  V, 
  float32_t * restrict  R, 
