...
void pfit_eval3(int16_t * restrict yi, const int16_t * restrict xi, const int32_t * restrict p, int P)
{
    ...
    BBE_LAVNX16_XP(vc, apoly, ppoly, 4 * 4); // 4 int32_t coefficients
    t = BBE_SEQNX16(); shr = BBE_MOVVSV(t, 0); // shift them by 0,1...3 bits right
    w = BBE_MOVSWV(vc, vc);
    w = BBE_SRANX40(w, shr);
    vc = BBE_MOVSVWL(w);
    zero = BBE_ZERONX16();
    ...
    for (n = 0; n<((P + BBE_SIMD_WIDTH - 1) >> LOG2_BBE_SIMD_WIDTH); n++)
    {
        ...
        lo = BBE_REPNX16(vc, 0); // p[0], low part
        hi = BBE_REPNX16(vc, 1); // p[0], high part
        Am = BBE_MULUSNX16(lo, xp);
        mvec = BBE_MULMNX16(hi, xp);
        BBE_SRAIWADDMNX40(Am, mvec, 16);
        vc = BBE_ADDNX16(vc, zero); // improves scheduling !!!
        t = BBE_REPNX16C(vc, 1); Ap = BBE_MOVSWV(t, t); // p[1]>>1
        ...
        BBE_SRAIWADDMNX40(Am, mvec, 16);
        vc = BBE_ADDNX16(vc, zero); // improves scheduling !!!
        t = BBE_REPNX16C(vc, 2); Ap = BBE_MOVSWV(t, t); // p[2]>>2
        Am = BBE_ADDNX40(Am, Ap);
        ...
        BBE_SRAIWADDMNX40(Am, mvec, 16);
        vc = BBE_ADDNX16(vc, zero); // improves scheduling !!!
        t = BBE_REPNX16C(vc, 3); Ap = BBE_MOVSWV(t, t); // p[3]>>3
        Am = BBE_ADDNX40(Am, Ap);
