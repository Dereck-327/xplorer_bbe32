...
static void makeV3(int16_t* restrict V, const int16_t* restrict x, int M)
{
    ...
    for (m = 0; m < (M >> LOG2_BBE_SIMD_WIDTH); m++)
    {
        ...
        BBE_SVNX16_X(one, pV, 3 * (Vstride << 1));
        BBE_SVNX16_X(X, pV, 2 * (Vstride << 1));
    ...
    //_M = (8 - 4) * (Vstride >> LOG2_BBE_SIMD_WIDTH);
    //pV = (xb_vecNx16*)XT_ADD(6*Vstride, (uintptr_t)pV);
    //for (m = 0; m < _M; m++)
    //{
...
void pfit_grid3 ( void * restrict pScr, int16_t * restrict  V, int16_t * restrict R, const int16_t * restrict x, int M )
{
    ...
    makeV3(V, x, M); /* construct Vandermonde matrix */
    pfit_chol((int16_t *)pScr, R, V, 100, M, 4); /* make Cholesky decomposition */
} /* pfit_grid3() */
