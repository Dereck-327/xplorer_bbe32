...
static void makeV4(int16_t* restrict V, const int16_t* restrict x, int M)
{
    ...
    for (m = 0; m<(M >> LOG2_BBE_SIMD_WIDTH); m++)
    {
        ...
        BBE_SVNX16_X(one, pV, 4 * (Vstride << 1));
        BBE_SVNX16_X(X, pV, 3 * (Vstride << 1));
    ...
    //_M = (8 - 5) * (Vstride >> LOG2_BBE_SIMD_WIDTH);
    //pV = (xb_vecNx16*)XT_ADDX8(Vstride, (uintptr_t)pV);
    //for (m = 0; m < _M; m++)
    //{
...
void pfit_grid4 ( void * restrict pScr, int16_t * restrict  V, int16_t * restrict R, const int16_t * restrict x, int M )
{
    ...
    makeV4(V, x, M); /* construct Vandermonde matrix */
    pfit_chol((int16_t *)pScr, R, V, 100, M, 5); /* make Cholesky decomposition */
} /* pfit_grid4() */
