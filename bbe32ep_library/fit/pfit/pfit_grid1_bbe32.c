...
static void makeV1(int16_t* restrict V, const int16_t* restrict x, int M)
{
    ...
    vboolN b;
    int m/*, _M*/;
    const int Vstride = ((M + (BBE_SIMD_WIDTH - 1)) & ~(BBE_SIMD_WIDTH - 1));
    ...
    //_M = (8 - 2) * (Vstride >> LOG2_BBE_SIMD_WIDTH);
    //pV = (xb_vecNx16*)XT_ADDX2(Vstride, (uintptr_t)pV);
    //for (m = 0; m < _M; m++)
    //{
...
)
{
    ...
    makeV1(V, x, M);
    pfit_chol((int16_t *)pScr, R, V, 100, M, 2); /* make Cholesky decomposition */
} /* pfit_grid1() */
