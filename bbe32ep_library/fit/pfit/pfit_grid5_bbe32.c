...
static void makeV5(int16_t* restrict V, const int16_t* restrict x, int M)
{
    ...
    for (m = 0; m<(M >> LOG2_BBE_SIMD_WIDTH); m++)
    {
        ...
        BBE_SVNX16_X(one, pV, 5 * (Vstride << 1));
        BBE_SVNX16_X(X, pV, 4 * (Vstride << 1));
    ...
    //_M = (8 - 6) * (Vstride >> LOG2_BBE_SIMD_WIDTH);
    //pV = (xb_vecNx16*)XT_ADD(10*Vstride, (uintptr_t)pV);
    //for (m = 0; m < _M; m++)
    //{
...
void pfit_grid5 ( void * restrict pScr, int16_t * restrict  V, int16_t * restrict R, const int16_t * restrict x, int M )
{
    ...
    makeV5(V, x, M); /* construct Vandermonde matrix */
    pfit_chol((int16_t *)pScr, R, V, 100, M, 6); /* make Cholesky decomposition */
} /* pfit_grid5() */
