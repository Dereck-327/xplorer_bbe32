...
DISCARD_FUN(void, pfit_process2, (
    void * restrict           pScr,
    int32_t * restrict        p,
...
static void pfit_discr3(int16_t* restrict f,
    ...
    )
{
    ...
    for (m = 0; m<(M >> LOG2_BBE_SIMD_WIDTH); m++)
    {
        ...
        BBE_LPNX16_XP(xx, px, -2 * 4); lo = BBE_REPNX16(xx, 0); hi = BBE_REPNX16(xx, 1);
        BBE_LVNX16_XP(aa, pA, 2 * BBE_SIMD_WIDTH - (3 - 1)*(M << 1));
        wB = BBE_MULUSNX16(lo, aa);
        wB = BBE_SRAINX40(wB, 16);
        BBE_MULANX16(wB, hi, aa);
        wA = BBE_SUBNX40(wA, wB);
...
void pfit_process2 ( void * restrict pScr, int32_t * restrict p, const int16_t * restrict V, const int16_t * restrict R, const int16_t * restrict y, int M, int maxIter )
{
    ...
    pfit_process(pScr, p, V, R, y, M, 2, maxIter, pfit_discr3);
} /* pfit_process2() */
