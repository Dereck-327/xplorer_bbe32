...
void pfit_eval6(int16_t * restrict yi, const int16_t * restrict xi, const int32_t * restrict p, int P)
{
    ...
    BBE_LAVNX16_XP(vc0, apoly, ppoly, 7 * 4); // 7 int32_t coefficients
    t = BBE_SEQNX16(); shr = BBE_MOVVSV(t, 0); // shift them by 0,1...6 bits right
    ...
    for (n = 0; n<((P + BBE_SIMD_WIDTH - 1) >> LOG2_BBE_SIMD_WIDTH); n++)
    {
        ...
        BBE_SRAIWADDMNX40(Am, mvec, 16);
        vc0 = BBE_ADDNX16(vc0, zero); // improves scheduling !!!
        t = BBE_REPNX16C(vc0, 6); Ap = BBE_MOVSWV(t, t); // p[6]>>6
        Am = BBE_ADDNX40(Am, Ap);
