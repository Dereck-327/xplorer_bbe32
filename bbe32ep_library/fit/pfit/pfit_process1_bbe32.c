...
DISCARD_FUN(void, pfit_process1, (
    void * restrict           pScr,
    int32_t * restrict        p,
...
static void pfit_discr2(int16_t* restrict f,
...
{
    xb_vecNx40 wA, wB;
    xb_vecNx16 aa, xx, bb, hi0, lo0, hi1, lo1, ff, _128 = BBE_MOVVA16(128);
    ...
    xx = BBE_LPNX16_X(x, 0);
    lo0 = BBE_REPNX16(xx, 0);
    hi0 = BBE_REPNX16(xx, 1);
    xx = BBE_LPNX16_X(x, 4);
    lo1 = BBE_REPNX16(xx, 0);
    hi1 = BBE_REPNX16(xx, 1);
    ...
    for (m = 0; m<(M >> LOG2_BBE_SIMD_WIDTH); m++)
    {
        ...
        wB = BBE_MULUSNX16(lo0, aa);
        wB = BBE_SRAINX40(wB, 16);
        BBE_MULANX16(wB, hi0, aa);
        wA = BBE_SUBNX40(wA, wB);

        BBE_LVNX16_XP(aa, pA, 2 * BBE_SIMD_WIDTH - (2 - 1)*(M << 1));
        wB = BBE_MULUSNX16(lo1, aa);
        wB = BBE_SRAINX40(wB, 16);
        BBE_MULANX16(wB, hi1, aa);
        wA = BBE_SUBNX40(wA, wB);

        ff = BBE_PACKVNX40(wA, _7);
        BBE_SVNX16_IP(ff, pf, 2 * BBE_SIMD_WIDTH);
        BBE_LVNX16_XP(aa, pA, M << 1);
    }
}
...
)
{
    ...
    pfit_process(pScr, p, V, R, y, M, 1, maxIter, pfit_discr2);
} /* pfit_process1() */
