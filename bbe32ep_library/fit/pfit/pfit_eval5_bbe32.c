...
void pfit_eval5(int16_t * restrict yi, const int16_t * restrict xi, const int32_t * restrict p, int P)
{
    ...
    BBE_LAVNX16_XP(vc0, apoly, ppoly, 6 * 4); // 6 int32_t coefficients
    t = BBE_SEQNX16(); shr = BBE_MOVVSV(t, 0); // shift them by 0,1...5 bits right
    ...
    for (n = 0; n<((P + BBE_SIMD_WIDTH - 1) >> LOG2_BBE_SIMD_WIDTH); n++)
    {
        ...
        mvec = BBE_MULMNX16(hi, xp);
        BBE_SRAIWADDMNX40(Am, mvec, 16);
        t = BBE_REPNX16C(vc0, 5); Ap = BBE_MOVSWV(t, t); // p[5]>>5
        Am = BBE_ADDNX40(Am, Ap);
        ...
        vc0 = BBE_MOVNX16T(vc0, vc0, _true);
    }
    BBE_SAPOS_FP(vy, pyi);
#else
  int n;
