...
static void makeV2(int16_t* restrict V, const int16_t* restrict x, int M)
{
    const xb_vecNx16* restrict px = (const xb_vecNx16*)x;
    xb_vecNx16* restrict pV = (xb_vecNx16*)V;
    xb_vecNx16 X, one, zero;
    valign vx;
    vboolN b;
    int m/*, _M*/;
    ...
    for (m = 0; m<(M >> LOG2_BBE_SIMD_WIDTH); m++)
    {
        ...
        BBE_SVNX16_X(one, pV, 2 * (Vstride << 1));
        BBE_SVNX16_X(X, pV, 1 * (Vstride << 1));
        BBE_MULNX16PACKQ_SAT(X, X, X);
        BBE_SVNX16_IP(X, pV, 2 * BBE_SIMD_WIDTH);
    }
    M &= (BBE_SIMD_WIDTH - 1);
    if (M != 0)
    {
    ...
    //_M = (8 - 3) * (Vstride >> LOG2_BBE_SIMD_WIDTH);
    //pV = (xb_vecNx16*)XT_ADDX4(Vstride, (uintptr_t)pV);
    //for (m = 0; m < _M; m++)
    //{
...
void pfit_grid2 ( void * restrict pScr, int16_t * restrict  V, int16_t * restrict R, const int16_t * restrict x, int M )
{
    ...
    makeV2(V, x, M);
    pfit_chol((int16_t *)pScr, R, V, 100, M, 3); /* make Cholesky decomposition */
} /* pfit_grid2() */
