...
void cholnConv0_7(int32_t* Z,const int16_t* A,int n,int M,int N,int L,int SA)
{
    ...
    valign alignA, align;
    const vselN sel= BBE_MOVVSELNX16(BBE_MOVVA16C(((2*(n-0)+1) << 16) + 2*(n-0)),0);
    NASSERT(n>=0 && n<=7);
    ...
    for (l=0; l<L; l++)
    {
        ...
        for (j=0; j < (M>>1); j++)
        {
            ...
            BBE_LVNX16_XP(A0, pA0, N*2*sizeof(int16_t)*2);

            alignA = BBE_LANX16_PP(pA1);
            BBE_LAVNX16_XP(A1, alignA, pA1, N*sizeof(int16_t)*2);

            repA0 = BBE_SHFLNX16(A0,sel);
            repA1 = BBE_SHFLNX16(A1,sel);
            BBE_MULANX16J(acc0,repA0,A0);
            BBE_MULANX16J(acc1,repA1,A1);
        }
        // store res
        acc0=BBE_ADDNX40(acc0,acc1);
        Zl=BBE_MOVVWL(acc0);
        Zh=BBE_MOVVWH(acc0);
        BBE_SAVNX16_XP(Zl,align,pZ,(n+1)*2*4 - 0*BBE_SIMD_WIDTH);
