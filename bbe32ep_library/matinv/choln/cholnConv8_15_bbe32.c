...
void cholnConv8_15(int32_t* Z,const int16_t* A,int n,int M,int N,int L,int SA)
{
    ...
    valign alignA, align;
    const vselN sel= BBE_MOVVSELNX16(BBE_MOVVA16C(((2*(n-8)+1) << 16) + 2*(n-8)),0);
    NASSERT(n>=8 && n<=15);
    ...
    for (l=0; l<L; l++)
    {
        acc0= acc1= BBE_ZERONX40();
        __Pragma("loop_count min=2, factor=2");
        for (j=0; j<(M>>1); j++)
        {
            pA1 = (const xb_vecNx16*)((int16_t *)(pA0)+N*2);
            BBE_LVNX16_IP(A00, pA0, 2*BBE_SIMD_WIDTH);
            BBE_LVNX16_XP(A01, pA0, N*2*sizeof(int16_t)*2-2*BBE_SIMD_WIDTH);

            alignA = BBE_LANX16_PP(pA1);
            BBE_LANX16_IP (A10, alignA, pA1);
            BBE_LAVNX16_XP(A11, alignA, pA1, N*sizeof(int16_t)*2-2*BBE_SIMD_WIDTH);

            repA0 = BBE_SHFLNX16(A01,sel);
            repA1 = BBE_SHFLNX16(A11,sel);
        ...
        Zl=BBE_MOVVWL(acc1);
        Zh=BBE_MOVVWH(acc1);
        BBE_SAVNX16_XP(Zl,align,pZ,(n+1)*2*4 - 4*BBE_SIMD_WIDTH);
        BBE_SAVNX16_XP(Zh,align,pZ,(n+1)*2*4 - 6*BBE_SIMD_WIDTH);
    }
