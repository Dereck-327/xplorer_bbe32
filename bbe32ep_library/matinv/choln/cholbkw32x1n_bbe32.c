...
/*
    ...
    Input:
    Rt[L][32*32][2]           L transformed R matrices
...

{
    ...
    for (k=N-1; k>=0; k--)
    {
        ...
        for(l=0; l<L; l++)
        {
            ...
            BBE_LVNX16_IP(xx2, pXrd, 2*BBE_SIMD_WIDTH);
            BBE_LVNX16_IP(xx3, pXrd, 2*BBE_SIMD_WIDTH);
            // load rr
            BBE_LVNX16_IP(rr0, pRrd, 2*BBE_SIMD_WIDTH);
            BBE_LVNX16_IP(rr1, pRrd, 2*BBE_SIMD_WIDTH);
            BBE_LVNX16_IP(rr2, pRrd, 2*BBE_SIMD_WIDTH);
            BBE_LVNX16_XP(rr3, pRrd, 2*2*N*N - 6*BBE_SIMD_WIDTH);
            //B-= xx*rr
...
/*
    Transpose Src complex matrix 32x32 and store it to Dst
    Input:
    Dst[32][32][2]        input complex matrix
    Output:
    Src[32][32][2]        output complex matrix
*/
static void transpose32x32(int16_t *restrict Dst, const int16_t *restrict Src)
{
    int k, j;
    int wstride, zstride;
    ...
    xb_vecNx16 *restrict pDst = (xb_vecNx16 *)Dst;
    const xb_vecNx16 *restrict pSrc = (const xb_vecNx16 *)Src;
    //-quarter0-------------------------------------------------------------------
    for(k=0; k<4; k++)
    {
        wstride = 4*2*BBE_SIMD_WIDTH;
        zstride = 2*BBE_SIMD_WIDTH + 2*2*BBE_SIMD_WIDTH;
        j=XT_AND(k,1);
        XT_MOVNEZ(wstride,-15*4*2*BBE_SIMD_WIDTH+2*BBE_SIMD_WIDTH,j);
        XT_MOVEQZ(zstride,-7*4*2*BBE_SIMD_WIDTH+2*BBE_SIMD_WIDTH,j);

        BBE_LVNX16_IP( x0, pSrc, 4*2*BBE_SIMD_WIDTH);
        BBE_LVNX16_IP( x1, pSrc, 4*2*BBE_SIMD_WIDTH);
        BBE_LVNX16_IP( x2, pSrc, 4*2*BBE_SIMD_WIDTH);
        BBE_LVNX16_IP( x3, pSrc, 4*2*BBE_SIMD_WIDTH);
        BBE_LVNX16_IP( x4, pSrc, 4*2*BBE_SIMD_WIDTH);
        BBE_LVNX16_IP( x5, pSrc, 4*2*BBE_SIMD_WIDTH);
        BBE_LVNX16_IP( x6, pSrc, 4*2*BBE_SIMD_WIDTH);
        BBE_LVNX16_XP( x7, pSrc, wstride);
        ...
        BBE_SVNX16_IP( x0, pDst, 4*2*BBE_SIMD_WIDTH); 
        BBE_SVNX16_IP( x1, pDst, 4*2*BBE_SIMD_WIDTH);
        BBE_SVNX16_IP( x2, pDst, 4*2*BBE_SIMD_WIDTH); 
        BBE_SVNX16_IP( x3, pDst, 4*2*BBE_SIMD_WIDTH);
        BBE_SVNX16_IP( x4, pDst, 4*2*BBE_SIMD_WIDTH); 
        BBE_SVNX16_IP( x5, pDst, 4*2*BBE_SIMD_WIDTH);
        BBE_SVNX16_IP( x6, pDst, 4*2*BBE_SIMD_WIDTH); 
        BBE_SVNX16_XP( x7, pDst, zstride);
    }
    //-quarter1-------------------------------------------------------------------
    pSrc= (xb_vecNx16*)XT_ADDX2(2*2*16,(uintptr_t)Src);
    pDst= (xb_vecNx16*)XT_ADDX2(2*16*32,(uintptr_t)Dst);
    for(k=0; k<4; k++)
    {
        wstride = 4*2*BBE_SIMD_WIDTH;
        zstride = 2*BBE_SIMD_WIDTH + 2*2*BBE_SIMD_WIDTH;
    ...
    //-quarter2-------------------------------------------------------------------
    pSrc= (xb_vecNx16*)XT_ADDX2(2*16*32+2*16,(uintptr_t)Src);
    pDst= (xb_vecNx16*)XT_ADDX2(2*16*32+2*16,(uintptr_t)Dst);
    for(k=0; k<4; k++)
    {
        wstride = 4*2*BBE_SIMD_WIDTH;
        zstride = 2*BBE_SIMD_WIDTH + 2*2*BBE_SIMD_WIDTH;
    ...
    //-quarter3-------------------------------------------------------------------
    pSrc= (xb_vecNx16*)XT_ADDX2(2*16*32,(uintptr_t)Src);
    pDst= (xb_vecNx16*)XT_ADDX2(2*16,(uintptr_t)Dst);
    for(k=0; k<4; k++)
    {
        wstride = 4*2*BBE_SIMD_WIDTH;
        zstride = 2*BBE_SIMD_WIDTH + 2*2*BBE_SIMD_WIDTH;
...
----------------------------------------------------------------------------------*/
void transformR32x32(int16_t* Rt, void *pScr,const int16_t* R,int L)
{
    #define N 32
    #define SR 1056
    ...
    
    {
        ...
        r3 = BBE_ZERONX16();
        BBE_SVNX16_IP(r3,pUR,2*BBE_SIMD_WIDTH);
        BBE_SVNX16_IP(r3,pUR,2*BBE_SIMD_WIDTH);
        BBE_SVNX16_IP(r3,pUR,2*BBE_SIMD_WIDTH);
        BBE_SVNX16_IP(r3,pUR,4*BBE_SIMD_WIDTH);
        for (k = 2; k <= 8; k++)
        {
            BBE_SVNX16_IP(r3,pUR,2*BBE_SIMD_WIDTH);
            BBE_SVNX16_IP(r3,pUR,2*BBE_SIMD_WIDTH);
        ...
        {
            BBE_SVNX16_IP(r3,pUR,2*BBE_SIMD_WIDTH);
            BBE_SVNX16_IP(r3,pUR,6*BBE_SIMD_WIDTH);
        }

        BBE_SVNX16_IP(r3,pUR,2*BBE_SIMD_WIDTH);
        for (k = 17; k <= 24; k++)
        {
            BBE_SVNX16_IP(r3,pUR,8*BBE_SIMD_WIDTH);
        }
    }
    ...
    for(l=0; l<L; l++)
    {
        pUR=(xb_vecNx16*)((uintptr_t)pScr+4*2*BBE_SIMD_WIDTH);
        r3 = BBE_ZERONX16();
        // skip R[0]
        ...
        for (k = 2; k <= 8; k++)
        {
            ...
            BBE_SVNX16_IP(r0,pUR,8*BBE_SIMD_WIDTH);
        }
        for (k = 9; k <= 16; k++)
        {
            BBE_LAVNX16_XP(r0,align_rd,pR,2*BBE_SIMD_WIDTH);
            BBE_LAVNX16_XP(r1,align_rd,pR,(k*4)-2*BBE_SIMD_WIDTH);

            BBE_SVNX16_IP(r0,pUR,2*BBE_SIMD_WIDTH);
            BBE_SVNX16_IP(r1,pUR,6*BBE_SIMD_WIDTH);
        }
        for (k = 17; k <= 24; k++)
        {
            BBE_LAVNX16_XP(r0,align_rd,pR,2*BBE_SIMD_WIDTH);
            BBE_LAVNX16_XP(r1,align_rd,pR,2*BBE_SIMD_WIDTH);
            BBE_LAVNX16_XP(r2,align_rd,pR,(k*4)-4*BBE_SIMD_WIDTH);

            BBE_SVNX16_IP(r0,pUR,2*BBE_SIMD_WIDTH);
            BBE_SVNX16_IP(r1,pUR,2*BBE_SIMD_WIDTH);
            BBE_SVNX16_IP(r2,pUR,4*BBE_SIMD_WIDTH);
        }
        for (k = 25; k <= 32; k++)
        {
            BBE_LAVNX16_XP(r0,align_rd,pR,2*BBE_SIMD_WIDTH);
            BBE_LAVNX16_XP(r1,align_rd,pR,2*BBE_SIMD_WIDTH);
            BBE_LAVNX16_XP(r2,align_rd,pR,2*BBE_SIMD_WIDTH);
            BBE_LAVNX16_XP(r3,align_rd,pR,(k*4)-6*BBE_SIMD_WIDTH);

            BBE_SVNX16_IP(r0,pUR,2*BBE_SIMD_WIDTH);
            BBE_SVNX16_IP(r1,pUR,2*BBE_SIMD_WIDTH);
            BBE_SVNX16_IP(r2,pUR,2*BBE_SIMD_WIDTH);
            BBE_SVNX16_IP(r3,pUR,2*BBE_SIMD_WIDTH);
        }
        // transpose R 
        transpose32x32((int16_t*)pRt,(const int16_t*)pScr);
        pRt=(xb_vecNx16*)XT_ADDX2(2*N*N,(uintptr_t)pRt);
    }
    #undef N
    #undef SR
}
                    ...
                    int L)
{
    ...
    transformR32x32(Rt,pUR,R,L);
    bkw32x1(x,Rt,D,y,qXYA,L);
    #undef N
} /* cholbkw32x1n() */

size_t cholbkw32x1n_getScratchSize (int N,int P,int L)
{
    NASSERT(N==32 && P==1 && L>0);
    return (2*N*N*L + 2*N*N)*sizeof(int16_t);
} /* cholbkw32x1n_getScratchSize() */

#else
DISCARD_FUN(void,cholbkw32x1n,(
            void *pScr,
            ...
            int qA, int qY, int qX,
            int L))
size_t cholbkw32x1n_getScratchSize(int N,int P,int L) { (void)N,(void)P,(void)L; return 0; }
