...
#else
{
    const int VECLEN=(int)(sizeof(xb_vecNx16)/(2*sizeof(int16_t)));
    int l;
    int n, m;
    ...
    for(n=0; n<N; n++)
    {
        ...
        for(l=0; l<L; l++)
        {
            ...
            Y_align= BBE_LA_PP(pYrd);
            __Pragma("loop_count min=1");
            for (m= n; m > 0; m-=VECLEN)
            {
                BBE_LAVNX16_XP(rr,R_align,pRrd,m*4);
                BBE_LAVNX16_XP(yy,Y_align,pYrd,m*4);
                //B-= rr*yy
                BBE_MULSNX16J(B,yy,rr);
            }
            // pRrd+=SR-n*2
            pRrd=(const xb_vecNx16*)XT_ADDX2(SR-2*n,(uintptr_t)pRrd);
            // pYrd+=SY-n*2
            pYrd=(const xb_vecNx16*)XT_ADDX2(SY-2*n,(uintptr_t)pYrd);
            // reduced add
