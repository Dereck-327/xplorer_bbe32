...
/* ------------------------------------------------------------------------ */
/*
    BBE32 code for Cholesky decomposition, block format
    IntegrIT, 2006-2016
*/
...
void cholnConv24_31(int32_t* Z,const int16_t* A,int n,int M,int N,int L,int SA)
{
    ...
    valign alignA, align;
    const vselN sel= BBE_MOVVSELNX16(BBE_MOVVA16C(((2*(n-24)+1) << 16) + 2*(n-24)),0);
    NASSERT(n>=24 && n<=31);
    ...
    for (l=0; l<L; l++)
    {
        acc0= acc1= acc2= acc3= BBE_ZERONX40();
        ...
        for (j=0; j<(M>>1); j++)
        {
            ...
            BBE_LVNX16_IP(A02, pA0,2*BBE_SIMD_WIDTH);
            BBE_LVNX16_XP(A03, pA0,N*2*sizeof(int16_t)*2-6*BBE_SIMD_WIDTH);
            
            alignA = BBE_LANX16_PP(pA1);
            BBE_LANX16_IP (A10, alignA, pA1);
            BBE_LANX16_IP (A11, alignA, pA1);
            BBE_LANX16_IP (A12, alignA, pA1);
            BBE_LAVNX16_XP(A13, alignA, pA1, N*sizeof(int16_t)*2-6*BBE_SIMD_WIDTH);

            repA0 = BBE_SHFLNX16(A03,sel);
            repA1 = BBE_SHFLNX16(A13,sel);
            BBE_MULANX16J(acc0,repA0,A00);
            BBE_MULANX16J(acc1,repA0,A01);
            BBE_MULANX16J(acc2,repA0,A02);
            BBE_MULANX16J(acc3,repA0,A03);
            BBE_MULANX16J(acc0,repA1,A10);
            BBE_MULANX16J(acc1,repA1,A11);
            BBE_MULANX16J(acc2,repA1,A12);
            BBE_MULANX16J(acc3,repA1,A13);
        }
        // store res
        Zl=BBE_MOVSVWL(acc0);
        Zh=BBE_MOVSVWH(acc0);
        ...
        Zl=BBE_MOVSVWL(acc3);
        Zh=BBE_MOVSVWH(acc3);
        BBE_SAVNX16_XP(Zl,align,pZ,(n+1)*2*4 - 12*BBE_SIMD_WIDTH);
        BBE_SAVNX16_XP(Zh,align,pZ,(n+1)*2*4 - 14*BBE_SIMD_WIDTH);
    }
