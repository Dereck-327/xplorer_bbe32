...
*/
static void computeAB16x16x1(int32_t* Z,const int16_t* A,const int16_t* B, int qYB,int L)
#if 0
{
    int64_t B_re,B_im;
    int n,m,l;
    ...
    for(l=0; l<L; l++)
    {
        ...
        Z[2*l*16+2*n+0]=(int32_t)B_re;
        Z[2*l*16+2*n+1]=(int32_t)B_im;
    }
}
#else
{
    ...
    for(l=0; l<L*2; l++)
    {
        ...
        a_stride=2*2*BBE_SIMD_WIDTH;
        z_stride=2*BBE_SIMD_WIDTH + 2*2*BBE_SIMD_WIDTH;
        XT_MOVEQZ(a_stride,3*2*BBE_SIMD_WIDTH-16*2*2*BBE_SIMD_WIDTH*L,j);
        XT_MOVEQZ(z_stride,5*2*BBE_SIMD_WIDTH-4*2*BBE_SIMD_WIDTH*L,j);
        //computing half of Z[l]------------------------------------------------
        //--------part0 of MULACC--------------------------------------------------
        // load B[l] row
        BBE_LVNX16_IP(b, pb0, 2*BBE_SIMD_WIDTH);

        BBE_LVNX16_IP(a, pa0, 2*2*BBE_SIMD_WIDTH);b1 = BBE_REPNX16C(b,0);acc=BBE_MULNX16J( b1,a);
        BBE_LVNX16_IP(a, pa0, 2*2*BBE_SIMD_WIDTH);b1 = BBE_REPNX16C(b,1);BBE_MULANX16J(acc,b1,a);
        BBE_LVNX16_IP(a, pa0, 2*2*BBE_SIMD_WIDTH);b1 = BBE_REPNX16C(b,2);BBE_MULANX16J(acc,b1,a);
        BBE_LVNX16_IP(a, pa0, 2*2*BBE_SIMD_WIDTH);b1 = BBE_REPNX16C(b,3);BBE_MULANX16J(acc,b1,a);
        BBE_LVNX16_IP(a, pa0, 2*2*BBE_SIMD_WIDTH);b1 = BBE_REPNX16C(b,4);BBE_MULANX16J(acc,b1,a);
        BBE_LVNX16_IP(a, pa0, 2*2*BBE_SIMD_WIDTH);b1 = BBE_REPNX16C(b,5);BBE_MULANX16J(acc,b1,a);
        BBE_LVNX16_IP(a, pa0, 2*2*BBE_SIMD_WIDTH);b1 = BBE_REPNX16C(b,6);BBE_MULANX16J(acc,b1,a);
        BBE_LVNX16_IP(a, pa0, 2*2*BBE_SIMD_WIDTH);b1 = BBE_REPNX16C(b,7);BBE_MULANX16J(acc,b1,a);
        //--------part1 of MULACC--------------------------------------------------
        // load B[l] row
        BBE_LVNX16_IC(b, pb0);  /* use circular addressing for automatic returning to the beginning of B */

        BBE_LVNX16_IP(a, pa0, 2*2*BBE_SIMD_WIDTH);b1 = BBE_REPNX16C(b,0);BBE_MULANX16J(acc,b1,a);
        BBE_LVNX16_IP(a, pa0, 2*2*BBE_SIMD_WIDTH);b1 = BBE_REPNX16C(b,1);BBE_MULANX16J(acc,b1,a);
        BBE_LVNX16_IP(a, pa0, 2*2*BBE_SIMD_WIDTH);b1 = BBE_REPNX16C(b,2);BBE_MULANX16J(acc,b1,a);
        BBE_LVNX16_IP(a, pa0, 2*2*BBE_SIMD_WIDTH);b1 = BBE_REPNX16C(b,3);BBE_MULANX16J(acc,b1,a);
        BBE_LVNX16_IP(a, pa0, 2*2*BBE_SIMD_WIDTH);b1 = BBE_REPNX16C(b,4);BBE_MULANX16J(acc,b1,a);
...
/*
   ...
   Input:
    ...
    Z[L][16*1]      L complex matrices 16x1
            ...
            int L)
{
    ...
    for(n=0; n<N; n++)
    {
        ...
        for(l=0; l<L; l++)
        {
            ...
            B=BBE_MOVSWVL(b);
            // pZrd+=32 (2*N = 32)
            pZrd=(const int32_t*)XT_ADDX4(32,(uintptr_t)pZrd);
            // load rr
            ...
            // load D
            BBE_LPNX16_XP(dd,pDrd,2*2*BBE_SIMD_WIDTH);
            // replicate dd[0]
            d0 = BBE_REPNX16(dd,0);
            ...
            BBE_SPNX16_XP(b_res,pYwr,2*2*BBE_SIMD_WIDTH);
        }
        __Pragma("no_reorder")
    }
    #undef N
...
void cholfwd16x16x1n ( void    * pScr,
                       ...
                       int L)
{
    ...
    // compute A'*B
    computeAB16x16x1(Z,A,B,qYB,L);
    //cholnFwdrec16(y,R,D,Z,16,L,2*8*17,2*16,2*16,2*16);
    fwd16x1(y,R,D,Z,L);
} /* cholfwd16x16x1n() */
...
size_t cholfwd16x16x1n_getScratchSize (int M, int N, int P,int L)
{
    ...
    return (Z_size);
} /* cholfwd16x16x1n_getScratchSize() */

#else
DISCARD_FUN(void,cholfwd16x16x1n,( void    * pScr,
                     complex_fract16 * restrict _y,
               const complex_fract16 * restrict _R, 
               const complex_fract16 * restrict _D,
               const complex_fract16 * restrict _A, 
               const complex_fract16 * restrict _B, 
                                   int qB,int qY,int L ) )
size_t cholfwd16x16x1n_getScratchSize  (int M,int N, int P,int L) { (void)M,(void)N,(void)P,(void)L; return 0; }
#endif
