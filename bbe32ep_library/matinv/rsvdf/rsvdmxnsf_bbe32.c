...
/*
    ...
    Real MxN stream ordered matrices
...
#define RSVDSF_BLK  (BBE_SIMD_WIDTH/2)
            ...
            int M, int N, int L )
{
...

  {
    ...
    F = (float32_t*)p; p = F + L*(N-1);
    pScr_grsvd = p; p = (uint8_t*)pScr_grsvd + rgrsvdsf_getScratchSize(M,N,L);
    /* Make sure that scratch arrays fit into the reserved space. */
    NASSERT( (uint8_t*)p - (uint8_t*)pScr <= (int)rsvdmxnsf_getScratchSize(M,N,L) );
  }

  /* Reduce input matrixes to upper-bidiagonal form. Left-hand transform matrix
   * U replaces orginial matrix A. */
  rsvd_bidiag_mxnsf(s,F,V,A,M,N,L,U!=0);
  if (U) memcpy(U,A,M*N*L*sz_f32);
  /* Apply the Golub-Reinsch SVD to bidiagonal matrices. */
  rgrsvdsf(pScr_grsvd,s,F,U,V,M,N,L);
...
size_t rsvdmxnsf_getScratchSize ( int M, int N, int L )
{
  NASSERT( M>=N && N>1 );
  NASSERT( 0==(L%(BBE_SIMD_WIDTH/2)) );
  return ( L*(N-1)*sz_f32 +                  /* F: superdiagonal in stream order */
           rgrsvdsf_getScratchSize(M,N,L) ); /* Scratch for Golub-Reinsch SVD implementation */
}

#else /* HAVE_VFPU */

DISCARD_FUN( void, rsvdmxnsf, ( void * pScr,
                       float32_t * restrict U,
                       float32_t * restrict s,
                       float32_t * restrict V,
                       float32_t * restrict A,
