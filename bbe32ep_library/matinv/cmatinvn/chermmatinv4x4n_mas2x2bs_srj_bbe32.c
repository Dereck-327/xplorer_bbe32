...
/*
  NatureDSP_Baseband library. Direct Matrix Inversion
    Blockwise inversion of 4x4 complex matrices
             ...
             int L, int Qzy, int rsh )
{
  ...
  const xb_vecNx16 * restrict EZ_rd;
  ...
  NASSERT_ALIGN( Qz, BBE_SIMD_WIDTH*2 );
  NASSERT_ALIGN( Qx, BBE_SIMD_WIDTH*2 );
  NASSERT_ALIGN( Ez, BBE_SIMD_WIDTH*2 );

  NASSERT( !( L & (BBE_SIMD_WIDTH/2-1) ) );

  csh = BBE_MOVVA16( 15 + rsh - Qzy ); 
  ...
  for ( l=0; l<L/(BBE_SIMD_WIDTH/2); l++ )
  {
    ...
    BBE_LAVNX16_XP( qZ, QZ_va, QZ_rd, (BBE_SIMD_WIDTH/2)*2 );
    BBE_LAVNX16_XP( qX, QX_va, QX_rd, (BBE_SIMD_WIDTH/2)*2 );
    BBE_LAVNX16_XP( eZ, EZ_va, EZ_rd, (BBE_SIMD_WIDTH/2)*2 );
    ...
    // CQ(qX)
    x01 = BBE_LVNX16_I( X_rd, 1*4*BBE_SIMD_WIDTH/2 );
    x10 = BBE_LVNX16_I( X_rd, 2*4*BBE_SIMD_WIDTH/2 );
    x11 = BBE_LVNX16_I( X_rd, 3*4*BBE_SIMD_WIDTH/2 );
    BBE_LVNX16_IP( x00, X_rd, 4*4*BBE_SIMD_WIDTH/2 );

    // CQ(qZY)
    y01 = BBE_LVNX16_I( Y_rd, 1*4*BBE_SIMD_WIDTH/2 );
    y10 = BBE_LVNX16_I( Y_rd, 2*4*BBE_SIMD_WIDTH/2 );
    y11 = BBE_LVNX16_I( Y_rd, 3*4*BBE_SIMD_WIDTH/2 );
    BBE_LVNX16_IP( y00, Y_rd, 4*4*BBE_SIMD_WIDTH/2 );

    // CQ(qZ)
    z01 = BBE_LVNX16_I( Z_rd, 1*4*BBE_SIMD_WIDTH/2 );
    z10 = BBE_LVNX16_I( Z_rd, 2*4*BBE_SIMD_WIDTH/2 );
    z11 = BBE_LVNX16_I( Z_rd, 3*4*BBE_SIMD_WIDTH/2 );
    BBE_LVNX16_IP( z00, Z_rd, 4*4*BBE_SIMD_WIDTH/2 );
    ...
    BBE_MULSNX16J( w00, y00, x00 );
    BBE_MULSNX16J( w01, y01, x00 );
    BBE_MULSNX16J( w10, y00, x01 );
    BBE_MULSNX16J( w11, y01, x01 ); 
                           
    BBE_MULSNX16J( w00, y10, x10 );
    BBE_MULSNX16J( w01, y11, x10 );
    BBE_MULSNX16J( w10, y10, x11 );
    BBE_MULSNX16J( w11, y11, x11 );
