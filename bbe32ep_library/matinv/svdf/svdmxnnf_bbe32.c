...
/*
    ...
    Complex MxN block ordered matrices
...
{
  void *pScr_grsvd;
  complex_float *Ab,*As,*Us,*Vs,*Ds,*Fs;
  float32_t *ss;
  int _L,SA;
  ...
  SA = getSpace(sz_f32c, M*N);

  {
    void * p = pScr;
    /* Partition the scratch area. */
    Ab = (complex_float*)p; p = Ab + SA*_L;
    As = (complex_float*)p; p = As + M*N*_L;
    Ds = (complex_float*)p; p = Ds + N*_L;
    Fs = (complex_float*)p; p = Fs + (N-1)*_L;
    ss = (float32_t    *)p; p = ss + N*_L;
    pScr_grsvd = p; p = (uint8_t*)pScr_grsvd + grsvdsf_getScratchSize(M,N,_L);
    /* Make sure that scratch arrays fit into the reserved space. */
    NASSERT( (uint8_t*)p - (uint8_t*)pScr <= (int)svdmxnnf_getScratchSize(M,N,L) );
  ...
  memcpy(Ab, A, L*SA*sz_f32c); memset(Ab+L*SA, 0, (_L-L)*SA*sz_f32c);
  /* Convert input data to stream order. */
  cbsmxnf(As,Ab,M,N,_L);
  /* Reduce input matrixes to upper-bidiagonal form. Left-hand transform matrix
   * U replaces orginial matrix A. */
  svd_bidiag_mxnsf(Ds,Fs,Vs,As,M,N,_L,(U!=0));
  /* Apply the Golub-Reinsch SVD to bidiagonal matrices. */
  grsvdsf(pScr_grsvd,ss,Ds,Fs,Us,Vs,M,N,_L);
  /* Convert results to block order. */
  svd_rsbmxnxsf(s,ss,1,N,L,_L);
  if (U) svd_csbmxnxsf(U,Us,M,N,L,_L);
  if (V) svd_csbmxnxsf(V,Vs,N,N,L,_L);
...
size_t svdmxnnf_getScratchSize ( int M, int N, int L )
{
  ...
  /* Space occupoed by single input matrix in block order. */
  SA = getSpace(sz_f32c, M*N);
  return ( _L*SA   *sz_f32c + /* Ab: input matrices in block order padded with zeros        */
           _L*M*N  *sz_f32c + /* As: input matrix and left-singular vectors in stream order */
           _L*N    *sz_f32c + /* Ds: main diagonal in stream order                          */
           _L*(N-1)*sz_f32c + /* Fs: superdiagonal in stream order                          */
           _L*N    *sz_f32  + /* ss: singular values in stream order                        */
           grsvdsf_getScratchSize(M,N,_L) ); /* Golub-Reinsch SVD implementation req's      */
}

#else /* HAVE_VFPU */

DISCARD_FUN( void, svdmxnnf, ( void * pScr,
                      complex_float * restrict U,
                      float32_t     * restrict s,
                      complex_float * restrict V,
                      complex_float * restrict A,
