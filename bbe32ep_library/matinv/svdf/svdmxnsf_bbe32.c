...
/*
    ...
    Complex MxN stream ordered matrices
            ...
            int M, int N, int L )
{
...

  {
    ...
    D = (complex_float*)p; p = D + L*N;
    F = (complex_float*)p; p = F + L*(N-1);
    pScr_grsvd = p; p = (uint8_t*)pScr_grsvd + grsvdsf_getScratchSize(M,N,L);
    /* Make sure that scratch arrays fit into the reserved space. */
    NASSERT( (uint8_t*)p - (uint8_t*)pScr <= (int)svdmxnsf_getScratchSize(M,N,L) );
  }

  /* Reduce input matrixes to upper-bidiagonal form. Left-hand transform matrix
   * U replaces orginial matrix A. */
  svd_bidiag_mxnsf(D,F,V,A,M,N,L,U!=0);
  if (U) memcpy(U,A,M*N*L*sz_f32c);
  /* Apply the Golub-Reinsch SVD to bidiagonal matrices. */
  grsvdsf(pScr_grsvd,s,D,F,U,V,M,N,L);
...
size_t svdmxnsf_getScratchSize ( int M, int N, int L )
{
  NASSERT( M>=N && N>1 );
  NASSERT( 0==(L%(BBE_SIMD_WIDTH/2)) );
  return ( L*N*sz_f32c +                    /* D: main diagonal in stream order             */
           L*(N-1)*sz_f32c +                /* F: superdiagonal in stream order             */
           grsvdsf_getScratchSize(M,N,L) ); /* Scratch for Golub-Reinsch SVD implementation */
}

#else /* HAVE_VFPU */

DISCARD_FUN( void, svdmxnsf, ( void * pScr,
                      complex_float * restrict U,
                      float32_t     * restrict s,
                      complex_float * restrict V,
                      complex_float * restrict A,
