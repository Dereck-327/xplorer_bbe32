...
---------------------------------------------------------------------------*/
#if !(HAVE_VFPU)
DISCARD_FUN(void, cholmmsemxnxpsf,(
            void * pScr,
      ...
      int M, int N, int P, int L )
{
    ...
    y=(complex_float*)(R+N*N*L);
    D=(complex_float*)(y+M*P*L);
    cholmxnsf(R,D,A,sigma2,M,N,L);
    cholfwdmxnxpsf(y,R,D,A,B,M,N,P,L);
    cholbkwnxpsf(x,R,D,y,N,P,L);
}
...
size_t cholmmsemxnxpsf_getScratchSize ( int M, int N, int P, int L )
{
    ...
    NASSERT(L>1);
    NASSERT((L&(BBE_SIMD_WIDTH/4-1))==0);
    sz=sizeof(complex_float)*(N*N*L+M*P*L+N*L);
    return sz;
}
#endif
