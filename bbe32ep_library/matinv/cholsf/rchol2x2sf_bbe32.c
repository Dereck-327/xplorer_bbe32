            ...
            int L)
{
    ...
    {
        BBE_LVN_2XF32_IP(Acc, pS, 0);
        A0 = BBE_LVN_2XF32_X(pAm, 2 * L * sizeof(float32_t));
        BBE_MULAN_2XF32(Acc, A0, A0);
        BBE_LVN_2XF32_IP(A0, pAm, 0);
        BBE_MULAN_2XF32(Acc, A0, A0);
        ...
        BBE_SVN_2XF32_X(Acc, pRk, -L * sizeof(float32_t));


        BBE_LVN_2XF32_IP(Acc, pS, 2 * BBE_SIMD_WIDTH);
        A0 = BBE_LVN_2XF32_X(pAm, 2 * L * sizeof(float32_t));
        A1 = BBE_LVN_2XF32_X(pAk, 2 * L * sizeof(float32_t));
    ...
    for (l = 0; l < L; l += VECLEN)
    {
        BBE_LVN_2XF32_IP(Acc, pS, 2 * BBE_SIMD_WIDTH);
        A1 = BBE_LVN_2XF32_X(pAk, 2 * L * sizeof(float32_t));
        Acc1 = BBE_MULN_2XF32(A1, A1);
        BBE_LVN_2XF32_IP(A1, pAk, 2 * BBE_SIMD_WIDTH);
        BBE_MULAN_2XF32(Acc1, A1, A1);
        ...
        Acc = BBE_MULN_2XF32(Acc, rsqrt);
        BBE_SVN_2XF32_X(BBE_ZERON_2XF32(), pRk, -L * sizeof(float32_t));
        BBE_SVN_2XF32_IP(Acc, pRk, 2 * BBE_SIMD_WIDTH);
    }
}
#endif
