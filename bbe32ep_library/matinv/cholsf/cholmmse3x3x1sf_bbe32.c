...
---------------------------------------------------------------------------*/
#if !(HAVE_VFPU)
DISCARD_FUN(void, cholmmse3x3x1sf,(
            void * pScr,
      ...
      int L )
{
    ...
    y=(complex_float*)(R+3*3*L);
    D=(complex_float*)(y+3*1*L);
    chol3x3sf(R,D,A,sigma2,L);
    cholfwd3x3x1sf(y,R,D,A,B,L);
    cholbkw3x1sf(x,R,D,y,L);
}
...
size_t cholmmse3x3x1sf_getScratchSize ( int M, int N, int P, int L )
{
    ...
    NASSERT(L>1);
    NASSERT((L&(BBE_SIMD_WIDTH/4-1))==0);
    sz=sizeof(complex_float)*(3*3*L+3*1*L+3*L);
    return sz;
}
#endif
