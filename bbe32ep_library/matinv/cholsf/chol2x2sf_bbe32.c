            ...
            int L)
{
    ...
    valign vs;
    vboolN_2 vpred;
    xb_vecN_2xf32 Acc, Acc1, rsqrt, sigma;
    xb_vecN_2xf32 A0, A1;
    xb_vecN_2xf32 R0;
    ...
    for (l = 0; l < L; l += VECLEN)
    {
        ...
        A0 = BBE_LVN_2XF32_X(pAm, 2 * 2 * L * sizeof(float32_t));
        ...
        A1 = BBE_SHFLN_2XF32I(Acc, BBE_SHFLI_SWAP_2);
        Acc = BBE_ADDN_2XF32(Acc, A1);

        vpred = BBE_OGTN_2XF32(Acc, BBE_ZERON_2XF32());
        rsqrt = 1.f;
        BBE_RSQRTN_2XF32T(rsqrt, Acc, vpred);
        BBE_SVN_2XF32_IP(rsqrt, pD, 4 * BBE_SIMD_WIDTH);
    ...
    for (l = 0; l < L; l += VECLEN)
    {
        ...
        A0 = BBE_LVN_2XF32_X(pAm, 2 * 2 * L * sizeof(float32_t));
        A1 = BBE_LVN_2XF32_X(pAk, 2 * 2 * L * sizeof(float32_t));
        ...
        BBE_LVN_2XF32_IP(rsqrt, pD, 4 * BBE_SIMD_WIDTH);
        Acc = BBE_MULN_2XF32(Acc, rsqrt);
        BBE_SVN_2XF32_XP(Acc, pRk, 2 * L * sizeof(float32_t));
        
        BBE_SVN_2XF32_XP(BBE_ZERON_2XF32(), pRk, 2 * BBE_SIMD_WIDTH - 2 * L * sizeof(float32_t));
    }
    ...
    pRk = (xb_vecN_2xf32 *)(_R + 3 * L);
    for (l = 0; l < L; l += VECLEN)
    {
        BBE_LAVN_2XF32_XP(sigma, vs, pS, BBE_SIMD_WIDTH);
        Acc = BBE_SELN_2XF32I(BBE_ZERON_2XF32(), sigma, BBE_SELI_INTERLEAVE_2_LO);

        A0 = BBE_LVN_2XF32_X(pAk, 2 * 2 * L * sizeof(float32_t));
        BBE_MULAN_2XF32(Acc, A0, A0);
        BBE_LVN_2XF32_IP(A0, pAk, 2 * BBE_SIMD_WIDTH);
        BBE_MULAN_2XF32(Acc, A0, A0);
