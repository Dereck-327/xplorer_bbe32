            ...
            int L)
{
    ...
    xb_vecN_2xf32 Acc, Acc1, Acc2, Acc3;
    ...
    {
        Acc = BBE_LVN_2XF32_X(pY, 2 * L * sizeof(float32_t));
        BBE_LVN_2XF32_IP(d0, pD, -2 * BBE_SIMD_WIDTH);
        Acc = BBE_MULN_2XF32(Acc, d0);
        BBE_SVN_2XF32_X(Acc, pXw, 2 * L * sizeof(float32_t));
    ...
    for (l = 0; l < L; l += VECLEN)
    {
        BBE_LVN_2XF32_IP(Acc1, pY, 2 * BBE_SIMD_WIDTH);
    ...
    for (l = 0; l < L; l += VECLEN)
    {
        BBE_LVN_2XF32_IP(Acc2, pY, 2 * BBE_SIMD_WIDTH);

        BBE_LVN_2XF32_XP(x0, pXr, 2 * L * sizeof(float32_t));
        ...
        BBE_LVN_2XF32_XP(x0, pXr, 2 * BBE_SIMD_WIDTH - 2 * 2 * L * sizeof(float32_t));
        BBE_LVN_2XF32_XP(r0, pR, 2 * BBE_SIMD_WIDTH - 2 * 2 * L * sizeof(float32_t));
        BBE_MULMASN_2XF32(Acc, x0, r0, 3, 4);
        BBE_MULMASN_2XF32(Acc1, x0, r0, 2, 11);
