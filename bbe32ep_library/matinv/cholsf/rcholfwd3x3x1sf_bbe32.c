...
---------------------------------------------------------------------------*/
#if !(HAVE_VFPU)
DISCARD_FUN(void, rcholfwd3x3x1sf,(
          ...
          int L)
{
    ...
    pYr = (const xb_vecN_2xf32 *)(y + L);
    pY = (xb_vecN_2xf32 *)(y);
    pD = (const xb_vecN_2xf32 *)D;
    for (l = 0; l < L; l += VECLEN)
    {
        A0 = BBE_LVN_2XF32_X(pA, 2 * 3 * L * sizeof(float32_t));
        B0 = BBE_LVN_2XF32_X(pB, 2 * L * sizeof(float32_t));
        Acc0 = BBE_MULN_2XF32(A0, B0);
        A0 = BBE_LVN_2XF32_X(pA, 3 * L * sizeof(float32_t));
        B0 = BBE_LVN_2XF32_X(pB, L * sizeof(float32_t));
        Acc2 = BBE_MULN_2XF32(A0, B0);
        BBE_LVN_2XF32_IP(A0, pA, 2 * BBE_SIMD_WIDTH);
        BBE_LVN_2XF32_IP(B0, pB, 2 * BBE_SIMD_WIDTH);
        BBE_MULAN_2XF32(Acc0, A0, B0);

        BBE_LVN_2XF32_XP(R0, pR, 2 * BBE_SIMD_WIDTH); //for compiler
        //BBE_LVN_2XF32_XP(Y0, pYr, -2 * BBE_SIMD_WIDTH); //for compiler
        Acc0 = BBE_ADDN_2XF32(Acc0, Acc2);

        BBE_LVN_2XF32_XP(D0, pD, 6 * BBE_SIMD_WIDTH);
        Acc0 = BBE_MULN_2XF32(Acc0, D0);
        BBE_SVN_2XF32_XP(Acc0, pY, 2 * BBE_SIMD_WIDTH);
    }
    ...
    for (l = 0; l < L; l += VECLEN)
    {
        ...
        R0 = BBE_LVN_2XF32_X(pR, 3 * L * sizeof(float32_t));
        Y0 = BBE_LVN_2XF32_X(pYr, L * sizeof(float32_t));
        BBE_MULSN_2XF32(Acc2, R0, Y0);
        BBE_LVN_2XF32_XP(R0, pR, 2 * BBE_SIMD_WIDTH);
        BBE_LVN_2XF32_XP(Y0, pYr, 2 * BBE_SIMD_WIDTH);
        BBE_MULSN_2XF32(Acc0, R0, Y0);
