...
---------------------------------------------------------------------------*/
#if !(HAVE_VFPU)
DISCARD_FUN(void, cholmmse4x4x1sf,(
            void * pScr,
      ...
      int L )
{
    ...
    y=(complex_float*)(R+4*4*L);
    D=(complex_float*)(y+4*1*L);
    chol4x4sf(R,D,A,sigma2,L);
    cholfwd4x4x1sf(y,R,D,A,B,L);
    cholbkw4x1sf(x,R,D,y,L);
}
...
size_t cholmmse4x4x1sf_getScratchSize ( int M, int N, int P, int L )
{
    ...
    NASSERT(L>1);
    NASSERT((L&(BBE_SIMD_WIDTH/4-1))==0);
    sz=sizeof(complex_float)*(4*4*L+4*1*L+4*L);
    return sz;
}
#endif
