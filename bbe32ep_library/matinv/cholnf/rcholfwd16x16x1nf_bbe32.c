...
*/
static void rcomputeAB16x16x1f(float32_t* Z,const float32_t* A,const float32_t* B, int L)
{
    int l;
    ...
    for (l = 0; l < L; l++)
    {
        ...
        BBE_LVN_2XF32_XP(A0, pA, 4 * BBE_SIMD_WIDTH);
        BBE_MULAN_2XF32(Acc, A0, t0);
        BBE_LVN_2XF32_XP(A0, pA, 4 * BBE_SIMD_WIDTH);
        BBE_MULAN_2XF32(Acc1, A0, t1);
        BBE_LVN_2XF32_XP(A0, pA, 4 * BBE_SIMD_WIDTH);
        BBE_MULAN_2XF32(Acc2, A0, t2);
        BBE_LVN_2XF32_XP(A0, pA, 4 * BBE_SIMD_WIDTH);
        BBE_MULAN_2XF32(Acc3, A0, t3);
        BBE_LVN_2XF32_XP(A0, pA, 4 * BBE_SIMD_WIDTH);
        BBE_MULAN_2XF32(Acc, A0, t4);
        BBE_LVN_2XF32_XP(A0, pA, 4 * BBE_SIMD_WIDTH);
        BBE_MULAN_2XF32(Acc1, A0, t5);
    ...
    pA = (const xb_vecN_2xf32 *)(A) + 1;
    pB = (const xb_vecN_2xf32 *)(B);
    pZ = (      xb_vecN_2xf32 *)(Z) + 1;
            ...
            int L)
{
    ...
    // compute A'*B
    rcomputeAB16x16x1f(Z,A,B, L);
    rcholnfFwdrec16(y,R,D,Z,16,L,136,16,16,16);
}
...
#else
DISCARD_FUN(void, rcholfwd16x16x1nf,(
            void * pScr,
            float32_t * restrict y,
      const float32_t * restrict R, 
      const float32_t * restrict D,
