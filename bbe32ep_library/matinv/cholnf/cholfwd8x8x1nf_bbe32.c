...
*/
static void computeAB8x8x1f(float32_t* Z,const float32_t* A,const float32_t* B, int L)
{
#if 0
    float32_t B_re, B_im;
    int n, m, l;
    ...
    const xb_vecN_2xf32 * restrict pA1 = (const xb_vecN_2xf32 *)(A) + 1;
    const xb_vecN_2xf32 * restrict pB  = (const xb_vecN_2xf32 *)(B);
    const xb_vecN_2xf32 * restrict pB1 = (const xb_vecN_2xf32 *)(B);
          xb_vecN_2xf32 * restrict pZ  = (      xb_vecN_2xf32 *)(Z);
          xb_vecN_2xf32 * restrict pZ1 = (      xb_vecN_2xf32 *)(Z) + 1;
    xb_vecN_2xf32 Acc, Acc1, Acc2, Acc3, A0, A1, B0, temp;
    ...
    for (l = 0; l < L; l++)
    {
        ...
        BBE_LVN_2XF32_XP(A0, pA, 4 * BBE_SIMD_WIDTH);
        ...
        BBE_SVN_2XF32_IP(Acc, pZ, 4 * BBE_SIMD_WIDTH);
    }
    for (l = 0; l < L; l++)
    {
        BBE_LVN_2XF32_XP(temp, pB1, 2 * BBE_SIMD_WIDTH);

        B0 = BBE_SHFLN_2XF32I(temp, BBE_SHFLI_REP_0X4);
        BBE_LVN_2XF32_XP(A1, pA1, 4 * BBE_SIMD_WIDTH);
        Acc = BBE_MULMN_2XF32(A1, B0, 0, 4);
        Acc1 = BBE_MULMN_2XF32(A1, B0, 2, 11);

        B0 = BBE_SHFLN_2XF32I(temp, BBE_SHFLI_REP_1X4);
        BBE_LVN_2XF32_XP(A1, pA1, 4 * BBE_SIMD_WIDTH);
        Acc2 = BBE_MULMN_2XF32(A1, B0, 0, 4);
        Acc3 = BBE_MULMN_2XF32(A1, B0, 2, 11);

        B0 = BBE_SHFLN_2XF32I(temp, BBE_SHFLI_REP_2X4);
        BBE_LVN_2XF32_XP(A1, pA1, 4 * BBE_SIMD_WIDTH);
        BBE_MULMASN_2XF32(Acc, A1, B0, 0, 4);
        BBE_MULMASN_2XF32(Acc1, A1, B0, 2, 11);

        B0 = BBE_SHFLN_2XF32I(temp, BBE_SHFLI_REP_3X4);
        BBE_LVN_2XF32_XP(A1, pA1, 4 * BBE_SIMD_WIDTH);
        BBE_MULMASN_2XF32(Acc2, A1, B0, 0, 4);
        BBE_MULMASN_2XF32(Acc3, A1, B0, 2, 11);
        ...
        Acc = BBE_ADDN_2XF32(Acc, Acc1);
        Acc = BBE_ADDN_2XF32(Acc, Acc2);
        BBE_SVN_2XF32_IP(Acc, pZ1, 4 * BBE_SIMD_WIDTH);
    }
}
            ...
            int L)
{
    ...
    // compute A'*B
    computeAB8x8x1f(Z,A,B, L);
    cholnfFwdrec8(y,R,D,Z,8,L,2*((8*(8+1))>>1),2*8,2*8,2*8);
}
...
#else
DISCARD_FUN(void, cholfwd8x8x1nf,(
            void * pScr,
            complex_float * restrict _y,
      const complex_float * restrict _R, 
      const complex_float * restrict _D,
