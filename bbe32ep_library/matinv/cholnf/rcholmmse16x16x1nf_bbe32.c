      ...
      int L )
{
    ...
    if (L<=0) return;
    SR=L*(getSpace((16*(16+1))>>1))*sizeof(float32_t);
    SD=L*(getSpace(16  )          )*sizeof(float32_t);
    SY=L*(getSpace(16*1)          )*sizeof(float32_t);
    ...
    N_ = rcholmmse16x16x1nf_getScratchSize(16, 16, 1, L) >> (LOG2_BBE_SIMD_WIDTH + 1);
    for (n = 0; n < N_; n++)
    {
    ...
    rchol16x16nf(pScr,R,D,A,sigma2,L);
    rcholfwd16x16x1nf(pScr,Y,R,D,A,B,L);
    rcholbkw16x1nf(pScr,x,R,D,Y,L);
}

size_t rcholmmse16x16x1nf_getScratchSize ( int M, int N, int P, int L )
{
    NASSERT(M==16 && N==16 && P==1);
    return rcholmmsemxnxpnf_getScratchSize(M,N,P,L);
}

#else
DISCARD_FUN(void, rcholmmse16x16x1nf,(
            void * pScr,
            float32_t * restrict x,
      const float32_t * restrict A,
      const float32_t * restrict B,
