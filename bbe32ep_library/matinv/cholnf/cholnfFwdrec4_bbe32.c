...
--------------------------------------------------*/
void cholnfFwdrec4(float32_t* y,const float32_t* R,const float32_t* D,const float32_t* Z,int N,int L,int SR,int SD,int SY,int SZ)
{
    int l, n;
    ...
    n = 0;
    pYw = (long long *)(y);
    pZ = (const long long *)(Z);
    pD = (const long long *)(D);
    for (l = 0; l < L; l++)
    {
    ...
    for (n = 1; n < N; n++)
    {
        ...
        for (l = 0; l < L; l++)
        {
            ...
            BBE_LAVN_2XF32_XP(R0, vr, pR, 2 * n * sizeof(float32_t));
