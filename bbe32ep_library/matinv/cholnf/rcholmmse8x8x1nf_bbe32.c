      ...
      int L )
{
    ...
    if (L<=0) return;
    SR=L*(getSpace((8*(8+1))>>1))*sizeof(float32_t);
    SD=L*(getSpace(8  )         )*sizeof(float32_t);
    SY=L*(getSpace(8*1)         )*sizeof(float32_t);
    ...
    N_ = rcholmmse8x8x1nf_getScratchSize(8, 8, 1, L) >> (LOG2_BBE_SIMD_WIDTH + 1);
    for (n = 0; n < N_; n++)
    {
    ...
    rchol8x8nf(pScr,R,D,A,sigma2,L);
    rcholfwd8x8x1nf(pScr,Y,R,D,A,B,L);
    rcholbkw8x1nf(pScr,x,R,D,Y,L);
}
...
#else
DISCARD_FUN(void, rcholmmse8x8x1nf,(
            void * pScr,
            float32_t * restrict x,
      const float32_t * restrict A,
      const float32_t * restrict B,
