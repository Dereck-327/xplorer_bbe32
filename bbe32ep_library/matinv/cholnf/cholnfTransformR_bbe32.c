...
void cholnfTransformR(float32_t* Rt, const float32_t* R, int N, int L)
{
    ...
    const int delta3 = 2 * (N + 1) * sizeof(float32_t);
    xb_vecN_4x64 vTmp;
    ...
    for (l = 0; l < L; l++)
    {
        ...
        for (n = 0; n < N; n++)
        {
            pR = (const long long *)XT_ADDX4((dpR)*(dpR + 3) - 2, (uintptr_t)pR2);
            delta = delta2;
            for (m = 0; m < n; m++)
            {
                BBE_LSN_4X64_XP(vTmp, pR, delta);
                BBE_SSN_4X64_XP(vTmp, pRw, 2 * sizeof(float32_t));
                delta = XT_ADDX4(2, delta);
            }
            delta2 = XT_ADDX4(-2, delta2);
            dpR = XT_ADDI(dpR, -1);
        }
