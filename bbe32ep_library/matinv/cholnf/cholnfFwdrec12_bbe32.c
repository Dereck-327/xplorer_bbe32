...
--------------------------------------------------*/
void cholnfFwdrec12(float32_t* y,const float32_t* R,const float32_t* D,const float32_t* Z,int N,int L,int SR,int SD,int SY,int SZ)
{
    int l, n;
    ...
    cholnfFwdrec8(y, R, D, Z, 8, L, SR, SD, SY, SZ);
    ...
    for (n = 8; n < N; n++)
    {
        ...
        for (l = 0; l < L; l++)
        {
            ...
            BBE_LAVN_2XF32_XP(R0, vr, pR, 2 * (n - 2 * (BBE_SIMD_WIDTH / 4)) * sizeof(float32_t));
            BBE_LAVN_2XF32_XP(Y0, vy, pY, 2 * (n - 2 * (BBE_SIMD_WIDTH / 4)) * sizeof(float32_t));
            BBE_MULMASN_2XF32(Acc1, R0, Y0, 3, 4);
            BBE_MULMASN_2XF32(Acc, R0, Y0, 1, 11);
