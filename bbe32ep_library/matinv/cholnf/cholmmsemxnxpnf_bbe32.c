      ...
      int M, int N, int P, int L )
{
    ...
    if (L<=0) return;
    SR=L*(getSpace((N*(N+1)))>>1)*sizeof(complex_float);
    SD=L*(getSpace(N  *2)>>1    )*sizeof(complex_float);
    SY=L*(getSpace(N*P*2)>>1    )*sizeof(complex_float);
    ...
    N_ = cholmmsemxnxpnf_getScratchSize(M, N, P, L) >> (LOG2_BBE_SIMD_WIDTH + 1);
    for (n = 0; n < N_; n++)
    {
    ...
    cholmxnnf(pScr,R,D,A,sigma2,M,N,L);
    cholfwdmxnxpnf(pScr,Y,R,D,A,B,M,N,P,L);
    cholbkwnxpnf(pScr,x,R,D,Y,N,P,L);
}
...
#else
DISCARD_FUN(void, cholmmsemxnxpnf,(
            void * pScr,
            complex_float * restrict x,
      const complex_float * restrict A,
      const complex_float * restrict B,
