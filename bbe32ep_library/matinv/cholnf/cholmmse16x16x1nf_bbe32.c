      ...
      int L )
{
    ...
    if (L<=0) return;
    SR=L*(((16*(16+1)))>>1)*sizeof(complex_float);
    SD=L*((16  *2)>>1     )*sizeof(complex_float);
    SY=L*((16*1*2)>>1     )*sizeof(complex_float);
    ...
    N_ = cholmmse16x16x1nf_getScratchSize(16, 16, 1, L) >> (LOG2_BBE_SIMD_WIDTH + 1);
    for (n = 0; n < N_; n++)
    {
    ...
    chol16x16nf(pScr,R,D,A,sigma2,L);
    cholfwd16x16x1nf(pScr,Y,R,D,A,B,L);
    cholbkw16x1nf(pScr,x,R,D,Y,L);
}
...
DISCARD_FUN(void, cholmmse16x16x1nf,(
            void * pScr,
            complex_float * restrict x,
      const complex_float * restrict A,
      const complex_float * restrict B,
