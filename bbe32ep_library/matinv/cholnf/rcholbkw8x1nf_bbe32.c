...
/* ------------------------------------------------------------------------ */
/*
    Cholesky backward recursion, block format real floating point
...
static void rcholnfTransformR8(float32_t* Rt, const float32_t* R, int N, int L)
{
    const int SR = 40;
    int l;
    ...
    int L)
{
    ...
    rcholnfTransformR8(Rt, R, 8, L);
    rcholnfBkwnx1(x, Rt, D, y, 8, L);
}
...
#else
DISCARD_FUN(void, rcholbkw8x1nf,(
            void * pScr,
            float32_t * restrict x, 
      const float32_t * restrict R,
      const float32_t * restrict D,
