...
--------------------------------------------------*/
void cholnfFwdrec8(float32_t* y,const float32_t* R,const float32_t* D,const float32_t* Z,int N,int L,int SR,int SD,int SY,int SZ)
{
    int l, n;
    ...
    cholnfFwdrec4(y, R, D, Z, 4, L, SR, SD, SY, SZ);
    ...
    for (n = 4; n < N; n++)
    {
        ...
        for (l = 0; l < L; l++)
        {
            ...
            BBE_LAVN_2XF32_XP(R0, vr, pR, 2 * (n - (BBE_SIMD_WIDTH / 4)) * sizeof(float32_t));
