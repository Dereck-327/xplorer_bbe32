...
/*---------------------------------------------------
   ...
   Z[L][n+1][2] results
---------------------------------------------------*/
void cholnfConv12_15(float32_t* Z,const float32_t* A,const float32_t * restrict sigma2,int n,int M,int N,int L,int SA)
{
    int l, m;
    ...
    for (l = 0; l < L; l++)
    {
        ...
        for (m = 0; m < M; m++)
        {
            ...
            A0 = BBE_LVN_2XF32_I(pAk, 6 * BBE_SIMD_WIDTH);
        ...
        BBE_SAVN_2XF32_XP(Acc3, vz, pZ, 2 * (n + 1) * sizeof(float32_t));
        BBE_SAVN_2XF32_XP(Acc2, vz, pZ, 2 * (n + 1 - (BBE_SIMD_WIDTH / 4)) * sizeof(float32_t));
        BBE_SAVN_2XF32_XP(Acc1, vz, pZ, 2 * (n + 1 - 2 * (BBE_SIMD_WIDTH / 4)) * sizeof(float32_t));
        BBE_SAVN_2XF32_XP(Acc, vz, pZ, 2 * (n + 1 - 3 * (BBE_SIMD_WIDTH / 4)) * sizeof(float32_t));
