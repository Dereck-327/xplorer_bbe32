...
/*---------------------------------------------------
   ...
   Z[L][n+1][1] results
---------------------------------------------------*/
void rcholnfConv8_15(float32_t* Z,const float32_t* A,const float32_t * restrict sigma2,int n,int M,int N,int L,int SA)
{
    int l, m;
    ...
    for (l = 0; l < L; l++)
    {
        ...
        for (m = 0; m < M; m++)
        {
            ...
            pAk = (const xb_vecN_2xf32 *)XT_ADDX4(N - 16, (uintptr_t)pAk);
        }
        BBE_SAN_2XF32_IP(Acc, vz, pZ);
        BBE_SAVN_2XF32_XP(Acc1, vz, pZ, (n + 1 - (BBE_SIMD_WIDTH / 2)) * sizeof(float32_t));
