...
static void conv0_3(float32_t* Z, const float32_t* A, const float32_t * restrict sigma2, int n, int M, int N, int L, int SA)
{
    ...
    valign vz;
    xb_vecN_2xf32 Acc, Acc1, Acc2, sigma;
    xb_vecN_2xf32 A0, A1;
    xb_vecN_4x64 temp;

    NASSERT(n >= 0 && n <= 3);
    NASSERT(N == 8 && M == 8 && SA == 2 * (M*N));
    ...
    for (l = 0; l < L; l++)
    {
        ...
        BBE_LSN_4X64_XP(temp, pAn, (SA - 7 * 2 * 8) * sizeof(float32_t));
        A1 = BBE_MOVN_2XF32_FROMNX16(BBE_MOVNX16_FROMN_4X64(temp));
        A1 = BBE_SHFLN_2XF32I(A1, BBE_SHFLI_REP_0X4);
        BBE_LVN_2XF32_XP(A0, pAk, (SA - 7 * 2 * 8) * sizeof(float32_t));
        BBE_MULMASN_2XF32(Acc, A0, A1, 0, 4);
        BBE_MULMASN_2XF32(Acc1, A0, A1, 2, 11);
            ...
            int L)
{
    ...
    for (n=0; n<8; n++)
    {
        ...
        it[n].conv(Z,A,sigma2,n,8,8,L,SA);
