...
static void cqrRotateR5x5(int16_t* R, const int16_t* pFi, int L)
{
    ...
    strideFi = 2 * BBE_SIMD_WIDTH*(SIZE_OF_V(5, 5) + 1);
    strideR = -(5 * 5 - 1) * 4 * L + 2 * BBE_SIMD_WIDTH;
...
-------------------------------------------------------------------------*/
void cqr_build_r5x5s  ( void* pScr, complex_fract16 * restrict _V, complex_fract16 * restrict _R, int L)
{
    ...
    pFi = V + BBE_SIMD_WIDTH* SIZE_OF_V(5, 5);  pR = R;    pV = V;
    cqrHouseholder5(pScr, pR, pV, pFi, 5, 5, L);
    cqrUpdateR5(pR, pV, 0, 5, 5, L);
    pFi += BBE_SIMD_WIDTH; pV += BBE_SIMD_WIDTH * 5; pR += (5 + 1)*L * 2;
    cqrHouseholder4(pR, pV, pFi, 5, 5, L);
    cqrUpdateR4(pR, pV, 1, 5, 5, L);
    pFi += BBE_SIMD_WIDTH; pV += BBE_SIMD_WIDTH * 4; pR += (5 + 1)*L * 2;
    cqrHouseholder3(pR, pV, pFi, 5, 5, L);
    cqrUpdateR3(pR, pV, 2, 5, 5, L);
    pFi += BBE_SIMD_WIDTH; pV += BBE_SIMD_WIDTH * 3; pR += (5 + 1)*L * 2;
    cqrHouseholder2(pR, pV, pFi, 5, 5, L);
    cqrUpdateR2(pR, pV, 3, 5, 5, L);
    // for square matrices we have to calculate last rotation explicitely
    cqrComputeLastRot(V + BBE_SIMD_WIDTH* SIZE_OF_V(5, 5), R, 5, L);
    // final rotation
    pFi = V + BBE_SIMD_WIDTH* SIZE_OF_V(5, 5);
    cqrRotateR5x5(R, pFi, L);
}
...
size_t cqr_build_r5x5s_getScratchSize (int M, int N,int L)
{
    ...
    return 4 * L;
} /* cqr_build_r5x5s_getScratchSize() */
#else
DISCARD_FUN(void, cqr_build_r5x5s, ( void* pScr,complex_fract16 * restrict V, complex_fract16 * restrict R, int L))
size_t cqr_build_r5x5s_getScratchSize (int M, int N,int L) { (void)M;(void)N;(void)L;return (0); }
#endif
