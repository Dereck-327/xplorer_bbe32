...
---------------------------------------------------------------------------*/
void cqr_calc_qb8x4x1s (void *pScr,complex_fract16 *_B, const complex_fract16 *_V, int L)
{
    ...
    sizeV = (SIZE_OF_V(8, 4) + SIZE_OF_FI(8)) * 2 * BBE_SIMD_WIDTH;
    vinc = 8 * 2 * BBE_SIMD_WIDTH;
    voff = 0;
    strideV = -vinc;
    strideR = -8 * 4 * L;
    ...
    __Pragma("no_reorder")
        ...
        _pr = (const xb_vecNx16 *)B;
    _pw = (xb_vecNx16 *)B;
    pFi = (const xb_vecNx16*)(V + BBE_SIMD_WIDTH*SIZE_OF_V(8, 4));
    strideR = -(8 - 1) * 4 * L + 2 * BBE_SIMD_WIDTH;
    strideV = (SIZE_OF_V(8, 4) + 1) * 2 * BBE_SIMD_WIDTH;
    for (l = 0; l < (L >> (LOG2_BBE_SIMD_WIDTH - 1)); l++)
    {
        xb_vecNx16 fi0, r0;
        xb_vecNx40 acc;
...
size_t cqr_calc_qb8x4x1s_getScratchSize (int M, int P, int L)
{
    ...
    return 0;
} /* cqr_calc_qb8x4x1s_getScratchSize() */
