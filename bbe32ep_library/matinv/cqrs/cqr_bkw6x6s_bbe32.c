    ...
    )
{
    ...
    xb_vecNx40 acc0;
    xb_vecNx16 x0, x1, x2, x3, x4, x5;
    int R_last_step0 = -4 * 6 * L;
    int R_last_step_inc = 4 * L;
    int X_last_step = (6 * 6 - 1) * 4 * L;
    int R_last_step;
    vsaN q11 = BBE_MOVVSA32(11);

    pR00 = (const xb_vecNx16 *)(R + (6 * 6 - 6 - 1)*L * 2);
    pX = (xb_vecNx16 *)(X + (6 * 6 - 1)*L * 2);
    ...
    for (k = 6 - 1; k >= 0; k--)
    {
        ...
        //m=3;
        {
            ...
            R_last_step += R_last_step_inc;
            dp = BBE_PACKVNX40(acc0, q11);
            BBE_LVNX16_XP(b, pB, -4 * 4 * L * 6 + X_last_step);
            acc0 = BBE_MULNX16(f0, b);
            acc0 = BBE_SRAINX40(acc0, 1);
            BBE_MULSNX16(acc0, f0, dp);
            ex = BBE_ADDSAVSN(-1, ex);
        ...
        BBE_SVNX16_XP(x5, pX, -4 * L * 6);
        BBE_SVNX16_XP(x4, pX, -4 * L * 6);
        BBE_SVNX16_XP(x3, pX, -4 * 4 * L * 6 + X_last_step);
    } //for(k=P-1; k>=0; k--)
    ...
    __Pragma("no_reorder") /* this pragma protects from memory spills due to the
                               ...
                               // m=2...0
                               pR00 = (const xb_vecNx16 *)(R + (6 * 6 - 6 - 1)*L * 2);
    pB = (const xb_vecNx16 *)(X + (6 * 6 - 1)*L * 2);
    pX = (xb_vecNx16 *)XT_ADD(-3 * 4 * L * 6, (uintptr_t)pB);
    pR00 = (const xb_vecNx16*)XT_ADD(2 * R_last_step0 + R_last_step_inc - 4 * L, (uintptr_t)pR00);
    R_last_step0 += 2 * R_last_step_inc;
    pf = (const xb_vecNx16  *)XT_ADD(-3 * 2 * BBE_SIMD_WIDTH, (uintptr_t)invDiag);
    for (k = 6 - 1; k >= 0; k--)
    {
        pR0 = pR00;
        R_last_step = R_last_step0;
        BBE_LVNX16_XP(x5, pB, -4 * L * 6);
        BBE_LVNX16_XP(x4, pB, -4 * L * 6);
        BBE_LVNX16_XP(x3, pB, -4 * L * 6);
        //m=2;
        {
            BBE_LVNX16_IP(f0, pf, -2 * BBE_SIMD_WIDTH);
            ex = BBE_SHFLNX16I(f0, BBE_SHFLI_DOUBLE_1_HI);
        ...
        //m=0;
        {
            ...
            R_last_step += R_last_step_inc;
            dp = BBE_PACKVNX40(acc0, q11);
            BBE_LVNX16_XP(b, pB, -4 * L * 6 + X_last_step);
            acc0 = BBE_MULNX16(f0, b);
            acc0 = BBE_SRAINX40(acc0, 1);
            BBE_MULSNX16(acc0, f0, dp);
            ex = BBE_ADDSAVSN(-1, ex);
        ...
        BBE_SVNX16_XP(x2, pX, -4 * L * 6);
        BBE_SVNX16_XP(x1, pX, -4 * L * 6);
        BBE_SVNX16_XP(x0, pX, -4 * 4 * L * 6 + X_last_step);
    } //for(k=P-1; k>=0; k--)
...
void cqr_bkw6x6s (void* pScr, complex_fract16* restrict _B, const complex_fract16* restrict _R, int L)
{
...
} /* cqr_bkw6x6s_getScratchSize() */
#else
DISCARD_FUN(void,cqr_bkw6x6s,(void* pScr, complex_fract16* restrict B,const complex_fract16* restrict R, int L))
size_t cqr_bkw6x6s_getScratchSize (int N, int P, int L) { (void)N;(void)P;(void)L; return 0; }
#endif
