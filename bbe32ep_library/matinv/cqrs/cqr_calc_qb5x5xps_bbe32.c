...
{
#define SIZE_OF_V5x5   14
#define SIZE_OF_FI5x5  5
    int p;
    int l;
    vsaN q = BBE_MOVVSA32(14);
    xb_vecNx16 vr, _0x4000 = 0x4000;
    xb_vecNx16 v0, v1, v2, v3, v4;
    xb_vecNx16 r0, r1, r2, r3, r4;
    xb_vecNx16 fi0, fi1, fi2, fi3, fi4;
    xb_vecNx40 acc;
    ...
    for (p = 0; p < P; p++)
    {
        ...
        _pw = (xb_vecNx16 *)B;
        // prologue of manual sw-pipeline
        BBE_LVNX16_XP(r0, _pr, 4 * L*P);
        BBE_LVNX16_XP(r1, _pr, 4 * L*P);
        BBE_LVNX16_XP(r2, _pr, 4 * L*P);
        BBE_LVNX16_XP(r3, _pr, 4 * L*P);
        BBE_LVNX16_XP(r4, _pr, -4 * 4 * L*P + 2 * BBE_SIMD_WIDTH);
        // 1-st column
        v1 = BBE_LVNX16_I(_pv, 1 * 2 * BBE_SIMD_WIDTH);
        v2 = BBE_LVNX16_I(_pv, 2 * 2 * BBE_SIMD_WIDTH);
        v3 = BBE_LVNX16_I(_pv, 3 * 2 * BBE_SIMD_WIDTH);
        v4 = BBE_LVNX16_I(_pv, 4 * 2 * BBE_SIMD_WIDTH);
        BBE_LVNX16_IP(v0, _pv, 5 * 2 * BBE_SIMD_WIDTH);
        for (l = 0; l < (L >> (LOG2_BBE_SIMD_WIDTH - 1)) - 1; l++)
        {
            ...
            acc = BBE_MULRNX16(r0, _0x4000, q); BBE_MULSNX16C(acc, v0, vr); fi0 = BBE_PACKVNX40(acc, q);
            ...
            // second column
            v2 = BBE_LVNX16_I(_pv, 1 * 2 * BBE_SIMD_WIDTH);
            v3 = BBE_LVNX16_I(_pv, 2 * 2 * BBE_SIMD_WIDTH);
            v4 = BBE_LVNX16_I(_pv, 3 * 2 * BBE_SIMD_WIDTH);
            BBE_LVNX16_XP(v1, _pv, (SIZE_OF_FI5x5 + 5 + 4) * 2 * BBE_SIMD_WIDTH);
            acc = BBE_MULRNX16J(r1, v1, q);
            BBE_MULANX16J(acc, r2, v2);
            BBE_MULANX16J(acc, r3, v3);
            BBE_MULANX16J(acc, r4, v4);
            vr = BBE_PACKVNX40(acc, q);
            acc = BBE_MULRNX16(r1, _0x4000, q); BBE_MULSNX16C(acc, v1, vr); fi1 = BBE_PACKVNX40(acc, q);
            acc = BBE_MULRNX16(r2, _0x4000, q); BBE_MULSNX16C(acc, v2, vr); fi2 = BBE_PACKVNX40(acc, q);
            acc = BBE_MULRNX16(r3, _0x4000, q); BBE_MULSNX16C(acc, v3, vr); fi3 = BBE_PACKVNX40(acc, q);
            acc = BBE_MULRNX16(r4, _0x4000, q); BBE_MULSNX16C(acc, v4, vr); fi4 = BBE_PACKVNX40(acc, q);
            // begin of the next iteration       
            BBE_LVNX16_XP(r0, _pr, 4 * L*P);
            BBE_LVNX16_XP(r1, _pr, 4 * L*P);
            BBE_LVNX16_XP(r2, _pr, 4 * L*P);
            BBE_LVNX16_XP(r3, _pr, 4 * L*P);
            ...
            // save results
            BBE_SVNX16_XP(fi0, _pw, 4 * L*P);
            BBE_SVNX16_XP(fi1, _pw, 4 * L*P);
            BBE_SVNX16_XP(fi2, _pw, 4 * L*P);
            BBE_SVNX16_XP(fi3, _pw, 4 * L*P);
            BBE_SVNX16_XP(fi4, _pw, -4 * 4 * L*P + 2 * BBE_SIMD_WIDTH);
        }
        // epilogue of sw pipelining 
        acc = BBE_MULRNX16J(r0, v0, q);
        BBE_MULANX16J(acc, r1, v1);
        BBE_MULANX16J(acc, r2, v2);
        BBE_MULANX16J(acc, r3, v3);
        ...
        // 3-rd and 4-th columns
        _pv = (const xb_vecNx16 *)(V + 9 * BBE_SIMD_WIDTH);
        _pr = (const xb_vecNx16 *)XT_ADDX2(4 * L*P, (uintptr_t)B);
        _pw = (xb_vecNx16 *)_pr;
        for (l = 0; l < (L >> (LOG2_BBE_SIMD_WIDTH - 1)); l++)
        {
            BBE_LVNX16_XP(r2, _pr, 4 * L*P);
            BBE_LVNX16_XP(r3, _pr, 4 * L*P);
            BBE_LVNX16_XP(r4, _pr, -2 * 4 * L*P + 2 * BBE_SIMD_WIDTH);
            // third column
            v3 = BBE_LVNX16_I(_pv, 1 * 2 * BBE_SIMD_WIDTH);
            v4 = BBE_LVNX16_I(_pv, 2 * 2 * BBE_SIMD_WIDTH);
            BBE_LVNX16_IP(v2, _pv, 3 * 2 * BBE_SIMD_WIDTH);
            ...
            acc = BBE_MULRNX16(r4, _0x4000, q); BBE_MULSNX16C(acc, v4, vr); r4 = BBE_PACKVNX40(acc, q);
            // 4-th column
            v4 = BBE_LVNX16_I(_pv, 1 * 2 * BBE_SIMD_WIDTH);
            BBE_LVNX16_XP(v3, _pv, (SIZE_OF_FI5x5 + 2 + 9) * 2 * BBE_SIMD_WIDTH);
            ...
            BBE_SVNX16_XP(r4, _pw, -2 * 4 * L*P + 2 * BBE_SIMD_WIDTH);
        }

        __Pragma("no_reorder")  // flush writing via _pw

            // final rotation
            _pr = (const xb_vecNx16 *)B;
        _pw = (xb_vecNx16 *)B;
        pFi = (const xb_vecNx16*)(V + BBE_SIMD_WIDTH*SIZE_OF_V5x5);
        for (l = 0; l < (L >> (LOG2_BBE_SIMD_WIDTH - 1)); l++)
        {
            ...
            BBE_LVNX16_XP(fi4, pFi, (SIZE_OF_V5x5 + 1) * 2 * BBE_SIMD_WIDTH);
            BBE_LVNX16_XP(r4, _pr, -4 * 4 * L*P + 2 * BBE_SIMD_WIDTH);
            acc = BBE_MULRNX16J(r4, fi4, q);
            r4 = BBE_PACKVNX40(acc, q);
            BBE_SVNX16_XP(r4, _pw, -4 * 4 * L*P + 2 * BBE_SIMD_WIDTH);
        }
