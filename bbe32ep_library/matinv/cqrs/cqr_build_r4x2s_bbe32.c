...
DISCARD_FUN(void, cqr_build_r4x2s, ( void* pScr,complex_fract16 * restrict V, complex_fract16 * restrict R, int L))
#else
// final rotation of matrices R(M,N)xL
static void cqrRotateR4x2(int16_t* R, const int16_t* pFi, int L)
{
    ...
    strideFi = 2 * BBE_SIMD_WIDTH*(SIZE_OF_V(4, 2) + 1);
    strideR = -(4 * 2 - 1) * 4 * L + 2 * BBE_SIMD_WIDTH;
...
-------------------------------------------------------------------------*/void cqr_build_r4x2s  ( void* pScr, complex_fract16 * restrict _V, complex_fract16 * restrict _R, int L)
{
    ...
    (void)pScr;
    pFi = V + BBE_SIMD_WIDTH* SIZE_OF_V(4, 2);  pR = R; pV = V;
    cqrHouseholder4(pR, pV, pFi, 4, 2, L);
    cqrUpdateR4(pR, pV, 0, 4, 2, L);
    pFi += BBE_SIMD_WIDTH; pV += BBE_SIMD_WIDTH * 4; pR += (2 + 1)*L * 2;
    cqrHouseholder3(pR, pV, pFi, 4, 2, L);
    cqrUpdateR3(pR, pV, 1, 4, 2, L);
    // final rotation
    pFi = V + BBE_SIMD_WIDTH* SIZE_OF_V(4, 2);
    cqrRotateR4x2(R, pFi, L);
} /* cqr_build_r4x2s() */
#endif
...
size_t cqr_build_r4x2s_getScratchSize (int M, int N,int L)
{
    ...
    return 0;
} /* cqr_build_r4x2s_getScratchSize() */
