...
static void cqrRotateR16x16(int16_t* R, const int16_t* pFi, int L)
{
    vsaN q14 = BBE_MOVVSA32(14);
    int k, i;
    xb_vecNx16  fi, r;
    ...
    for (k = i = 0; i < 16 * (L >> (LOG2_BBE_SIMD_WIDTH - 1)); i++)
    {
        ...
        strideR = 4 * L;
        XT_MOVEQZ(strideFi, 2 * BBE_SIMD_WIDTH*(SIZE_OF_V(16, 16) + 1), k);
        XT_MOVEQZ(strideR, -(16 * 16 - 1) * 4 * L + 2 * BBE_SIMD_WIDTH, k);
...
-------------------------------------------------------------------------*/
void cqr_build_r16x16s  ( void* pScr, complex_fract16 * restrict _V, complex_fract16 * restrict _R, int L)
{
    ...
    pFi = V + BBE_SIMD_WIDTH* SIZE_OF_V(16, 16);
    pR = R;
    pV = V;
    for (m = 0; m < 3; m++)
    {
        cqrHouseholder(pScr, pR, pV, pFi, 16, 16, (16 - m), L);
        cqrUpdateR(pR, pV, m, 16, 16, (16 - m), L);
        pFi += BBE_SIMD_WIDTH;
        pV += BBE_SIMD_WIDTH*(16 - m);
        pR += (16 + 1)*L * 2;
    }
    // use dedicated cqrUpdateR functions
    cqrHouseholder(pScr, pR, pV, pFi, 16, 16, 13, L);
    cqrUpdateR13(pR, pV, 3, 16, 16, L);
    pFi += BBE_SIMD_WIDTH; pV += BBE_SIMD_WIDTH * 13; pR += (16 + 1)*L * 2;
    cqrHouseholder(pScr, pR, pV, pFi, 16, 16, 12, L);
    cqrUpdateR12(pR, pV, 4, 16, 16, L);
    pFi += BBE_SIMD_WIDTH; pV += BBE_SIMD_WIDTH * 12; pR += (16 + 1)*L * 2;
    cqrHouseholder(pScr, pR, pV, pFi, 16, 16, 11, L);
    cqrUpdateR11(pR, pV, 5, 16, 16, L);
    pFi += BBE_SIMD_WIDTH; pV += BBE_SIMD_WIDTH * 11; pR += (16 + 1)*L * 2;
    cqrHouseholder(pScr, pR, pV, pFi, 16, 16, 10, L);
    cqrUpdateR10(pR, pV, 6, 16, 16, L);
    pFi += BBE_SIMD_WIDTH; pV += BBE_SIMD_WIDTH * 10; pR += (16 + 1)*L * 2;
    cqrHouseholder(pScr, pR, pV, pFi, 16, 16, 9, L);
    cqrUpdateR9(pR, pV, 7, 16, 16, L);
    pFi += BBE_SIMD_WIDTH; pV += BBE_SIMD_WIDTH * 9; pR += (16 + 1)*L * 2;
    // use dedicated cqrHouseholder/cqrUpdateR functions
    cqrHouseholder8(pScr, pR, pV, pFi, 16, 16, L);
    cqrUpdateR8(pR, pV, 8, 16, 16, L);
    pFi += BBE_SIMD_WIDTH; pV += BBE_SIMD_WIDTH * 8; pR += (16 + 1)*L * 2;
    cqrHouseholder7(pScr, pR, pV, pFi, 16, 16, L);
    cqrUpdateR7(pR, pV, 9, 16, 16, L);
    pFi += BBE_SIMD_WIDTH; pV += BBE_SIMD_WIDTH * 7; pR += (16 + 1)*L * 2;
    cqrHouseholder6(pScr, pR, pV, pFi, 16, 16, L);
    cqrUpdateR6(pR, pV, 10, 16, 16, L);
    pFi += BBE_SIMD_WIDTH; pV += BBE_SIMD_WIDTH * 6; pR += (16 + 1)*L * 2;
    cqrHouseholder5(pScr, pR, pV, pFi, 16, 16, L);
    cqrUpdateR5(pR, pV, 11, 16, 16, L);
    pFi += BBE_SIMD_WIDTH; pV += BBE_SIMD_WIDTH * 5; pR += (16 + 1)*L * 2;
    cqrHouseholder4(pR, pV, pFi, 16, 16, L);
    cqrUpdateR4(pR, pV, 12, 16, 16, L);
    pFi += BBE_SIMD_WIDTH; pV += BBE_SIMD_WIDTH * 4; pR += (16 + 1)*L * 2;
    cqrHouseholder3(pR, pV, pFi, 16, 16, L);
    cqrUpdateR3(pR, pV, 13, 16, 16, L);
    pFi += BBE_SIMD_WIDTH; pV += BBE_SIMD_WIDTH * 3; pR += (16 + 1)*L * 2;
    cqrHouseholder2(pR, pV, pFi, 16, 16, L);
    cqrUpdateR2(pR, pV, 14, 16, 16, L);
    // for square matrices we have to calculate last rotation explicitely
    cqrComputeLastRot(V + BBE_SIMD_WIDTH* SIZE_OF_V(16, 16), R, 16, L);
    // final rotation
    pFi = V + BBE_SIMD_WIDTH* SIZE_OF_V(16, 16);
    cqrRotateR16x16(R, pFi, L);
}
...
size_t cqr_build_r16x16s_getScratchSize (int M, int N,int L)
{
    (void)M; (void)N;
    return 4 * L;
} /* cqr_build_r16x16s_getScratchSize() */
#else
DISCARD_FUN(void, cqr_build_r16x16s, ( void* pScr,complex_fract16 * restrict V, complex_fract16 * restrict R, int L))
size_t cqr_build_r16x16s_getScratchSize (int M,int N,int L) { (void)M;(void)N;(void)L; return 0; }
#endif
