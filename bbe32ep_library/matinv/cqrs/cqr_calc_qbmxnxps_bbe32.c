...
---------------------------------------------------------------------------*/
void cqr_calc_qbmxnxps (void *pScr, complex_fract16 *_B, const complex_fract16 *_V , int M, int N, int P, int L)
{
    ...
    for (p = 0; p < P; p++)
    {
        ...
        {
            const int16_t *v = V + l*BBE_SIMD_WIDTH*(SIZE_OF_V(M, N) + SIZE_OF_FI(M));
            for (m = 0; m < Ncolumns; m++)
            {
                ...
                for (i = 1; i < M - m; i++)
                {
                    BBE_LVNX16_XP(r0, _pr, 4 * L*P);
                    BBE_LVNX16_IP(v0, _pv, 2 * BBE_SIMD_WIDTH);
                    BBE_MULANX16J(acc, r0, v0);
                }
                vr = BBE_PACKVNX40(acc, q);
                _pv = (const xb_vecNx16 *)v;
                _pr = (const xb_vecNx16*)_pw;
                //                __Pragma("loop_count min=2");
                __Pragma("super_swp ii=5,unroll=2")
                    for (i = 0; i < M - m; i++)
                    {
                        BBE_LVNX16_XP(r0, _pr, 4 * L*P);
                        BBE_LVNX16_IP(v0, _pv, 2 * BBE_SIMD_WIDTH);
                ...
                __Pragma("no_reorder")
                    v += BBE_SIMD_WIDTH*(M - m);
                b = (int16_t*)XT_ADD((uintptr_t)b, 4 * L*P);
            }
            b -= Ncolumns * 2 * L*P - BBE_SIMD_WIDTH;
        }
        // final rotation
        _pr = (const xb_vecNx16 *)B;
        _pw = (xb_vecNx16 *)B;
        pFi = (const xb_vecNx16*)(V + BBE_SIMD_WIDTH*SIZE_OF_V(M, N));
        for (l = 0; l < (L >> (LOG2_BBE_SIMD_WIDTH - 1)); l++)
        {
            xb_vecNx16 fi0, r0;
            xb_vecNx40 acc;
            //            __Pragma("loop_count min=2");
            __Pragma("super_swp ii=4,unroll=2")
                for (m = 0; m < M; m++)
                {
            ...
            pFi = (const xb_vecNx16*)XT_ADD((uintptr_t)pFi, (SIZE_OF_V(M, N) * 2 * BBE_SIMD_WIDTH));
            _pr = (const xb_vecNx16*)XT_ADD((uintptr_t)_pr, -M * 4 * L*P + 2 * BBE_SIMD_WIDTH);
            _pw = (xb_vecNx16*)XT_ADD((uintptr_t)_pw, -M * 4 * L*P + 2 * BBE_SIMD_WIDTH);
        }
        B += 2 * L;
    }
} /* cqr_calc_qbmxnxps() */
...
size_t cqr_calc_qbmxnxps_getScratchSize (int M, int P, int L)
{
    ...
    return 0;
} /* cqr_calc_qbmxnxps_getScratchSize() */
