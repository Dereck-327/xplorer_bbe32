...
#if 0
ALIGN(32)  static int16_t test_buf[16]; 

#define TEST16(x) \
{                                                    \
    int i;                                              \
    BBE_SVNX16_I(x, (xb_vecNx16*)test_buf, 0);          \
    for(i=0; i<16; i++)                                 \
    {                                                   \
        ASSERT(test_buf[i]<MAX_INT16);                    \
        ASSERT(test_buf[i]>MIN_INT16);                    \
    }                                                   \
}
#endif
...
static void SolveRxP_M16_P16x8(
                               int16_t *X,
                               const int16_t *R,
                               int16_t *QB,
                               int L,
...
static void SolveRxP_M16_P16x8_last8(
                               int16_t *X,
                               const int16_t *R,
                               int16_t *QB,
                               int L,
...
static void UpdateB_M16_P16x8(int16_t *X,
                              const int16_t *R,
                              int16_t *QB,
                              int16_t *exp,
                              int16_t *invDiagR,
                              ...
                              int _end_m)
{
    ...
    int m;
    xb_vecNx40 acc0;
    xb_vecNx16 * restrict pe = (xb_vecNx16 *) (exp      +  (start_m)*BBE_SIMD_WIDTH);
    xb_vecNx16 * restrict pf = (xb_vecNx16 *) (invDiagR +  (start_m)*BBE_SIMD_WIDTH);
    xb_vecNx16 b;
    xb_vecNx16 *pe_end = pe, *pf_end = pf;
    ...
    for(k=CURRENT_P-1; k>=0; k--)
    {
        ...
        for(m=start_m; m>=end_m; m--)
        {
            ...
            //for(n=CURRENT_M-1; n>start_m; n--)//for( n = start_m+1; n<CURRENT_M; n++)
            {
...
void cqr_bkw16x16s (void* pScr, complex_fract16* restrict _B, const complex_fract16* restrict _R, int L)
{
          int16_t* restrict B=(      int16_t*)_B;
    const int16_t* restrict R=(const int16_t*)_R;
#if 0
    return cqr_bkwnxps(pScr, B, R, 16, 16, L); 
#else
    const int q = 10;
    int i;
    ...
    for(i=0; i<L; i+=(BBE_SIMD_WIDTH/2))
    {
        ...
        UpdateB_M16_P16x8(X, R, QB, exp, invDiagR, CURRENT_M, CURRENT_P, L, 10, CURRENT_M/2-1, 0);
...
} /* cqr_bkw16x16s_getScratchSize() */
#else
DISCARD_FUN(void,cqr_bkw16x16s,(void* pScr, complex_fract16* restrict B,const complex_fract16* restrict R, int L))
size_t cqr_bkw16x16s_getScratchSize (int N, int P, int L) { (void)N;(void)P;(void)L; return 0; }
#endif
