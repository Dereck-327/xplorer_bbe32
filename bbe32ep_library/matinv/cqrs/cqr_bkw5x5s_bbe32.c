    ...
    )
{
    ...
    int R_last_step0 = -4 * 5 * L;
    int R_last_step_inc = 4 * L;
    int X_last_step = (5 * 5 - 1) * 4 * L;
    int R_last_step;
    pR00 = (const xb_vecNx16 *)(R + (5 * 5 - 5 - 1)*L * 2);
    pX = (xb_vecNx16 *)(X + (5 * 5 - 1)*L * 2);
    ...
    for (k = 5 - 1; k >= 0; k--)
    {
        ...
        //m=2;
        {
            ...
            R_last_step += R_last_step_inc;
            dp = BBE_PACKVNX40(acc0, q11);
            BBE_LVNX16_XP(b, pB, -3 * 4 * L * 5 + X_last_step);
            acc0 = BBE_MULNX16(f0, b);
            acc0 = BBE_SRAINX40(acc0, 1);
            BBE_MULSNX16(acc0, f0, dp);
            ex = BBE_ADDSAVSN(-1, ex);
        ...
        BBE_SVNX16_XP(x4, pX, -4 * L * 5);
        BBE_SVNX16_XP(x3, pX, -4 * L * 5);
        BBE_SVNX16_XP(x2, pX, -3 * 4 * L * 5 + X_last_step);
    } //for(k=P-1; k>=0; k--)
    ...
    __Pragma("no_reorder") /* this pragma protects from memory spills due to the
                               ...
                               loops */
                               // m=1..0
                               pB = (xb_vecNx16 *)XT_ADD((5 * 5 - 1)*L * 4, (uintptr_t)X);
    pX = (xb_vecNx16 *)XT_ADD(-3 * 4 * L * 5, (uintptr_t)pB);
    pR00 = (const xb_vecNx16*)XT_ADD(2 * R_last_step0 + R_last_step_inc - 4 * L, (uintptr_t)pR00);
    R_last_step0 += 2 * R_last_step_inc;
    invDiag = (const xb_vecNx16  *)XT_ADD(-3 * 2 * BBE_SIMD_WIDTH, (uintptr_t)invDiag);
    pf = invDiag;
    for (k = 5 - 1; k >= 0; k--)
    {
        pR0 = pR00;
        R_last_step = R_last_step0;
        BBE_LVNX16_XP(x4, pB, -4 * L * 5);
        BBE_LVNX16_XP(x3, pB, -4 * L * 5);
        BBE_LVNX16_XP(x2, pB, -4 * L * 5);
        //m=1;
        {
            BBE_LVNX16_IP(f0, pf, -2 * BBE_SIMD_WIDTH);
            ex = BBE_SHFLNX16I(f0, BBE_SHFLI_DOUBLE_1_HI);
        ...
        // m=0
        {
            ...
            R_last_step += R_last_step_inc;
            dp = BBE_PACKVNX40(acc0, q11);
            BBE_LVNX16_XP(b, pB, -4 * L * 5 + X_last_step);
            acc0 = BBE_MULNX16(f0, b);
            acc0 = BBE_SRAINX40(acc0, 1);
            BBE_MULSNX16(acc0, f0, dp);
            ex = BBE_ADDSAVSN(-1, ex);
        ...
        BBE_SVNX16_XP(x1, pX, -4 * L * 5);
        BBE_SVNX16_XP(x0, pX, -4 * 4 * L * 5 + X_last_step);
    } //for(k=P-1; k>=0; k--)
}
...
void cqr_bkw5x5s (void* pScr, complex_fract16* restrict _B, const complex_fract16* restrict _R, int L)
{
...
} /* cqr_bkw5x5s_getScratchSize() */
#else
DISCARD_FUN(void,cqr_bkw5x5s , (void* pScr, complex_fract16* restrict B,const complex_fract16* restrict R, int L))
size_t cqr_bkw5x5s_getScratchSize (int N, int P, int L) { (void)N;(void)P;(void)L; return 0; }
#endif
