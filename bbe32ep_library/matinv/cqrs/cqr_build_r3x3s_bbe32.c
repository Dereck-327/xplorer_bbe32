...
DISCARD_FUN(void, cqr_build_r3x3s, ( void* pScr,complex_fract16 * restrict V, complex_fract16 * restrict R, int L))
#else

// final rotation of matrices R(M,N)xL
static void cqrRotateR3x3(int16_t* R, const int16_t* pFi, int L)
{
    ...
    strideFi = 2 * BBE_SIMD_WIDTH*(SIZE_OF_V(3, 3) + 1);
    strideR = -(3 * 3 - 1) * 4 * L + 2 * BBE_SIMD_WIDTH;
...
-------------------------------------------------------------------------*/
void cqr_build_r3x3s ( void* pScr, complex_fract16 * restrict _V, complex_fract16 * restrict _R, int L)
{
    ...
    pFi = V + BBE_SIMD_WIDTH* SIZE_OF_V(3, 3);  pR = R; pV = V;
    cqrHouseholder3(pR, pV, pFi, 3, 3, L);
    cqrUpdateR3(pR, pV, 0, 3, 3, L);
    pFi += BBE_SIMD_WIDTH; pV += BBE_SIMD_WIDTH * 3; pR += (3 + 1)*L * 2;
    cqrHouseholder2(pR, pV, pFi, 3, 3, L);
    cqrUpdateR2(pR, pV, 1, 3, 3, L);
    // for square matrices we have to calculate last rotation explicitely
    cqrComputeLastRot(V + BBE_SIMD_WIDTH* SIZE_OF_V(3, 3), R, 3, L);
    // final rotation
    pFi = V + BBE_SIMD_WIDTH* SIZE_OF_V(3, 3);
    cqrRotateR3x3(R, pFi, L);
} /* cqr_build_r3x3s() */
#endif
...
size_t cqr_build_r3x3s_getScratchSize (int M, int N,int L)
{
    ...
    return 0;
} /* cqr_build_r3x3s_getScratchSize() */
