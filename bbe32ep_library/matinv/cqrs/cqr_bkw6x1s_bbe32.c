...
void cqr_bkw6x1s (void* pScr, complex_fract16* restrict _B, const complex_fract16* restrict _R, int L)
{
    ...
    invDiagR = (xb_vecNx16*)pScr;
    {
        ...
        for (i = k = 0; k < 6 * (L / (BBE_SIMD_WIDTH / 2)); k++)
        {
            i = BBE_ADDMOD16U(i, (6 << 16) | 1);
            stride = (6 + 1) * 4 * L;
            XT_MOVEQZ(stride, -(6 - 1)*(6 + 1) * 4 * L + 2 * BBE_SIMD_WIDTH, i);
    ...
    for (l = 0; l < L; l += BBE_SIMD_WIDTH / 2)
    {
        ...
        BBE_SVNX16_XP(x3, pX, 4 * L * 2 + 2 * BBE_SIMD_WIDTH);
        BBE_LVNX16_IP(x0, pR00, 2 * BBE_SIMD_WIDTH);//        pR00++;
    }
    __Pragma("no_reorder")
        invDiagR = (xb_vecNx16*)XT_ADD(2 * BBE_SIMD_WIDTH * 2, (uintptr_t)pScr);
    pB = (const xb_vecNx16 *)(B + (6 - 1)*L * 2);
    pX = (xb_vecNx16 *)pB;
    pR00 = (const xb_vecNx16 *)(R + (6 * 6 - 6 - 1 - 12)*L * 2);
    pX = (xb_vecNx16 *)XT_ADD(-4 * L * 3, (uintptr_t)pX);
    for (l = 0; l < L; l += BBE_SIMD_WIDTH / 2)
    {
        pR = pR00;
        R_last_step = -4 * 4 * L;
        ...
        //m=0;
        {
            ...
            R_last_step += R_last_step_inc;
            dp = BBE_PACKVNX40(acc0, q11);
            BBE_LVNX16_XP(b, pB, 4 * L * 5 + 2 * BBE_SIMD_WIDTH);
            acc0 = BBE_MULNX16(f0, b);
            acc0 = BBE_SRAINX40(acc0, 1);
            BBE_MULSNX16(acc0, f0, dp);
            ex = BBE_ADDSAVSN(-1, ex);
        ...
        BBE_SVNX16_XP(x0, pX, 4 * L*(5 - 3) + 2 * BBE_SIMD_WIDTH);
        BBE_LVNX16_IP(x0, pR00, 2 * BBE_SIMD_WIDTH);//        pR00++;
    }
} /* cqr_bkw6x1s() */
...
size_t cqr_bkw6x1s_getScratchSize (int N, int P, int L)
{
    (void)P;
    return 4 * N*L;
} /* cqr_bkw6x1s_getScratchSize() */
#else
DISCARD_FUN(void,cqr_bkw6x1s,(void* pScr, complex_fract16* restrict B,const complex_fract16* restrict R, int L))
size_t cqr_bkw6x1s_getScratchSize (int N, int P, int L) { (void)N;(void)P;(void)L; return 0; }
#endif
