...
void cqr_bkw7x1s (void* pScr, complex_fract16* restrict _B, const complex_fract16* restrict _R, int L)
{
    ...
    invDiagR = (xb_vecNx16*)pScr;
    {
        ...
        for (i = k = 0; k < 7 * (L / (BBE_SIMD_WIDTH / 2)); k++)
        {
            i = BBE_ADDMOD16U(i, (7 << 16) | 1);
            stride = (7 + 1) * 4 * L;
            XT_MOVEQZ(stride, -(7 - 1)*(7 + 1) * 4 * L + 2 * BBE_SIMD_WIDTH, i);
    ...
    // m=6...4
    invDiagR = (xb_vecNx16*)XT_ADD(2 * BBE_SIMD_WIDTH * 7, (uintptr_t)pScr);
    invDiagR--;
    pX = (xb_vecNx16 *)(B + (7 - 1)*L * 2);
    ...
    for (l = 0; l < L; l += BBE_SIMD_WIDTH / 2)
    {
        ...
        //m=4;
        {
            BBE_LVNX16_IP(f0, invDiagR, 9 * 2 * BBE_SIMD_WIDTH);
            ex = BBE_SHFLNX16I(f0, BBE_SHFLI_DOUBLE_1_HI);
            f0 = BBE_SHFLNX16I(f0, BBE_SHFLI_DOUBLE_1_LO);
            ...
            R_last_step += R_last_step_inc;
            dp = BBE_PACKVNX40(acc0, q11);
            BBE_LVNX16_XP(b, pB, 2 * L * 4 + 2 * BBE_SIMD_WIDTH);
            acc0 = BBE_MULNX16(f0, b);
            acc0 = BBE_SRAINX40(acc0, 1);
            BBE_MULSNX16(acc0, f0, dp);
            ex = BBE_ADDSAVSN(-1, ex);
        ...
        BBE_SVNX16_XP(x6, pX, -4 * L);
        BBE_SVNX16_XP(x5, pX, -4 * L);
        BBE_SVNX16_XP(x4, pX, 2 * L * 4 + 2 * BBE_SIMD_WIDTH);
        BBE_LVNX16_IP(x0, pR00, 2 * BBE_SIMD_WIDTH); //pR00++;
    }
    // m=3...2
    __Pragma("no_reorder")
        invDiagR = (xb_vecNx16*)XT_ADD(2 * BBE_SIMD_WIDTH*(7 - 3), (uintptr_t)pScr);
    invDiagR--;
    pB = (xb_vecNx16 *)(B + (7 - 1)*L * 2);
    pX = (xb_vecNx16 *)XT_ADD(-4 * L * 4, (uintptr_t)pB);
    pR00 = (const xb_vecNx16 *)(R + (7 * 7 - 7 - 1 - 1 - 13)*L * 2);
    for (l = 0; l < L; l += BBE_SIMD_WIDTH / 2)
    {
        pR = pR00;
        R_last_step = -4 * 5 * L;
        BBE_LVNX16_XP(x6, pB, -4 * L);
        BBE_LVNX16_XP(x5, pB, -4 * L);
        ...
        //m=2;
        {
            BBE_LVNX16_IP(f0, invDiagR, 8 * 2 * BBE_SIMD_WIDTH);
            ex = BBE_SHFLNX16I(f0, BBE_SHFLI_DOUBLE_1_HI);
            f0 = BBE_SHFLNX16I(f0, BBE_SHFLI_DOUBLE_1_LO);
            BBE_LVNX16_XP(r0, pR, -4 * L);
            acc0 = BBE_MULRNX16C(x6, r0, q11);
        ...
        BBE_SVNX16_X(x3, pX, 4 * L);
        BBE_SVNX16_IP(x2, pX, 2 * BBE_SIMD_WIDTH);
        BBE_LVNX16_IP(x0, pR00, 2 * BBE_SIMD_WIDTH); //pR00++;
    }
    // m=1...0
    __Pragma("no_reorder")
        invDiagR = (xb_vecNx16*)XT_ADD(2 * BBE_SIMD_WIDTH*(7 - 6), (uintptr_t)pScr);
    pB = (const xb_vecNx16 *)(B + (7 - 1)*L * 2);
    pX = (xb_vecNx16 *)XT_ADD(-4 * L * 6, (uintptr_t)pB);
    pR00 = (const xb_vecNx16 *)(R + (7 * 7 - 7 - 1 - 6 - 22)*L * 2);
    for (l = 0; l < L; l += BBE_SIMD_WIDTH / 2)
    {
        pR = pR00;
        R_last_step = -4 * 3 * L;
        ...
        //m=0;
        {
            ...
            R_last_step += R_last_step_inc;
            dp = BBE_PACKVNX40(acc0, q11);
            BBE_LVNX16_XP(b, pB, 4 * L * 6 + 2 * BBE_SIMD_WIDTH);
            acc0 = BBE_MULNX16(f0, b);
            acc0 = BBE_SRAINX40(acc0, 1);
            BBE_MULSNX16(acc0, f0, dp);
            ex = BBE_ADDSAVSN(-1, ex);
...
size_t cqr_bkw7x1s_getScratchSize (int N, int P, int L)
{
    (void)P;
    return 4 * N*L;
} /* cqr_bkw7x1s_getScratchSize() */
#else
DISCARD_FUN(void,cqr_bkw7x1s,(void* pScr, complex_fract16* restrict B,const complex_fract16* restrict R, int L))
size_t cqr_bkw7x1s_getScratchSize (int N, int P, int L) { (void)P; return 0; }
#endif
