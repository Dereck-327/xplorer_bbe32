    ...
    )
{
    ...
    const int R_last_step0 = -4 * 3 * L;
    const int R_last_step_inc = L;
    const int X_last_step = (3 * 3 - 1) * 4 * L;
    int R_last_step;
    pR00 = (const xb_vecNx16 *)(R + (3 * 3 - 3 - 1)*L * 2);
    pX = (xb_vecNx16 *)(X + (3 * 3 - 1)*L * 2);
...
//    __Pragma("concurrent")
        for (k = 3 - 1; k >= 0; k--)
        {
            ...
            R_last_step = XT_ADDX4(R_last_step_inc, R_last_step);
            ...
            R_last_step = XT_ADDX4(R_last_step_inc, R_last_step);
            dp = BBE_PACKVNX40(acc0, q11);
            BBE_LVNX16_XP(b, pB, -4 * L * 3 + X_last_step);
            acc0 = BBE_MULNX16(f0, b);
            acc0 = BBE_SRAINX40(acc0, 1);
            BBE_MULSNX16(acc0, f0, dp);
            ex = BBE_ADDSAVSN(-1, ex);
            ...
            BBE_SVNX16_XP(x2, pX, -4 * L * 3);
            BBE_SVNX16_XP(x1, pX, -4 * L * 3);
            BBE_SVNX16_XP(x0, pX, -4 * L * 3 + X_last_step);
        } //for(k=P-1; k>=0; k--)
}
...
void cqr_bkw3x3s (void* pScr, complex_fract16* restrict _B, const complex_fract16* restrict _R, int L)
{
    ...
    for (i = 0; i < L; i += BBE_SIMD_WIDTH / 2)
    {
        ...
        QUO8X32(f0, _0x20000000, r0);
        f0 = BBE_SELNX16I(e0, f0, BBE_SELI_EXTRACT_1_OF_2_OFF_0);
        BBE_SVNX16_IP(f0, invDiagR, 0);

        __Solve3x3(X, R, L, invDiagR);
...
} /* cqr_bkw3x3s_getScratchSize() */
#else
DISCARD_FUN(void,cqr_bkw3x3s,(void* pScr, complex_fract16* restrict B,const complex_fract16* restrict R, int L))
size_t cqr_bkw3x3s_getScratchSize (int N, int P, int L) { return (0); }
#endif
