...
---------------------------------------------------------------------------*/
void cqr_calc_qb3x3x3s (void *pScr,complex_fract16 *_B, const complex_fract16 *_V, int L)
{
    ...
    for (i = 0; i < P; i++)
    {
        ...
        for (l = 0; l < (L>>(LOG2_BBE_SIMD_WIDTH-1)); l++)
        {
            ...
            BBE_LVNX16_XP(r2, _pr, -2 * 4 * L*P + 2 * BBE_SIMD_WIDTH);
            ...
            r2 = BBE_PACKVNX40(acc, q);
            BBE_SVNX16_XP(r2, _pw, -2 * 4 * L*P + 2 * BBE_SIMD_WIDTH);
...
size_t cqr_calc_qb3x3x3s_getScratchSize (int M, int P, int L)
{
    ...
    return 0;
} /* cqr_calc_qb3x3x3s_getScratchSize() */
