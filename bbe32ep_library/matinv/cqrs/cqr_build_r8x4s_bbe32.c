...
static void cqrRotateR8x4(int16_t* R, const int16_t* pFi, int L)
{
    ...
    for (k = i = 0; i < 2 * (L >> (LOG2_BBE_SIMD_WIDTH - 1)); i++)
    {
        ...
        strideR = 4 * L;
        XT_MOVEQZ(strideFi, 2 * BBE_SIMD_WIDTH*(SIZE_OF_V(8, 4) + 1), k);
        XT_MOVEQZ(strideR, -(8 * 4 - 1) * 4 * L + 2 * BBE_SIMD_WIDTH, k);
...
-------------------------------------------------------------------------*/
void cqr_build_r8x4s  ( void* pScr, complex_fract16 * restrict _V, complex_fract16 * restrict _R, int L)
{
    ...
    pFi = V + BBE_SIMD_WIDTH* SIZE_OF_V(8, 4);  pR = R; pV = V;
    cqrHouseholder8(pScr, pR, pV, pFi, 8, 4, L);
    cqrUpdateR8(pR, pV, 0, 8, 4, L);
    pFi += BBE_SIMD_WIDTH; pV += BBE_SIMD_WIDTH * 8; pR += (4 + 1)*L * 2;
    cqrHouseholder7(pScr, pR, pV, pFi, 8, 4, L);
    cqrUpdateR7(pR, pV, 1, 8, 4, L);
    pFi += BBE_SIMD_WIDTH; pV += BBE_SIMD_WIDTH * 7; pR += (4 + 1)*L * 2;
    cqrHouseholder6(pScr, pR, pV, pFi, 8, 4, L);
    cqrUpdateR6(pR, pV, 2, 8, 4, L);
    pFi += BBE_SIMD_WIDTH; pV += BBE_SIMD_WIDTH * 6; pR += (4 + 1)*L * 2;
    cqrHouseholder5(pScr, pR, pV, pFi, 8, 4, L);
    cqrUpdateR5(pR, pV, 3, 8, 4, L);
    // final rotation
    pFi = V + BBE_SIMD_WIDTH* SIZE_OF_V(8, 4);
    cqrRotateR8x4(R, pFi, L);
} 
...
size_t cqr_build_r8x4s_getScratchSize (int M, int N,int L)
{
    (void)M; (void)N;
    return 4 * L;
} /* cqr_build_r8x4s_getScratchSize() */
#else
DISCARD_FUN(void, cqr_build_r8x4s, ( void* pScr,complex_fract16 * restrict V, complex_fract16 * restrict R, int L))
size_t cqr_build_r8x4s_getScratchSize (int M, int N, int L) { (void)M;(void)N;(void)L; return 0; }
#endif
