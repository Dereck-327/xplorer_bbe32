...
/*
  NatureDSP_Baseband library. QR-based matrix decomposition and inversion for streaming order
    helper routines for Housholder algorithm, len_x==9
...
DISCARD_FUN(void,cqrUpdateR9,(  int16_t* restrict R,
    ...
    int m, int M, int N, int L)
{
    ...
    xb_vecNx40 acc;
    xb_vecNx16 r0, r1, r2, r3, r4, r5, r6, r7, r8, v0, v1, v2, v3, v4, v5, v6, v7, v8, vr, _0x4000 = 0x4000;
    ...
    for (j = l = 0; l < (N - m)*((L >> (LOG2_BBE_SIMD_WIDTH - 1))); l++)
    {
        ...
        strideV = 2 * BBE_SIMD_WIDTH*(sizeV - 8);
        XT_MOVEQZ(strideV, 2 * BBE_SIMD_WIDTH*(sizeV - 8) - 4 * L*sizeV, j);
        ...
        BBE_LVNX16_XP(r6, pr, 4 * N*L);
        BBE_LVNX16_XP(r7, pr, 4 * N*L);
        BBE_LVNX16_XP(r8, pr, 2 * BBE_SIMD_WIDTH - 8 * 4 * N*L);
        ...
        BBE_SVNX16_XP(r8, pw, 2 * BBE_SIMD_WIDTH - 8 * 4 * N*L);
    }
}
#endif
