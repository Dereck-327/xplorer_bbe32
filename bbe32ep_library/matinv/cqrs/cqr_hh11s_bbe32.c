...
/*
  NatureDSP_Baseband library. QR-based matrix decomposition and inversion for streaming order
    helper routines for Housholder algorithm, len_x==11
...
DISCARD_FUN(void,cqrUpdateR11,(  int16_t* restrict R,
    ...
    int m, int M, int N, int L)
{
    ...
    xb_vecNx40 acc;
    xb_vecNx16 r0, r1, r2, r3, r4, r5, r6, r7, r8, r9, ra, v0, v1, v2, v3, v4, v5, v6, v7, v8, v9, va, vr, _0x4000 = 0x4000;
    ...
    for (j = l = 0; l < (N - m)*((L >> (LOG2_BBE_SIMD_WIDTH - 1))); l++)
    {
        ...
        strideV = 2 * BBE_SIMD_WIDTH*(sizeV - 10);
        XT_MOVEQZ(strideV, 2 * BBE_SIMD_WIDTH*(sizeV - 10) - 4 * L*sizeV, j);
        ...
        v7 = BBE_LVNX16_I(pv, 0 * 2 * BBE_SIMD_WIDTH);
        v8 = BBE_LVNX16_I(pv, 1 * 2 * BBE_SIMD_WIDTH);
        v9 = BBE_LVNX16_I(pv, 2 * 2 * BBE_SIMD_WIDTH);
        va = BBE_LVNX16_I(pv, 3 * 2 * BBE_SIMD_WIDTH);
        ...
        BBE_SVNX16_XP(ra, pw, 2 * BBE_SIMD_WIDTH - 10 * 4 * N*L);
    }
}
#endif
