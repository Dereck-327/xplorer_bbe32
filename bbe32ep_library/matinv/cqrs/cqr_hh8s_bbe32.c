...
DISCARD_FUN(void,cqrHouseholder8,( void* pScr, 
    const int16_t *   A,
    int16_t *    v ,
    int16_t*    Fi_out,
    const int M,
    ...
    DISCARD_FUN(void,cqrUpdateR8,(  int16_t* restrict R,
    ...
    )
{
    ...
    for (l = 0; l < ((L >> (LOG2_BBE_SIMD_WIDTH - 1))); l++)
    {
        ...
        BBE_LVNX16_XP(x, px, 4 * N*L); BBE_MULANX16J(a, x, x);
        BBE_LVNX16_XP(x, px, 4 * N*L); BBE_MULANX16J(a, x, x);
        BBE_LVNX16_XP(x, px, -7 * 4 * N*L + 2 * BBE_SIMD_WIDTH); BBE_MULANX16J(a, x, x);
    ...
    for (l = 0; l < ((L >> (LOG2_BBE_SIMD_WIDTH - 1))); l++)
    {
        ...
        BBE_LVNX16_XP(x, px, 4 * N*L); a = BBE_MULUSRNX16(mant, x, exp); x = BBE_PACKVNX40(a, exp); x = BBE_MULNX16PACKQ(x, k_norm); BBE_SVNX16_IP(x, pv, BBE_SIMD_WIDTH * 2);
        BBE_LVNX16_XP(x, px, 4 * N*L); a = BBE_MULUSRNX16(mant, x, exp); x = BBE_PACKVNX40(a, exp); x = BBE_MULNX16PACKQ(x, k_norm); BBE_SVNX16_IP(x, pv, BBE_SIMD_WIDTH * 2);
        BBE_LVNX16_XP(x, px, -7 * 4 * N*L + BBE_SIMD_WIDTH * 2); a = BBE_MULUSRNX16(mant, x, exp); x = BBE_PACKVNX40(a, exp); x = BBE_MULNX16PACKQ(x, k_norm); BBE_SVNX16_XP(x, pv, (V_Lstep - 8 + 1)*BBE_SIMD_WIDTH * 2);
    }
}
    ...
    int m, int M, int N, int L)
{
    ...
    for (j = l = 0; l < (N - m)*((L >> (LOG2_BBE_SIMD_WIDTH - 1))); l++)
    {
        ...
        XT_MOVEQZ(strideV, 2 * BBE_SIMD_WIDTH*(sizeV - 7) - 4 * L*sizeV, j);
        ...
        BBE_LVNX16_XP(r5, pr, 4 * N*L);
        BBE_LVNX16_XP(r6, pr, 4 * N*L);
        BBE_LVNX16_XP(r7, pr, 2 * BBE_SIMD_WIDTH - 7 * 4 * N*L);
        ...
        BBE_SVNX16_XP(r7, pw, 2 * BBE_SIMD_WIDTH - 7 * 4 * N*L);
    }
}
#endif
