...
/*
  NatureDSP_Baseband library. QR-based matrix decomposition and inversion for streaming order
    helper routines for Housholder algorithm, len_x==7
...
DISCARD_FUN(void,cqrHouseholder7,(void* pScr,          /* scratch (4*L bytes)        */
    const int16_t *   A,
    int16_t *    v ,
    int16_t*    Fi_out,
    const int M,
    ...
    DISCARD_FUN(void,cqrUpdateR7,(  int16_t* restrict R,
    ...
    )
{
    ...
    for (l = 0; l < ((L >> (LOG2_BBE_SIMD_WIDTH - 1))); l++)
    {
        ...
        //-----------------------------
        BBE_LVNX16_XP(x, px, 4 * N*L); a = BBE_MULNX16J(x, x);
        BBE_LVNX16_XP(x, px, 4 * N*L); BBE_MULANX16J(a, x, x);
        BBE_LVNX16_XP(x, px, 4 * N*L); BBE_MULANX16J(a, x, x);
        BBE_LVNX16_XP(x, px, 4 * N*L); BBE_MULANX16J(a, x, x);
        BBE_LVNX16_XP(x, px, 4 * N*L); BBE_MULANX16J(a, x, x);
        BBE_LVNX16_XP(x, px, 4 * N*L); BBE_MULANX16J(a, x, x);
        BBE_LVNX16_XP(x, px, -6 * 4 * N*L + 2 * BBE_SIMD_WIDTH); BBE_MULANX16J(a, x, x);
    ...
    for (l = 0; l < ((L >> (LOG2_BBE_SIMD_WIDTH - 1))); l++)
    {
        ...
        BBE_LVNX16_XP(x, px, 4 * N*L); a = BBE_MULUSRNX16(mant, x, exp); x = BBE_PACKVNX40(a, exp); x = BBE_MULNX16PACKQ(x, k_norm); BBE_SVNX16_IP(x, pv, BBE_SIMD_WIDTH * 2);
        BBE_LVNX16_XP(x, px, 4 * N*L); a = BBE_MULUSRNX16(mant, x, exp); x = BBE_PACKVNX40(a, exp); x = BBE_MULNX16PACKQ(x, k_norm); BBE_SVNX16_IP(x, pv, BBE_SIMD_WIDTH * 2);
        BBE_LVNX16_XP(x, px, 4 * N*L); a = BBE_MULUSRNX16(mant, x, exp); x = BBE_PACKVNX40(a, exp); x = BBE_MULNX16PACKQ(x, k_norm); BBE_SVNX16_IP(x, pv, BBE_SIMD_WIDTH * 2);
        BBE_LVNX16_XP(x, px, 4 * N*L); a = BBE_MULUSRNX16(mant, x, exp); x = BBE_PACKVNX40(a, exp); x = BBE_MULNX16PACKQ(x, k_norm); BBE_SVNX16_IP(x, pv, BBE_SIMD_WIDTH * 2);
        BBE_LVNX16_XP(x, px, 4 * N*L); a = BBE_MULUSRNX16(mant, x, exp); x = BBE_PACKVNX40(a, exp); x = BBE_MULNX16PACKQ(x, k_norm); BBE_SVNX16_IP(x, pv, BBE_SIMD_WIDTH * 2);
        BBE_LVNX16_XP(x, px, -6 * 4 * N*L + BBE_SIMD_WIDTH * 2); a = BBE_MULUSRNX16(mant, x, exp); x = BBE_PACKVNX40(a, exp); x = BBE_MULNX16PACKQ(x, k_norm); BBE_SVNX16_XP(x, pv, (V_Lstep - 7 + 1)*BBE_SIMD_WIDTH * 2);
    }
}
    ...
    int m, int M, int N, int L)
{
    ...
    xb_vecNx40 acc;
    xb_vecNx16 r0, r1, r2, r3, r4, r5, r6, v0, v1, v2, v3, v4, v5, v6, vr, _0x4000 = 0x4000;
    ...
    for (j = l = 0; l < (N - m)*((L >> (LOG2_BBE_SIMD_WIDTH - 1))); l++)
    {
        ...
        strideV = 2 * BBE_SIMD_WIDTH*(sizeV - 6);
        XT_MOVEQZ(strideV, 2 * BBE_SIMD_WIDTH*(sizeV - 6) - 4 * L*sizeV, j);
        ...
        BBE_LVNX16_XP(r4, pr, 4 * N*L);
        BBE_LVNX16_XP(r5, pr, 4 * N*L);
        BBE_LVNX16_XP(r6, pr, 2 * BBE_SIMD_WIDTH - 6 * 4 * N*L);
        ...
        BBE_SVNX16_XP(r6, pw, 2 * BBE_SIMD_WIDTH - 6 * 4 * N*L);
    }
}
#endif
