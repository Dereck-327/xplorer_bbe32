...
/*
  NatureDSP_Baseband library. QR-based matrix decomposition and inversion for streaming order
    helper routines for Housholder algorithm, len_x==4
...
DISCARD_FUN(void,cqrHouseholder4,( 
    const int16_t *   A,
    int16_t *    v ,         
    int16_t*    Fi_out,                                       
    const int M,        
    ...
    DISCARD_FUN(void,cqrUpdateR4,(  int16_t* restrict R,
    ...
    )
{
    ...
    xb_vecNx16 x, t, mant, k_norm, norm_x, fi, x0, x1, x2, x3;
    vsaN exp;
    xb_vecNx40 a;
    ...
    for (l = 0; l < ((L >> (LOG2_BBE_SIMD_WIDTH - 1))); l++)
    {
        ...
        //-----------------------------
        BBE_LVNX16_XP(x1, px, 4 * N*L);
        BBE_LVNX16_XP(x2, px, 4 * N*L);
        BBE_LVNX16_XP(x3, px, -3 * 4 * N*L + BBE_SIMD_WIDTH * 2);
        a = BBE_MULNX16J(x1, x1);
        BBE_MULANX16J(a, x0, x0);
        BBE_MULANX16J(a, x2, x2);
        BBE_MULANX16J(a, x3, x3);
        ...
        a = BBE_MULUSRNX16(mant, x1, exp); x = BBE_PACKVNX40(a, exp); x = BBE_MULNX16PACKQ(x, k_norm); BBE_SVNX16_IP(x, pv, BBE_SIMD_WIDTH * 2);
        a = BBE_MULUSRNX16(mant, x2, exp); x = BBE_PACKVNX40(a, exp); x = BBE_MULNX16PACKQ(x, k_norm); BBE_SVNX16_IP(x, pv, BBE_SIMD_WIDTH * 2);
        a = BBE_MULUSRNX16(mant, x3, exp); x = BBE_PACKVNX40(a, exp); x = BBE_MULNX16PACKQ(x, k_norm); BBE_SVNX16_XP(x, pv, (V_Lstep - 4 + 1)*BBE_SIMD_WIDTH * 2);
    }
}
    ...
    int m, int M, int N, int L)
{
    ...
    xb_vecNx40 acc;
    xb_vecNx16 r0, r1, r2, r3, v0, v1, v2, v3, vr, _0x4000;
    ...
    for (j = l = 0; l < (N - m)*((L >> (LOG2_BBE_SIMD_WIDTH - 1))); l++)
    {
        ...
        BBE_SVNX16_XP(r3, pw, 2 * BBE_SIMD_WIDTH - 3 * 4 * N*L);
    }
}
#endif
