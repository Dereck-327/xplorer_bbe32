...
*/
    ...
    const xb_vecNx16 * restrict V_rd = (const xb_vecNx16*) V;

    vsaN _11=BBE_MOVVSA32(11);
    vsaN _14=BBE_MOVVSA32(14);
    xb_vecNx16 _0x4000=0x4000;
    xb_vecNx16 V0,V1,V2,V3,B0,B1,B2,B3,Z0;
    xb_vecNx40 A;
    xb_c40 Im;
    valign aV;
    vboolN mask;
    ...
    for (m=0; m<8; m++)
    {
        ...
        for (l=0; l<L; l++)
        {
            ...
            BBE_LANX16_IP (V1,aV,V_rd );
            BBE_LANX16_IP (V2,aV,V_rd );
            BBE_LAVNX16_XP(V3,aV,V_rd ,4*(8-m));
            V3=BBE_SELNX16(V2,V3,rotate);
            V2=BBE_SELNX16(V1,V2,rotate);
            ...
            BBE_LVNX16_IP (B2,pBr,2*BBE_SIMD_WIDTH);
            BBE_LVNX16_IP (B3,pBr,2*BBE_SIMD_WIDTH);
            A=BBE_MULRNX16J(B0,V0,_11); 
            BBE_MULANX16J (A,B1,V1); 
            BBE_MULANX16J (A,B2,V2); 
            BBE_MULANX16J (A,B3,V3); 
            ...
            A=BBE_MULRNX16(B2,_0x4000,_14);
            BBE_MULSNX16C(A,Z0,V2);
            B2=BBE_PACKVNX40(A,_14);

            A=BBE_MULRNX16(B3,_0x4000,_14);
            BBE_MULSNX16C(A,Z0,V3);
            B3=BBE_PACKVNX40(A,_14);

            BBE_SVNX16F_IP(B0,pBw,2*BBE_SIMD_WIDTH,mask);
            BBE_SVNX16_IP (B1,pBw,2*BBE_SIMD_WIDTH);
            BBE_SVNX16_IP (B2,pBw,2*BBE_SIMD_WIDTH);
            BBE_SVNX16_IP (B3,pBw,2*BBE_SIMD_WIDTH);
        }
    }
    __Pragma("no_reorder"); 
    /* update first 8-16 columns */
    for (m=0; m<8; m++)
    {
        ...
        for (l=0; l<L; l++)
        {
            BBE_LANX16_IP (V0,aV,V_rd );
            BBE_LANX16_IP (V1,aV,V_rd );
            BBE_LAVNX16_XP(V2,aV,V_rd ,4*(8-m));
            V2=BBE_SELNX16(V1,V2,rotate);
            V1=BBE_SELNX16(V0,V1,rotate);
            V0=BBE_SELNX16(V0,V0,rotate);
            BBE_LVNX16F_IP(B0,pBr,2*BBE_SIMD_WIDTH,mask);
            BBE_LVNX16_IP (B1,pBr,2*BBE_SIMD_WIDTH);
            BBE_LVNX16_IP (B2,pBr,4*BBE_SIMD_WIDTH);
            A=BBE_MULRNX16J(B0,V0,_11); 
            BBE_MULANX16J (A,B1,V1); 
            BBE_MULANX16J (A,B2,V2); 
            Im=BBE_RADDNX40C(A); A=BBE_MOVNX40_FROMC40(Im);
            ...
            BBE_SVNX16F_IP(B0,pBw,2*BBE_SIMD_WIDTH,mask);
            BBE_SVNX16_IP (B1,pBw,2*BBE_SIMD_WIDTH);
            BBE_SVNX16_IP (B2,pBw,4*BBE_SIMD_WIDTH);
        }
    }
    __Pragma("no_reorder");
    /* update 16-24 columns */
    for (m=0; m<8; m++)
    {
        pBr=(const xb_vecNx16 * )(B+32);
        pBw=(      xb_vecNx16 * )(B+32);
        aV=BBE_LA_PP(V_rd);
        mask = BBE_LTRN(2*m);
        Z0=BBE_SEQNX16(); B0=BBE_MOVVA16(2*m); Z0=BBE_SUBNX16(Z0,B0);
        ...
        {
            BBE_LANX16_IP (V0,aV,V_rd );
            BBE_LAVNX16_XP(V1,aV,V_rd ,4*(8-m));
            V1=BBE_SELNX16(V0,V1,rotate);
            V0=BBE_SELNX16(V0,V0,rotate);
            BBE_LVNX16F_IP(B0,pBr,2*BBE_SIMD_WIDTH,mask);
            BBE_LVNX16_IP (B1,pBr,6*BBE_SIMD_WIDTH);
            A=BBE_MULRNX16J(B0,V0,_11); 
            BBE_MULANX16J (A,B1,V1); 
            Im=BBE_RADDNX40C(A); A=BBE_MOVNX40_FROMC40(Im);
            Z0=BBE_PACKVNX40(A,_14);
            ...
            B1=BBE_PACKVNX40(A,_14);
            BBE_SVNX16F_IP(B0,pBw,2*BBE_SIMD_WIDTH,mask);
            BBE_SVNX16_IP (B1,pBw,6*BBE_SIMD_WIDTH);
        }
    }
    __Pragma("no_reorder");
    /* update last 24-32 columns */
    for (m=0; m<8; m++)
    {
        pBr=(const xb_vecNx16 * )(B+48);
        pBw=(      xb_vecNx16 * )(B+48);
        aV=BBE_LA_PP(V_rd);
        mask = BBE_LTRN(m*2);
        Z0=BBE_SEQNX16(); B0=BBE_MOVVA16(m*2); Z0=BBE_SUBNX16(Z0,B0);
        rotate=BBE_MOVVSELNX16(Z0,0);
        __Pragma("loop_count min=1");
        for (l=0; l<L; l++)
        {
            ...
            BBE_LVNX16F_IP(B0,pBr,4*2*BBE_SIMD_WIDTH,mask);
            ...
            BBE_MULSNX16C(A,Z0,V0);
            B0=BBE_PACKVNX40(A,_14);
            BBE_SVNX16F_IP(B0,pBw,4*2*BBE_SIMD_WIDTH,mask);
        }
    }

    /* Rotate matrix B by diagonal matrix Fi' */
    cqrnRotateB32(pScr,B,V+(32+1)*(32/2)*2*L,L);
} 
...
size_t cqr_calc_qb32x32x1n_getScratchSize (int M, int N,int P,int L)
{
    ...
    return Bsize;
} /* cqr_calc_qb32x32x1n_getScratchSize() */
#else

DISCARD_FUN(void,cqr_calc_qb32x32x1n,(void *pScr,
                          complex_fract16* B,
