                    ...
                    int L)
{
    ...
    static const fnTakeColumn takeColumn[]=
    {
        cqrnTakeColumn8,
        cqrnTakeColumn8,
        cqrnTakeColumn8,
    ...
    for(m=0; m<7; m++)
    {
        takeColumn[m](Z,A+2*m*(8+1),8-m,8,SA,L);
        cqrnHousholder8(pScr,pV,Fi+2*m*L,D+2*m,Z,8-m,SD,L);
        cqrnUpdateR8(Z,A+2*m*(8+1),pV,SA,8-m,8-m,8,L);
        pV+=2*(8-m)*L;
    }
    cqrnTakeColumn1(Z,A+2*m*(8+1),SA,L);
    cqrnHousholder1(pV,Fi+2*m*L,D+2*m,Z,SD,L);
    cqrnUpdateR1   (A+2*m*(8+1),pV,SA,L);

    // final rotation Fi'*R
    cqrnRotateR8(A,Fi,L,0);
#if EXTRA_SHIFT
...
size_t cqr_build_r8x8n_getScratchSize (int M, int N,int L)
{
    ...
    return Zsize+Hsize;
} /* cqr_build_r8x8n_getScratchSize() */

#else
DISCARD_FUN(void,cqr_build_r8x8n,(void *pScr,
                    complex_fract16* A,
                    complex_fract16* V,
                    complex_fract16* D,
                    int L))
size_t cqr_build_r8x8n_getScratchSize(int M, int N,int L) { (void)M,(void)N,(void)L; return 0; }
#endif
