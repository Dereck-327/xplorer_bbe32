...
void cqrnTakeColumn(int16_t* restrict x,const int16_t* restrict A,int M,int N,int SA,int L)
{
    ...
    M_ = (M) & (BBE_SIMD_WIDTH/2-1);
    NbM  = sz_i16*2*(M & (BBE_SIMD_WIDTH/2-1));
    XT_MOVEQZ(NbM, sz_i16*BBE_SIMD_WIDTH, NbM);
    pX = (xb_vecNx16 *)(x);
    al_X = BBE_ZALIGN();
    ...
    for (l=0; l<L; l++)
    {
        pA = (const xb_vecNx16 *)(A+l*SA);

        /* load the first 8 values */
        ...
        pA = (const xb_vecNx16 *)(A+l*SA+2*M_*N);
