...
#if !HAVE_CQRN
DISCARD_FUN(void,cqr_bkw32x32x32n,(void *pScr,
                          complex_fract16* X,
                    ...
                    int L)
{
    ...
    xb_vecNx16 dd, d0, d1;
    xb_vecNx16 tl0, th0, b_res0, b0;
    xb_vecNx16 tl1, th1, b_res1, b1;
    xb_vecNx16 tl2, th2, b_res2, b2;
    xb_vecNx16 tl3, th3, b_res3, b3;
    xb_vecNx16 xx0, xx1, xx2, xx3;
    xb_vecNx16 rr;
    xb_vecNx40 B0, B1, B2, B3;
    vsaN q;
    ...
    for (k=N-1; k>=0; k--)
    {
        //pXrd_first= (const xb_vecNx16*)&X[2*k*P];
        ...
        for(l=0; l<L; l++)
        {
            ...
            // load P (P=32) 16 bit complex value (16 bit re and 16 bit im)
            BBE_LVNX16_IP(b0, pXrd_first, 2*BBE_SIMD_WIDTH);
            BBE_LVNX16_IP(b1, pXrd_first, 2*BBE_SIMD_WIDTH);
            BBE_LVNX16_IP(b2, pXrd_first, 2*BBE_SIMD_WIDTH);
            BBE_LVNX16_XP(b3, pXrd_first, 2*SX-3*2*BBE_SIMD_WIDTH);
            B0 = BBE_UNPKSNX16(b0);
            B1 = BBE_UNPKSNX16(b1);
            B2 = BBE_UNPKSNX16(b2);
            B3 = BBE_UNPKSNX16(b3);
            ...
            B2 = BBE_SLSNX40(B2, vqABX);
            B3 = BBE_SLSNX40(B3, vqABX);
            ...
            for (m=0; m<N-k-1; m++)
            {
                ...
                BBE_MULSNX16C(B2,xx2,rr);
                BBE_MULSNX16C(B3,xx3,rr);
            }
            ...
            tl2 = BBE_PACKLNX40(B2);
            tl3 = BBE_PACKLNX40(B3);
            // get high 16 bits of B
            th0 = BBE_PACKVNX40(B0,sh16);
            th1 = BBE_PACKVNX40(B1,sh16);
            th2 = BBE_PACKVNX40(B2,sh16);
            th3 = BBE_PACKVNX40(B3,sh16);
            // mul low (unsigned*unsigned)
            B0 = BBE_MULUUNX16(d0,tl0);
            B1 = BBE_MULUUNX16(d0,tl1);
            B2 = BBE_MULUUNX16(d0,tl2);
            B3 = BBE_MULUUNX16(d0,tl3);
            // shift low left
            B0 = BBE_SRAINX40(B0,16);
            B1 = BBE_SRAINX40(B1,16);
            B2 = BBE_SRAINX40(B2,16);
            B3 = BBE_SRAINX40(B3,16);
            // Bres+= mul high (unsigned*signed)
            BBE_MULUSANX16(B0,d0,th0);
            BBE_MULUSANX16(B1,d0,th1);
            BBE_MULUSANX16(B2,d0,th2);
            BBE_MULUSANX16(B3,d0,th3);
            // result rounding
            B0 = BBE_RNDADJNX40(B0,q);
            B1 = BBE_RNDADJNX40(B1,q);
            B2 = BBE_RNDADJNX40(B2,q);
            B3 = BBE_RNDADJNX40(B3,q);
            b_res0 = BBE_PACKVNX40(B0,q);
            b_res1 = BBE_PACKVNX40(B1,q);
            b_res2 = BBE_PACKVNX40(B2,q);
            b_res3 = BBE_PACKVNX40(B3,q);
            // store res
            BBE_SVNX16_IP(b_res0,pXwr,2*BBE_SIMD_WIDTH);
            BBE_SVNX16_IP(b_res1,pXwr,2*BBE_SIMD_WIDTH);
            BBE_SVNX16_IP(b_res2,pXwr,2*BBE_SIMD_WIDTH);
            BBE_SVNX16_XP(b_res3,pXwr,2*SX-3*2*BBE_SIMD_WIDTH);
            //-------------------------------------------------------------------
        }
    }
    #undef N
...
size_t cqr_bkw32x32x32n_getScratchSize (int M, int N,int P,int L)
{
    ...
    (void)L;
    return 0;
} /* cqr_bkw32x32x32n_getScratchSize() */
