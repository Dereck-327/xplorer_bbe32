                    ...
                    int L)
{
    ...
    for(m=0; m<15; m++)
    {
        it[m].takeColumn (Z,A+2*m*(16+1),16-m,16,SA,L);
        it[m].housholder (pScr,pV,Fi+2*m*L,D+2*m,Z,16-m,SD,L);
        it[m].updateR (Z,A+2*m*(16+1),pV,SA,16-m,16-m,16,L);
        pV+=2*(16-m)*L;
    }
    cqrnTakeColumn1(Z,A+2*m*(16+1),SA,L);
    cqrnHousholder1(pV,Fi+2*m*L,D+2*m,Z,SD,L);
    cqrnUpdateR1   (A+2*m*(16+1),pV,SA,L);

    // final rotation Fi'*R
    cqrnRotateR16(A,Fi,L,0);
#if EXTRA_SHIFT
...
size_t cqr_build_r16x16n_getScratchSize (int M, int N,int L)
{
    ...
    return Zsize+Hsize;
} /* cqr_build_r16x16n_getScratchSize() */

#else
DISCARD_FUN(void,cqr_build_r16x16n,(void *pScr,
                    complex_fract16* A,
                    complex_fract16* V,
                    complex_fract16* D,
                    int L))
size_t cqr_build_r16x16n_getScratchSize(int M, int N,int L) { (void)M,(void)N,(void)L; return 0; }
#endif
