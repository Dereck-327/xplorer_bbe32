...
#if !HAVE_CQRN
DISCARD_FUN(void,cqr_bkw8x8x8n,(void *pScr,
                          complex_fract16* X,
                    ...
                    int L)
{
    ...
    for (k=N-1; k>=0; k--)
    {
        ...
        for(l=0; l<L; l++)
        {
            ...
            // load P(=8) 16 bit complex value (16 bit re and 16 bit im)
            BBE_LVNX16_XP(b0, pXrd_first, 2*SX);
            B0 = BBE_UNPKSNX16(b0);
            ...
            // store res
            BBE_SVNX16_XP(b_res0,pXwr,2*SX);
            //-------------------------------------------------------------------
        }
    }
    #undef N
