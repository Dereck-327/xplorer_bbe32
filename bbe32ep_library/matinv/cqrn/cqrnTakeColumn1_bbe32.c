...
void cqrnTakeColumn1(int16_t* restrict x,const int16_t* restrict A,int SA,int L)
{
    ...
    /* Extract by SIMD_WIDTH/2 == 8 values per iteration */
    for (l=0; l<(L>>(LOG2_BBE_SIMD_WIDTH-1)); l++)
    {
        /* load 8 values and put it to the one register */
        BBE_LPNX16_XP(T0, pA, SA*sz_i16);
        BBE_LPNX16_XP(T1, pA, SA*sz_i16);
        BBE_LPNX16_XP(T2, pA, SA*sz_i16);
        BBE_LPNX16_XP(T3, pA, SA*sz_i16);
        BBE_LPNX16_XP(T4, pA, SA*sz_i16);
        BBE_LPNX16_XP(T5, pA, SA*sz_i16);
        BBE_LPNX16_XP(T6, pA, SA*sz_i16);
        BBE_LPNX16_XP(T7, pA, SA*sz_i16);
        ...
        /* save values */
        BBE_SVNX16_IP(T0, pX, sz_i16*BBE_SIMD_WIDTH);
    }

    /* Extract last L%8 values */
    __Pragma("no_unroll")
    for (l=0; l<Lmod; l++)
    {
        BBE_LPNX16_XP(T0, pA, SA*sz_i16);
        BBE_SPNX16_IP(T0, pX, 2*sz_i16);
    }
}
