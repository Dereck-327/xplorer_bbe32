...
void cqrnRotateB16(void* pScr, int16_t* B,const int16_t* Fi,int L)
{
    ...
    if (Lmod)
    {
        ...
        for (m=0; m<(N/8); m++)
        {
            ...
            BBE_SVNX16_IP(F0, pscr_st, 2*sz_i16*BBE_SIMD_WIDTH);
            BBE_SVNX16_IP(F1, pscr_st, 2*sz_i16*BBE_SIMD_WIDTH);
            BBE_SVNX16_IP(F2, pscr_st, 2*sz_i16*BBE_SIMD_WIDTH);
            BBE_SVNX16_IP(F3, pscr_st, 2*sz_i16*BBE_SIMD_WIDTH);
            BBE_SVNX16_IP(F4, pscr_st, 2*sz_i16*BBE_SIMD_WIDTH);
            BBE_SVNX16_IP(F5, pscr_st, 2*sz_i16*BBE_SIMD_WIDTH);
            BBE_SVNX16_XP(F6, pscr_st, (1-12)*(int)sz_i16*BBE_SIMD_WIDTH);
        }
