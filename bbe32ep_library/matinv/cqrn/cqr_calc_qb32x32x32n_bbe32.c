                    ...
                    int L)
{
  ...
  const int16_t* V=(const int16_t*)_V;
    ...
    for (pV=V,m=0; m<32; m++)
    {
        cqrnUpdateR32(Z,B+2*m*32,pV,SB,(32-m),32,32,L); 
        pV+=2*(32-m)*L;
    }

    /* Rotate matrix B by diagonal matrix Fi' */
    cqrnRotateR(B,pV,32,SB,L,1);
} /* cqr_calc_qb32x32x32n() */

size_t cqr_calc_qb32x32x32n_getScratchSize (int M, int N,int P,int L)
{
    NASSERT(M==32 && N==32 && P==32 && L>0);
    (void)N;(void)M;
    return 2*P*L*sizeof(int16_t);
} /* cqr_calc_qb32x32x32n_getScratchSize() */
#else
DISCARD_FUN(void,cqr_calc_qb32x32x32n,(void *pScr,
                          complex_fract16* B,
