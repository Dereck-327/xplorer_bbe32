...
#if !HAVE_CQRN
DISCARD_FUN(void,cqr_bkw16x16x1n,(void *pScr,
                          complex_fract16* X,
                    ...
                    int L)
{
    ...
    for (k=N-1; k>=0; k--)
    {
        ...
        cmp0 = BBE_SEQNX16();
        BBE_DSELNX16I(cmp1, cmp0, cmp0, cmp0, BBE_DSELI_INTERLEAVE_1);
        mask0 = BBE_LTNX16(BBE_MOVVA16(k), cmp0);
        mask1 = BBE_LTNX16(BBE_MOVVA16(k), cmp1);
        ...
        for(l=0; l<L; l++)
        {
            ...
            BBE_LVNX16T_IP(rr0,pRrd,     2*BBE_SIMD_WIDTH,mask0);
            BBE_LVNX16T_XP(rr1,pRrd,2*SR-2*BBE_SIMD_WIDTH,mask1);
            BBE_LVNX16T_IP(xx0,pXrd,     2*BBE_SIMD_WIDTH,mask0);
            BBE_LVNX16T_XP(xx1,pXrd,2*SX-2*BBE_SIMD_WIDTH,mask1);
            //B-= rr*xx
...
size_t cqr_bkw16x16x1n_getScratchSize (int M, int N,int P,int L)
{
    ...
    (void)L;
    return 0;
} /* cqr_bkw16x16x1n_getScratchSize() */
