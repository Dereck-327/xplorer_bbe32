                    ...
                    int L)
{
  ...
  const int16_t* V=(const int16_t*)_V;
    ...
    for (pV=V,m=0; m<16; m++)
    {
        cqrnUpdateR16(Z,B+2*m*16,pV,SB,(16-m),16,16,L); 
        pV+=2*(16-m)*L;
    }

    /* Rotate matrix B by diagonal matrix Fi' */
    cqrnRotateR16(B,pV,L,1);
} 

size_t cqr_calc_qb16x16x16n_getScratchSize (int M, int N,int P,int L)
{
    NASSERT(M==16 && N==16 && P==16 && L>0);
    (void)N;(void)M;
    return 2*P*L*sizeof(int16_t);
} /* cqr_calc_qb16x16x16n_getScratchSize() */
#else
DISCARD_FUN(void,cqr_calc_qb16x16x16n,(void *pScr,
                          complex_fract16* B,
