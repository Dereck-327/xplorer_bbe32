...
DISCARD_FUN(void, qrHouseholder7,(  
    ...
    DISCARD_FUN(void, qrUpdateR7,(  int16_t* restrict R,
...
#if 0
{
    ...
    vboolN sign;
    vsaN d1,exp;
    int strideV=2*BBE_SIMD_WIDTH*(SIZE_OF_V(M,N)-7+1);
    ...
    for(p = 0; p<L; p+=BBE_SIMD_WIDTH)
    {
        ...
        BBE_LVNX16_XP(x0,x,2*BBE_SIMD_WIDTH-2*L*N*(7-1));
        a=BBE_MULUSRNX16(d0,x0,d1);
        x0=BBE_PACKVNX40(a,d1);
        x0 = BBE_MULNX16PACKQ(k_norm,x0);
...
#else
{
    ...
    int strideV2 = 2 * BBE_SIMD_WIDTH*(SIZE_OF_V(M, N) - 6);
    ...
    int m, int M, int N, int L)
{
    ...
    for (j = l = 0; l < (N - m)*((L >> (LOG2_BBE_SIMD_WIDTH))); l++)
    {
        ...
        XT_MOVEQZ(strideV, 2 * BBE_SIMD_WIDTH*(sizeV - 6) - 2 * L*sizeV, j);
        ...
        BBE_LVNX16_XP(r4, pr, 2 * N*L);
        BBE_LVNX16_XP(r5, pr, 2 * N*L);
        BBE_LVNX16_XP(r6, pr, 2 * BBE_SIMD_WIDTH - 6 * 2 * N*L);
        ...
        BBE_SVNX16_XP(r6, pw, 2 * BBE_SIMD_WIDTH - 6 * 2 * N*L);
    }
}
#endif
