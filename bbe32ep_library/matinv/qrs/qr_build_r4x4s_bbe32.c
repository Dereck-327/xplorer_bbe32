...
DISCARD_FUN(void, qr_build_r4x4s, (void* pScr,int16_t * restrict V, int16_t * restrict R, int L))
#else
...
void  qr_build_r4x4s ( void* pScr, int16_t * restrict V, int16_t * restrict R, int L)
{
    ...
    pR = R;
    qrHouseholder4(pR, pV, 4, 4, L); qrUpdateR4(pR, pV, 0, 4, 4, L); pR += (4 + 1)*L; pV += 4 * BBE_SIMD_WIDTH;
    qrHouseholder3(pR, pV, 4, 4, L); qrUpdateR3(pR, pV, 1, 4, 4, L); pR += (4 + 1)*L; pV += 3 * BBE_SIMD_WIDTH;
    qrHouseholder2(pR, pV, 4, 4, L); qrUpdateR2(pR, pV, 2, 4, 4, L);
} /* qr_build_r4x4s() */
#endif
