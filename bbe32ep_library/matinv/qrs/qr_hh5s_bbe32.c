...
DISCARD_FUN(void, qrHouseholder5,(  
    ...
    DISCARD_FUN(void, qrUpdateR5,(  int16_t* restrict R,
    ...
    )
{
    ...
    vboolN sign;
    vsaN d1, exp;
    int strideV = 2 * BBE_SIMD_WIDTH*(SIZE_OF_V(M, N) - 5 + 1);
    ...
    for (p = 0; p < L; p += BBE_SIMD_WIDTH)
    {
        ...
        BBE_LVNX16_XP(x0, x, 2 * BBE_SIMD_WIDTH - 2 * L*N*(5 - 1));
        a = BBE_MULUSRNX16(d0, x0, d1);
        x0 = BBE_PACKVNX40(a, d1);
        x0 = BBE_MULNX16PACKQ(k_norm, x0);
    ...
    int m, int M, int N, int L)
{
    ...
    for (j = l = 0; l < (N - m)*((L >> (LOG2_BBE_SIMD_WIDTH))); l++)
    {
        ...
        BBE_LVNX16_XP(r2, pr, 2 * N*L);
        BBE_LVNX16_XP(r3, pr, 2 * N*L);
        BBE_LVNX16_XP(r4, pr, 2 * BBE_SIMD_WIDTH - 4 * 2 * N*L);
        ...
        BBE_SVNX16_XP(r4, pw, 2 * BBE_SIMD_WIDTH - 4 * 2 * N*L);
    }
}
#endif
