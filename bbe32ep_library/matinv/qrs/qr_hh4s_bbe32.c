...
DISCARD_FUN(void, qrHouseholder4,(  
    ...
    DISCARD_FUN(void, qrUpdateR4,(  int16_t* restrict R,
    ...
    )
{
    ...
    vboolN sign;
    vsaN d1, exp;
    int strideV = 2 * BBE_SIMD_WIDTH*(SIZE_OF_V(M, N) - 4 + 1);
    ...
    for (p = 0; p < L; p += BBE_SIMD_WIDTH)
    {
        ...
        BBE_LVNX16_XP(x0, x, 2 * L*N); BBE_MULANX16(a, x0, x0);
        BBE_LVNX16_XP(x0, x, -2 * L*N*(4 - 1)); BBE_MULANX16(a, x0, x0);
        ...
        BBE_LVNX16_XP(x0, x, 2 * BBE_SIMD_WIDTH - 2 * L*N*(4 - 1));
        a = BBE_MULUSRNX16(d0, x0, d1);
        x0 = BBE_PACKVNX40(a, d1);
        x0 = BBE_MULNX16PACKQ(k_norm, x0);
    ...
    int m, int M, int N, int L)
{
    ...
    xb_vecNx40 acc;
    xb_vecNx16 r0, r1, r2, r3, v0, v1, v2, v3, vr, _0x4000 = 0x4000;
    ...
    for (j = l = 0; l < (N - m)*((L >> (LOG2_BBE_SIMD_WIDTH))); l++)
    {
        ...
        XT_MOVEQZ(strideV, 2 * BBE_SIMD_WIDTH*(sizeV - 3) - 2 * L*sizeV, j);
        BBE_LVNX16_XP(r0, pr, 2 * N*L);
        BBE_LVNX16_XP(r1, pr, 2 * N*L);
        BBE_LVNX16_XP(r2, pr, 2 * N*L);
        BBE_LVNX16_XP(r3, pr, 2 * BBE_SIMD_WIDTH - 3 * 2 * N*L);
        ...
        BBE_SVNX16_XP(r3, pw, 2 * BBE_SIMD_WIDTH - 3 * 2 * N*L);
    }
}
#endif
