                        ...
                        int K,int M,int N, int L)
{
    ...
    xb_vecN_2xf32 Acc, V0, V1, V2, R0, R1, R2, Z0;
    ...
    for (l = 0; l < (N - K)*L; l += (BBE_SIMD_WIDTH / 2))
    {
        ...
        R2 = BBE_LVN_2XF32_X(pRr, 2 * N*L * sizeof(float32_t));
        Acc = BBE_MULN_2XF32(V2, R2);
        V1 = BBE_LVN_2XF32_X(pV, L * sizeof(float32_t));
        R1 = BBE_LVN_2XF32_X(pRr, N*L * sizeof(float32_t));
        BBE_MULAN_2XF32(Acc, V1, R1);
        BBE_LVN_2XF32_IC(V0, pV);
        BBE_LVN_2XF32_IP(R0, pRr, 0);
