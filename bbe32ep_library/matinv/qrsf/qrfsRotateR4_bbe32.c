...
void qrfsRotateR4(float32_t* restrict R,const float32_t* restrict Fi,int M, int N,int L)
{
    ...
    pF2 = (const xb_vecN_2xf32 *)(Fi + 3 * L);
    pR = (xb_vecN_2xf32 *)R;
    ...
    for (l = 0; l < L; l += (BBE_SIMD_WIDTH / 2))
    {
        ...
        BBE_LVN_2XF32_XP(R0, pRr, L * sizeof(float32_t));
        Acc = BBE_MULN_2XF32(F0, R0);
        BBE_SVN_2XF32_XP(Acc, pR, L * sizeof(float32_t));
        ...
        BBE_LVN_2XF32_XP(R0, pRr, 2 * BBE_SIMD_WIDTH - 15 * L * sizeof(float32_t));
        Acc = BBE_MULN_2XF32(F0, R0);
        BBE_SVN_2XF32_XP(Acc, pR, 2 * BBE_SIMD_WIDTH - 15 * L * sizeof(float32_t));
    }
}
#endif
