...
{
    int SV=((2*3-3+1)*3)>>1; // number of elements in Householder without rotator
    float32_t* Fi;
    float32_t* pV;
    float32_t* pA;
    ...
    qrfsHousholder3(pScr,pV,Fi,D,pA,SV,3-0,3,L); qrfsUpdateR3(pA,pV,0,3,3,L); pA+=(3+1)*L;  pV+=(3-0)*L; D+=L; Fi+=L;
    qrfsHousholder2(pScr,pV,Fi,D,pA,SV,3-1,3,L); qrfsUpdateR2(pA,pV,1,2,3,L); pA+=(3+1)*L;  pV+=(3-1)*L; D+=L; Fi+=L;
    qrfsHousholder1(pV,Fi,D,pA,SV,3-2,3,L);      qrfsUpdateR1(pA,pV,2,1,3,L);

    Fi=V+SV*L;
    // final rotation Fi'*R
    qrfsRotateR3(R,Fi,3,3,L);
}
...
#else
DISCARD_FUN(void, qr_build_r3x3sf, ( void* pScr, float32_t* restrict V, float32_t * restrict R, float32_t * restrict D, int L) )
size_t qr_build_r3x3sf_getScratchSize  (int M, int N,int L) 
{
    (void)M,(void)N,(void)L;
    NASSERT(M==3 && N==3);
