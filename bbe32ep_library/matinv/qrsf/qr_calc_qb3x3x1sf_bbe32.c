...
void qr_calc_qb3x3x1sf  (void *pScr, float32_t *B, const float32_t *V , int L)
{
    ...
    qrsfUpdateB3(B+0*L,V,L);  V+=(3-0)*L;
    qrsfUpdateB2(B+1*L,V,L);  V+=(3-1)*L;
    qrsfUpdateB1(B+2*L,V,L);  V+=(3-2)*L;
    qrsfRotateBconj1(B,V,3,L);
}
...
#else
DISCARD_FUN(void, qr_calc_qb3x3x1sf, (void *pScr, float32_t *B, const float32_t *V , int L))
size_t qr_calc_qb3x3x1sf_getScratchSize(int M, int P, int L) 
{ 
    (void)M,(void)P,(void)L;
    NASSERT(L>0 && L%(BBE_SIMD_WIDTH/2)==0);
