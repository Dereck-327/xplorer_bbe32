...
void qr_calc_qb4x4x1sf  (void *pScr, float32_t *B, const float32_t *V , int L)
{
    ...
    qrsfUpdateB4(B+0*L,V,L);  V+=(4-0)*L;
    qrsfUpdateB3(B+1*L,V,L);  V+=(4-1)*L;
    qrsfUpdateB2(B+2*L,V,L);  V+=(4-2)*L;
    qrsfUpdateB1(B+3*L,V,L);  V+=(4-3)*L;
    qrsfRotateBconj1(B,V,4,L);
}
...
#else
DISCARD_FUN(void, qr_calc_qb4x4x1sf, (void *pScr, float32_t *B, const float32_t *V , int L))
size_t qr_calc_qb4x4x1sf_getScratchSize(int M, int P, int L) 
{ 
    (void)M,(void)P,(void)L;
    NASSERT(L>0 && L%(BBE_SIMD_WIDTH/2)==0);
