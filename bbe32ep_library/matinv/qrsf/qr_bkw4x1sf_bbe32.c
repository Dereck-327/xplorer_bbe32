            ...
            int N,int P,int L)
{
    ...
    pD = (const xb_vecN_2xf32 *)(D + L);
    pX = (const xb_vecN_2xf32 *)(x + L);
    ...
    for (l = 0; l < L; l += (BBE_SIMD_WIDTH / 2))
    {
        ...
        Acc = BBE_MULN_2XF32(X0, R0);

        X0 = BBE_LVN_2XF32_X(pX, L * sizeof(float32_t));
        BBE_LVN_2XF32_IP(Acc1, pX, 2 * BBE_SIMD_WIDTH);
        BBE_LVN_2XF32_IP(R0, pR, 2 * BBE_SIMD_WIDTH);
        BBE_MULSN_2XF32(Acc1, X0, R0);

        Acc = BBE_SUBN_2XF32(Acc1, Acc);
    ...
    for (l = 0; l < L; l += (BBE_SIMD_WIDTH / 2))
    {
        ...
        X2 = BBE_LVN_2XF32_X(pX, 3 * L * sizeof(float32_t));
        X1 = BBE_LVN_2XF32_X(pX, 2 * L * sizeof(float32_t));
        X0 = BBE_LVN_2XF32_X(pX, L * sizeof(float32_t));
        BBE_LVN_2XF32_IP(Acc, pX, 2 * BBE_SIMD_WIDTH);

        R0 = BBE_LVN_2XF32_X(pR, 2 * L * sizeof(float32_t));
        Acc1 = BBE_MULN_2XF32(X2, R0);

        R0 = BBE_LVN_2XF32_X(pR, L * sizeof(float32_t));
        BBE_MULSN_2XF32(Acc, X1, R0);
...
#else
DISCARD_FUN(void, qr_bkw4x1sf, (void* pScr, float32_t* restrict B, const float32_t* restrict R, const float32_t* restrict D,int L))
size_t qr_bkw4x1sf_getScratchSize(int N, int P, int L)
{ 
    (void)N,(void)P,(void)L;
    NASSERT(L>0 && L%(BBE_SIMD_WIDTH/2)==0);
