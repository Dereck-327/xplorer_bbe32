...
#else
DISCARD_FUN(void, qr_bkw3x1sf, (void* pScr, float32_t* restrict B, const float32_t* restrict R, const float32_t* restrict D,int L))
size_t qr_bkw3x1sf_getScratchSize(int N, int P, int L) 
{ 
    (void)N,(void)P,(void)L;
    NASSERT(L>0 && L%(BBE_SIMD_WIDTH/2)==0);
