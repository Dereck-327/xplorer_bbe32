...
{
    int SV=((2*4-4+1)*4)>>1; // number of elements in Householder without rotator
    float32_t* Fi;
    float32_t* pV;
    float32_t* pA;
    ...
    qrfsHousholder4(pScr,pV,Fi,D,pA,SV,4-0,4,L); qrfsUpdateR4(pA,pV,0,4-0,4,L);pA+=(4+1)*L; pV+=(4-0)*L;D+=L;Fi+=L;
    qrfsHousholder3(pScr,pV,Fi,D,pA,SV,4-1,4,L); qrfsUpdateR3(pA,pV,1,4-1,4,L);pA+=(4+1)*L; pV+=(4-1)*L;D+=L;Fi+=L;
    qrfsHousholder2(pScr,pV,Fi,D,pA,SV,4-2,4,L); qrfsUpdateR2(pA,pV,2,4-2,4,L);pA+=(4+1)*L; pV+=(4-2)*L;D+=L;Fi+=L;
    qrfsHousholder1(     pV,Fi,D,pA,SV,4-3,4,L); qrfsUpdateR1(pA,pV,3,4-3,4,L);

    Fi=V+SV*L;
    // final rotation Fi'*R
    qrfsRotateR4(R,Fi,4,4,L);
}
...
#else
DISCARD_FUN(void, qr_build_r4x4sf, ( void* pScr, float32_t* restrict V, float32_t * restrict R, float32_t * restrict D, int L) )
size_t qr_build_r4x4sf_getScratchSize  (int M, int N,int L) 
{
    (void)M,(void)N,(void)L;
    NASSERT(M==4 && N==4);
