                    ...
                    int SV, int M, int N, int L)
{
    ...
    xb_vecN_2xf32 Acc, Acc1, Acc2, X0, X1, rsqrt;
    vboolN_2 tmp;
    ...
    pX2 = (const xb_vecN_2xf32 *)(x + 3 * N*L);
    ...
    for (l = 0; l < L; l += (BBE_SIMD_WIDTH / 2))
    {
        ...
        BBE_LVN_2XF32_XP(X0, pX0, 2 * BBE_SIMD_WIDTH);
    ...
    pX = (const xb_vecN_2xf32 *)(x + N*L);
    pZr = (const xb_vecN_2xf32 *)pScr;
    pV = (xb_vecN_2xf32 *)(v + L);
    for (l = 0; l < L; l += (BBE_SIMD_WIDTH / 2))
    {
        ...
        Acc = BBE_MULN_2XF32(X0, rsqrt);
        BBE_SVN_2XF32_X(Acc, pV, L * sizeof(float32_t));
        BBE_LVN_2XF32_IP(X0, pX, 2 * BBE_SIMD_WIDTH);
        Acc = BBE_MULN_2XF32(X0, rsqrt);
        BBE_SVN_2XF32_IP(Acc, pV, 2 * BBE_SIMD_WIDTH);
    }
