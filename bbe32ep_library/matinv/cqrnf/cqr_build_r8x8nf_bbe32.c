...
5. Number of columns for input matrices A must not exceed the number
   of rows: N <= M.
---------------------------------------------------------------------------*/
void  cqr_build_r8x8nf  (void *pScr,complex_float* _A,complex_float* _V,complex_float* _D,int L)
{
    ...
    NASSERT(L>0);
    if (L<=0) return;
    Hsize=cqrnfHousholder_getScratchSize(8,L);
    ...
    for(m=0; m<7; m++)
    {
        it[m].takeColumn (Z,Zt,A+2*m*(8+1),8-m,8,SA,L);
        it[m].housholder (pScr,pV,Fi+2*m*L,D+2*m,Z,Zt,8-m,SD,L);
        it[m].updateR (Z,A+2*m*(8+1),pV,SA,8-m,8-m,8,L);
        pV+=2*(8-m)*L;
    }
    cqrnfTakeColumn1(Z,A+2*m*(8+1),SA,L);
    cqrnfHousholder1(pV,Fi+2*m*L,D+2*m,Z,SD,L);
    cqrnfUpdateR1   (A+2*m*(8+1),pV,SA,L);

    // final rotation Fi'*R
    cqrnfrotateR8(A,Fi,L);
}
...
#else
DISCARD_FUN(void, cqr_build_r8x8nf, (void *pScr,complex_float* _A,complex_float* _V,complex_float* _D,int L))
size_t  cqr_build_r8x8nf_getScratchSize(int M, int N, int L)
{
    NASSERT(N == 8 && M == 8);
    NASSERT(L>0);
