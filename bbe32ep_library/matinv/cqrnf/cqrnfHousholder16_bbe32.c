                           ...
                           int M, int SD, int L)
{
    ...
    xb_vecN_2xf32 Acc, Acc0, Acc1, X0, X1, F0, rsqrt;
    ...
    for (l = 0; l < L; l++)
    {
        ...
        BBE_MULMASN_2XF32(Acc0, Acc0, 1.f, 0, 6);
            ...
            BBE_LAVN_2XF32_XP(X0, vX, pX, 2 * (M - 9)*sizeof(float32_t));
            BBE_MULAN_2XF32(Acc, X0, X0);
            BBE_LAVN_2XF32_XP(X0, vX, pX, 2 * (M - 13)*sizeof(float32_t));
    ...
    for (l = 0; l < L; l++)
    {
        ...
        BBE_SAN_2XF32_IP(Acc, vV, pV);
        Acc = X0;
        BBE_LAN_2XF32_IP(X0, vX, pX);
        X0 = BBE_MULN_2XF32(X0, rsqrt);
        Acc = BBE_SELN_2XF32I(X0, Acc, BBE_SELI_ROTATE_RIGHT_12);
        BBE_SAN_2XF32_IP(Acc, vV, pV);
        Acc = X0;
        BBE_LAVN_2XF32_XP(X0, vX, pX, 2 * (M - 8)*sizeof(float32_t));
        X0 = BBE_MULN_2XF32(X0, rsqrt);
        Acc = BBE_SELN_2XF32I(X0, Acc, BBE_SELI_ROTATE_RIGHT_12);
        BBE_SAVN_2XF32_XP(Acc, vV, pV, 2 * (M - 8)*sizeof(float32_t));
        Acc = X0;
        BBE_LAVN_2XF32_XP(X0, vX, pX, 2 * (M - 12)*sizeof(float32_t));
        X0 = BBE_MULN_2XF32(X0, rsqrt);
        Acc = BBE_SELN_2XF32I(X0, Acc, BBE_SELI_ROTATE_RIGHT_12);
        BBE_SAVN_2XF32_XP(Acc, vV, pV, 2 * (M - 12)*sizeof(float32_t));
    }
