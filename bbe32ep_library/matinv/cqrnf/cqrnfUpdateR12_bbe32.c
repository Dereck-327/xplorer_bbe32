                        ...
                        int SA, int M,int N, int N0, int L)
{
    ...
    xb_vecN_4x64 vTmp;
    xb_vecN_2xf32 Acc, Acc1, Acc2, Acc3, Acc4, Acc5, V0, R0, Z0, Z1, Z2;
    ...
    for (l = 0; l < L; l++)
    {
        ...
        for (m = 0; m < M; m++)
        {
            ...
            BBE_MULMASN_2XF32(Acc4, V0, R0, 2, 11);

            BBE_LVN_2XF32_XP(R0, pR, 2 * N0*sizeof(float32_t) - 2 * 2 * BBE_SIMD_WIDTH);
            BBE_MULMASN_2XF32(Acc2, V0, R0, 0, 4);
            BBE_MULMASN_2XF32(Acc5, V0, R0, 2, 11);
        }
        ...
        BBE_SVN_2XF32_IP(Z2, pZ, -2 * 2 * BBE_SIMD_WIDTH);
        ...
        for (m = 0; m < M; m++)
        {
            ...
            BBE_LVN_2XF32_IP(Z2, pZ, -2 * 2 * BBE_SIMD_WIDTH);
            ...
            BBE_LVN_2XF32_XP(Acc, pR, 2 * N0*sizeof(float32_t) - 2 * 2 * BBE_SIMD_WIDTH);
            BBE_MULMASN_2XF32(Acc, Z2, V0, 3, 4);
            BBE_MULMASN_2XF32(Acc, Z2, V0, 2, 11);
            BBE_SVN_2XF32_XP(Acc, pRw, 2 * N0*sizeof(float32_t) - 2 * 2 * BBE_SIMD_WIDTH);
        }
