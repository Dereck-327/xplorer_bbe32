...
#if !__OPTIMIZED__
{
    ...
    {
        for (l=0; l<L; l+=BBE_SIMD_WIDTH/2,X+=BBE_SIMD_WIDTH/2)
        {
    ...
    {
        NASSERT(layout==e3x3_block || layout==e4x4_block);
        for (l=0; l<L; l+=BBE_SIMD_WIDTH/2,X+=16*BBE_SIMD_WIDTH/2)
        {
            for (p=0; p<BBE_SIMD_WIDTH/2; p++)
            {
                for (n=0; n<16; n++) x[n][p]=X[p+n*BBE_SIMD_WIDTH/2];
            }
            for (p=0; p<BBE_SIMD_WIDTH/2; p++)
            {
...
#else
{
    ...
    {
        if (layout==e3x3_block) mask=BBE_LTRN(0);
        NASSERT( layout==e3x3_block || layout==e4x4_block);
    ...
    {
        BBE_LVNX16_XP(x0 , pX, estride);
        BBE_LVNX16_XP(x1 , pX, estride);
        BBE_LVNX16_XP(x2 , pX, estride);
        BBE_LVNX16_XP(x3 , pX, estride);
        BBE_LVNX16_XP(x4 , pX, estride);
        BBE_LVNX16_XP(x5 , pX, estride);
        BBE_LVNX16_XP(x6 , pX, estride);
        BBE_LVNX16_XP(x7 , pX, estride);
        BBE_LVNX16_XP(x8 , pX, estride);
        BBE_LVNX16T_XP(x9 , pX, estride,mask);
        BBE_LVNX16T_XP(x10, pX, estride,mask);
        BBE_LVNX16T_XP(x11, pX, estride,mask);
        BBE_LVNX16T_XP(x12, pX, estride,mask);
        BBE_LVNX16T_XP(x13, pX, estride,mask);
        BBE_LVNX16T_XP(x14, pX, estride,mask);
        BBE_LVNX16T_XP(x15, pX, -15*estride+mstride,mask);
        ...
        BBE_SVNX16_XP(y0 , pY, 2*estride);
        BBE_SVNX16_XP(y1 , pY, 2*estride); 
        BBE_SVNX16_XP(y2 , pY, 2*estride); 
        BBE_SVNX16_XP(y3 , pY, 2*estride); 
        BBE_SVNX16_XP(y4 , pY, 2*estride); 
        BBE_SVNX16_XP(y5 , pY, 2*estride); 
        BBE_SVNX16_XP(y6 , pY, 2*estride); 
        BBE_SVNX16_XP(y7 , pY, -13*estride); 

        BBE_SVNX16_XP(y8 , pY, 2*estride); 
        BBE_SVNX16_XP(y9 , pY, 2*estride); 
        BBE_SVNX16_XP(y10, pY, 2*estride); 
        BBE_SVNX16_XP(y11, pY, 2*estride); 
        BBE_SVNX16_XP(y12, pY, 2*estride); 
        BBE_SVNX16_XP(y13, pY, 2*estride); 
        BBE_SVNX16_XP(y14, pY, 2*estride); 
