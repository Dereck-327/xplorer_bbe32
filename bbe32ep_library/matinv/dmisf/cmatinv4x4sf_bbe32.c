...
/*
    ...
    Direct inversion of 4x4 complex floating point matrices, streaming data
...
void cmatinv4x4sf(void * restrict pScr, complex_float * restrict A, int L)
{
    ...
    cmatinv4x4sf_findAndPerm(permIx,A,L);
    cmatinv4x4f_inv(pScr, A, L, e4x4_stream);
    cmatinv4x4f_csb4x4_inplace(A,L,e4x4_stream);
    cmatinv4x4f_permute(A,A,permIx,matinv4x4_bkw_perm_tbl_bbe32,L,e4x4_stream);
    cmatinv4x4f_cbs4x4_inplace(A,L,e4x4_stream);
...
size_t cmatinv4x4sf_getScratchSize ( int L )
{
    ...
    sz+=cmatinv4x4f_inv_getScratchSize(L);
    return sz;
}
#else
DISCARD_FUN(void, cmatinv4x4sf,(void * pScr, complex_float * A, int L))
size_t cmatinv4x4sf_getScratchSize ( int L )
{
  return (0);
}
