...
/*
    ...
    Direct inversion of 3x3 complex floating point matrices, streaming data
...
void cmatinv3x3sf(void * restrict pScr, complex_float * restrict A, int L)
{
    ...
    cmatinv3x3f_csb3x3_inplace(A,L,e3x3_stream);
    cmatinv3x3f_search(permIx,A,L,e3x3_stream);
    cmatinv3x3f_permute(A,A,permIx,matinv3x3_fwd_perm_tbl_bbe32,L,e3x3_stream);
    cmatinv3x3f_cbs3x3_inplace(A,L,e3x3_stream);
    cmatinv3x3f_inv(pScr, A, L, e3x3_stream);
    cmatinv3x3f_csb3x3_inplace(A,L,e3x3_stream);
    cmatinv3x3f_permute(A,A,permIx,matinv3x3_bkw_perm_tbl_bbe32,L,e3x3_stream);
    cmatinv3x3f_cbs3x3_inplace(A,L,e3x3_stream);
...
size_t cmatinv3x3sf_getScratchSize ( int L )
{
    ...
    sz+=cmatinv3x3f_inv_getScratchSize(L);
    return sz;
}
#else
DISCARD_FUN(void, cmatinv3x3sf,(void * pScr, complex_float * A, int L))
size_t cmatinv3x3sf_getScratchSize ( int L )
{
  return (0);
}
