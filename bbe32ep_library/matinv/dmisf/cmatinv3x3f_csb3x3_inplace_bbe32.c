...
#if !__OPTIMIZED__
{
    ...
    {
        static const union { float32_t f[2]; complex_float c;} zero={{0.f,0.f}};
        ...
        for (l=0; l<L; l+=BBE_SIMD_WIDTH/4)
        {
            ...
            {
                x[p][ 0]=X[p+ 0*(BBE_SIMD_WIDTH/4)]; x[p][ 1]=X[p+ 1*(BBE_SIMD_WIDTH/4)]; x[p][ 2]=X[p+ 2*(BBE_SIMD_WIDTH/4)]; 
                x[p][ 3]=X[p+ 3*(BBE_SIMD_WIDTH/4)]; x[p][ 4]=X[p+ 4*(BBE_SIMD_WIDTH/4)]; x[p][ 5]=X[p+ 5*(BBE_SIMD_WIDTH/4)]; 
                x[p][ 6]=X[p+ 6*(BBE_SIMD_WIDTH/4)]; x[p][ 7]=X[p+ 7*(BBE_SIMD_WIDTH/4)]; x[p][ 8]=X[p+ 8*(BBE_SIMD_WIDTH/4)]; 
                x[p][ 9]=x[p][10]=x[p][11]=zero.c;
            }
...
#else
{
    ...
    if (layout==e3x3_stream)
    {
      ...
      for ( l=0; l<(L>>(LOG2_BBE_SIMD_WIDTH-2)); l++ )
      {
            ...
            X1 = BBE_LVN_4XCF32_X(pX,-6*L*sizeof(complex_float));
            X2 = BBE_LVN_4XCF32_X(pX,-5*L*sizeof(complex_float));
            X3 = BBE_LVN_4XCF32_X(pX,-4*L*sizeof(complex_float));
            X4 = BBE_LVN_4XCF32_X(pX,-3*L*sizeof(complex_float));
            X5 = BBE_LVN_4XCF32_X(pX,-2*L*sizeof(complex_float));
            X6 = BBE_LVN_4XCF32_X(pX,-1*L*sizeof(complex_float));
    ...
    else
    {
        ...
        for ( l=0; l<(L>>(LOG2_BBE_SIMD_WIDTH-2)); l++ )
        {
            BBE_LVN_4XCF32_XP(X0, pX, 2*BBE_SIMD_WIDTH);
            BBE_LVN_4XCF32_XP(X1, pX, 2*BBE_SIMD_WIDTH);
            BBE_LVN_4XCF32_XP(X2, pX, 2*BBE_SIMD_WIDTH);
            BBE_LVN_4XCF32_XP(X3, pX, 2*BBE_SIMD_WIDTH);
            BBE_LVN_4XCF32_XP(X4, pX, 2*BBE_SIMD_WIDTH);
            BBE_LVN_4XCF32_XP(X5, pX, 2*BBE_SIMD_WIDTH);
            BBE_LVN_4XCF32_XP(X6, pX, 2*BBE_SIMD_WIDTH);
            BBE_LVN_4XCF32_IP(X7, pX, 2*BBE_SIMD_WIDTH);
            BBE_LVN_4XCF32_IP(X8, pX, 4*2*BBE_SIMD_WIDTH);
            ...
            BBE_DSELN_4XCF32I(X5, X4, X5, X4, BBE_DSELI_INTERLEAVE_4);
            BBE_DSELN_4XCF32I(X7, X6, X7, X6, BBE_DSELI_INTERLEAVE_4);
            X9 = BBE_REPN_4XCF32(X8, 1);
            X10= BBE_REPN_4XCF32(X8, 2);
            X11= BBE_REPN_4XCF32(X8, 3);

            BBE_SVN_4XCF32_I (X1 , pY,  3*2*BBE_SIMD_WIDTH); //3
            BBE_SVN_4XCF32_I (X2 , pY,  6*2*BBE_SIMD_WIDTH); //6
            BBE_SVN_4XCF32_I (X3 , pY,  9*2*BBE_SIMD_WIDTH); //9
            BBE_SVN_4XCF32_I (X4 , pY,  1*2*BBE_SIMD_WIDTH); //1
            BBE_SVN_4XCF32_I (X5 , pY,  4*2*BBE_SIMD_WIDTH); //4
            BBE_SVN_4XCF32_I (X6 , pY,  7*2*BBE_SIMD_WIDTH); //7
            BBE_SVN_4XCF32_I (X7 , pY, 10*2*BBE_SIMD_WIDTH); //10
            BBE_SVN_4XCF32_I (X8 , pY,  2*2*BBE_SIMD_WIDTH); //2
            BBE_SVN_4XCF32_I (X9 , pY,  5*2*BBE_SIMD_WIDTH); //5
            BBE_SVN_4XCF32_I (X10, pY,  8*2*BBE_SIMD_WIDTH); //8
            BBE_SVN_4XCF32_I (X11, pY, 11*2*BBE_SIMD_WIDTH); //11
            BBE_SVN_4XCF32_IP( X0, pY, 12*2*BBE_SIMD_WIDTH); //0
        }
    }
}
#endif
