...
void matinv3x3nf(void * restrict pScr, float32_t* restrict A, int L)
{
    ...
    matinv3x3f_search(permIx,A, L, e3x3_block);
    if (L0)
    {
        matinv3x3f_permute(A,A,permIx,matinv3x3_fwd_perm_tbl_bbe32,L0,e3x3_block);
        matinv4x4f_rbs4x4_inplace(A,L0,e4x4_block);
        matinv3x3f_inv(pScr,A,L0,e3x3_block);
        matinv4x4f_rsb4x4_inplace(A,L0,e3x3_block);
        matinv3x3f_permute(A,A,permIx,matinv3x3_bkw_perm_tbl_bbe32,L0,e3x3_block);
    }
    if (L0!=L)
    {
        A+=16*L0;
        permIx+=L0;
        L-=L0;
        matinv3x3f_permute(X,A,permIx,matinv3x3_fwd_perm_tbl_bbe32,L,e3x3_block);
        matinv4x4f_rbs4x4_inplace(X,BBE_SIMD_WIDTH/2,e4x4_block);
        matinv3x3f_inv(pScr,X,BBE_SIMD_WIDTH/2,e3x3_block);
        matinv4x4f_rsb4x4_inplace(X,BBE_SIMD_WIDTH/2,e3x3_block);
        matinv3x3f_permute(A,X,permIx,matinv3x3_bkw_perm_tbl_bbe32,L,e3x3_block);
    }
}
...
size_t matinv3x3nf_getScratchSize ( int L )
{
    ...
    sz+=16*(BBE_SIMD_WIDTH/2)*sizeof(float32_t); /* X */
    sz+=L;  /* permIx */
    sz=(sz+2*BBE_SIMD_WIDTH-1)&~(2*BBE_SIMD_WIDTH-1);
    sz+=matinv3x3f_inv_getScratchSize(XT_MAX(L0,BBE_SIMD_WIDTH/2)); /* for matinv3x3f_inv */
    return sz;
    return (0);
}
#else
DISCARD_FUN(void, matinv3x3nf,(void * restrict pScr, float32_t* restrict A, int L))
size_t matinv3x3nf_getScratchSize ( int L )
{
  return (0);
}
