...
/*
    ...
    Direct inversion of 4x4 complex floating point matrices 
...
-------------------------------------------------------------------------*/
void cmatinv4x4nf(void * restrict pScr, complex_float * restrict _A, int L)
{
    complex_float *X;
    int16_t *permIx;
    int L0;
    ...
    /* allocate data in the scratch */
    {
        ...
        permIx=(int16_t*)(X+16*(BBE_SIMD_WIDTH/4));
        pScr=permIx+L;
    ...
    cmatinv4x4f_findUL(permIx,_A, L,e4x4_block);
    cmatinv4x4f_truncatedSearch(permIx,_A,L,e4x4_block);
    if (L0)
    {
        cmatinv4x4f_permute(_A,_A,permIx,matinv4x4_fwd_perm_tbl_bbe32,L0,e4x4_block);
        cmatinv4x4f_cbs4x4_inplace(_A,L0,e4x4_block);
        cmatinv4x4f_inv(pScr,_A,L0,e4x4_block);
        cmatinv4x4f_csb4x4_inplace(_A,L0,e4x4_block);
        cmatinv4x4f_permute(_A,_A,permIx,matinv4x4_bkw_perm_tbl_bbe32,L0,e4x4_block);
    }
    if (L!=L0)
    {   /* process remainder */
        _A+=16*L0;
        permIx+=L0;
        L-=L0;
        cmatinv4x4f_permute(X,_A,permIx,matinv4x4_fwd_perm_tbl_bbe32,L,e4x4_block);
        cmatinv4x4f_cbs4x4_inplace(X,BBE_SIMD_WIDTH/4,e4x4_block);
        cmatinv4x4f_inv(pScr,X,BBE_SIMD_WIDTH/4,e4x4_block);
        cmatinv4x4f_csb4x4_inplace(X,BBE_SIMD_WIDTH/4,e4x4_block);
        cmatinv4x4f_permute(_A,X,permIx,matinv4x4_bkw_perm_tbl_bbe32,L,e4x4_block);
...
size_t cmatinv4x4nf_getScratchSize ( int L )
{
    ...
    if (L<=0) return 0;
    L0=L&~(BBE_SIMD_WIDTH/4-1);
    sz+=16*(BBE_SIMD_WIDTH/4)*sizeof(complex_float); /* for A,B,C,D parts */
    sz+=L*sizeof(int16_t);  /* permIx */
    sz=(sz+2*BBE_SIMD_WIDTH-1)&~(2*BBE_SIMD_WIDTH-1);
    sz+=cmatinv4x4f_inv_getScratchSize(XT_MAX(BBE_SIMD_WIDTH/4,L0)); /* for cmatinv4x4hfs */
    return sz;
}

#else
DISCARD_FUN(void, cmatinv4x4nf,(void * pScr, complex_float * A, int L))
size_t cmatinv4x4nf_getScratchSize ( int L )
{
    (void)L;
    return (0);
