...
void lunf_update16(float32_t* A, const float32_t* restrict norm, int i, int N, int L)
{
    ...
    NASSERT(N-i==16);
    pNorm=(xtfloat*)norm;
    bi=BBE_LTRN_2I(1) &~ BBE_LTRN_2I(0);    /* 1 in i-th position */
    bk=~BBE_LTRN_2I(1);                    /* 1 for k>i          */
    pAw=pAj=(xb_vecN_2xf32*)&A[N ];
    pAi=(xb_vecN_2xf32 *)&A[0];
    lastinc=(SA*sizeof(float32_t) - (N*sizeof(float32_t) * (16-1)));
    for (l=0; l<L; l++)
    {
        int j;
