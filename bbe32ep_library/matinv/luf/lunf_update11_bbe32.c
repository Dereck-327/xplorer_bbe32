...
#if 0
{
    ...
    NASSERT(N-i==11);
    pNorm=(xtfloat*)norm;
    bi=BBE_LTRN_2I(6) &~ BBE_LTRN_2I(5);    /* 1 in i-th position */
    bk=~BBE_LTRN_2I(6);                    /* 1 for k>i          */
    pAw=pAj=(xb_vecN_2xf32*)&A[N ];
    pAi=(xb_vecN_2xf32 *)&A[0];
    lastinc=(SA*sizeof(float32_t) - (N*sizeof(float32_t) * (11-1)));
    for (l=0; l<L; l++)
    {
        int j;
...
#else
{
    ...
    for (l=0; l<L; l++)
    {
        ...
        Aj1=BBE_LVN_2XF32_I (pAj,2*BBE_SIMD_WIDTH);
        BBE_LVN_2XF32_XP(Aj0,pAj,lastinc+N*sizeof(float32_t));
        BBE_MULN_2XF32T(Aj0,Aj0,NORM,bi);
        Aji=BBE_REPN_2XF32(Aj0,5);
        BBE_MULSN_2XF32T(Aj0,Aji,Ai0,bk);
        BBE_MULSN_2XF32 (Aj1,Aji,Ai1);
        BBE_SVN_2XF32_I (Aj1,pAw,2*BBE_SIMD_WIDTH);
        BBE_SVN_2XF32_XP(Aj0,pAw,lastinc+N*sizeof(float32_t));
    }
}
#endif
#endif
