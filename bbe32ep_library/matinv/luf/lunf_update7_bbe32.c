...
void lunf_update7(float32_t* A, const float32_t* restrict norm, int i, int N, int L)
{
    ...
    NASSERT(N-i==7);
    pNorm=(xtfloat*)norm;
    bi=BBE_LTRN_2I(2) &~ BBE_LTRN_2I(1);    /* 1 in i-th position */
    bk=~BBE_LTRN_2I(2);                    /* 1 for k>i          */
    ...
    lastinc=(SA*sizeof(float32_t) - (N*sizeof(float32_t) * (7-1)));
    for (l=0; l<L; l++)
    {
