...
#if 0
{
    ...
    for (l=0; l<L; l++)
    {
        D[l]=mulc(mulc(mulc(LU[l],LU[l+L*(1+1*4)]),LU[l+L*(2+2*4)]),LU[l+L*(3+3*4)]);
    }
}
#else
{
    const xb_vecN_4xcf32 * restrict pLU00=(const xb_vecN_4xcf32 *)LU;
    const xb_vecN_4xcf32 * restrict pLU11=(const xb_vecN_4xcf32 *)(LU+(1+1*4)*L);
    const xb_vecN_4xcf32 * restrict pLU22=(const xb_vecN_4xcf32 *)(LU+(2+2*4)*L);
    const xb_vecN_4xcf32 * restrict pLU33=(const xb_vecN_4xcf32 *)(LU+(3+3*4)*L);
    ...
    {
        xb_vecN_4xcf32 d,a00,a11,a22,a33;
        BBE_LVN_4XCF32_IP(a00,pLU00,2*BBE_SIMD_WIDTH);
        BBE_LVN_4XCF32_IP(a11,pLU11,2*BBE_SIMD_WIDTH);
        BBE_LVN_4XCF32_IP(a22,pLU22,2*BBE_SIMD_WIDTH);
        BBE_LVN_4XCF32_IP(a33,pLU33,2*BBE_SIMD_WIDTH);
        d=BBE_MULN_4XCF32(BBE_MULN_4XCF32(a00,a11),BBE_MULN_4XCF32(a22,a33));
...
#else
DISCARD_FUN(void, cdet4x4sf, ( complex_float * restrict D, const complex_float * restrict LU, int L ))
#endif
