...
void clunf_update4(complex_float* A, const complex_float* restrict norm, int i, int N, int L)
{
    ...
    bi=BBE_LTRN_4I(1) &~ BBE_LTRN_4I(0);    /* 1 in i-th position */
    bk=~BBE_LTRN_4I(1);                    /* 1 for k>i          */
    pAw=pAj=(xb_vecN_4xcf32*)&A[N + (i&~3)];
    pAi=(xb_vecN_4xcf32 *)&A[(i&~3)];
    lastinc=(SA*sizeof(complex_float) - (N*sizeof(complex_float) * (4-1)));
    for (l=0; l<L; l++)
    {
