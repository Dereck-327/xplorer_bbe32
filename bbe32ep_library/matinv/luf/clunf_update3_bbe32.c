...
void clunf_update3(complex_float* A, const complex_float* restrict norm, int i, int N, int L)
{
    ...
    xb_vecN_4xcf32 Aj,Ai,Aji,NORM;
    int lastinc;

    NASSERT_ALIGN(A,2*BBE_SIMD_WIDTH);
    NASSERT(N-i==3);
    pNorm=(xtcomplexfloat*)norm;
    bi=BBE_LTRN_4I(2) &~ BBE_LTRN_4I(1);    /* 1 in i-th position */
    bk=~BBE_LTRN_4I(2);                    /* 1 for k>i          */
    pAw=pAj=(xb_vecN_4xcf32*)&A[N + (i&~3)];
    pAi=(xb_vecN_4xcf32 *)&A[(i&~3)];
    lastinc=(SA*sizeof(complex_float) - (N*sizeof(complex_float) * (3-1)));
    for (l=0; l<L; l++)
    {
        BBE_LVN_4XCF32_XP(Ai,pAi,SA*sizeof(complex_float));
        BBE_LSN_4XCF32_IP(NORM,pNorm,sizeof(complex_float));
        NORM=BBE_REPN_4XCF32(NORM,0);

        BBE_LVN_4XCF32_XP(Aj,pAj,N*sizeof(complex_float));
        BBE_MULN_4XCF32T(Aj,Aj,NORM,bi);
        Aji=BBE_REPN_4XCF32(Aj,1);
        BBE_MULSN_4XCF32T(Aj,Aji,Ai,bk);
        BBE_SVN_4XCF32_XP(Aj,pAw,N*sizeof(complex_float));

        BBE_LVN_4XCF32_XP(Aj,pAj,lastinc+N*sizeof(complex_float));
        BBE_MULN_4XCF32T(Aj,Aj,NORM,bi);
        Aji=BBE_REPN_4XCF32(Aj,1);
        BBE_MULSN_4XCF32T(Aj,Aji,Ai,bk);
        BBE_SVN_4XCF32_XP(Aj,pAw,lastinc+N*sizeof(complex_float));
    }
}
#endif
