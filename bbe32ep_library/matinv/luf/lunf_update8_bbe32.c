...
void lunf_update8(float32_t* A, const float32_t* restrict norm, int i, int N, int L)
{
    ...
    NASSERT(N-i==8);
    pNorm=(xtfloat*)norm;
    bi=BBE_LTRN_2(1);        /* 1 in i-th position */
    bk=~BBE_LTRN_2(1);       /* 1 for k>i          */
    ...
    lastinc=(SA*sizeof(float32_t) - (N*sizeof(float32_t) * (8-1)));
    for (l=0; l<L; l++)
    {
