...
#if 0
{
    ...
    for (l=0; l<L; l++)
    {
        D[l]=mulc(mulc(LU[l],LU[l+L*(1+1*3)]),LU[l+L*(2+2*3)]);
    }
}
#else
{
    ...
    {
        xb_vecN_4xcf32 d,a00,a11,a22;
        BBE_LVN_4XCF32_XP(a00,pLU00,2*BBE_SIMD_WIDTH);
        BBE_LVN_4XCF32_XP(a11,pLU11,2*BBE_SIMD_WIDTH);
        BBE_LVN_4XCF32_XP(a22,pLU22,2*BBE_SIMD_WIDTH);
        d=BBE_MULN_4XCF32(BBE_MULN_4XCF32(a00,a11),a22);
        BBE_SVN_4XCF32_IP(d,pD,2*BBE_SIMD_WIDTH);
    }
}
#endif
#else
DISCARD_FUN(void, cdet3x3sf, ( complex_float * restrict D, const complex_float * restrict LU, int L ))
#endif
