...
#if 0
{
    return bcholfwdwxnx1n (Yt,Rt, D, At, Bt, qB,qY,16,32,L);
}
#else
{
    const int W=16;
    ...
    int l,n,M;
    const int SB=2*getSpace((W+N-1));
    const int SY=2*getSpace(N);
    const int SA=2*N*W;
    ...
    for(n=0; n<N; n++)
    {
        ...
        for (l=0; l<L; l++)
        {
            ...
            BBE_LANX16_IP(AN0,va,pAt);
            BBE_LANX16_IP(AN1,va,pAt);
            ...
            vb=BBE_LA_PP(pB);
            BBE_LANX16_IP(BP0,vb,pB);
            BBE_LANX16_IP(BP1,vb,pB);
