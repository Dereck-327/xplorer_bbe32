            ...
            int qXYA, int N, int L)
{
    ...
    xb_vecNx40 W0;
    xb_c40 I0;
    xb_vecNx16 D0,t,YP,RN,XN,lo,hi,cqXRY;
    vsaN D1,vqXRY=BBE_MOVVSA32(qXYA);
    int n,M,l,SN=getSpace(N);
    uint32_t idx,mod;
    vselN reverseW1;    // reverse first W-1 complex elements
    // check alignment
    ...
    t=BBE_SEQNX16();
    XN=BBE_MOVVA16((W-1)<<1);
    t=BBE_ADDNX16(t,XN);
    t=BBE_SHFLNX16I(t,BBE_SHFLI_REVERSE_2);
    reverseW1=BBE_MOVVSELNX16(t,0);
    mod=((N*W)<<18)|((W-1)<<2);
    RN=0;
    ...
    for (n=0; n<N; n++)
    {
        ...
        for (l=0; l<L; l++)
        {
            ...
            vx=BBE_LA_PP(pX);
            BBE_LAVNX16_XP(XN,vx,pX,M*4);
            pX =(const xb_vecNx16*)XT_ADDX4(-M+SN,(uintptr_t)pX);
            BBE_MULSNX16C(W0,RN,XN);
            I0=BBE_RADDNX40C(W0);
