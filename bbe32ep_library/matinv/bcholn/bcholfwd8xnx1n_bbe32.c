            ...
            int N, int L)
{
    ...
    xb_vecNx16 t,AN,RN,BP,D0,YP,lo,hi,ZERO=0;
    xb_vecNx40 W0;
    xb_c40 I0;
    valign vb,vy;
    vsaN D1,vsa_qYB=BBE_MOVVSA32(qYB);
    vselN rotate;
    vboolN validM;  // true for first M valid elements in R
    int l,n,M;
    ...
    {
        M = XT_MIN((W-1),n);
        // make select register: (14-2*M),(15-2*M),...
        t=BBE_SEQNX16(); rotate=BBE_MOVVSELNX16(t,0); BBE_SELUNX16(t,t,t,rotate,14-(M<<1));
        validM =  BBE_LTRN(M<<1);
        Yt=_Yt;
        D =_D;
        Bt=_Bt;
        ...
        for (l=0; l<L; l++)
        {
            NASSERT_ALIGN32(pRt);
            NASSERT_ALIGN32(pAt);
            BBE_LVNX16_XP(t,pRt,SA*2);
            t=BBE_SELNX16(ZERO,t,rotate);
            RN=BBE_MOVNX16T(t,ZERO,validM);
            BBE_LVNX16_XP(AN,pAt,SA*2);
            // load diagonals
