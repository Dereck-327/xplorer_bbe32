...
/*
    ...
    Convert real square upper-Hessenberg matrices from full stream to 
    compact block order
...
void reigen_s2hn_nxnf(float32_t * restrict y,     /* y[L][S(sz_f32, N*(N+3)/2-1)] */
                ...
                int N, int L)
{
    const xb_vecN_2xf32 * restrict pX_r;
          xb_vecN_2xf32 * restrict pX_w;
    ...
    for (i = 0; i<N; i++){
        int J = (i>1 ? i - 1 : 0);
        for (j = J; j < N; j++){
            pX_w = (xb_vecN_2xf32 *)&x[(offset + j)*L];
            pX_r = (xb_vecN_2xf32 *)&x[(i*N + j)*L];
            for (k = 0; k < L / (BBE_SIMD_WIDTH / 2); k++) {
                BBE_LVN_2XF32_IP(val, pX_r, sz_f32*(BBE_SIMD_WIDTH / 2));
                BBE_SVN_2XF32_IP(val, pX_w, sz_f32*(BBE_SIMD_WIDTH / 2));
            }
        }        
        offset += (N - i);
    }
