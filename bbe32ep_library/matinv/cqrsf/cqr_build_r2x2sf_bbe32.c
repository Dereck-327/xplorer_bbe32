...
-------------------------------------------------------------------------*/
void cqr_build_r2x2sf   ( void* pScr, complex_float* restrict _V, complex_float * restrict _R, complex_float * restrict _D, int L) 
{
    float32_t *V=(float32_t *)_V;
    float32_t *R=(float32_t *)_R;
    float32_t *D=(float32_t *)_D;

    int SV=(2*2-2+1)*2; // number of elements in Householder without rotator
    float32_t* Fi;
    float32_t* pV;
    float32_t* pA;
    ...
    cqrfsHousholder2(pScr,pV,Fi,D,pA,SV,2,2,L);
    cqrfsUpdateR2(pA,pV,0,2,2,L);
    pA+=2*(2+1)*L; 
    pV+=2*(2)*L;
    D+=2*L;
    Fi+=2*L;
    cqrfsHousholder1(pV,Fi,D,pA,SV,1,2,L);
    cqrfsUpdateR1(pA,pV,1,1,2,L);

    Fi=V+SV*L;
    // final rotation Fi'*R
    cqrfsRotateR2(R,Fi,2,2,L);
}
...
#else
DISCARD_FUN(void, cqr_build_r2x2sf, ( void* pScr, complex_float* restrict _V, complex_float * restrict _R, complex_float * restrict _D, int L))
size_t cqr_build_r2x2sf_getScratchSize  (int M, int N,int L) 
{
    (void)M,(void)N,(void)L;
    NASSERT(M==2 && N==2);
