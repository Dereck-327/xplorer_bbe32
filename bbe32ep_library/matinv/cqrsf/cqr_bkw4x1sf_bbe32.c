            ...
            int N,int P,int L)
{
    ...
    xb_vecN_2xf32 Acc, Acc1, X0, R0, D0;
    ...
    pD = (const xb_vecN_2xf32 *)(D + 4 * L);
    pX = (const xb_vecN_2xf32 *)(x + 4 * L);
    pXw = (xb_vecN_2xf32 *)(x + 4 * L);
    pR = (const xb_vecN_2xf32 *)(R + 22 * L);
    for (l = 0; l < L; l += (BBE_SIMD_WIDTH / 4))
    {
        X0 = BBE_LVN_2XF32_X(pX, 2 * L * sizeof(float32_t));
        D0 = BBE_LVN_2XF32_X(pD, 2 * L * sizeof(float32_t));
        X0 = BBE_MULN_2XF32(X0, D0);
        BBE_SVN_2XF32_X(X0, pXw, 2 * L * sizeof(float32_t));
    ...
    pD = (const xb_vecN_2xf32 *)(D + 2 * L);
    pX = (const xb_vecN_2xf32 *)(x + 2 * L);
    pXw = (xb_vecN_2xf32 *)(x + 2 * L);
    pR = (const xb_vecN_2xf32 *)(R + 12 * L);
    for (l = 0; l < L; l += (BBE_SIMD_WIDTH / 4))
    {
        X0 = BBE_LVN_2XF32_X(pX, 4 * L * sizeof(float32_t));
        R0 = BBE_LVN_2XF32_X(pR, 2 * L * sizeof(float32_t));
        Acc = BBE_MULMN_2XF32(X0, R0, 3, 4);
        BBE_MULMASN_2XF32(Acc, X0, R0, 2, 11);

        X0 = BBE_LVN_2XF32_X(pX, 2 * L * sizeof(float32_t));
        BBE_LVN_2XF32_IP(Acc1, pX, 2 * BBE_SIMD_WIDTH);
        BBE_LVN_2XF32_IP(R0, pR, 2 * BBE_SIMD_WIDTH);
    ...
    for (l = 0; l < L; l += (BBE_SIMD_WIDTH / 4))
    {
        ...
        x2 = BBE_LVN_2XF32_X(pX, 6 * L * sizeof(float32_t));
        x1 = BBE_LVN_2XF32_X(pX, 4 * L * sizeof(float32_t));
        BBE_LVN_2XF32_IP(Acc, pX, 0);

        X0 = x2;
        R0 = BBE_LVN_2XF32_X(pR, 4 * L * sizeof(float32_t));
...
void cqr_bkw4x1sf  (void* pScr, complex_float* restrict B, const complex_float* restrict R, const complex_float* restrict D,int L)
{
    ...
    cqrsfBkw4x1((float32_t*)B, (float32_t*)R, (float32_t*)D,4,1,L);
}
...
#else
DISCARD_FUN(void, cqr_bkw4x1sf, (void* pScr, complex_float* restrict B, const complex_float* restrict R, const complex_float* restrict D,int L))
size_t cqr_bkw4x1sf_getScratchSize(int N, int P, int L)
{ 
    (void)N,(void)P,(void)L;
    NASSERT(L>0 && L%(BBE_SIMD_WIDTH/4)==0);
