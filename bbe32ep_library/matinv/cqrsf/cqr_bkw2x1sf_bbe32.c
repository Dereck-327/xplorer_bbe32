...
void cqr_bkw2x1sf  (void* pScr, complex_float* restrict B, const complex_float* restrict R, const complex_float* restrict D,int L)
{
    ...
    cqrsfBkw2x1((float32_t*)B, (float32_t*)R, (float32_t*)D,2,1,L);
}
...
#else
DISCARD_FUN(void, cqr_bkw2x1sf, (void* pScr, complex_float* restrict B, const complex_float* restrict R, const complex_float* restrict D,int L))
size_t cqr_bkw2x1sf_getScratchSize(int N, int P, int L) 
{ 
    (void)N,(void)P,(void)L;
    NASSERT(L>0 && L%(BBE_SIMD_WIDTH/4)==0);
