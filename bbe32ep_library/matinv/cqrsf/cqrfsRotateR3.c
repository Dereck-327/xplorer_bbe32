...
void cqrfsRotateR3(float32_t* restrict R,const float32_t* restrict Fi,int M, int N,int L)
{
    ...
    pRr0 = (const xb_vecN_2xf32 *)(R + 6 * L);
    pRr1 = (const xb_vecN_2xf32 *)(R + 12 * L);
    pF = (const xb_vecN_2xf32 *)Fi;
    pF0 = (const xb_vecN_2xf32 *)(Fi + 2 * L);
    pF1 = (const xb_vecN_2xf32 *)(Fi + 4 * L);
    pR = (xb_vecN_2xf32 *)R;
    pR0 = (xb_vecN_2xf32 *)(R + 6 * L);
    pR1 = (xb_vecN_2xf32 *)(R + 12 * L);

    for (l = 0; l < L; l += (BBE_SIMD_WIDTH / 4))
    {
        BBE_LVN_2XF32_IP(F0, pF, 2 * BBE_SIMD_WIDTH);

        R0 = BBE_LVN_2XF32_X(pRr, 4 * L * sizeof(float32_t));
        Acc = BBE_MULMN_2XF32(F0, R0, 0, 4);
        BBE_MULMASN_2XF32(Acc, F0, R0, 2, 11);
        BBE_SVN_2XF32_X(Acc, pR, 4 * L * sizeof(float32_t));

        R0 = BBE_LVN_2XF32_X(pRr, 2 * L * sizeof(float32_t));
        Acc = BBE_MULMN_2XF32(F0, R0, 0, 4);
        BBE_MULMASN_2XF32(Acc, F0, R0, 2, 11);
        BBE_SVN_2XF32_X(Acc, pR, 2 * L * sizeof(float32_t));
        ...
        R0 = BBE_LVN_2XF32_X(pRr0, 4 * L * sizeof(float32_t));
        Acc = BBE_MULMN_2XF32(F0, R0, 0, 4);
        BBE_MULMASN_2XF32(Acc, F0, R0, 2, 11);
        BBE_SVN_2XF32_X(Acc, pR0, 4 * L * sizeof(float32_t));

        R0 = BBE_LVN_2XF32_X(pRr0, 2 * L * sizeof(float32_t));
        Acc = BBE_MULMN_2XF32(F0, R0, 0, 4);
        BBE_MULMASN_2XF32(Acc, F0, R0, 2, 11);
        BBE_SVN_2XF32_X(Acc, pR0, 2 * L * sizeof(float32_t));
        ...
        BBE_LVN_2XF32_IP(F0, pF1, 2 * BBE_SIMD_WIDTH);

        R0 = BBE_LVN_2XF32_X(pRr1, 4 * L * sizeof(float32_t));
        Acc = BBE_MULMN_2XF32(F0, R0, 0, 4);
        BBE_MULMASN_2XF32(Acc, F0, R0, 2, 11);
        BBE_SVN_2XF32_X(Acc, pR1, 4 * L * sizeof(float32_t));

        R0 = BBE_LVN_2XF32_X(pRr1, 2 * L * sizeof(float32_t));
        Acc = BBE_MULMN_2XF32(F0, R0, 0, 4);
        BBE_MULMASN_2XF32(Acc, F0, R0, 2, 11);
        BBE_SVN_2XF32_X(Acc, pR1, 2 * L * sizeof(float32_t));

        BBE_LVN_2XF32_IP(R0, pRr1, 2 * BBE_SIMD_WIDTH);
        Acc = BBE_MULMN_2XF32(F0, R0, 0, 4);
        BBE_MULMASN_2XF32(Acc, F0, R0, 2, 11);
        BBE_SVN_2XF32_IP(Acc, pR1, 2 * BBE_SIMD_WIDTH);
    }
}
#endif
