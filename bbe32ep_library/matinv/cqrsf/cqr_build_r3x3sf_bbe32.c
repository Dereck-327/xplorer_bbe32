...
void cqr_build_r3x3sf   ( void* pScr, complex_float* restrict _V, complex_float * restrict _R, complex_float * restrict _D, int L) 
{
    float32_t *V=(float32_t *)_V;
    float32_t *R=(float32_t *)_R;
    float32_t *D=(float32_t *)_D;
    int SV=(2*3-3+1)*3; // number of elements in Householder without rotator
    float32_t* Fi;
    float32_t* pV;
    float32_t* pA;
    ...
    {
        cqrfsHousholder3(pScr,pV,Fi,D,pA,SV,3,3,L);
        cqrfsUpdateR3(pA,pV,0,3,3,L);
        pA+=2*(3+1)*L; 
        pV+=2*(3)*L;
        D+=2*L;
        Fi+=2*L;
    ...
    {
        cqrfsHousholder2(pScr,pV,Fi,D,pA,SV,2,3,L);
        cqrfsUpdateR2(pA,pV,1,2,3,L);
        pA+=2*(3+1)*L; 
        pV+=2*(2)*L;
        D+=2*L;
        Fi+=2*L;
    }
    cqrfsHousholder1(pV,Fi,D,pA,SV,1,3,L);
    cqrfsUpdateR1(pA,pV,2,1,3,L);

    Fi=V+SV*L;
    // final rotation Fi'*R
    cqrfsRotateR3(R,Fi,3,3,L);
}
...
#else
DISCARD_FUN(void, cqr_build_r3x3sf, ( void* pScr, complex_float* restrict _V, complex_float * restrict _R, complex_float * restrict _D, int L))
size_t cqr_build_r3x3sf_getScratchSize  (int M, int N,int L) 
{
    (void)M,(void)N,(void)L;
    NASSERT(M==3 && N==3);
