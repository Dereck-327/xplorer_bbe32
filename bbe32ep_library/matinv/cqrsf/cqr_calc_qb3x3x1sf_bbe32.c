...
---------------------------------------------------------------------------*/
void cqr_calc_qb3x3x1sf  (void *pScr, complex_float *_B, const complex_float *_V , int L)
{
    ...
    cqrsfUpdateB3(B+2*0*L,V,L);   V+=2*3*L;
    cqrsfUpdateB2(B+2*1*L,V,L);   V+=2*2*L;
    cqrsfUpdateB1(B+2*2*L,V,L);   V+=2*1*L;
    cqrsfRotateBconj1(B,V,3,L);
}
...
#else
DISCARD_FUN(void,cqr_calc_qb3x3x1sf, (void *pScr, complex_float *B, const complex_float *V , int L))
size_t cqr_calc_qb3x3x1sf_getScratchSize(int M, int P, int L) 
{ 
    (void)M,(void)P,(void)L;
    NASSERT(L>0 && L%(BBE_SIMD_WIDTH/4)==0);
