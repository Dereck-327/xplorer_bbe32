...
/*
    ...
    Complex NxN block ordered matrices
...
#define EIGNF_BLK  (BBE_SIMD_WIDTH/4)
...
void eigennxnnf ( 
            ...
            int N, int L )
{
  #define BLK  EIGNF_BLK

  complex_float *Ab,*As,*Hb,*Pb,*Ps,*Ts,*Vs;
  int SA,Se,SV,SH;
  int k,p,P;
...
{ 
  NASSERT( N>0 && 0==(N%4) );
  return ( EIGNF_BLK*        (          N*N )*sz_f32c +  /* As: a block of input matrices in stream order */
           EIGNF_BLK*getSpace( sz_f32c, N*N )*sz_f32c +  /* Pb: transformation matrix in block order */
           EIGNF_BLK*        (          N*N )*sz_f32c +  /* Ps: transformation matrix in stream order */
           EIGNF_BLK*        (          N*N )*sz_f32c);  /* Vs: matrix of eigenvectors in stream order  */
}

#else /* HAVE_VFPU */

DISCARD_FUN( void, eigennxnnf, ( void * pScr,
                        complex_float * restrict e,
                        complex_float * restrict V,
                        complex_float * restrict A,
                        int N, int L ) )
