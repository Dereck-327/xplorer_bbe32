...
void  qr_build_r16x16nf(void *pScr,float32_t* A,float32_t* V,float32_t* D,int L)
{
    ...
    float32_t* Z,*Zt;
    float32_t* Fi;
    float32_t* pV;    NASSERT_ALIGN(pScr,2*BBE_SIMD_WIDTH);
    NASSERT_ALIGN(A,2*BBE_SIMD_WIDTH);
    NASSERT_ALIGN(V,2*BBE_SIMD_WIDTH);
    NASSERT_ALIGN(D,2*BBE_SIMD_WIDTH);
    NASSERT(L>0);
    if (L<=0) return;
    Hsize=qrnfHousholder_getScratchSize(16,L);
    ...
    Fi=V+(((2*16-16+1)*16)>>1)*L;
    pV=V;
    for(m=0; m<16-1; m++)
    {
        it[m].takeColumn (Z,Zt,A+m*(16+1),16-m,16,16*16,L);
        it[m].housholder (pScr,pV,Fi+m*L,D+m,Z,Zt,16-m,16,L);
        it[m].updateR (Z,A+m*(16+1),pV,16*16,16-m,16-m,16,L);
        pV+=(16-m)*L;
    }
    qrnfTakeColumn1(Z,A+m*(16+1),16*16,L);
    qrnfHousholder1(pV,Fi+m*L,D+m,Z,16,L);
    qrnfUpdateR1   (A+m*(16+1),pV,16*16,L);

    // final rotation Fi'*R
    qrnfRotateR16(A,Fi,L);
}
...
#else
DISCARD_FUN(void, qr_build_r16x16nf, (void *pScr,float32_t* A,float32_t* V,float32_t* D,int L))
size_t  qr_build_r16x16nf_getScratchSize(int M, int N,int L)
{
    NASSERT(N==16 && M==16);
    NASSERT(L>0);
