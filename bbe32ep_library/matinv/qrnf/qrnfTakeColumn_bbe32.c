...
void qrnfTakeColumn(float32_t* restrict x,float32_t* restrict xt,const float32_t* restrict A,int M,int N,int SA,int L)
{
    ...
    const xtfloat       * restrict pA0 = (const xtfloat       *)A;
    ...
    for (l = 0; l < L; l++)
    {
        ...
        for (m = 0; m < M / (BBE_SIMD_WIDTH / 2); m++)
        {
            ...
            BBE_SSN_2XF32_XP(t1, pXt, L * sizeof(float32_t));
            BBE_LSN_2XF32_XP(t2, pA, N * sizeof(float32_t));
            BBE_SSN_2XF32_XP(t2, pXt, L * sizeof(float32_t));
            BBE_LSN_2XF32_XP(t3, pA, N * sizeof(float32_t));
            BBE_SSN_2XF32_XP(t3, pXt, L * sizeof(float32_t));
            BBE_LSN_2XF32_XP(t4, pA, N * sizeof(float32_t));
            BBE_SSN_2XF32_XP(t4, pXt, L * sizeof(float32_t));
            BBE_LSN_2XF32_XP(t5, pA, N * sizeof(float32_t));
            BBE_SSN_2XF32_XP(t5, pXt, L * sizeof(float32_t));
            BBE_LSN_2XF32_XP(t6, pA, N * sizeof(float32_t));
            BBE_SSN_2XF32_XP(t6, pXt, L * sizeof(float32_t));
            BBE_LSN_2XF32_XP(t7, pA, N * sizeof(float32_t));
            BBE_SSN_2XF32_XP(t7, pXt, L * sizeof(float32_t));
        ...
        pA0 = (const xtfloat *)XT_ADDX4(SA, (uintptr_t)pA0);
    }
