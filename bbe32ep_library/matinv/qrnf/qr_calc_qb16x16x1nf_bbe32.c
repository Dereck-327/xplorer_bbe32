...
static void qrnfUpdateB16(float32_t* B, const float32_t* V, int M, int L)
{
    int l;
    int count;
    ...
    xb_vecN_2xf32 Acc, V0, V1, B0, B1, Z0;
    valign vV;
    ...
    count = (M - 8) * sizeof(float32_t);
    mask_ = vselN_loadx((const vselN *)p_mask, count * 8);
    mask_2 = BBE_LTRN_2((16 - M));
...
static void qrnfUpdateB8(float32_t* B, const float32_t* V, int M, int L)
{
    ...
    for (l = 0; l < L; l++)
    {
        BBE_LAVN_2XF32_XP(V0, vV, pV_, count);
        V0 = BBE_MOVN_2XF32_FROMNX16(BBE_SELNX16(BBE_MOVNX16_FROMN_2XF32(V0), BBE_MOVNX16_FROMN_2XF32(V0), mask_));
        BBE_LVN_2XF32T_XP(B0, pB, 4 * BBE_SIMD_WIDTH, mask_2);
        Acc = BBE_MULN_2XF32(V0, B0);
        ...
        BBE_MULSN_2XF32(B0, Z0, V0);
        BBE_SVN_2XF32T_XP(B0, pBw, 4 * BBE_SIMD_WIDTH, mask_2);
    }
}

static void qrnfRotateB1_16(void *pScr, float32_t* B, const float32_t* Fi, int N, int SB, int L)
{
    int l;

    const xtfloat       * restrict pF;
    const xtfloat       * restrict pF1;
    const xb_vecN_2xf32 * restrict pB;
    xb_vecN_2xf32 * restrict pBw;
    ...
    for (l = 0; l < L; l++)
    {
        ...
        t7 = BBE_LSN_2XF32_X(pF1, 7 * L*sizeof(float32_t));
        t6 = BBE_LSN_2XF32_X(pF1, 6 * L*sizeof(float32_t));
        t5 = BBE_LSN_2XF32_X(pF1, 5 * L*sizeof(float32_t));
        t4 = BBE_LSN_2XF32_X(pF1, 4 * L*sizeof(float32_t));
        t3 = BBE_LSN_2XF32_X(pF1, 3 * L*sizeof(float32_t));
        t2 = BBE_LSN_2XF32_X(pF1, 2 * L*sizeof(float32_t));
        t1 = BBE_LSN_2XF32_X(pF1, L*sizeof(float32_t));
        BBE_LSN_2XF32_IP(t0, pF1, sizeof(float32_t));
        t0 = BBE_SELN_2XF32I(t1, t0, BBE_SELI_INTERLEAVE_2_LO);
        t2 = BBE_SELN_2XF32I(t3, t2, BBE_SELI_INTERLEAVE_2_LO);
        t4 = BBE_SELN_2XF32I(t5, t4, BBE_SELI_INTERLEAVE_2_LO);
        t6 = BBE_SELN_2XF32I(t7, t6, BBE_SELI_INTERLEAVE_2_LO);
                    ...
                    float32_t* B,const float32_t* V,int L)
{
    ...
    for (pV = V, m = 0; m < 16; m++)
    {
        qrnfUpdateB(B + m, pV, (16 - m), L);
        pV += (16 - m)*L;
    }
    qrnfRotateB1_16(pScr, B, V + (((2 * 16 - 16 + 1) * 16) >> 1)*L, 16, 16, L);
#endif // 0
    ...
    qrnfRotateB1_16(pScr, B, V, 16, 16, L);
}
...
#else
DISCARD_FUN(void, qr_calc_qb16x16x1nf, (void *pScr,
                    float32_t* B,const float32_t* V,int L))
