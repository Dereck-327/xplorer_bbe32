                           ...
                           int M, int SD, int L)
{
    ...
    for (l = 0; l < L; l++)
    {
        ...
        BBE_LAVN_2XF32_XP(X0, vX, pX, (M - 9)*sizeof(float32_t));
    ...
    for (l = 0; l < L; l++)
    {
        ...
        Acc = X0;
        BBE_LAVN_2XF32_XP(X0, vX, pX, (M - 8)*sizeof(float32_t));
        X0 = BBE_MULN_2XF32(X0, rsqrt);
        Acc = BBE_SELN_2XF32I(X0, Acc, BBE_SELI_ROTATE_RIGHT_14);
        BBE_SAVN_2XF32_XP(Acc, vV, pV, (M - 8)*sizeof(float32_t));
    }
