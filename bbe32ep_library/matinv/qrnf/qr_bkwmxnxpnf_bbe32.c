            ...
            int M,int N,int P, int L)
{
    ...
    for (k = N - 1; k >= 0; k--)
    {
        for (l = 0; l < L; l++)
        {
            const float32_t* pRt = R + l*SR + (k*N + (k + 1));
    ...
    for (l = 0; l < L; l++)
    {
        ...
        for (k = N - 1; k >= 0; k--)
        {
            ...
            pY = (const xb_vecN_2xf32 *)(x + l*SX + k*P);
            ...
            for (p = P; p > 0; p -= BBE_SIMD_WIDTH)
            {
                pR = (const xtfloat *)(R + l*SR + (k*N + (k + 1)));
                pX0 = pX_;
            ...
            int M,int N,int L)
{
    ...
    for (; k >= XT_MAX(N - 16 - 1, 0); k--, M++)
    {
        ...
        for (l = 0; l < L; l++)
        {
            ...
            BBE_LAVN_2XF32_XP(R0, vR, pR, M * sizeof(float32_t));
            BBE_MULSN_2XF32(Acc, X0, R0);
            BBE_LAVN_2XF32_XP(X0, vX, pX, (M - (BBE_SIMD_WIDTH / 2)) * sizeof(float32_t));
            BBE_LAVN_2XF32_XP(R0, vR, pR, (M - (BBE_SIMD_WIDTH / 2)) * sizeof(float32_t));
            BBE_MULSN_2XF32(Acc, X0, R0);
                    ...
                    int L)
{
    ...
    if (P==1)  qrnfBkwnx1(X,R,D,M,N,L);
    else       qrnfBkwnxp(X,R,D,M,N,P,L);
}
...
#else
DISCARD_FUN(void, qr_bkwmxnxpnf, (void *pScr,
                    float32_t* X,
                    const float32_t* R,
                    const float32_t* D,
                    int M, int N, int P,
