...
{
    int n;
    static const int16_t ALIGN(32) SEL[BBE_SIMD_WIDTH] = { 0, 24, 0, 24, 0, 24, 0, 24, 0, 24, 0, 24, 0, 24, 0, 24 };
    ...
    x0 = BBE_LVNX16_I((const xb_vecNx16*)SEL, 0);
    sel0 = BBE_MOVVSELNX16(x0, 0);

#ifdef COMPILER_XTENSA
#pragma ymemory(pY)
#pragma loop_count min=4, factor=4
#endif

    for (n = 0; n<(N >> (LOG2_BBE_SIMD_WIDTH - 3)); n++)
    {
        BBE_LVNX16_IP(x0, pX0, 6 * BBE_SIMD_WIDTH);
        BBE_LVNX16_IP(x1, pX1, 6 * BBE_SIMD_WIDTH);
