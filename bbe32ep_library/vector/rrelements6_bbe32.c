...
void rrelements6 ( int16_t* restrict y, const int16_t* restrict x, int N )
{
    ...
    static const int16_t ALIGN(32) SEL[][BBE_SIMD_WIDTH] =
    {
        ...
        { 4, 5, 6, 7, 9, 10, 11, 12, 13, 15, 16, 17, 18, 19, 21, 22 },
        { 7, 8, 9, 11, 12, 13, 14, 15, 17, 18, 19, 20, 21, 23, 24, 25 },
        { 10, 11, 13, 14, 15, 16, 17, 19, 20, 21, 22, 23, 25, 26, 27, 28 },
    ...
    vselN sel0, sel1, sel2, sel3, sel4;
    ...
    x2 = BBE_LVNX16_I(pS, 2 * 2 * BBE_SIMD_WIDTH);
    x3 = BBE_LVNX16_I(pS, 3 * 2 * BBE_SIMD_WIDTH);
    x4 = BBE_LVNX16_I(pS, 4 * 2 * BBE_SIMD_WIDTH);
    sel0 = BBE_MOVVSELNX16(x0, 0);
    sel1 = BBE_MOVVSELNX16(x1, 0);
    sel2 = BBE_MOVVSELNX16(x2, 0);
    sel3 = BBE_MOVVSELNX16(x3, 0);
    sel4 = BBE_MOVVSELNX16(x4, 0);
    ...
    for (n = 0; n<(N >> LOG2_BBE_SIMD_WIDTH); n++)
    {
        ...
        y1 = BBE_SELNX16(x2, x1, sel1);
        BBE_SVNX16_IP(y1, pY, 2 * BBE_SIMD_WIDTH);
        y2 = BBE_SELNX16(x3, x2, sel2);
        BBE_SVNX16_IP(y2, pY, 2 * BBE_SIMD_WIDTH);
        y0 = BBE_SELNX16(x4, x3, sel3);
        BBE_SVNX16_IP(y0, pY, 2 * BBE_SIMD_WIDTH);
        y1 = BBE_SELNX16(x5, x4, sel4);
        BBE_SVNX16_IP(y1, pY, 2 * BBE_SIMD_WIDTH);
    }
} /* rrelements6() */
