...
{
    int n;
    static const int16_t ALIGN(32) SEL[BBE_SIMD_WIDTH] = { 0, 12, 24, 4, 16, 28, 8, 20, 0, 0, 0, 0, 0, 0, 0, 0 };
    ...
    x1 = BBE_SELNX16I(x0, x0, BBE_SELI_ROTATE_RIGHT_3);
    y0 = BBE_SELNX16I(x0, x0, BBE_SELI_ROTATE_RIGHT_6);
    sel0 = BBE_MOVVSELNX16(x0, 0);
    sel1 = BBE_MOVVSELNX16(x1, 0);
    sel2 = BBE_MOVVSELNX16(y0, 0);
...
#pragma ymemory(pY)
    ...
    for (n = 0; n<(N >> (LOG2_BBE_SIMD_WIDTH - 1)); n++)
    {
        ...
        BBE_SAVNX16_XP(y0, y_align, pY, 2 * 3);
        BBE_LVNX16_IP(x0, pX0, 4 * BBE_SIMD_WIDTH);
        BBE_LVNX16_IP(x1, pX1, 4 * BBE_SIMD_WIDTH);
        y0 = BBE_SELNX16(x1, x0, sel1);
        BBE_SAVNX16_XP(y0, y_align, pY, 2 * 3);
        BBE_LVNX16_IP(x0, pX0, 4 * BBE_SIMD_WIDTH);
        BBE_LVNX16_IP(x1, pX1, 4 * BBE_SIMD_WIDTH);
        y0 = BBE_SELNX16(x1, x0, sel2);
        BBE_SAVNX16_XP(y0, y_align, pY, 2 * 2);
    }
