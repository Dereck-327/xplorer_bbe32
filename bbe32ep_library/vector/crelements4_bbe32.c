...
void crelements4 ( complex_fract16 * restrict y, const complex_fract16 * restrict x, int N )
{
    ...
    static const int16_t ALIGN(32) SEL[][BBE_SIMD_WIDTH] =
    {
        ...
        { 6, 7, 10, 11, 12, 13, 14, 15, 18, 19, 20, 21, 22, 23, 26, 27 },
        { 12, 13, 14, 15, 18, 19, 20, 21, 22, 23, 26, 27, 28, 29, 30, 31 }
    };
    ...
    for (n = 0; n<(N >> (LOG2_BBE_SIMD_WIDTH - 1)); n++)
    {
        ...
        y0 = BBE_SELNX16(x2, x1, sel1);
        BBE_SVNX16_IP(y0, pY, 2 * BBE_SIMD_WIDTH);
        y0 = BBE_SELNX16(x3, x2, sel2);
        BBE_SVNX16_IP(y0, pY, 2 * BBE_SIMD_WIDTH);
    }
} /* crelements4() */
