             ...
             int L )
{
  ...
  for (i = 0; i<2; i++)
  {
    ...
    py = (xb_vecNx16 *)(y);
    px1 = (const xb_vecNx16 *)XT_ADDX4((4 * L), (uintptr_t)px);
    py1 = (xb_vecNx16 *)XT_ADDX2((2 * L), (uintptr_t)py);

    k = L >> LOG2_BBE_SIMD_WIDTH;
    do
    {
      x00 = BBE_LVNX16_X(px, L * 2);    BBE_SVNX16_X(x00, py, 4 * 2 * L);
      x00 = BBE_LVNX16_X(px, 2 * L * 2);    BBE_SVNX16_X(x00, py, 2 * 4 * 2 * L);
      x00 = BBE_LVNX16_X(px, 3 * L * 2);    BBE_SVNX16_X(x00, py, 3 * 4 * 2 * L);
      BBE_LVNX16_IP(x00, px, 2 * BBE_SIMD_WIDTH); BBE_SVNX16_IP(x00, py, 2 * BBE_SIMD_WIDTH);

      x00 = BBE_LVNX16_X(px1, L * 2);   BBE_SVNX16_X(x00, py1, 4 * 2 * L);
      x00 = BBE_LVNX16_X(px1, 2 * L * 2);   BBE_SVNX16_X(x00, py1, 2 * 4 * 2 * L);
      x00 = BBE_LVNX16_X(px1, 3 * L * 2);   BBE_SVNX16_X(x00, py1, 3 * 4 * 2 * L);
      BBE_LVNX16_IP(x00, px1, 2 * BBE_SIMD_WIDTH);	BBE_SVNX16_IP(x00, py1, 2 * BBE_SIMD_WIDTH);
    } while (--k);
    x = (const int16_t*)XT_ADD((2 * 4 * L), (uintptr_t)x);
    y = (int16_t*)XT_ADD((2 * L), (uintptr_t)y);
  }
} /* mattran4x4s() */
