...
#if !(HAVE_PACKEDMUL && 1)
DISCARD_FUN(void, rcmatmulnxmn_M4, (void * pScr,
                      complex_fract16 * restrict z,
                      const int16_t * restrict x,
                      const complex_fract16 * restrict y,
                      int N, int L, int Q))
size_t rcmatmulnxmn_M4_getScratchSize(int N) {(void)N; return 0; }
#else
/*-------------------------------------------------------------------------
Real Matrix by Complex Matrix/Vector Multiply 
...
*/
void rcmatmulnxmn_M4 ( void * pScr,
                    complex_fract16 * restrict z, 
              const int16_t * restrict x, 
              const complex_fract16 * restrict y, 
              int N, int L, int Q )
{
  ...
  int Sx = getSpaceR(N * 4);
  int Sy = N * 4;

  xb_vecNx16 * restrict X_scr = (xb_vecNx16 *)((int32_t)pScr + 0 * 4 * N * 4);
  const xb_vecNx16 *          X_rd;
  xb_vecNx16 * restrict X_wr;
  xb_vecNx16 * restrict Y_scr = (xb_vecNx16 *)((int32_t)pScr + 4 * 4 * N * 4);
  const xb_vecNx16 *          Y_rd;
  xb_vecNx16 * restrict Y_wr;
  xb_vecNx16 * restrict Z_scr = (xb_vecNx16 *)((int32_t)pScr + 8 * 4 * N * 4);
  ...
  for (l = 0; l<L; l += 4)
  {
    ...
    X_wr = X_scr;
    __Pragma("loop_count min=1")
    for (k = 0; k<4 * N; k += BBE_SIMD_WIDTH)
    {
    ...
    px = (const xb_vecNx16 *)XT_ADDX2(4 * Sx - 4 * N, (int32_t)px);
    Y_wr = Y_scr;
    /* compute offsets */
    ...
    for (k = 0; k<2 * 4 * N; k += BBE_SIMD_WIDTH)
    {
    ...
    Y_rd = (const xb_vecNx16 *)XT_ADDX2(2 * 4 * N * 4, (int32_t)pScr);
    Z_wr = Z_scr;
    for (k = 0; k<4; k++)
    {
      /* load input matrix X */
      ...
      /* Load input matrix Y */
      BBE_LVNX16_XP(Y0, Y_rd, 2 * BBE_SIMD_WIDTH); Acc0 = BBE_MULRNX16C(Y0, X0, q);
      BBE_LVNX16_XP(Y0, Y_rd, 2 * BBE_SIMD_WIDTH); Acc1 = BBE_MULRNX16C(Y0, X0, q);
      BBE_LVNX16_XP(Y1, Y_rd, 2 * BBE_SIMD_WIDTH); BBE_MULANX16C(Acc0, Y1, X1);
      BBE_LVNX16_XP(Y1, Y_rd, 2 * BBE_SIMD_WIDTH); BBE_MULANX16C(Acc1, Y1, X1);
      BBE_LVNX16_XP(Y2, Y_rd, 2 * BBE_SIMD_WIDTH); BBE_MULANX16C(Acc0, Y2, X2);
      BBE_LVNX16_XP(Y2, Y_rd, 2 * BBE_SIMD_WIDTH); BBE_MULANX16C(Acc1, Y2, X2);
      BBE_LVNX16_XP(Y3, Y_rd, 2 * BBE_SIMD_WIDTH); BBE_MULANX16C(Acc0, Y3, X3);
      BBE_LVNX16_XP(Y3, Y_rd, 2 * BBE_SIMD_WIDTH); BBE_MULANX16C(Acc1, Y3, X3);
      __Pragma("loop_count min=1")
      for (n = 0; n<N - 4; n += 4)
      {
        /* load input matrix X */
        ...
        /* Load input matrix Y */
        BBE_LVNX16_XP(Y0, Y_rd, 2 * BBE_SIMD_WIDTH); BBE_MULANX16C(Acc0, Y0, X0);
        BBE_LVNX16_XP(Y0, Y_rd, 2 * BBE_SIMD_WIDTH); BBE_MULANX16C(Acc1, Y0, X0);
        BBE_LVNX16_XP(Y1, Y_rd, 2 * BBE_SIMD_WIDTH); BBE_MULANX16C(Acc0, Y1, X1);
        BBE_LVNX16_XP(Y1, Y_rd, 2 * BBE_SIMD_WIDTH); BBE_MULANX16C(Acc1, Y1, X1);
        BBE_LVNX16_XP(Y2, Y_rd, 2 * BBE_SIMD_WIDTH); BBE_MULANX16C(Acc0, Y2, X2);
        BBE_LVNX16_XP(Y2, Y_rd, 2 * BBE_SIMD_WIDTH); BBE_MULANX16C(Acc1, Y2, X2);
      ...
      Z = BBE_PACKVNX40(Acc0, q);
      BBE_SVNX16_IP(Z, Z_wr, 2 * BBE_SIMD_WIDTH);
      Z = BBE_PACKVNX40(Acc1, q);
      BBE_SVNX16_IP(Z, Z_wr, 2 * BBE_SIMD_WIDTH);
      Y_rd = (const xb_vecNx16 *)XT_ADDX2(-2 * 4 * 4 * N, (int32_t)Y_rd);
    }

    /* Convert from streaming format to block format */
    Z_rd = (const xb_vecNx16 *)((int32_t)pScr + 8 * 4 * N * 4);
    X0 = BBE_SEQNX16(); X1 = BBE_MOVVA16(16 << 2);
    X1 = BBE_MOVNX16T(X1, zero, b);
    W = BBE_MULNX16(X0, X1);
    X0 = BBE_MOVVWL(W);
    off1 = BBE_EXTRNX16C(X0, 1);
    ...
    BBE_SVNX16_X(Z2, pz, off2);
    BBE_SVNX16_X(Z3, pz, off3);
    BBE_SVNX16_IP(Z0, pz, 2 * BBE_SIMD_WIDTH + 12 * BBE_SIMD_WIDTH);
  }
} /* rcmatmulnxmn_M4() */
...
size_t rcmatmulnxmn_M4_getScratchSize ( int N )
{
  int Sx;
  Sx = getSpaceR(N*4);
  return (Sx * 8 * 2 + 16*8)*sizeof(int16_t);
} /* rcmatmulnxmn_M4_getScratchSize() */
#endif
