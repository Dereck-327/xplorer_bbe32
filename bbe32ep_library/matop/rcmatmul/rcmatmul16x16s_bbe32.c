                ...
                int L, int Q )
{
  ...
  {
    py = (xb_vecNx16 *)y;
    px = (xb_vecNx16 *)(x + 16 * L*k);

    __Pragma("ymemory( py )");
    __Pragma("loop_count min=16");
    for (k_ = l = 0; l<L; l++)
    {
      k_ = BBE_ADDMOD16U(k_, (L << 16) | BBE_SIMD_WIDTH);
      stage = -15 * L + BBE_SIMD_WIDTH;
      XT_MOVEQZ(stage, -16 * L + BBE_SIMD_WIDTH, k_);
      ...
      BBE_LVNX16_XP(y0l, py, 16 * 2 * 2 * L);
      y1h = BBE_LVNX16_I(py, 2 * BBE_SIMD_WIDTH);
      BBE_LVNX16_XP(y1l, py, 16 * 2 * 2 * L);
      ...
      px = (const xb_vecNx16 *)XT_ADDX2(stage, (int32_t)px);
      ...
      BBE_LVNX16_XP(y0l, py, 16 * 2 * 2 * L);
      y1h = BBE_LVNX16_I(py, 2 * BBE_SIMD_WIDTH);
      BBE_LVNX16_XP(y1l, py, -15 * 16 * 2 * 2 * L + 2 * 2 * BBE_SIMD_WIDTH);
