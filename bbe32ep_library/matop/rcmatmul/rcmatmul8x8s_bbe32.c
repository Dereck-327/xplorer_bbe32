              ...
              int L, int Q )
{
  ...
  {
    py = (xb_vecNx16 *)y;
    px = (xb_vecNx16 *)(x + 8 * L*k);

    __Pragma("ymemory( py )");
    __Pragma("loop_count min=8");
    for (j = l = 0; l<8 * (L >> LOG2_BBE_SIMD_WIDTH); l++)
    {
      j = BBE_ADDMOD16U(j, (L << 16) | BBE_SIMD_WIDTH);
      x_stride = -7 * L + BBE_SIMD_WIDTH;
      XT_MOVEQZ(x_stride, -8 * L + BBE_SIMD_WIDTH, j);
      ...
      BBE_LVNX16_XP(y0l, py, 8 * 2 * 2 * L);
      y1h = BBE_LVNX16_I(py, 2 * BBE_SIMD_WIDTH);
      BBE_LVNX16_XP(y1l, py, 8 * 2 * 2 * L);
      ...
      BBE_LVNX16_XP(xx01, px, 2 * L);
      BBE_LVNX16_XP(xx02, px, 2 * L);
      xx03 = BBE_LVNX16_I(px, 0);

      px = (const xb_vecNx16*)XT_ADDX2(x_stride, (uintptr_t)px);
      ...
      BBE_LVNX16_XP(y0l, py, 8 * 2 * 2 * L);
      y1h = BBE_LVNX16_I(py, 2 * BBE_SIMD_WIDTH);
      BBE_LVNX16_XP(y1l, py, -7 * 8 * 2 * 2 * L + 2 * 2 * BBE_SIMD_WIDTH);
