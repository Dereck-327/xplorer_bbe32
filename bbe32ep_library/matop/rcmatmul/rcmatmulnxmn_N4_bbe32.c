...
#if !(HAVE_PACKEDMUL && 1)
DISCARD_FUN(void, rcmatmulnxmn_N4, (void * pScr,
                      complex_fract16 * restrict z,
                      const int16_t * restrict x,
                      const complex_fract16 * restrict y,
                      int M, int L, int Q))
size_t rcmatmulnxmn_N4_getScratchSize(int M) { (void)M; return 0; }
#else
/*-------------------------------------------------------------------------
Real Matrix by Complex Matrix/Vector Multiply 
...
*/
void rcmatmulnxmn_N4 ( void * pScr,
                    complex_fract16 * restrict z, 
              const int16_t * restrict x, 
              const complex_fract16 * restrict y, 
              int M, int L, int Q )
{
  ...
  int Sx = getSpaceR(4 * M);
  int Sy = 4 * M;
  int Sz = M*M;

        xb_vecNx16 * restrict X_scr = (xb_vecNx16 *)((int32_t)pScr + 0 * M * 4 * 4);
  const xb_vecNx16 *          X_rd;
        xb_vecNx16 * restrict X_wr;
        xb_vecNx16 * restrict Y_scr = (xb_vecNx16 *)((int32_t)pScr + 4 * M * 4 * 4);
  const xb_vecNx16 *          Y_rd;
        xb_vecNx16 * restrict Y_wr;
        xb_vecNx16 * restrict Z_scr = (xb_vecNx16 *)((int32_t)pScr + 8 * M * 4 * 4);
  ...
  for (l = 0; l<L; l += 4)
  {
    ...
    X_wr = X_scr;
    __Pragma("loop_count min=1")
    for (k = 0; k<M * 4; k += BBE_SIMD_WIDTH)
    {
    ...
    px = (const xb_vecNx16 *)XT_ADDX2(4 * Sx - M * 4, (int32_t)px);
    Y_wr = Y_scr;
    /* compute offsets */
    ...
    for (k = 0; k<2 * M * 4; k += BBE_SIMD_WIDTH)
    {
    ...
    Y_rd = (const xb_vecNx16 *)XT_ADDX2(2 * M * 4 * 4, (int32_t)pScr);
    Z_wr = Z_scr;
    __Pragma("loop_count min=1")
    for (k = 0; k<M; k++)
    {
      ...
      __Pragma("loop_count min=2")
        for (m = 0; m<M; m += 2)
        {
          ...
          BBE_LVNX16_XP(Y3, Y_rd, -3 * 2 * 2 * 4 * M + 2 * BBE_SIMD_WIDTH);
    ...
    Z_rd = (const xb_vecNx16 *)((int32_t)pScr + 8 * M * 4 * 4);
...
} /* rcmatmulnxmn_N4_getScratchSize() */
#endif
