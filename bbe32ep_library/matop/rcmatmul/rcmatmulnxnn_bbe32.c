...
-------------------------------------------------------------------------*/
#if !(HAVE_PACKEDMUL && 1)
DISCARD_FUN(void ,rcmatmulnxnn,(void* pScr,
                  complex_fract16 * restrict z, 
            const int16_t * restrict x, 
            const complex_fract16 * restrict y, 
            int N, int L, int Q))
size_t rcmatmulnxnn_getScratchSize ( int N, int L ) { (void)N; (void)L; return 0; }
#else
              ...
              int N, int L, int Q )
{
  ...
  NASSERT(N % 4 == 0);
  if (L <= 0 || N <= 0) return;
  rcmatmulnxmn(pScr, z, x, y, N, N, L, Q);
} /* rcmatmulnxnn() */

/* Return the scratch area size, in bytes. */
size_t rcmatmulnxnn_getScratchSize ( int N, int L )
{
  return rcmatmulnxmn_getScratchSize(N, N, L);
} /* rcmatmulnxnn_getScratchSize() */
#endif
