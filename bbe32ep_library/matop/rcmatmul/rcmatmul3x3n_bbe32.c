...
#if !(HAVE_PACKEDMUL && 1)
DISCARD_FUN(void, rcmatmul3x3n,(complex_fract16 * restrict z, 
              ...
              int L, int Q )
{
  ...
  int l;
  xb_vecNx16 vTmp;
  xb_vecNx16 vX, vX0, vX1, vY0, vY1, vZ;
  ...
  for (l = 0; l<L; l++)
  {
    ...
    BBE_LVNX16_IP(vX, pXrd, 2 * BBE_SIMD_WIDTH);
    BBE_DSELNX16I(vX1, vX0, vZero, vX, BBE_DSELI_INTERLEAVE_1);
...
#else
    vXsel = BBE_REPNX16C(vX0, 6);
    vYsel = BBE_REPNX16C(vY0, 2);
    wvZ = BBE_MULRNX16C(vXsel, vYsel, q);

    vXsel = BBE_REPNX16C(vX0, 7);
    vYsel = BBE_REPNX16C(vY0, 5);
    BBE_MULANX16C(wvZ, vXsel, vYsel);

    BBE_MULANX16C(wvZ, vX1, vY1);
#endif
    /* pack and save 8-th element */
    vZ = BBE_PACKVNX40(wvZ, q);
    BBE_SVNX16_IP(vZ, pZwr, 2 * BBE_SIMD_WIDTH);
