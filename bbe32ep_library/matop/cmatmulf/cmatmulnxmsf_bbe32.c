...
#if !(HAVE_VFPU)
DISCARD_FUN(void, cmatmulnxmsf, ( complex_float * restrict z, 
              ...
              int N, int M, int L )
{
  ...
  const xb_vecN_4xcf32 * restrict inY;
        xb_vecN_4xcf32 * restrict out = (xb_vecN_4xcf32 *)z;

  xb_vecN_4xcf32 regX0, regX1, regX2, regX3, regY_, regY;
  ...
  if (N <= 0)
  {
    memset(z, 0, M*M*L*sizeof(complex_float));
    return;
  }
  ...
  for (m = 0; m < (M>>2); m++)
  {
    ...
    inXmod = (const xb_vecN_4xcf32 *)(x + m*4*N*L);
    out    = (      xb_vecN_4xcf32 *)(z + m*4*M*L);
    ...
    for (p = 0; p < M*(L >> (LOG2_BBE_SIMD_WIDTH-2)); p++)
    {
      ...
      for (n = 0; n < N; n++)
      {
        ...
        BBE_LVN_4XCF32_XP(regY, inY, sz_cf32*L*M);
      ...
      BBE_SVN_4XCF32_XP(regout0, out,    sz_cf32*L*M);
      BBE_SVN_4XCF32_XP(regout1, out,    sz_cf32*L*M);
      BBE_SVN_4XCF32_XP(regout2, out,    sz_cf32*L*M);
      BBE_SVN_4XCF32_XP(regout3, out, -3*sz_cf32*L*M+2*BBE_SIMD_WIDTH);
      /* move pointer to the next matrices or next value in a row */
      inY = (const xb_vecN_4xcf32 *)XT_ADDX8(BBE_SIMD_WIDTH/4 - N*M*L, (uintptr_t)inY);
    }
  }
  ...
  if (M&2)
  {
    ...
    inY    = (const xb_vecN_4xcf32 *)(y);
    inXmod = (const xb_vecN_4xcf32 *)(x + _M*N*L);
    out    = (      xb_vecN_4xcf32 *)(z + _M*M*L);
    WUR_CBEGIN((uintptr_t)inXmod);
    WUR_CEND((uintptr_t)((complex_float *)inXmod + L));
    ...
    for (p = 0; p < M*(L >> (LOG2_BBE_SIMD_WIDTH-2)); p++)
    {
      ...
      {
        BBE_LVN_4XCF32_XP(regX0, inX,  sz_cf32*L*N);
        BBE_LVN_4XCF32_XP(regX1, inX, -sz_cf32*L*N+sz_cf32*L);
        BBE_LVN_4XCF32_XP(regY , inY, sz_cf32*L*M);
      ...
      BBE_SVN_4XCF32_XP(regout0, out,  sz_cf32*L*M);
      BBE_SVN_4XCF32_XP(regout2, out, -sz_cf32*L*M+2*BBE_SIMD_WIDTH);
      inY = (const xb_vecN_4xcf32 *)XT_ADDX8(BBE_SIMD_WIDTH/4 - N*M*L, (uintptr_t)inY);
    }
  }
  if (M&1)
