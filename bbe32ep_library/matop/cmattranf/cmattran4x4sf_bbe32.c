...
#if !(HAVE_VFPU)
DISCARD_FUN(void, cmattran4x4sf, ( complex_float * restrict y, 
               ...
               int L )
{
  ...
  for (i = 0; i<2; i++)
  {
    ...
    px1 = (const xb_vecN_4xcf32 *)((complex_float *)px0+L*4);
    py1 = (      xb_vecN_4xcf32 *)((complex_float *)py0+L);

    for (l=0; l<(L>>(LOG2_BBE_SIMD_WIDTH-2)); l++)
    {
      vreg = BBE_LVN_4XCF32_X(px0, 1*L*sizeof(complex_float));  vreg = BBE_CONJN_4XCF32(vreg); BBE_SVN_4XCF32_X (vreg, py0, 1*4*L*sizeof(complex_float));
      vreg = BBE_LVN_4XCF32_X(px0, 2*L*sizeof(complex_float));  vreg = BBE_CONJN_4XCF32(vreg); BBE_SVN_4XCF32_X (vreg, py0, 2*4*L*sizeof(complex_float));
      vreg = BBE_LVN_4XCF32_X(px0, 3*L*sizeof(complex_float));  vreg = BBE_CONJN_4XCF32(vreg); BBE_SVN_4XCF32_X (vreg, py0, 3*4*L*sizeof(complex_float));
      BBE_LVN_4XCF32_IP(vreg, px0, 2*BBE_SIMD_WIDTH);           vreg = BBE_CONJN_4XCF32(vreg); BBE_SVN_4XCF32_IP(vreg, py0, 2*BBE_SIMD_WIDTH);

      vreg = BBE_LVN_4XCF32_X(px1, 1*L*sizeof(complex_float));  vreg = BBE_CONJN_4XCF32(vreg); BBE_SVN_4XCF32_X (vreg, py1, 1*4*L*sizeof(complex_float));
      vreg = BBE_LVN_4XCF32_X(px1, 2*L*sizeof(complex_float));  vreg = BBE_CONJN_4XCF32(vreg); BBE_SVN_4XCF32_X (vreg, py1, 2*4*L*sizeof(complex_float));
      vreg = BBE_LVN_4XCF32_X(px1, 3*L*sizeof(complex_float));  vreg = BBE_CONJN_4XCF32(vreg); BBE_SVN_4XCF32_X (vreg, py1, 3*4*L*sizeof(complex_float));
