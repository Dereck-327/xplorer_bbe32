...
/* M=3, N=3, Sx=9, Sy=12 */
void csb3x3f ( complex_float * restrict y, const complex_float * restrict x, int L )
{
  ...
  for ( l=0; l<(L>>(LOG2_BBE_SIMD_WIDTH-2)); l++ )
  {
    ...
    BBE_SVNX16_IP(Y0, py0, 3*2*BBE_SIMD_WIDTH);
    BBE_SVNX16_IP(Y1, py0, 3*2*BBE_SIMD_WIDTH);
    BBE_SVNX16_IP(Y2, py0, 3*2*BBE_SIMD_WIDTH);
    BBE_SVNX16_IP(Y3, py0, 3*2*BBE_SIMD_WIDTH);
    ...
    BBE_SVNX16_IP(Y0, py1, 3*2*BBE_SIMD_WIDTH);
    BBE_SVNX16_IP(Y1, py1, 3*2*BBE_SIMD_WIDTH);
    BBE_SVNX16_IP(Y2, py1, 3*2*BBE_SIMD_WIDTH);
    BBE_SVNX16_IP(Y3, py1, 3*2*BBE_SIMD_WIDTH);
  }

  px0 = px1;
  py0 = (xb_vecNx16 *)(y+8);

  /* Process last 9th value for all L matrices */
  __Pragma("loop_count min=1");
  for (l = 0; l<(L>>(LOG2_BBE_SIMD_WIDTH-2)); l++)
  {
#ifdef BBE_REPN_4X64
    ...
    Y0 = BBE_MOVNX16_FROMN_4X64(BBE_REPN_4X64(vtmp64, 0));
    Y1 = BBE_MOVNX16_FROMN_4X64(BBE_REPN_4X64(vtmp64, 1));
    Y2 = BBE_MOVNX16_FROMN_4X64(BBE_REPN_4X64(vtmp64, 2));
    Y3 = BBE_MOVNX16_FROMN_4X64(BBE_REPN_4X64(vtmp64, 3));
#else
    BBE_LVNX16_IP(X0, px0, 2*BBE_SIMD_WIDTH);
