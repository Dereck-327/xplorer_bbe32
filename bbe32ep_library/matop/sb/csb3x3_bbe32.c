...
/* M=3, N=3, Sx=9, Sy=16 */
void csb3x3(complex_fract16 * restrict y, const complex_fract16 * restrict x, int L)
{
  const xb_vecNx16* restrict X = (const xb_vecNx16*)x;
  xb_vecNx16* restrict Y = (xb_vecNx16*)y;
  xb_vecNx16* y0; const xb_vecNx16* x0;
  ...
  for (l = 0; l<(L >> (LOG2_BBE_SIMD_WIDTH - 1)); l++)
  {
    ...
    BBE_LVNX16_XP(X7, X, 2 * BBE_SIMD_WIDTH - 7 * (L << 2));
    ...
    BBE_SVNX16_IP(Y7, Y, 4 * BBE_SIMD_WIDTH);
  }
  /* process last 9-th element */
  X = x0 + L;
  Y = y0 + 1;
  __Pragma("loop_count min=1")
  for (l = 0; l<(L >> (LOG2_BBE_SIMD_WIDTH - 1)); l++)
