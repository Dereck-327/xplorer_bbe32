...
-------------------------------------------------------------------------*/
#if !(HAVE_PACKEDMUL && 1)
DISCARD_FUN(void, rcmatvmulnxnn,(void* pScr,
                  complex_fract16 * restrict z, 
            const int16_t * restrict x, 
            const complex_fract16 * restrict y, 
            int N, int L, int Q))
size_t rcmatvmulnxnn_getScratchSize(int N, int L) { (void)N; (void)L; return 0; }
#else
               ...
               int N, int L, int Q )
{
  ...
  rcmatvmulnxmn(pScr, z, x, y, N, N, L, Q);
} /* rcmatvmulnxnn() */

/* Return the scratch area size, in bytes. */
size_t rcmatvmulnxnn_getScratchSize ( int N, int L )
{
  return rcmatvmulnxmn_getScratchSize(N, N, L);
} /* rcmatvmulnxnn_getScratchSize() */
#endif
