...
#if !(HAVE_VFPU)
DISCARD_FUN(void, cmatvmul8x8nf,( complex_float * restrict z, 
               ...
               int L )
{
  ...
  px2 = (const xb_vecN_4xcf32 *)(x+16);
  px3 = (const xb_vecN_4xcf32 *)(x+24);
  ...
  {
    i = i ^ 1;
    ystride = -2*2*BBE_SIMD_WIDTH;
    XT_MOVEQZ(ystride, i, i);

    /* Load 1-st part of input matrices X and Y */
    ...
    /* Load 2-nd part of input matrices X and Y */
    BBE_LVN_4XCF32_IP(X0, px0, 7*2*BBE_SIMD_WIDTH);
    BBE_LVN_4XCF32_IP(X1, px1, 7*2*BBE_SIMD_WIDTH);
    BBE_LVN_4XCF32_IP(X2, px2, 7*2*BBE_SIMD_WIDTH);
    BBE_LVN_4XCF32_IP(X3, px3, 7*2*BBE_SIMD_WIDTH);
    BBE_LVN_4XCF32_IP(Y , py, 2*BBE_SIMD_WIDTH);
    py = (const xb_vecN_4xcf32 *)((intptr_t)py + ystride);
    /* Multiply input matrices X and Y */
    BBE_MULAN_4XCF32(Z0, X0, Y);
    BBE_MULAN_4XCF32(Z1, X1, Y);
    BBE_MULAN_4XCF32(Z2, X2, Y);
    BBE_MULAN_4XCF32(Z3, X3, Y);
