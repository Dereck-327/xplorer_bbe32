...
#if !(HAVE_VFPU)
DISCARD_FUN(void, matmul2x2nf,( float32_t * restrict z,
            ...
            int L )
{
  ...
  for (l = 0; l<(L>>1); l++)
  {
    ...
    BBE_LVN_2XF32_IP(X, px, 2*BBE_SIMD_WIDTH);
    BBE_LVN_2XF32_IP(Y, py, 2*BBE_SIMD_WIDTH);

    y0 = BBE_SHFLN_2XF32I(Y, BBE_SHFLI_DUPLICATE_4_EVEN);
    y1 = BBE_SHFLN_2XF32I(Y, BBE_SHFLI_DUPLICATE_4_ODD );

    /* Multiply input matrices X and Y */
    Z = BBE_MULMN_2XF32( X, y0, 0, 0x4);
    BBE_MULMASN_2XF32(Z, X, y1, 0, 0xE);
