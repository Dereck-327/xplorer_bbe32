              ...
              int N, int M, int L )
{
  ...
  const int16_t * x0 = (const int16_t *)x;
  ...
  for (i = 0; i<(M >> 1); i++)
  {
    ...
    for (j = 0; j<(N >> 2); j++)
    {
      ...
      {
        x00 = BBE_LVNX16_X(px, 2 * L); x00 = BBE_CONJSNX16C(x00); BBE_SVNX16_X(x00, py, 2 * M*L);
        x00 = BBE_LVNX16_X(px, 2 * 2 * L); x00 = BBE_CONJSNX16C(x00); BBE_SVNX16_X(x00, py, 2 * 2 * M*L);
        x00 = BBE_LVNX16_X(px, 3 * 2 * L); x00 = BBE_CONJSNX16C(x00); BBE_SVNX16_X(x00, py, 3 * 2 * M*L);
        BBE_LVNX16_IP(x00, px, 2 * BBE_SIMD_WIDTH); x00 = BBE_CONJSNX16C(x00); BBE_SVNX16_IP(x00, py, 2 * BBE_SIMD_WIDTH);

        x00 = BBE_LVNX16_X(px1, 2 * L); x00 = BBE_CONJSNX16C(x00); BBE_SVNX16_X(x00, py1, 2 * M*L);
        x00 = BBE_LVNX16_X(px1, 2 * 2 * L); x00 = BBE_CONJSNX16C(x00); BBE_SVNX16_X(x00, py1, 2 * 2 * M*L);
        x00 = BBE_LVNX16_X(px1, 3 * 2 * L); x00 = BBE_CONJSNX16C(x00); BBE_SVNX16_X(x00, py1, 3 * 2 * M*L);
        BBE_LVNX16_IP(x00, px1, 2 * BBE_SIMD_WIDTH); x00 = BBE_CONJSNX16C(x00); BBE_SVNX16_IP(x00, py1, 2 * BBE_SIMD_WIDTH);
      } //while (--k);

      x = (const complex_fract16*)XT_ADDX4((2 * L), (uintptr_t)x);
      y = (complex_fract16*)XT_ADDX4((2 * M*L), (uintptr_t)y);
    }
    x += ((N & 3)*L)>>1;
    y += ((1 - _N*M)*L)>>1;
  }
  ...
  if (M & 1)
  {
    x = (complex_fract16*)XT_ADDX2((_M*N*L), (uintptr_t)x0);
    y = (complex_fract16*)XT_ADDX2((_M*L), (uintptr_t)y0);
    for (j = 0; j<N; j++)
    {
      px = (const xb_vecNx16 *)(x);
      py = (xb_vecNx16 *)(y);
      ...
      do
      {
        BBE_LVNX16_IP(x00, px, 2 * BBE_SIMD_WIDTH); x00 = BBE_CONJSNX16C(x00);
        BBE_SVNX16_IP(x00, py, 2 * BBE_SIMD_WIDTH);
      } while (--k);
      x = (const complex_fract16*)XT_ADDX2((L), (uintptr_t)x);
      y = (complex_fract16*)XT_ADDX2((M*L), (uintptr_t)y);
    }
  }
  /* right border: M rows x (N&3) columns */
  x = (const complex_fract16*)XT_ADDX2(_N*L, (uintptr_t)x0);
  y = (complex_fract16*)XT_ADDX2(_N*M*L, (uintptr_t)y0);
  for (j = 0; j<(N & 3); j++)
  {
    for (i = 0; i<M; i++)
    {
      ...
      x = (const complex_fract16*)XT_ADDX2((N*L), (uintptr_t)x);
      y = (complex_fract16*)XT_ADDX2((L), (uintptr_t)y);
    }
    x = (const complex_fract16*)XT_ADDX2(((1 - M*N)*L), (uintptr_t)x);
  }
} /* cmattrannxms() */
