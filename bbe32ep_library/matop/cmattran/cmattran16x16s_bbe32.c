...
/* Streaming Order, 16x16->16x16, S=256
   Restrictions:
              ...
              int L )
{
  ...
  for (i = 0; i<M / 2; i++)
  {
    for (j = 0; j<N / 4; j++)
    {
      ...
      py = (xb_vecNx16 *)(y);
      px1 = (const xb_vecNx16 *)XT_ADDX2((M*N / 2 * L), (uintptr_t)px);
      py1 = (xb_vecNx16 *)XT_ADDX2((M / 2 * L), (uintptr_t)py);

      k = L >> LOG2_BBE_SIMD_WIDTH;
      do
      {
        x00 = BBE_LVNX16_X(px, N / 4 * L * 2);   x00 = BBE_CONJSNX16C(x00); BBE_SVNX16_X(x00, py, N*N / 4 * L * 2);
        x00 = BBE_LVNX16_X(px, 2 * N / 4 * L * 2);   x00 = BBE_CONJSNX16C(x00); BBE_SVNX16_X(x00, py, 2 * N*N / 4 * L * 2);
        x00 = BBE_LVNX16_X(px, 3 * N / 4 * L * 2);   x00 = BBE_CONJSNX16C(x00); BBE_SVNX16_X(x00, py, 3 * N*N / 4 * L * 2);
        BBE_LVNX16_IP(x00, px, sizeof(*px));  x00 = BBE_CONJSNX16C(x00); BBE_SVNX16_IP(x00, py, sizeof(*py));

        x00 = BBE_LVNX16_X(px1, N / 4 * L * 2);  x00 = BBE_CONJSNX16C(x00); BBE_SVNX16_X(x00, py1, N*N / 4 * L * 2);
        x00 = BBE_LVNX16_X(px1, 2 * N / 4 * L * 2);  x00 = BBE_CONJSNX16C(x00); BBE_SVNX16_X(x00, py1, 2 * N*N / 4 * L * 2);
        x00 = BBE_LVNX16_X(px1, 3 * N / 4 * L * 2);  x00 = BBE_CONJSNX16C(x00); BBE_SVNX16_X(x00, py1, 3 * N*N / 4 * L * 2);
        BBE_LVNX16_IP(x00, px1, sizeof(*px1)); x00 = BBE_CONJSNX16C(x00); BBE_SVNX16_IP(x00, py1, sizeof(*py1));
      } while (--k);
      x += L>>1;
      y = (complex_fract16*)XT_ADDX4((M / 2 * L), (uintptr_t)y);
    }
    x += (N - N / 4)*L>>1;
    y += (1 - (N*M) / 4)*L>>1;
  }
