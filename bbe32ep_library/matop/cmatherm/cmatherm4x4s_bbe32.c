...
/* Streaming Order, 4x4*4x4->4x4, Sx=16, Sy=16
   Restrictions:
              ...
              int L, int Q )
{
  ...
  for (k = 0; k < L8; k++)
  {
    ...
    x11 = BBE_LVNX16_X(px02, 4 * L);
    BBE_LVNX16_XP(x00, px02, -12 * 4 * L + 2 * BBE_SIMD_WIDTH);
    BBE_MAGIANX16C(zout_00, x11, x00);
    BBE_MULANX16J(zout_10, x11, x00);
    ...
    BBE_SVNX16_X(x01, pz00, 4 * 4 * L);
    BBE_SVNX16_X(x10, pz00, 4 * L);
    BBE_SVNX16_X(x11, pz00, 4 * L * 5);
    BBE_SVNX16_IP(x00, pz00, 2 * BBE_SIMD_WIDTH);
  }

  py01 = (xb_vecNx16 *)XT_ADDX2(4 * L, (int32_t)px01);
  pz00 = ((xb_vecNx16*)y) + (0 + 2 * 4)*L8;
  pz01 = ((xb_vecNx16*)y) + (2 + 0 * 4)*L8;
  px02 = px01;
  py02 = py01;
  ...
  for (k = 0; k < L8; k++)
  {
    ...
    px02 = (xb_vecNx16 *)XT_ADD(-4 * 4 * 4 * L + 2 * BBE_SIMD_WIDTH, (int32_t)px02);
    py02 = (xb_vecNx16 *)XT_ADD(-4 * 4 * 4 * L + 2 * BBE_SIMD_WIDTH, (int32_t)py02);
    ...
    BBE_SVNX16_X(x11, pz00, 4 * 5 * L);
    BBE_SVNX16_X(x10, pz01, 4 * 5 * L);
