...
{                                 \
    xb_vecNx16 x0,y0,x1;          \
    x1=BBE_LVNX16_X(px, 64*L);    \
    BBE_LVNX16_XP(x0, px, 4*L);   \
    BBE_LVNX16_XP(y0, py, 64*L);  \
  ...
  for (m = 0; m<8; m++)
  {
    ...
    for (k = i = 0; i<L * 16; i += BBE_SIMD_WIDTH / 2)
    {
      k = BBE_ADDMOD16U(k, (L << 16) | (BBE_SIMD_WIDTH / 2));
      xstride = -15 * 4 * L + 2 * BBE_SIMD_WIDTH;
      XT_MOVEQZ(xstride, -16 * 4 * L + 2 * BBE_SIMD_WIDTH, k);
      x1 = BBE_LVNX16_X(px, 64 * L);
      BBE_LVNX16_XP(x0, px, 4 * L);
      BBE_LVNX16_XP(y0, py, 64 * L);
      ...
      x1 = BBE_LVNX16_X(px, 16 * 4 * L);
      BBE_LVNX16_XP(x0, px, xstride);
      BBE_LVNX16_XP(y0, py, -15 * 16 * 4 * L + 2 * BBE_SIMD_WIDTH);
      BBE_MULANX16C(Z0, x0, y0);
      BBE_MULANX16C(Z1, x1, y0);
      ...
      BBE_SVNX16_X(z1, pz, 64 * L);
      BBE_SVNX16_IP(z0, pz, 2 * BBE_SIMD_WIDTH);
    }
    /* jump over row */
    px = (const xb_vecNx16*)XT_ADDX2(64 * L, (uintptr_t)px);
    pz = (xb_vecNx16*)XT_ADD(64 * L, (uintptr_t)pz);
  }
} /* cmatmul16x16s() */
