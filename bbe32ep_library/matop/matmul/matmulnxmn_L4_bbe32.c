...
/* Compute L matrix products MxN * NxM -> MxM, where L is a multiple of 4. */
void matmulnxmn_L4( void * pScr,
                      int16_t * restrict z, 
                ...
                int N, int M, int L, int Q )
{
  ...
  */
  ...
  /*
  * Main loop computes BBE_SIMD_WIDTH/4 matrix products at a time.
  */

  for ( l=0; l<L/(BBE_SIMD_WIDTH/4); l++ )
  {
    /*
    * Convert BBE_SIMD_WIDTH/4 MxN matrices x[] and y[] to streaming format.
    */
...

    {
      ...
      for ( n=0; n<MN/(BBE_SIMD_WIDTH); n++ )
      {
        ...
        BBE_LVNX16_XP( a03, X_rd, 2*BBE_SIMD_WIDTH - 3*MN*2 );

        BBE_LVNX16_XP( a10, Y_rd, MN*2 );
        BBE_LVNX16_XP( a11, Y_rd, MN*2 );
        BBE_LVNX16_XP( a12, Y_rd, MN*2 );

        BBE_LVNX16_XP( a13, Y_rd, 2*BBE_SIMD_WIDTH - 3*MN*2 );
        ...
        BBE_SVNX16_IP( a00, X_wr, 2*BBE_SIMD_WIDTH );
        BBE_SVNX16_IP( a01, X_wr, 2*BBE_SIMD_WIDTH );
        BBE_SVNX16_IP( a02, X_wr, 2*BBE_SIMD_WIDTH );
        BBE_SVNX16_IP( a03, X_wr, 2*BBE_SIMD_WIDTH );
    ...
    */
...

    {
      ...
      static const int16_t ALIGN(32) seli[][BBE_SIMD_WIDTH] = {
      ...
      for ( i=0; i<M/4; i++ )
      {
        ...
        for ( j=0; j<M/4; j++ )
        {
          ...
          /*
          * 4x4 sub-blocks of BBE_SIMD_WIDTH/4 resulting matrices z[] appear
          * transposed here.
          */
    ...
    /*
    * Convert BBE_SIMD_WIDTH/4 MxM matrices z[] to block format.
    */
...

    {
      ...
      for ( n=0; n<MM/(BBE_SIMD_WIDTH); n++ )
      {
        ...
        BBE_SVNX16_XP( a0, Z_wr, MM*2 );
        BBE_SVNX16_XP( a1, Z_wr, MM*2 );
        BBE_SVNX16_XP( a2, Z_wr, MM*2 );

        BBE_SVNX16_XP( a3, Z_wr, 4*BBE_SIMD_WIDTH/2 - 3*MM*2 );
      }
    }
  }
} /* matmulnxmn_L4() */
...
size_t matmulnxmn_L4_getScratchSize ( int N, int M )
{
  int Sx, Sz;
  Sx = getSpaceR(N*M);
  Sz = getSpaceR(M*M);
  return (Sx * 4 * 2 + Sz * 4)*sizeof(int16_t);
} /* matmulnxmn_L4_getScratchSize() */
