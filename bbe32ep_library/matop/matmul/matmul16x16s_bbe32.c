...
void matmul16x16s ( int16_t * restrict z, 
              ...
              int L, int Q )
{
#define ITER(Z1,Z0,px,py,L)     \
{                               \
  xb_vecNx16 x0,y0,x1;          \
  x1=BBE_LVNX16_X(px, 32*L);    \
  BBE_LVNX16_XP(x0, px, 2*L);   \
  BBE_LVNX16_XP(y0, py, 32*L);  \
  BBE_MULANX16(Z0,x0, y0);      \
  BBE_MULANX16(Z1,x1, y0);      \
}  
  ...
  for (m = 0; m<8; m++)
  {
    ...
    for (k = i = 0; i<L * 16; i += BBE_SIMD_WIDTH)
    {
      k = BBE_ADDMOD16U(k, (L << 16) | BBE_SIMD_WIDTH);
      xstride = -15 * 2 * L + 2 * BBE_SIMD_WIDTH;
      XT_MOVEQZ(xstride, -16 * 2 * L + 2 * BBE_SIMD_WIDTH, k);

      x1 = BBE_LVNX16_X(px, 32 * L);
      BBE_LVNX16_XP(x0, px, 2 * L);
      BBE_LVNX16_XP(y0, py, 32 * L);
      Z0 = BBE_MULNX16(x0, y0);
      Z1 = BBE_MULNX16(x1, y0);
      ...
      x1 = BBE_LVNX16_X(px, 16 * 2 * L);
      BBE_LVNX16_XP(x0, px, xstride);
      BBE_LVNX16_XP(y0, py, -15 * 16 * 2 * L + 2 * BBE_SIMD_WIDTH);
      BBE_MULANX16(Z0, x0, y0);
      BBE_MULANX16(Z1, x1, y0);
