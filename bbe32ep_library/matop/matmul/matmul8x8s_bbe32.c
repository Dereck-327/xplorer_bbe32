            ...
            int L, int Q )
{
  ...
  for (m = 0; m<4; m++)
  {
    ...
    for (k = i = 0; i<L * 8; i += BBE_SIMD_WIDTH)
    {
      k = BBE_ADDMOD16U(k, (L << 16) | BBE_SIMD_WIDTH);
      xstride = -7 * 2 * L + 2 * BBE_SIMD_WIDTH;
      XT_MOVEQZ(xstride, -8 * 2 * L + 2 * BBE_SIMD_WIDTH, k);

      x1 = BBE_LVNX16_X(px, 8 * 2 * L);
      BBE_LVNX16_XP(x0, px, 2 * L);
      BBE_LVNX16_XP(y0, py, 8 * 2 * L);
      ...
      BBE_LVNX16_XP(y0, py, -7 * 8 * 2 * L + 2 * BBE_SIMD_WIDTH);
      BBE_MULANX16(Z0, x0, y0);
      BBE_MULANX16(Z1, x1, y0);
      ...
      BBE_SVNX16_X(z1, pz, 8 * 2 * L);
      BBE_SVNX16_IP(z0, pz, 2 * BBE_SIMD_WIDTH);
    }
    /* jump over row */
    px = (const xb_vecNx16*)XT_ADDX2(16 * L, (uintptr_t)px);
    pz = (xb_vecNx16*)XT_ADDX8(2 * L, (uintptr_t)pz);
  }
} /* matmul8x8s() */
