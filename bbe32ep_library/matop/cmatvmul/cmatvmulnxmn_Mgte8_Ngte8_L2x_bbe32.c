...
#if !(HAVE_MULPC && 1)
DISCARD_FUN(void, cmatvmulnxmn_Mgte8_Ngte8_L2x,( void    *          pScr,
  ...
  for ( l=0; l<L/(BBE_SIMD_WIDTH/8); l++ )
  {
    ...
    */
...

    {
      ...
      for ( i=0; i<M/4; i++ )
      {
        ...
        X0_rd = (const xb_vecNx16*)( (uintptr_t)X0_rd + 3*BBE_SIMD_WIDTH/8*N*4 );
        X1_rd = (const xb_vecNx16*)( (uintptr_t)X1_rd + 3*BBE_SIMD_WIDTH/8*N*4 );
        X2_rd = (const xb_vecNx16*)( (uintptr_t)X2_rd + 3*BBE_SIMD_WIDTH/8*N*4 );
        X3_rd = (const xb_vecNx16*)( (uintptr_t)X3_rd + 3*BBE_SIMD_WIDTH/8*N*4 );

        Y_rd = (const xb_vecNx16*)( (uintptr_t)Y_rd - BBE_SIMD_WIDTH/8*N*4 );
    ...
    */
...

    {
      ...
      {
        a1 = BBE_LVNX16_X(Z_rd, off*4*BBE_SIMD_WIDTH/2);
        BBE_LVNX16_IP( a0, Z_rd, 8*BBE_SIMD_WIDTH/2 );
...
size_t cmatvmulnxmn_Mgte8_Ngte8_L2x_getScratchSize(int N, int M)
{
  ...
  size_t  sz = 0;
  sz = (M*N + _N + _M) * 2 * 2 * sizeof(int16_t);
  return sz;
} /* cmatvmulnxmn_Mgte8_Ngte8_L2x_getScratchSize() */
#endif
