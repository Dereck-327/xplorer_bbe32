...
#if !(HAVE_MULPC && 1)
DISCARD_FUN(void, cmatvmulnxmn_Mgte8_Ngte8_L8x,( 
                   void    *          pScr,
    ...
    */
...

    {
      ...
      for ( i=0; i<M/4; i++ )
      {
        ...
        X0_rd = (const xb_vecNx16*)( (uintptr_t)X0_rd + 3*BBE_SIMD_WIDTH/2*N*4 );
        X1_rd = (const xb_vecNx16*)( (uintptr_t)X1_rd + 3*BBE_SIMD_WIDTH/2*N*4 );
        X2_rd = (const xb_vecNx16*)( (uintptr_t)X2_rd + 3*BBE_SIMD_WIDTH/2*N*4 );
        X3_rd = (const xb_vecNx16*)( (uintptr_t)X3_rd + 3*BBE_SIMD_WIDTH/2*N*4 );

        Y_rd = (const xb_vecNx16*)( (uintptr_t)Y_rd - BBE_SIMD_WIDTH/2*N*4 );
...
size_t cmatvmulnxmn_Mgte8_Ngte8_L8x_getScratchSize(int N, int M)
{
  ...
  return sz;
} /* cmatvmulnxmn_Mgte8_Ngte8_L8x_getScratchSize() */
#endif
