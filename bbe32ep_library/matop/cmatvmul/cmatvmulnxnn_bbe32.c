...
-------------------------------------------------------------------------*/
#if !(HAVE_MULPC && 1)
DISCARD_FUN(void, cmatvmulnxnn,( void * pScr,
                   complex_fract16 * restrict z, 
             const complex_fract16 * restrict x, 
             const complex_fract16 * restrict y, 
             int N, int L, int Q ))
size_t cmatvmulnxnn_getScratchSize(int N, int L) { (void)N; (void)L; return 0; }
#else
              ...
              int N, int L, int Q )
{
  ...
  cmatvmulnxmn(pScr, z, x, y, N, N, L, Q);
} /* cmatvmulnxnn() */

/* Return the scratch area size, in bytes. */
size_t cmatvmulnxnn_getScratchSize ( int N, int L )
{
  return cmatvmulnxmn_getScratchSize(N, N, L);
} /* cmatvmulnxnn_getScratchSize() */
#endif
