              ...
              int L, int Q )
{
  ...
  for (l = 0; l<L; l += (BBE_SIMD_WIDTH / 2))
  {
    ...
    BBE_LVNX16_XP(X10, X_, 2 * 2 * L);
    BBE_LVNX16_XP(X11, X_, -3 * 2 * 2 * L + 2 * BBE_SIMD_WIDTH);

    /* Loading input vector Y */
    BBE_LVNX16_XP(Y0, Y_, 2 * 2 * L);
    BBE_LVNX16_XP(Y1, Y_, -2 * 2 * L + 2 * BBE_SIMD_WIDTH);

    /* Multiple input matrix X and Y */
    Z0 = BBE_MULNX16C(X00, Y0); Z1 = BBE_MULNX16C(X10, Y0);
    BBE_MULANX16C(Z0, X01, Y1); BBE_MULANX16C(Z1, X11, Y1);

    /* Pack and save result */
    z0 = BBE_PACKVNX40(Z0, q);   z1 = BBE_PACKVNX40(Z1, q);
    BBE_SVNX16_XP(z0, Z_, 2 * 2 * L); BBE_SVNX16_XP(z1, Z_, -2 * 2 * L + 2 * BBE_SIMD_WIDTH);
  }
} /* cmatvmul2x2s() */
