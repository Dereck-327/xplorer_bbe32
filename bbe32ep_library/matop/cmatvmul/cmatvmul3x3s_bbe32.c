              ...
              int L, int Q )
{
  ...
  const int dOffset = 2 * L*sizeof(int16_t);
  ...
  for (l = 0; l<L; l += (BBE_SIMD_WIDTH / 2) )
  {
    ...
    /* Loading 8 input vectors Y */
    BBE_LVNX16_XP(vY0, pYrd, dOffset);
    BBE_LVNX16_XP(vY1, pYrd, dOffset);
    BBE_LVNX16_XP(vY2, pYrd, -2 * dOffset + 2 * BBE_SIMD_WIDTH);

    Z0 = BBE_MULRNX16C(vX00, vY0, q);
    BBE_MULANX16C(Z0, vX01, vY1);
    BBE_MULANX16C(Z0, vX02, vY2);

    Z1 = BBE_MULRNX16C(vX10, vY0, q);
    BBE_MULANX16C(Z1, vX11, vY1);
    BBE_MULANX16C(Z1, vX12, vY2);

    Z2 = BBE_MULRNX16C(vX20, vY0, q);
    BBE_MULANX16C(Z2, vX21, vY1);
    BBE_MULANX16C(Z2, vX22, vY2);
