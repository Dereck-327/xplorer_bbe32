...
#if !(HAVE_MULPC && 1)
DISCARD_FUN(void, cmatvmulnxmn_Mgte8_Nlt8_L8x,( 
                   void    *          pScr,
  ...
  for ( l=0; l<L/(BBE_SIMD_WIDTH/2); l++ )
  {
    ...
    */
...

    {
      ...
      /*
      * 8x4 => 4x8 complex-valued transpose
      */

      BBE_DSELNX16I( y1, y0, y1, y0, BBE_DSELI_DEINTERLEAVE_2 );
      BBE_DSELNX16I( y3, y2, y3, y2, BBE_DSELI_DEINTERLEAVE_2 );

      BBE_DSELNX16I( y2, y0, y2, y0, BBE_DSELI_DEINTERLEAVE_2 );
      BBE_DSELNX16I( y3, y1, y3, y1, BBE_DSELI_DEINTERLEAVE_2 );
      ...
      for ( i=0; i<M/4; i++ )
      {
        ...
        BBE_LVNX16_IP( x20, X_rd, +4*BBE_SIMD_WIDTH/2 );
        BBE_LVNX16_IP( x21, X_rd, +4*BBE_SIMD_WIDTH/2 );
        BBE_LVNX16_IP( x22, X_rd, +4*BBE_SIMD_WIDTH/2 );
        BBE_LVNX16_IP( x23, X_rd, +4*BBE_SIMD_WIDTH/2 );
                             
        BBE_LVNX16_IP( x30, X_rd, +4*BBE_SIMD_WIDTH/2 );
        BBE_LVNX16_IP( x31, X_rd, +4*BBE_SIMD_WIDTH/2 );
        BBE_LVNX16_IP( x32, X_rd, +4*BBE_SIMD_WIDTH/2 );
        BBE_LVNX16_IP( x33, X_rd, +4*BBE_SIMD_WIDTH/2 );

        w0 = BBE_MULRNX16C( x00, y0, vsa0 );
        w1 = BBE_MULRNX16C( x10, y0, vsa0 );
        w2 = BBE_MULRNX16C( x20, y0, vsa0 );
        w3 = BBE_MULRNX16C( x30, y0, vsa0 );
    ...
    */
...

    {
      ...
      for ( m=1; m<_M/(BBE_SIMD_WIDTH/2); m++ )
      {
        ...
        */
        ...
        BBE_SVNX16_XP( a0, Z_wr, _M*4 );
        BBE_SVNX16_XP( a1, Z_wr, _M*4 );
        BBE_SVNX16_XP( a2, Z_wr, _M*4 );
        BBE_SVNX16_XP( a3, Z_wr, _M*4 );
        BBE_SVNX16_XP( a4, Z_wr, _M*4 );
        BBE_SVNX16_XP( a5, Z_wr, _M*4 );
        BBE_SVNX16_XP( a6, Z_wr, _M*4 );

        BBE_SVNX16_XP( a7, Z_wr, -7*_M*4 + 4*BBE_SIMD_WIDTH/2 );
      }
      {
        a4 = BBE_LVNX16_X(Z_rd, off*8*BBE_SIMD_WIDTH );
        a5 = BBE_LVNX16_X(Z_rd, off*10*BBE_SIMD_WIDTH);
        a6 = BBE_LVNX16_X(Z_rd, off*12*BBE_SIMD_WIDTH);
        a7 = BBE_LVNX16_X(Z_rd, off*14*BBE_SIMD_WIDTH);
        BBE_LVNX16_IP( a0, Z_rd, +4*BBE_SIMD_WIDTH/2 );
        BBE_LVNX16_IP( a1, Z_rd, +4*BBE_SIMD_WIDTH/2 );
        BBE_LVNX16_IP( a2, Z_rd, +4*BBE_SIMD_WIDTH/2 );
        BBE_LVNX16_IP( a3, Z_rd, +20*BBE_SIMD_WIDTH/2 );
...
size_t cmatvmulnxmn_Mgte8_Nlt8_L8x_getScratchSize(int N, int M)
{
  ...
  size_t  sz = 0;
  sz = (4 * M + _M) * 2 * 8 * sizeof(int16_t);
  return sz;
} /* cmatvmulnxmn_Mgte8_Nlt8_L8x_getScratchSize() */
#endif
