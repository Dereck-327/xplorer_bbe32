...
/* M=4, N=4, Sx=16, Sy=16 */
void cbs4x4f ( complex_float * restrict y, const complex_float * restrict x, int L )
{
  ...
  for ( l=0; l<(L>>(LOG2_BBE_SIMD_WIDTH-2))*2; l++ )
  {
    ...
    stridex = 4*2*BBE_SIMD_WIDTH;
    XT_MOVEQZ(stridex, -L*16*sz_cf32+6*2*BBE_SIMD_WIDTH, k);
    stridey = -6*L*sz_cf32+2*BBE_SIMD_WIDTH;
    XT_MOVEQZ(stridey, L*sz_cf32+2*BBE_SIMD_WIDTH, k);

    BBE_LVNX16_IP(X00, px0, 4*2*BBE_SIMD_WIDTH);
    BBE_LVNX16_IP(X01, px0, 4*2*BBE_SIMD_WIDTH);
    BBE_LVNX16_IP(X02, px0, 4*2*BBE_SIMD_WIDTH);
    BBE_LVNX16_XP(X03, px0, stridex);

    BBE_LVNX16_IP(X10, px1, 4*2*BBE_SIMD_WIDTH);
    BBE_LVNX16_IP(X11, px1, 4*2*BBE_SIMD_WIDTH);
    BBE_LVNX16_IP(X12, px1, 4*2*BBE_SIMD_WIDTH);
