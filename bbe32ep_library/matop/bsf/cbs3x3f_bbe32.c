...
/* M=3, N=3, Sx=12, Sy=9 */
void cbs3x3f ( complex_float * restrict y, const complex_float * restrict x, int L )
{
  ...
  for ( l=0; l<(L>>(LOG2_BBE_SIMD_WIDTH-2)); l++ )
  {
    ...
    BBE_LVNX16_IP(X0, px0, 3*2*BBE_SIMD_WIDTH);
    BBE_LVNX16_IP(X1, px0, 3*2*BBE_SIMD_WIDTH);
    BBE_LVNX16_IP(X2, px0, 3*2*BBE_SIMD_WIDTH);
    BBE_LVNX16_IP(X3, px0, 3*2*BBE_SIMD_WIDTH);
    ...
    BBE_LVNX16_IP(X0, px1, 3*2*BBE_SIMD_WIDTH);
    BBE_LVNX16_IP(X1, px1, 3*2*BBE_SIMD_WIDTH);
    BBE_LVNX16_IP(X2, px1, 3*2*BBE_SIMD_WIDTH);
    BBE_LVNX16_IP(X3, px1, 3*2*BBE_SIMD_WIDTH);
  ...
  for (l = 0; l<(L>>(LOG2_BBE_SIMD_WIDTH-2)); l++)
  {
    ...
    Y1 = BBE_SELNX16I(X1, X0, BBE_SELI_PACK_4);
    Y2 = BBE_SELNX16I(X3, X2, BBE_SELI_PACK_4);
    Y0 = BBE_SELNX16I(Y2, Y1, BBE_SELI_PACK_8);
