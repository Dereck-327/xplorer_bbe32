...
/* M=2, N=1, Sx=2, Sy=2 */
void rbs2x1 ( int16_t * restrict y, const int16_t * restrict x, int L )
{
  const xb_vecNx16* restrict X = (const xb_vecNx16*)x;
  xb_vecNx16* restrict Y = (xb_vecNx16*)y;
  xb_vecNx16* restrict y1 = Y + (L >> LOG2_BBE_SIMD_WIDTH);
  ...
  for (l = 0; l<(L >> LOG2_BBE_SIMD_WIDTH); l++)
  {
    ...
    Y1 = BBE_SELNX16I(X1, X0, BBE_SELI_EXTRACT_1_OF_2_OFF_1);
    BBE_SVNX16_IP(Y0, Y, 2 * BBE_SIMD_WIDTH);
    BBE_SVNX16_IP(Y1, y1, 2 * BBE_SIMD_WIDTH);
  }
} /* rbs2x1() */
