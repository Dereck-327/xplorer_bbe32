...
/* M=3, N=3, Sx=16, Sy=9 */
void cbs3x3(complex_fract16 * restrict y, const complex_fract16 * restrict x,int L)
{
  ...
  for (k = 0; k<(L >> (LOG2_BBE_SIMD_WIDTH - 1)); k++)
  {
    X4 = BBE_LVNX16_I(X, 4 * 4 * BBE_SIMD_WIDTH);
    X5 = BBE_LVNX16_I(X, 5 * 4 * BBE_SIMD_WIDTH);
    X6 = BBE_LVNX16_I(X, 6 * 4 * BBE_SIMD_WIDTH);
    X7 = BBE_LVNX16_I(X, 7 * 4 * BBE_SIMD_WIDTH);
    DEINTLV1(Y5, Y4, X5, X4);
    DEINTLV1(Y7, Y6, X7, X6);
    DEINTLV2(X5, X4, Y6, Y4);
    DEINTLV2(X7, X6, Y7, Y5);

    X1 = BBE_LVNX16_I(X, 1 * 4 * BBE_SIMD_WIDTH);
    X2 = BBE_LVNX16_I(X, 2 * 4 * BBE_SIMD_WIDTH);
    X3 = BBE_LVNX16_I(X, 3 * 4 * BBE_SIMD_WIDTH);
    ...
    X2 = BBE_LVNX16_I(X, 2 * 4 * BBE_SIMD_WIDTH);
    X3 = BBE_LVNX16_I(X, 3 * 4 * BBE_SIMD_WIDTH);
    BBE_LVNX16_XP(X0, X, 15 * 2 * BBE_SIMD_WIDTH);
    DEINTLV1(Y1, Y0, X1, X0);
    DEINTLV1(Y3, Y2, X3, X2);
    DEINTLV2(X1, X0, Y2, Y0);

    Y0 = BBE_SELNX16I(X4, X0, BBE_SELI_EXTRACT_LO_HALVES);
    BBE_SVNX16_XP(Y0, Y, 2 * BBE_SIMD_WIDTH - 8 * (L << 2));
  }
} /* cbs3x3() */
