...
void rbs5_7x1(int16_t* restrict y, const int16_t* restrict x, int MN,int L)
{
    ...
    int l,off1,off2,off3,off4,off5,off6;
    ...
    off3=3*(L<<1);
    off4=(MN>=5)?(L<<3):0;
    off5=(MN>=6)? off4+(L<<1):0;
    off6=(MN>=7)? off5+(L<<1):0;
    if (L <= 0) return;
    for (l = 0; l<(L >> LOG2_BBE_SIMD_WIDTH); l++)
    {
        ...
        BBE_SVNX16_X (X6,z, off6);
        BBE_SVNX16_X (X5,z, off5);
        BBE_SVNX16_X (X4,z, off4);
        BBE_SVNX16_X (X3,z, off3);
        BBE_SVNX16_X (X2,z, off2);
        BBE_SVNX16_X (X1,z, off1);
