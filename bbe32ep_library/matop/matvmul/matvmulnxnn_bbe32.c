...
DISCARD_FUN(void, matvmulnxnn,( void *             pScr,
                  int16_t * restrict z,
            const int16_t * restrict x,
            const int16_t * restrict y,
            int N, int L, int Q ))
size_t matvmulnxnn_getScratchSize ( int N, int L ) {(void)N; (void)L; return 0;};
#else


/* Block Order, NxN*Nx1->Nx1, Sx=NxN, Sy=(N>4)?((N+4)&~4):N, Sz=(N>4)?((N+4)&~4):N
   Restrictions:
              ...
              int N, int L, int Q )
{
  ...
  matvmulnxmn(pScr, z, x, y, N, N, L, Q);
} /* matvmulnxnn() */

/* Return the scratch area size, in bytes. */
size_t matvmulnxnn_getScratchSize ( int N, int L )
{
  return matvmulnxmn_getScratchSize(N, N, L);
} /* matvmulnxnn_getScratchSize() */
#endif
