...
#if !(HAVE_MULPC && 1)
DISCARD_FUN(void, matvmulnxmn_Mgte12_Ngte12_L4x, (void * pScr,
                   int16_t * restrict z, 
             const int16_t * restrict x, 
             const int16_t * restrict y, 
             int N, int M, int L, int Q ))
size_t matvmulnxmn_Mgte12_Ngte12_L4_getScratchSize(int N, int M) { (void)N; (void)M; return 0; }
#else
...
// M>=12 && N>=12 && !(L&3)
void matvmulnxmn_Mgte12_Ngte12_L4x( void    *          pScr,
                                    int16_t * restrict z, 
                              const int16_t * restrict x, 
                              const int16_t * restrict y, 
                              int N, int M, int L, int Q )
{
  ...
  for ( l=0; l<L/(BBE_SIMD_WIDTH/4); l++ )
  {
...

    {
      ...
      for ( i=0; i<M/4; i++ )
      {
        w0 = BBE_MOVWA32( ( 1L << Q ) >> 1 );

        __Pragma( "loop_count min=1" );
        for ( j=0; j<N/4; j++ )
        {
          BBE_LVNX16_XP( x0, X_rd, +1*BBE_SIMD_WIDTH/4*N*2                    );
          BBE_LVNX16_XP( x1, X_rd, +1*BBE_SIMD_WIDTH/4*N*2                    );
          BBE_LVNX16_XP( x2, X_rd, +1*BBE_SIMD_WIDTH/4*N*2                    );
          BBE_LVNX16_XP( x3, X_rd, -3*BBE_SIMD_WIDTH/4*N*2 + 2*BBE_SIMD_WIDTH );
        ...
        X_rd = (const xb_vecNx16*)( (uintptr_t)X_rd + 3*BBE_SIMD_WIDTH/4*N*2 );
        Y_rd = (const xb_vecNx16*)( (uintptr_t)Y_rd - 1*BBE_SIMD_WIDTH/4*N*2 );
...
size_t matvmulnxmn_Mgte12_Ngte12_L4_getScratchSize(int N, int M)
{
  ...
  _M = ((M + (BBE_SIMD_WIDTH - 1)) & ~(BBE_SIMD_WIDTH - 1));
  Sx = getSpaceR(N*M);
  return (Sx * 4 * 2 + _M * 4)*sizeof(int16_t);
} /* matvmulnxmn_Mgte12_Ngte12_L4_getScratchSize() */
#endif
