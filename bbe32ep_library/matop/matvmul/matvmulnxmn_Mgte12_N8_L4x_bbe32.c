...
#if !(HAVE_MULPC && 1)
DISCARD_FUN(void, matvmulnxmn_Mgte12_N8_L4x, (void * pScr,
                   int16_t * restrict z, 
             const int16_t * restrict x, 
             const int16_t * restrict y, 
             int N, int M, int L, int Q ))
size_t matvmulnxmn_Mgte12_N8_getScratchSize(int N, int M) { (void)N; (void)M; return 0; }
#else
...
// M>=12 && N==8 && !(L&3)
void matvmulnxmn_Mgte12_N8_L4x( void    *          pScr,
                                int16_t * restrict z, 
                          const int16_t * restrict x, 
                          const int16_t * restrict y, 
                          int N, int M, int L, int Q )
  ...
  for ( l=0; l<L/(BBE_SIMD_WIDTH/4); l++ )
  {
...

    {
...

      {
        ...
        BBE_LVNX16_IP( a1, Y_rd, 2*BBE_SIMD_WIDTH );

        b0 = BBE_SELNX16( a1, a0, sel0 );
        b1 = BBE_SELNX16( a1, a0, sel1 );

        z0 = 0;

        BBE_DSELNX16I( y1, y0, z0, b0, BBE_DSELI_INTERLEAVE_1 );
        BBE_DSELNX16I( y3, y2, z0, b1, BBE_DSELI_INTERLEAVE_1 );
      }
      ...
      for ( i=0; i<M/4; i++ )
      {
        ...
        BBE_LVNX16_IP( x20, X_rd, 2*BBE_SIMD_WIDTH );
        BBE_LVNX16_IP( x21, X_rd, 2*BBE_SIMD_WIDTH );
        BBE_LVNX16_IP( x30, X_rd, 2*BBE_SIMD_WIDTH );
        BBE_LVNX16_IP( x31, X_rd, 2*BBE_SIMD_WIDTH );

        BBE_DSELNX16I( a01, a00, x10, x00, BBE_DSELI_INTERLEAVE_1 );
        BBE_DSELNX16I( a11, a10, x30, x20, BBE_DSELI_INTERLEAVE_1 );

        BBE_DSELNX16I( a03, a02, x11, x01, BBE_DSELI_INTERLEAVE_1 );
        BBE_DSELNX16I( a13, a12, x31, x21, BBE_DSELI_INTERLEAVE_1 );

        w0 = BBE_MULRNX16PC_0( a00, y0, vsa0 );
        BBE_MULANX16PC_0 ( w0, a01, y1 );
        BBE_MULANX16PC_0 ( w0, a02, y2 );
        BBE_MULANX16PC_0 ( w0, a03, y3 );

        BBE_MULANX16PC_1( w0, a10, y0 );
        BBE_MULANX16PC_1( w0, a11, y1 );
        BBE_MULANX16PC_1( w0, a12, y2 );
        BBE_MULANX16PC_1( w0, a13, y3 );
...
} /* matvmulnxmn_Mgte12_N8_getScratchSize() */
#endif
