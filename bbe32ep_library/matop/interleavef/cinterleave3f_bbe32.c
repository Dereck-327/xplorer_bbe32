               ...
               const complex_float * restrict x2, int N )
{
  ...
  xb_vecNx16 X0, X1, X2, Xt, Y0, Y1, Y2, Y3;
  xb_vecNx16 zero;
  valign aly;
  int n;
  ...
  px0 = (const xb_vecNx16 *)(x0);
  px1 = (const xb_vecNx16 *)(x1);
  px2 = (const xb_vecNx16 *)(x2);
  py  = (      xb_vecNx16 *)(y);
  aly = BBE_ZALIGN();
  zero = BBE_ZERONX16();
  ...
  for (n = 0; n<(N>>(LOG2_BBE_SIMD_WIDTH-2)); n++)
  {
    ...
    BBE_DSELNX16I(Xt, X1, zero, X1, BBE_DSELI_INTERLEAVE_4);
    BBE_DSELNX16I(Y1, Y0, X1  , X0, BBE_DSELI_INTERLEAVE_4);
    BBE_DSELNX16I(Y3, Y2, Xt  , X2, BBE_DSELI_INTERLEAVE_4);

    BBE_SAVNX16_XP(Y0, aly, py, 3*sizeof(complex_float));
    BBE_SAVNX16_XP(Y1, aly, py, 3*sizeof(complex_float));
    BBE_SAVNX16_XP(Y2, aly, py, 3*sizeof(complex_float));
    BBE_SAVNX16_XP(Y3, aly, py, 3*sizeof(complex_float));
  }
  BBE_SANX16POS_FP(aly, py);
} /* cinterleave3f() */
