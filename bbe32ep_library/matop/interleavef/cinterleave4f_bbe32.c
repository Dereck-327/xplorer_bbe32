               ...
               const complex_float * restrict x3, int N )
{
  ...
  px3 = (const xb_vecNx16 *)(x3);
  py  = (      xb_vecNx16 *)(y);

  for (n = 0; n<(N>>(LOG2_BBE_SIMD_WIDTH-2)); n++)
  {
    BBE_LVNX16_IP(V0, px0, 2*BBE_SIMD_WIDTH);
    BBE_LVNX16_IP(V1, px1, 2*BBE_SIMD_WIDTH);
    BBE_LVNX16_IP(V2, px2, 2*BBE_SIMD_WIDTH);
    BBE_LVNX16_IP(V3, px3, 2*BBE_SIMD_WIDTH);
    ...
    BBE_SVNX16_IP(V0, py, 2*BBE_SIMD_WIDTH);
    BBE_SVNX16_IP(V1, py, 2*BBE_SIMD_WIDTH);
    BBE_SVNX16_IP(V2, py, 2*BBE_SIMD_WIDTH);
    BBE_SVNX16_IP(V3, py, 2*BBE_SIMD_WIDTH);
  }
} /* cinterleave4f() */
