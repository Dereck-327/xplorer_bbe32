...
---------------------------------------------------------------------------*/
void latrsf_process6( latrsf_t  *          latrs,
                      ...
                      int                  N )
{
    ...
    {
        DL5 = BBE_LVN_2XF32_I(delLine, 0*BBE_SIMD_WIDTH/2*sizeof(float32_t));
        DL4 = BBE_LVN_2XF32_I(delLine, 1*BBE_SIMD_WIDTH/2*sizeof(float32_t));
        DL3 = BBE_LVN_2XF32_I(delLine, 2*BBE_SIMD_WIDTH/2*sizeof(float32_t));
        DL2 = BBE_LVN_2XF32_I(delLine, 3*BBE_SIMD_WIDTH/2*sizeof(float32_t));
        DL1 = BBE_LVN_2XF32_I(delLine, 4*BBE_SIMD_WIDTH/2*sizeof(float32_t));
        DL0 = BBE_LVN_2XF32_I(delLine, 5*BBE_SIMD_WIDTH/2*sizeof(float32_t));
    ...
    xb_vecN_2xf32 Xin,R0,R1,R2,R3,R4,
                  DL0,DL1,DL2,DL3,DL4,DL5,
                  CF012345,CF0,CF1,CF2,CF3,CF4,CF5;
    ...
    pr   = (      xb_vecN_2xf32 *)r;
    delLine   = ( xb_vecN_2xf32 *)latrs->delLine;
    coef = (const xb_vecN_2xf32 *)latrs->coef;

    Gain    = latrs->gain;
    CF012345= BBE_LVN_2XF32_I(coef, 0);
    ...
    for ( l=0; l<(L>>(LOG2_BBE_SIMD_WIDTH-1)); l++ )
    {
        ...
        for ( n=0; n<(N>>1); n++ )
        {
            ...
            CF012345 = BBE_ADDN_2XF32(CF012345, 0.0f);

            BBE_LVN_2XF32_XP(Xin, px, L*sizeof(float32_t));
            ACCR = BBE_MULN_2XF32(Xin, Gain);
            BBE_MULSN_2XF32(ACCR, DL5, CF5);

            BBE_MULSN_2XF32(ACCR, DL4, CF4);  R4 = ACCR;
            BBE_MULAN_2XF32(DL4, R4, CF4);  DL5 = DL4;

            BBE_MULSN_2XF32(ACCR, DL3, CF3);  R3 = ACCR;
            BBE_MULAN_2XF32(DL3, R3, CF3);  DL4 = DL3;

            BBE_MULSN_2XF32(ACCR, DL2, CF2);  R2 = ACCR;
            BBE_MULAN_2XF32(DL2, R2, CF2);  DL3 = DL2;

            BBE_MULSN_2XF32(ACCR, DL1, CF1);  R1 = ACCR;
            BBE_MULAN_2XF32(DL1, R1, CF1);  DL2 = DL1;

            BBE_MULSN_2XF32(ACCR, DL0, CF0);  R0 = ACCR;
            BBE_MULAN_2XF32(DL0, R0, CF0);  DL1 = DL0;
            
            DL0 = ACCR;
            BBE_SVN_2XF32_XP(R0, pr, L*sizeof(float32_t));
