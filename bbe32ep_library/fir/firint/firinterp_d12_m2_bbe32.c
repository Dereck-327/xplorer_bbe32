...
/* Filter processing function for D=12 M=2 and N=8*n */
static void filter_proc_12d_2_8n (  
                              void* handle,
                                 int16_t *  restrict  y,
                          ...
                          )
{
  ...
  for (k=0;k<6;k++)
  {
    ...
    for (n=0;n<N/(BBE_SIMD_WIDTH/2);n++)
    {
      ...
      BBE_SVNX16_IP(y0,pY,  1*2*BBE_SIMD_WIDTH);
...
const tFirFxdxns interp_12d_2_8n   ={&firinterp_dx,filter_proc_12d_2_8n}  ;
