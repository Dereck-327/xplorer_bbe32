...
/* Filter processing function for D=2 M%16==0 and N%8==0 */
static void filter_proc_2d_mx_8n(      
                              void* handle,
                                 int16_t *  restrict  y,
                          ...
                          )
{
  ...
  for (n = 0; n<N / (BBE_SIMD_WIDTH / 2); n++)
  {
      ...
      pH = (const xb_vecNx16 *)(coef + 2 * M - BBE_SIMD_WIDTH);

      A0 = A1 = BBE_MOVWA32(0x2000);
      __Pragma("ymemory(pD)")
          __Pragma("ymemory(pH)")
          __Pragma("loop_count min=1")
...
const tFirFxdxns interp_2d_mx_8n  ={&firinterp_dx,filter_proc_2d_mx_8n} ;
