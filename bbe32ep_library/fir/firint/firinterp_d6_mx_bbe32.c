...
/* Filter processing function for D=6 M=8 and N=8*n */
static void filter_proc_6d_mx_8n (  
                              void* handle,
                                 int16_t *  restrict  y,
                          ...
                          )
{
    ...
    const xb_vecNx16 *  restrict pH_ = (const xb_vecNx16 *)coef;
    xb_vecNx16 *  restrict pD = (xb_vecNx16 *)delayLine;
    xb_vecNx16 *  restrict pD_ = (xb_vecNx16 *)delayLine;
    ...
    for (k = 0; k<D; k += 2)
    {
        ...
        for (n = 0; n<N / (BBE_SIMD_WIDTH / 2); n++)
        {
            ...
            pH = (const xb_vecNx16 *)(pH_ + (2 * M / BBE_SIMD_WIDTH) - 1);

            A0 = A1 = BBE_MOVWA32(0x800);
            __Pragma("ymemory(pD)")
                __Pragma("ymemory(pH)")
            ...
            for (m = 0; m<M / (BBE_SIMD_WIDTH); m++)
            {
                ...
                c01 = BBE_EXTRNX16C(CoefVec, 2);
                c11 = BBE_EXTRNX16C(CoefVec, 3);
                c02 = BBE_EXTRNX16C(CoefVec, 4);
                c12 = BBE_EXTRNX16C(CoefVec, 5);
                c03 = BBE_EXTRNX16C(CoefVec, 6);
                c13 = BBE_EXTRNX16C(CoefVec, 7);

                BBE_LVNX16_IP(CoefVec, pH, -2 * BBE_SIMD_WIDTH);
                c04 = BBE_EXTRNX16C(CoefVec, 0);
                c14 = BBE_EXTRNX16C(CoefVec, 1);
                c05 = BBE_EXTRNX16C(CoefVec, 2);
                c15 = BBE_EXTRNX16C(CoefVec, 3);
                c06 = BBE_EXTRNX16C(CoefVec, 4);
                c16 = BBE_EXTRNX16C(CoefVec, 5);
                c07 = BBE_EXTRNX16C(CoefVec, 6);
                c17 = BBE_EXTRNX16C(CoefVec, 7);
                ...
                BBE_MULANX16PR(A0, p06, p07, c00);
                BBE_MULANX16PR(A1, p06, p07, c10);
                BBE_MULANX16PR(A0, p04, p05, c01);
                BBE_MULANX16PR(A1, p04, p05, c11);
                BBE_MULANX16PR(A0, p02, p03, c02);
                BBE_MULANX16PR(A1, p02, p03, c12);
                BBE_MULANX16PR(A0, p00, p01, c03);
                BBE_MULANX16PR(A1, p00, p01, c13);

                BBE_MULANX16PR(A0, p16, p17, c04);
                BBE_MULANX16PR(A1, p16, p17, c14);
                BBE_MULANX16PR(A0, p14, p15, c05);
                BBE_MULANX16PR(A1, p14, p15, c15);
                BBE_MULANX16PR(A0, p12, p13, c06);
                BBE_MULANX16PR(A1, p12, p13, c16);
                BBE_MULANX16PR(A0, p10, p11, c07);
                BBE_MULANX16PR(A1, p10, p11, c17);
            }
            y0 = BBE_PACKVNX40(A0, shft);
            y1 = BBE_PACKVNX40(A1, shft);
            BBE_SVNX16_I(y1, pY, 2 * BBE_SIMD_WIDTH);
            BBE_SVNX16_XP(y0, pY, 2 * D*BBE_SIMD_WIDTH);
        }
        pD_ = pD + 1;
        pH = pH_ + (2 * M / BBE_SIMD_WIDTH);
    }

    firinterp_6d_intlv(y, N);
}
const tFirFxdxns interp_6d_mx_8n  ={&firinterp_d_6_12_mx,filter_proc_6d_mx_8n} ;
