...
/* processing function D==4, M==32 */
static void firdec_proc_D4_M32( int16_t * restrict y,
                           ...
                           int M, int N, int D )
{
    ...
    for (n = 0; n<N / (BBE_SIMD_WIDTH / 2); n++)
    {
        ...
        BBE_LVNX16_XP(x0, X, 2 * BBE_SIMD_WIDTH);
        BBE_LVNX16_XP(x1, X, 2 * BBE_SIMD_WIDTH);
        BBE_LVNX16_XP(x2, X, 2 * BBE_SIMD_WIDTH);
        BBE_LVNX16_XP(x3, X, 2 * BBE_SIMD_WIDTH);
        ...
        cf = BBE_LVNX16_I((const xb_vecNx16*)coef, 1 * 2 * BBE_SIMD_WIDTH);
        ...
        BBE_SELPCNX16I(p31, p30, x3, d3, 0);
        BBE_SELPCNX16I(p33, p32, x3, d3, 2);
        BBE_SELPCNX16I(p35, p34, x3, d3, 4);
        BBE_SELPCNX16I(p37, p36, x3, d3, 6);
        d3 = x3;
        BBE_SVNX16_I(d3, (xb_vecNx16*)delayLine, 3 * 2 * BBE_SIMD_WIDTH);

        q30 = BBE_EXTRNX16C(cf, 0);
        q31 = BBE_EXTRNX16C(cf, 1);
        q32 = BBE_EXTRNX16C(cf, 2);
        q33 = BBE_EXTRNX16C(cf, 3);

        BBE_MULANX16PR(w0, p31, p30, q30);
        BBE_MULANX16PR(w0, p33, p32, q31);
        BBE_MULANX16PR(w0, p35, p34, q32);
        BBE_MULANX16PR(w0, p37, p36, q33);


        cf = BBE_LVNX16_I((const xb_vecNx16*)coef, 0 * 2 * BBE_SIMD_WIDTH);
        //                            
        // Coefficients bank 0
        //                            
        ...
        BBE_SELPCNX16I(p21, p20, x2, d2, 0);
        BBE_SELPCNX16I(p23, p22, x2, d2, 2);
        BBE_SELPCNX16I(p25, p24, x2, d2, 4);
        BBE_SELPCNX16I(p27, p26, x2, d2, 6);
        d2 = x2;
        BBE_SVNX16_I(d2, (xb_vecNx16*)delayLine, 2 * 2 * BBE_SIMD_WIDTH);

        q20 = BBE_EXTRNX16C(cf, 4);
        q21 = BBE_EXTRNX16C(cf, 5);
        q22 = BBE_EXTRNX16C(cf, 6);
        q23 = BBE_EXTRNX16C(cf, 7);
...
const tFirFxdxns firdec_4d_32_8n ={&firdec_alloc_gen,firdec_proc_D4_M32};
