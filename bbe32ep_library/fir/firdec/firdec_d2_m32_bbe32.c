...
/* processing function D==2, M==32 */
static void firdec_proc_D2_M32( int16_t * restrict y,
                         ...
                         int M, int N, int D )
{
    ...
    uint32_t   q04, q05, q06, q07;
    uint32_t   q10, q11, q12, q13;
    uint32_t   q14, q15, q16, q17;
    ...
    // 1 bank
    d2 = BBE_LVNX16_I((const xb_vecNx16*)delayLine, 2 * 2 * BBE_SIMD_WIDTH);
    d3 = BBE_LVNX16_I((const xb_vecNx16*)delayLine, 3 * 2 * BBE_SIMD_WIDTH);


    //
    // Process data.
    //
    __Pragma("ymemory(coef)");
    __Pragma("ymemory(X)");
    for (n = 0; n<N / (BBE_SIMD_WIDTH / 2); n++)
    {
        // Load 8x2 input samples, CQ15
        BBE_LVNX16_IP(d4, X, 2 * BBE_SIMD_WIDTH);
        BBE_LVNX16_IP(d5, X, 2 * BBE_SIMD_WIDTH);

        // Transposition 8x2 -> 2x8 (2 banks, each of 8 samples).
        BBE_DSELNX16I(x1, x0, d5, d4, BBE_DSELI_DEINTERLEAVE_2);
        ...
        p06 = BBE_SELNX16I(d1, d0, BBE_SELI_ROTATE_LEFT_2);
        p07 = d1;
        BBE_SELPCNX16I(p09, p08, x0, d1, 1);
        BBE_SELPCNX16I(p0b, p0a, x0, d1, 3);
        BBE_SELPCNX16I(p0d, p0c, x0, d1, 5);
        p0e = BBE_SELNX16I(x0, d1, BBE_SELI_ROTATE_LEFT_2);
        p0f = x0;
        d0 = d1; d1 = x0;
        ...
        cf = BBE_LVNX16_I((const xb_vecNx16*)coef, 2 * BBE_SIMD_WIDTH);

        q00 = BBE_EXTRNX16C(cf, 0);
        q01 = BBE_EXTRNX16C(cf, 1);
        q02 = BBE_EXTRNX16C(cf, 2);
        q03 = BBE_EXTRNX16C(cf, 3);
        q04 = BBE_EXTRNX16C(cf, 4);
        q05 = BBE_EXTRNX16C(cf, 5);
        q06 = BBE_EXTRNX16C(cf, 6);
        q07 = BBE_EXTRNX16C(cf, 7);
        ...
        BBE_MULANX16PR(w0, p0d, p0c, q06);
        BBE_MULANX16PR(w0, p0f, p0e, q07);

        BBE_SELPCNX16I(p11, p10, d3, d2, 0);
        BBE_SELPCNX16I(p13, p12, d3, d2, 2);
        BBE_SELPCNX16I(p15, p14, d3, d2, 4);
        BBE_SELPCNX16I(p17, p16, d3, d2, 6);
        BBE_SELPCNX16I(p19, p18, x1, d3, 0);
        BBE_SELPCNX16I(p1b, p1a, x1, d3, 2);
        BBE_SELPCNX16I(p1d, p1c, x1, d3, 4);
        BBE_SELPCNX16I(p1f, p1e, x1, d3, 6);
        d2 = d3; d3 = x1;
        ...
        q14 = BBE_EXTRNX16C(cf, 4);
        q15 = BBE_EXTRNX16C(cf, 5);
        q16 = BBE_EXTRNX16C(cf, 6);
        q17 = BBE_EXTRNX16C(cf, 7);

        BBE_MULANX16PR(w0, p1f, p1e, q17);
        BBE_MULANX16PR(w0, p1d, p1c, q16);
    ...
    BBE_SVNX16_I(d2, (xb_vecNx16*)delayLine, 2 * 2 * BBE_SIMD_WIDTH);
    BBE_SVNX16_I(d3, (xb_vecNx16*)delayLine, 3 * 2 * BBE_SIMD_WIDTH);
}
const tFirFxdxns firdec_2d_32_8n ={&firdec_alloc_gen,firdec_proc_D2_M32};
