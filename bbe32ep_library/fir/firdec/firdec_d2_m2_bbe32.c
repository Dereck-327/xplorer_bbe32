...
/* processing function D==2, M==2 */

#if !(HAVE_DIPACK && HAVE_MULPC && 1)
DISCARD_FUN(void, firdec_proc_D2_M2,(  int16_t * restrict y,
                         const int16_t * restrict x,
                         const int16_t * restrict coef,
                               int16_t * restrict delayLine,
                         int M, int N, int D ))
#else
void firdec_proc_D2_M2(  int16_t * restrict y,
                         const int16_t * restrict x,
                         const int16_t * restrict coef,
                               int16_t * restrict delayLine,
                         int M, int N, int D )
{
  ...
  {
    // Load 16x2 input samples, CQ15
      BBE_LVNX16_IP(x1, X, 2 * BBE_SIMD_WIDTH);
      BBE_LVNX16_IP(x2, X, 2 * BBE_SIMD_WIDTH);

    d0 = BBE_SELNX16I(x1,x0,BBE_SELI_ROTATE_LEFT_2);
    d1 = BBE_SELNX16I(x2,x1,BBE_SELI_ROTATE_LEFT_2);
    
    w0 = BBE_MULNX16PC_0( d0, cf );
    BBE_MULNX16PC_1( w0, d1, cf );
...
const tFirFxdxns firdec_2d_2_8n  ={&firdec_alloc_d2_m2,firdec_proc_D2_M2 };
