...
/* processing function D==4, M==4 */
static void firdec_proc_D4_M4(  int16_t * restrict y,
                         ...
                         int M, int N, int D )
{
  ...
  c0 = ( ( (uint32_t)coef[1] << 16 ) | (uint16_t)coef[0] );
  c1 = ( ( (uint32_t)coef[3] << 16 ) | (uint16_t)coef[2] );
  ...
  for ( n=0; n<N/(BBE_SIMD_WIDTH/2); n++ )
  {
    ...
    BBE_DSELNX16I( d1, d0, x1, x0, BBE_DSELI_DEINTERLEAVE_2 );
    BBE_DSELNX16I( d3, d2, x3, x2, BBE_DSELI_DEINTERLEAVE_2 );
    BBE_DSELNX16I( d2, d0, d2, d0, BBE_DSELI_DEINTERLEAVE_2 );
    BBE_DSELNX16I( d3, d1, d3, d1, BBE_DSELI_DEINTERLEAVE_2 );

    w0 = BBE_MULNX16PR( d1, d0, c0 );
    y0 = BBE_PACKQNX40( w0 );
    BBE_MULANX16PR( w0, d3, d2, c1 );

    // CQ15 <- CQ30 - 15 w/ rounding and saturation.
    y0 = BBE_PACKQNX40( w0 );
    BBE_SVNX16_IP( y0, Y, 2 * BBE_SIMD_WIDTH );

    BBE_LAVNX16_XP( x0, X_va, X, 2*BBE_SIMD_WIDTH );
  }
...
const tFirFxdxns firdec_4d_4_8n  ={&firdec_alloc_gen,firdec_proc_D4_M4 };
