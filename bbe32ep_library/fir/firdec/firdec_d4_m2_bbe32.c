...
/* processing function D==4, M==2 */

#if !(HAVE_DIPACK && HAVE_MULPC && 1)
DISCARD_FUN(void, firdec_proc_D4_M2,(  int16_t * restrict y,
                         const int16_t * restrict x,
                         const int16_t * restrict coef,
                               int16_t * restrict delayLine,
                         int M, int N, int D ))
#else
void firdec_proc_D4_M2(  int16_t * restrict y,
                         const int16_t * restrict x,
                         const int16_t * restrict coef,
                               int16_t * restrict delayLine,
                         int M, int N, int D )
{
  ...
  for ( n=0; n<N/(BBE_SIMD_WIDTH/2); n++ )
  {
    ...
    BBE_LAVNX16_XP( d3, X_va, X, 2*BBE_SIMD_WIDTH );

    x0 = BBE_SELNX16I(d1, d0, BBE_SELI_EXTRACT_4_OF_8_OFF_0);
    x1 = BBE_SELNX16I(d3, d2, BBE_SELI_EXTRACT_4_OF_8_OFF_0);

    w0 = BBE_MULNX16PC_0( x0, h0 );
    BBE_MULNX16PC_1( w0, x1, h0 );
...
#endif
const tFirFxdxns firdec_4d_2_8n  ={&firdec_alloc_d2_m2,firdec_proc_D4_M2 };
