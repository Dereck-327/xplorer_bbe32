...
-------------------------------------------------------------------------*/
#if !HAVE_VFPU
DISCARD_FUN (void,fir_xcorrf_circ,(complex_float * restrict r,
               ...
               int N, int M)
{
    ...
    for (n = 0; n < N; n += BBE_SIMD_WIDTH / 4)
    {
        ...
        for (m = 0; m < M; m += BBE_SIMD_WIDTH / 4)
        {
            ...
            x2 = BBE_SELN_2XF32I(x_cur, x_pre, BBE_SELI_ROTATE_RIGHT_8);
            x3 = BBE_SELN_2XF32I(x_cur, x_pre, BBE_SELI_ROTATE_RIGHT_12);
            x_pre = x_cur;
