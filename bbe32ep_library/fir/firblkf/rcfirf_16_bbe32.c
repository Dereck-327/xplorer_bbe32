...
-------------------------------------------------------------------------*/
#if !HAVE_VFPU
DISCARD_FUN(void,rcfirf_process_16,(complex_float * restrict y, const complex_float * restrict x, const float32_t * restrict h, float32_t * restrict delay, int M, int N))
#else
void rcfirf_process_16(complex_float * restrict y, const complex_float * restrict x, const float32_t * restrict h, float32_t * restrict delay, int M, int N)
{
	int n;
    ...
    for (n = 0; n < (N / (BBE_SIMD_WIDTH / 4)); n++)
    {
        ...
        x3 = d3;
        x4 = BBE_SELN_2XF32I(d4, d3, BBE_SELI_ROTATE_RIGHT_4);
        x5 = BBE_SELN_2XF32I(d4, d3, BBE_SELI_ROTATE_RIGHT_8);
        x6 = BBE_SELN_2XF32I(d4, d3, BBE_SELI_ROTATE_RIGHT_12);
        x7 = d4;
