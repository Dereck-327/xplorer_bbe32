...
-------------------------------------------------------------------------*/
#if !HAVE_VFPU
DISCARD_FUN(void,cxfirf_process_8,(complex_float * restrict y, const complex_float * restrict x, const float32_t * restrict coef, float32_t * restrict delayLine, int M, int N))
#else
void cxfirf_process_8(complex_float * restrict y, const complex_float * restrict x, const float32_t * restrict coef, float32_t * restrict delayLine, int M, int N)
{
    int n;
    ...
    for (n = 0; n < N / (BBE_SIMD_WIDTH / 4); n++)
    {
        ...
        BBE_MULMASN_2XF32(A0, x4, h0, 0, 4);
        BBE_MULMASN_2XF32(A1, x4, h0, 1, 11);
        BBE_MULMASN_2XF32(A2, x5, h1, 0, 4);
        BBE_MULMASN_2XF32(A3, x5, h1, 1, 11);
        BBE_MULMASN_2XF32(A0, x6, h2, 0, 4);
        BBE_MULMASN_2XF32(A1, x6, h2, 1, 11);
        BBE_MULMASN_2XF32(A2, x7, h3, 0, 4);
        BBE_MULMASN_2XF32(A3, x7, h3, 1, 11);
