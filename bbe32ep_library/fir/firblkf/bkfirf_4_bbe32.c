...
-------------------------------------------------------------------------*/
#if !HAVE_VFPU
DISCARD_FUN (void,bkfirf_process_4,(float32_t* restrict delay, float32_t* restrict y, const float32_t * restrict x, const float32_t* restrict h, int M, int N))
#else
void bkfirf_process_4(float32_t* restrict delay, float32_t* restrict y, const float32_t * restrict x, const float32_t* restrict h, int M, int N)
{
    ...
    for (n = 0; n < (N / (BBE_SIMD_WIDTH / 2)); n++)
    {
        ...
        x0 = BBE_SELN_2XF32I(d1, d0, BBE_SELI_ROTATE_RIGHT_10);
        x1 = BBE_SELN_2XF32I(d1, d0, BBE_SELI_ROTATE_RIGHT_12);
        x2 = BBE_SELN_2XF32I(d1, d0, BBE_SELI_ROTATE_RIGHT_14);
        x3 = d1;
