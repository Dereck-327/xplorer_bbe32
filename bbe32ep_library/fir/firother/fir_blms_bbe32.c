                     ...
                     int32_t norm, int16_t mu, int N, int M)
{
    ...
    pHw = (xb_vecNx16*)(pScr);
    pHr = (const xb_vecNx16*)(h + (8 * M - 2 * BBE_SIMD_WIDTH) / sizeof(complex_fract32));
    for (m = 0; m < M / 8; m++)
    {
        xb_vecNx40 A;
        xb_vecNx16 H, H_;
        ...
        // compute error
    {
        ...
        for (n = 0; n<N / (BBE_SIMD_WIDTH / 2); n++)
        {
            ...
            pX  = (const xb_vecNx16*)XT_ADDX4(-M, (uintptr_t)pX);
            pHr = (const xb_vecNx16*)XT_ADDX4(-M, (uintptr_t)pHr);   // get back on the beginning
        }
    }
