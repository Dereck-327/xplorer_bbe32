...
#if !HAVE_DSPR
DISCARD_FUN(void, despread4, (complex_fract16 * restrict y,
...
#define DESPREAD4_ITERN( qpsk_type )                         \
{                                                            \
    xb_vecNx16 p0;                                           \
    ...
    BBE_LVNX16_IP( x15, X, +4*BBE_SIMD_WIDTH/2 );            \
                                                             \
    w0  = BBE_DSPR1DNX16CSF4( x0 , p0, 0 , qpsk_type );      \
    w1  = BBE_DSPR1DNX16CSF4( x1 , p0, 1 , qpsk_type );      \
    w2  = BBE_DSPR1DNX16CSF4( x2 , p0, 2 , qpsk_type );      \
    w3  = BBE_DSPR1DNX16CSF4( x3 , p0, 3 , qpsk_type );      \
    w4  = BBE_DSPR1DNX16CSF4( x4 , p0, 4 , qpsk_type );      \
    w5  = BBE_DSPR1DNX16CSF4( x5 , p0, 5 , qpsk_type );      \
    w6  = BBE_DSPR1DNX16CSF4( x6 , p0, 6 , qpsk_type );      \
    w7  = BBE_DSPR1DNX16CSF4( x7 , p0, 7 , qpsk_type );      \
                                                             \
    w8  = BBE_DSPR1DNX16CSF4( x8 , p0, 8 , qpsk_type );      \
    w9  = BBE_DSPR1DNX16CSF4( x9 , p0, 9 , qpsk_type );      \
    w10 = BBE_DSPR1DNX16CSF4( x10, p0, 10, qpsk_type );      \
    w11 = BBE_DSPR1DNX16CSF4( x11, p0, 11, qpsk_type );      \
    w12 = BBE_DSPR1DNX16CSF4( x12, p0, 12, qpsk_type );      \
    w13 = BBE_DSPR1DNX16CSF4( x13, p0, 13, qpsk_type );      \
    w14 = BBE_DSPR1DNX16CSF4( x14, p0, 14, qpsk_type );      \
    w15 = BBE_DSPR1DNX16CSF4( x15, p0, 15, qpsk_type );      \
                                                             \
    ...
    y4 = BBE_SELNX16( y9 , y8 , sel0 );                      \
    y5 = BBE_SELNX16( y11, y10, sel0 );                      \
    y6 = BBE_SELNX16( y13, y12, sel0 );                      \
    y7 = BBE_SELNX16( y15, y14, sel0 );                      \
                                                             \
    y0 = BBE_SELNX16I( y1, y0, BBE_SELI_EXTRACT_LO_HALVES ); \
    y1 = BBE_SELNX16I( y3, y2, BBE_SELI_EXTRACT_LO_HALVES ); \
    y2 = BBE_SELNX16I( y5, y4, BBE_SELI_EXTRACT_LO_HALVES ); \
    y3 = BBE_SELNX16I( y7, y6, BBE_SELI_EXTRACT_LO_HALVES ); \
                                                             \
    BBE_SVNX16_IP( y0, Y, +4*BBE_SIMD_WIDTH/2 );             \
    BBE_SVNX16_IP( y1, Y, +4*BBE_SIMD_WIDTH/2 );             \
    BBE_SVNX16_IP( y2, Y, +4*BBE_SIMD_WIDTH/2 );             \
    BBE_SVNX16_IP( y3, Y, +4*BBE_SIMD_WIDTH/2 );             \
}

#define DESPREAD4_ITER1( qpsk_type )                         \
{                                                            \
  xb_vecNx16 p0, x0, y0;                                     \
  xb_vecNx40 w0;                                             \
                                                             \
  ...
  y0 = BBE_PACKVNX40( w0, vsa0 );                            \
                                                             \
  BBE_SAVNX16_XP( y0, Y_va, Y, +4*(BBE_SIMD_WIDTH/2)/4 );    \
}
