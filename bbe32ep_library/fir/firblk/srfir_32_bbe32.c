...
DISCARD_FUN(void, srfir_process_32, (void* _srfir, int16_t* restrict delay, int16_t* restrict y, const int16_t * restrict x, const int16_t* restrict h, int M, int N))
#else
...
void srfir_process_32 (void* _srfir, int16_t* restrict delay, int16_t* restrict y, const int16_t * restrict x, const int16_t* restrict h, int M, int N)
{
    ...
    h89 = BBE_EXTRNX16C(t0, 4);
    hab = BBE_EXTRNX16C(t0, 5);
    hcd = BBE_EXTRNX16C(t0, 6);
    hef = BBE_EXTRNX16C(t0, 7);
    ...
    d = BBE_LVNX16_I(D, 1 * 2 * BBE_SIMD_WIDTH);
    BBE_LVNX16_IP(e, X, 2 * BBE_SIMD_WIDTH);
    // begin filtering
    a = BBE_SELNX16I(d, c, BBE_SELI_ROTATE_RIGHT_1);
    b = BBE_SELNX16I(e, d, BBE_SELI_ROTATE_RIGHT_1);
    ...
    for (n = 0; n<N / BBE_SIMD_WIDTH; n++)
    {
        ...
        BBE_ADDPNX16RRU(x1, x0); BBE_MULANX16PR(A, x0, x1, h67);
        BBE_ADDPNX16RRU(x1, x0); BBE_MULANX16PR(A, x0, x1, h89);
        BBE_ADDPNX16RRU(x1, x0); BBE_MULANX16PR(A, x0, x1, hab);
        BBE_ADDPNX16RRU(x1, x0); BBE_MULANX16PR(A, x0, x1, hcd);
        BBE_ADDPNX16RRUMBC(x1, x0, b, d); BBE_MULANX16PR(A, x0, x1, hef);
    ...
    BBE_SVNX16_I(c, D, 0);
    BBE_SVNX16_I(d, D, 2 * BBE_SIMD_WIDTH);
}
#endif
