...
void cxfir_process_4(complex_fract16 * restrict y, const complex_fract16 * restrict x, const int16_t * restrict coef, int16_t * restrict delayLine, int M, int N)
{
  ...
  BBE_LPNX16_IP(c0, coef, 4);
  BBE_LPNX16_IP(c1, coef, 4);
  BBE_LPNX16_IP(c2, coef, 4);
  BBE_LPNX16_IP(c3, coef, 4);

  c0 = BBE_REPNX16C(c0, 0);
  c1 = BBE_REPNX16C(c1, 0);
  c2 = BBE_REPNX16C(c2, 0);
  c3 = BBE_REPNX16C(c3, 0);
  ...
  for ( n=0; n<N/(BBE_SIMD_WIDTH/2); n++ )
  {
    ...
    p0 = BBE_SELNX16I( d1, d0, BBE_SELI_ROTATE_LEFT_6 );    
    p1 = BBE_SELNX16I( d1, d0, BBE_SELI_ROTATE_LEFT_4 );    
    p2 = BBE_SELNX16I( d1, d0, BBE_SELI_ROTATE_LEFT_2 );    
    p3 = d1;
    ...
    BBE_MULANX16C(w0, p1, c1);
    BBE_MULANX16C(w0, p2, c2);
    BBE_MULANX16C(w0, p3, c3);
  ...
  BBE_SVNX16_I( d0, (xb_vecNx16*)delayLine, 0*4*BBE_SIMD_WIDTH/2 );
