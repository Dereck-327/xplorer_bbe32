...
DISCARD_FUN(void, srcfir_process_16m,(void* _srcfir,
...
//--------------------------------------------------------
// processing function for M%16==0
//--------------------------------------------------------
void srcfir_process_16m(void* _srcfir,
                        int16_t * restrict delay, complex_fract16 * restrict y, 
                        const complex_fract16 * restrict x, const int16_t * restrict h,
                        int M, int N)
{
    ...
    srcfir_t *srcfir = (srcfir_t *)_srcfir;
    ...
    NASSERT_ALIGN32(h);
    NASSERT_ALIGN32(srcfir->p0);
    NASSERT_ALIGN32(srcfir->p1);

    shft = BBE_MOVVSA32(14);
    pX = (const xb_vecNx16*)x;
    Y = (xb_vecNx16*)y;
    W0 = (xb_vecNx16 *)srcfir->p0;
    W1 = (xb_vecNx16 *)srcfir->p1;
    // Load the delay line state.
    // move delay line and put new samples
    WUR_CBEGIN((uintptr_t)delay);
    WUR_CEND((uintptr_t)(delay + 2 * 2 * (M + BBE_SIMD_WIDTH / 2) * 2));
...

    {
        ...
        p0[2 * (2 * M + sh) + 0] = x[0].s.re;
        p0[2 * (2 * M + sh) + 1] = x[0].s.im;

        ix = p1 - delay;
        if (ix >= (4 * M + 2*BBE_SIMD_WIDTH * 2)) p1 -= (4 * M + 2*BBE_SIMD_WIDTH * 2);
    ...
    for (n = 0; n < N / (BBE_SIMD_WIDTH / 2); n++)
    {
        ...
        BBE_MOVSAV(d0);
        BBE_MOVSDV(p0);
        BBE_ADDPNX16RCUMBCIAD(x0, x1, d2, d4);
        A = BBE_MOVWA32(1L << 13);
        ...
        for (m = 0; m < M / (BBE_SIMD_WIDTH); m++)
        {
            ...
            BBE_ADDPNX16RCU(x1, x0); BBE_MULANX16PR(A, x0, x1, h01);
            BBE_LVNX16_IP(d0, R0, 2 * BBE_SIMD_WIDTH);
            BBE_LVNX16_IP(d1, R1, -2 * BBE_SIMD_WIDTH);
            BBE_ADDPNX16RCUMBC(x1, x0, d0, d1); BBE_MULANX16PR(A, x0, x1, h23);

            BBE_ADDPNX16RCU(x1, x0); BBE_MULANX16PR(A, x0, x1, h45);
            BBE_LVNX16_IP(d0, R0, 2 * BBE_SIMD_WIDTH);
            BBE_LVNX16_IP(d1, R1, -2 * BBE_SIMD_WIDTH);
            BBE_ADDPNX16RCUMBC(x1, x0, d0, d1); BBE_MULANX16PR(A, x0, x1, h67);
        }
    ...
    srcfir->p0 = (int16_t *)W0;
    srcfir->p1 = (int16_t *)W1;
}
#endif
