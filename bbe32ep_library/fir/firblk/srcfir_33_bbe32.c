...
DISCARD_FUN(void, srcfir_process_33,(void* _srcfir,
                       int16_t * restrict delay, complex_fract16 * restrict y, 
                       const complex_fract16 * restrict x, const int16_t * restrict h,
                       int M, int N))
#else
//--------------------------------------------------------
// processing function for M==33
//--------------------------------------------------------
void srcfir_process_33(void* _srcfir,
                       int16_t * restrict delay, complex_fract16 * restrict y, 
                       const complex_fract16 * restrict x, const int16_t * restrict h,
                       int M, int N)
{
    ...
    for (n = 0; n<N / (BBE_SIMD_WIDTH / 2); n++)
    {
        ...
        BBE_ADDPNX16RCUMBC(p07, p06, d2, d2);
        ...
        BBE_ADDPNX16RCUMBCIAD(p17, p16, d1, d3);

        BBE_MOVSAV(d0);
        BBE_MOVSDV(d4);

        A = BBE_MULRNX16(d1, hcv, vsa0); // Central tap
        BBE_MULANX16PR(A, p00, p01, h01);
