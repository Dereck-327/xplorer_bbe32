...
DISCARD_FUN(void, srcfir_process_17,(void* _srcfir,
                       int16_t * restrict delay, complex_fract16 * restrict y, 
                       const complex_fract16 * restrict x, const int16_t * restrict h,
                       int M, int N))
#else
//--------------------------------------------------------
// processing function for M==17
//--------------------------------------------------------
void srcfir_process_17(void* _srcfir,
                       int16_t * restrict delay, complex_fract16 * restrict y, 
                       const complex_fract16 * restrict x, const int16_t * restrict h,
                       int M, int N)
{
    ...
    for (n = 0; n<N / (BBE_SIMD_WIDTH / 2); n++)
    {
        ...
        BBE_ADDPNX16RCUMBC(x1, x0, d2, d2); BBE_MULANX16PR(A, x0, x1, h67);
    ...
    BBE_SVNX16_I(d1, D, 0);
    BBE_SVNX16_I(d2, D, 2 * BBE_SIMD_WIDTH);
}
#endif
