...
DISCARD_FUN(void, srcfir_process_32,(void* _srcfir,
...
//--------------------------------------------------------
void srcfir_process_32(void* _srcfir,
                       int16_t * restrict delay, complex_fract16 * restrict y, 
                       const complex_fract16 * restrict x, const int16_t * restrict h,
                       int M, int N)
{
    ...
    h89 = BBE_EXTRNX16C(d0, 4);
    hab = BBE_EXTRNX16C(d0, 5);
    hcd = BBE_EXTRNX16C(d0, 6);
    hef = BBE_EXTRNX16C(d0, 7);
    ...
    d2 = BBE_LVNX16_I(D, 2 * 2 * BBE_SIMD_WIDTH);
    d3 = BBE_LVNX16_I(D, 3 * 2 * BBE_SIMD_WIDTH);

    BBE_LVNX16_IP(d4, X0, 2 * BBE_SIMD_WIDTH);

    // begin filtering
    t0 = BBE_SELNX16I(d1, d0, BBE_SELI_ROTATE_RIGHT_2);
    t1 = BBE_SELNX16I(d2, d1, BBE_SELI_ROTATE_RIGHT_2);
    t2 = BBE_SELNX16I(d3, d2, BBE_SELI_ROTATE_RIGHT_2);
    t3 = BBE_SELNX16I(d4, d3, BBE_SELI_ROTATE_RIGHT_2);
    ...
    {
        BBE_ADDPNX16RCU(p01, p00);
        BBE_ADDPNX16RCU(p03, p02);
        BBE_ADDPNX16RCU(p05, p04);
        BBE_ADDPNX16RCUMBC(p07, p06, t2, d2);

        d0 = d1;
        d1 = d2;
        d2 = d3;
        d3 = BBE_LVNX16_I(X0, -2 * BBE_SIMD_WIDTH);
        BBE_LVNX16_IP(d4, X0, 2 * BBE_SIMD_WIDTH);

        BBE_ADDPNX16RCU(p11, p10);
        BBE_ADDPNX16RCU(p13, p12);
        BBE_ADDPNX16RCU(p15, p14);
        BBE_ADDPNX16RCUMBCIAD(p17, p16, t2, d3);
        BBE_MOVSAV(t1);
        BBE_MOVSDV(d4);

        t1 = t2;
        t2 = t3;
        BBE_LANX16_IP(t3, X1_va, X1);

        A = BBE_MULNX16PR(p00, p01, h01);
        BBE_MULANX16PR(A, p02, p03, h23);
        BBE_MULANX16PR(A, p04, p05, h45);
        BBE_MULANX16PR(A, p06, p07, h67);

        BBE_MULANX16PR(A, p10, p11, h89);
        BBE_MULANX16PR(A, p12, p13, hab);
        BBE_MULANX16PR(A, p14, p15, hcd);
        BBE_MULANX16PR(A, p16, p17, hef);

        x0 = BBE_PACKVNX40(A, vsa0);
        BBE_SVNX16_IP(x0, Y, 2 * BBE_SIMD_WIDTH);
    }

    // save last 4 samples in the delay line
    BBE_SVNX16_I(d0, D, 0 * 2 * BBE_SIMD_WIDTH);
    BBE_SVNX16_I(d1, D, 1 * 2 * BBE_SIMD_WIDTH);
    BBE_SVNX16_I(d2, D, 2 * 2 * BBE_SIMD_WIDTH);
    BBE_SVNX16_I(d3, D, 3 * 2 * BBE_SIMD_WIDTH);
}
#endif
