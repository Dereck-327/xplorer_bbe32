...
-------------------------------------------------------------------------*/
#if !HAVE_VFPU
DISCARD_FUN (void,firdecf_proc_D4_MX,( complex_float * restrict y,
                   ...
                   int M, int N, int D )
{
    ...
    for (n = 0; n < N / (BBE_SIMD_WIDTH / 4); n++)
    {
        ...
        BBE_SVN_2XF32_X(s3, pD, 4 * 2 * M + 3 * 2 * BBE_SIMD_WIDTH);
        ...
        for (m = 0; m < (M / BBE_SIMD_WIDTH); m++)
        {
            ...
            BBE_LVN_2XF32_IP(s2, pD, 2 * BBE_SIMD_WIDTH);
            BBE_LVN_2XF32_XP(s3, pD, -7 * 2 * BBE_SIMD_WIDTH);
...
#endif
const tFirFxdxns firdecf_4d_xm = { &firdecf_alloc_d2_4_mx, firdecf_proc_D4_MX };
