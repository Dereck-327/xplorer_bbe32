...
-------------------------------------------------------------------------*/
#if !HAVE_VFPU
DISCARD_FUN (void,firinterpf_proc_D4_M2,( complex_float * restrict y,
                             const complex_float * restrict x,
                             const float32_t     * restrict coef,
                                   float32_t     * restrict delayLine,
                             int M, int N, int D ))
#else
void firinterpf_proc_D4_M2( complex_float * restrict y,
                             const complex_float * restrict x,
                             const float32_t     * restrict coef,
                                   float32_t     * restrict delayLine,
                             int M, int N, int D )
{
    ...
    for (n = 0; n < N / (BBE_SIMD_WIDTH / 4); n++)
    {
        ...
        y1 = BBE_MULN_2XF32(x0, h0);
        y2 = BBE_MULN_2XF32(x1, h0);
        y3 = BBE_MULN_2XF32(x2, h0);
...
}// firinterpf_proc_D4_M2
#endif

const tFirFxdxns interpf_4d_2m = { &firinterpf_dx, firinterpf_proc_D4_M2 };
