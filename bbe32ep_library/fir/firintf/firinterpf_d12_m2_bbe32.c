...
-------------------------------------------------------------------------*/
#if !HAVE_VFPU
DISCARD_FUN (void,firinterpf_proc_D12_M2,( complex_float * restrict y,
                             const complex_float * restrict x,
                             const float32_t     * restrict coef,
                                   float32_t     * restrict delayLine,
                             int M, int N, int D ))
#else
void firinterpf_proc_D12_M2( complex_float * restrict y,
                              const complex_float * restrict x,
                              const float32_t     * restrict coef,
                                    float32_t     * restrict delayLine,
                              int M, int N, int D )
{
    ...
    for (n = 0; n < N / (BBE_SIMD_WIDTH / 4); n++)
    {
        ...
        h2 = BBE_SHFLN_2XF32I(c1, BBE_SHFLI_DOUBLE_2_LO);
        h3 = BBE_SHFLN_2XF32I(c1, BBE_SHFLI_DOUBLE_2_HI);
        h4 = BBE_SHFLN_2XF32I(c2, BBE_SHFLI_DOUBLE_2_LO);
        ...
        y1 = BBE_MULN_2XF32(d0, h1);
        y2 = BBE_MULN_2XF32(d0, h2);
        y3 = BBE_MULN_2XF32(x0, h0);
        y4 = BBE_MULN_2XF32(x0, h1);
        y5 = BBE_MULN_2XF32(x0, h2);
        y6 = BBE_MULN_2XF32(x1, h0);
        y7 = BBE_MULN_2XF32(x1, h1);
        y8 = BBE_MULN_2XF32(x1, h2);
        y9 = BBE_MULN_2XF32(x2, h0);
        ya = BBE_MULN_2XF32(x2, h1);
        yb = BBE_MULN_2XF32(x2, h2);
        BBE_MULAN_2XF32(y0, x0, h3);
        BBE_MULAN_2XF32(y1, x0, h4);
        BBE_MULAN_2XF32(y2, x0, h5);
        BBE_MULAN_2XF32(y3, x1, h3);
        BBE_MULAN_2XF32(y4, x1, h4);
        BBE_MULAN_2XF32(y5, x1, h5);
        BBE_MULAN_2XF32(y6, x2, h3);
        BBE_MULAN_2XF32(y7, x2, h4);
        BBE_MULAN_2XF32(y8, x2, h5);
        BBE_MULAN_2XF32(y9, x3, h3);
        BBE_MULAN_2XF32(ya, x3, h4);
        BBE_MULAN_2XF32(yb, x3, h5);
        ...
        BBE_SVN_2XF32_IP(y6, pY, 2 * BBE_SIMD_WIDTH);
        BBE_SVN_2XF32_IP(y7, pY, 2 * BBE_SIMD_WIDTH);
        BBE_SVN_2XF32_IP(y8, pY, 2 * BBE_SIMD_WIDTH);
        BBE_SVN_2XF32_IP(y9, pY, 2 * BBE_SIMD_WIDTH);
        BBE_SVN_2XF32_IP(ya, pY, 2 * BBE_SIMD_WIDTH);
        BBE_SVN_2XF32_IP(yb, pY, 2 * BBE_SIMD_WIDTH);
        d0 = x3;
    }
    BBE_SVN_2XF32_I(d0, pD, 0);
}// firinterpf_proc_D12_M2
#endif

const tFirFxdxns interpf_12d_2m = { &firinterpf_dx, firinterpf_proc_D12_M2 };
