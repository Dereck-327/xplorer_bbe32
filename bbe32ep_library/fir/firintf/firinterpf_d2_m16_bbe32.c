...
-------------------------------------------------------------------------*/
#if !HAVE_VFPU
DISCARD_FUN (void,firinterpf_proc_D2_M16,( complex_float * restrict y,
                             const complex_float * restrict x,
                             const float32_t     * restrict coef,
                                   float32_t     * restrict delayLine,
                             int M, int N, int D ))
#else
void firinterpf_proc_D2_M16( complex_float * restrict y,
                              const complex_float * restrict x,
                              const float32_t     * restrict coef,
                                    float32_t     * restrict delayLine,
                              int M, int N, int D )
{
    ...
    BBE_LVN_2XF32_XP(d3, pD, -3 * 2 * BBE_SIMD_WIDTH);
    ...
    for (n = 0; n < N / (BBE_SIMD_WIDTH / 4); n++)
    {
        ...
        y1 = BBE_MULN_2XF32(x0, h1);
        BBE_MULAN_2XF32(y0, x1, h2);
        BBE_MULAN_2XF32(y1, x1, h3);
        BBE_MULAN_2XF32(y0, x2, h4);
        BBE_MULAN_2XF32(y1, x2, h5);
        BBE_MULAN_2XF32(y0, x3, h6);
        ...
        BBE_DSELN_2XF32I(y1, y0, y1, y0, BBE_DSELI_INTERLEAVE_4);
    ...
    BBE_SVN_2XF32_XP(d3, pD, -3 * 2 * BBE_SIMD_WIDTH);
}// firinterpf_proc_D2_M16
#endif

const tFirFxdxns interpf_2d_16m = { &firinterpf_dx, firinterpf_proc_D2_M16 };
