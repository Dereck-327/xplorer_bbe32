...
-------------------------------------------------------------------------*/
#if !HAVE_VFPU
DISCARD_FUN (void,firinterpf_proc_D4_M16,( complex_float * restrict y,
                             const complex_float * restrict x,
                             const float32_t     * restrict coef,
                                   float32_t     * restrict delayLine,
                             int M, int N, int D ))
#else
void firinterpf_proc_D4_M16( complex_float * restrict y,
                              const complex_float * restrict x,
                              const float32_t     * restrict coef,
                                    float32_t     * restrict delayLine,
                              int M, int N, int D )
{
    ...
    for (n = 0; n < N / (BBE_SIMD_WIDTH / 4); n++)
    {
        ...
        BBE_LVN_2XF32_IP(xx, pD, -2 * BBE_SIMD_WIDTH);
        ...
        BBE_LVN_2XF32_XP(c0, pH, 2 * BBE_SIMD_WIDTH);
        h2 = BBE_SHFLN_2XF32I(c0, BBE_SHFLI_DOUBLE_2_LO);
        h3 = BBE_SHFLN_2XF32I(c0, BBE_SHFLI_DOUBLE_2_HI);

        BBE_MULAN_2XF32(y0, d2, h2);
        BBE_MULAN_2XF32(y1, x0, h2);
        BBE_MULAN_2XF32(y2, x1, h2);
        BBE_MULAN_2XF32(y3, x2, h2);
        BBE_MULAN_2XF32(y0, x0, h3);
        BBE_MULAN_2XF32(y1, x1, h3);
        BBE_MULAN_2XF32(y2, x2, h3);
        BBE_MULAN_2XF32(y3, x3, h3);

        BBE_SVN_2XF32_IP(xx, pD, 2 * 2 * BBE_SIMD_WIDTH);
        d0 = x1;
        d1 = x2;
        d2 = x3;
        ...
        BBE_MULAN_2XF32(y0, d0, h0);
        BBE_MULAN_2XF32(y1, d1, h0);
        BBE_MULAN_2XF32(y2, d2, h0);
        ...
        BBE_LVN_2XF32_XP(c0, pH, -7 * 2 * BBE_SIMD_WIDTH);
        h2 = BBE_SHFLN_2XF32I(c0, BBE_SHFLI_DOUBLE_2_LO);
        h3 = BBE_SHFLN_2XF32I(c0, BBE_SHFLI_DOUBLE_2_HI);
        ...
        BBE_SVN_2XF32_XP(xx, pD, -3 * 2 * BBE_SIMD_WIDTH);
...
}// firinterpf_proc_D4_M16
#endif

const tFirFxdxns interpf_4d_16m = { &firinterpf_dx, firinterpf_proc_D4_M16 };
