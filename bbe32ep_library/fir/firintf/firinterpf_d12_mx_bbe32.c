...
-------------------------------------------------------------------------*/
#if !HAVE_VFPU
DISCARD_FUN (void,firinterpf_proc_D12_MX,( complex_float * restrict y,
                             const complex_float * restrict x,
                             const float32_t     * restrict coef,
                                   float32_t     * restrict delayLine,
                             int M, int N, int D ))
#else
void firinterpf_proc_D12_MX( complex_float * restrict y,
                              const complex_float * restrict x,
                              const float32_t     * restrict coef,
                                    float32_t     * restrict delayLine,
                              int M, int N, int D )
{
    ...
    for (d = 0; d < D; d++)
    {
        ...
        pD_= (      xb_vecN_2xf32 *)delayLine + d * ((M / (BBE_SIMD_WIDTH / 4)) + 1);
...
}// firinterpf_proc_D12_MX
#endif

const tFirFxdxns interpf_12d_xm = { &firinterpf_d_6_12_mx, firinterpf_proc_D12_MX };
