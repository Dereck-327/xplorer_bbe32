...
#if !(HAVE_VFPU)
DISCARD_FUN(void, vcabsf, ( float32_t * restrict z, const complex_float   * restrict x, int N ))
#else
...
void vcabsf ( float32_t * restrict z, const complex_float   * restrict x, int N )
{
  ...
  xb_vecN_2xf32 temp, half, t0, t1, x0, x0_red, x0_adj, r0, r0_err;
  xb_vecNx16 temp0, exp_re, exp_im, exp_abs;
  int n;
  ...
  for ( n = 0; n < (N+BBE_SIMD_WIDTH/2-1)/(BBE_SIMD_WIDTH/2); n++ )
  {
    ...
    exp_re = BBE_MOVNX16_FROMN_2XF32(Xre);
    exp_re = BBE_SRLINX16(exp_re, 7);
    /*--------------------------*/

    /* Get the exponent of x.im */
    exp_im = BBE_MOVNX16_FROMN_2XF32(Xim);
    exp_im = BBE_SRLINX16(exp_im, 7);
    /*--------------------------*/

    /* Select maximum exponent */
    exp_abs = BBE_MAXNX16(exp_re, exp_im);
    exp_abs = BBE_MINNX16(exp_abs, 256-3);
    exp_abs = BBE_MAXNX16(exp_abs, 1    );
    /* Normalize range of input data using ldexpf */
    /* Xre = ldexpf(Xre, -exp_abs) */
    /* Xim = ldexpf(Xim, -exp_abs) */
    temp0 = BBE_SUBNX16(256-2, exp_abs);
    temp0 = BBE_SLLINX16(temp0, 7);
    temp0 = BBE_SELNX16I(temp0, 0, BBE_SELI_INTERLEAVE_1_ODD);
    temp = BBE_MOVN_2XF32_FROMNX16(temp0);

    Xre = BBE_MULN_2XF32(Xre, temp);
    Xim = BBE_MULN_2XF32(Xim, temp);
    /*-----------------------------*/
    ...
    temp0 = BBE_SLLINX16(exp_abs, 7);
    temp0 = BBE_SELNX16I(temp0, 0, BBE_SELI_INTERLEAVE_1_ODD);
    temp = BBE_MOVN_2XF32_FROMNX16(temp0);
    Zout = BBE_MULN_2XF32(Zout, temp);
    /*------------------------------*/
  ...
  half = BBE_CONSTN_2XF32(3);
  temp0 = BBE_MOVVINT16(1);
  mask = BBE_EQNX16(temp0, BBE_CONJNX16C(temp0));
  ...
  for ( n = 0; n < (N+BBE_SIMD_WIDTH/2-1)/(BBE_SIMD_WIDTH/2); n++ )
  {
    ...
    temp0 = BBE_SUBNX16(256-2, exp_abs);
    temp0 = BBE_SLLINX16(temp0, 7);
    BBE_SUBNX16T(temp0, temp0, temp0, mask);/* clear bits in mantissa */
    temp = BBE_MOVN_2XF32_FROMNX16(temp0);
