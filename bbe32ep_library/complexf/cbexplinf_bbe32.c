...
#if !(HAVE_VFPU)
DISCARD_FUN(float32_t, cbexplinf, ( complex_float   * restrict z,  float32_t phase0, float32_t delta, int N ))
#else
...
float32_t cbexplinf ( complex_float   * restrict z,  float32_t phase0, float32_t delta, int N )
{
  ...
  for (n = 0; n<(N>>(LOG2_BBE_SIMD_WIDTH-1)); n++)
  {
    ...
    */
    ...
    zsn = ph; BBE_MULAN_2XF32(zsn, sn2, ph);
    ...
    */
    ...
    ph = BBE_ABSN_2XF32(ph);
    bdom = BBE_ULEQN_2XF32(ph, sinf_maxval.f);
  ...
  if (N&4)
  {
    ...
    cs0 = BBE_ABSN_2XF32(ph);
    bdom = BBE_ULEQN_2XF32(cs0, sinf_maxval.f);
    bdom = BBE_NOTBN_2(bdom);
