...
-------------------------------------------------------------------------*/
#if !HAVE_VFPU
DISCARD_FUN(void,vtanhf,( float32_t * restrict y, const float32_t * restrict x, int N ))
#else
void vtanhf ( float32_t * restrict y, const float32_t * restrict x, int N )
{
  ...
  vboolN_2 b_nan, b_snan, b_inf, b_gt;
  __fenv_t fenv;
  ...
  /* Process errors */
  {
    ...
    __fesetenv(&fenv);
    /* Raise the FE_INVALID exception. */
