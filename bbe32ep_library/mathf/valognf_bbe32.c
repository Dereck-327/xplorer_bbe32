...
-------------------------------------------------------------------------*/
#if !HAVE_VFPU
DISCARD_FUN(void,valognf,( float32_t * restrict y, const float32_t * restrict x, int N ))
#else
void valognf ( float32_t * restrict y, const float32_t * restrict x, int N )
{
  ...
  xb_vecN_2xf32 t0, t1, t2, t3, t4, t5, t6, tx,_0;
  xb_vecN_2x32v temp0, temp1;
  vboolN_2 b_edom, b_erange;
  vboolN_2 b_nan, b_max, b_snan;
  vboolN_2 b_outb, b_inv, b_inf;
  ...
  for (n = 0; n<(N >> (LOG2_BBE_SIMD_WIDTH - 1)); n++)
  {
    ...
    b_max = BBE_OLEN_2XF32(expfminmax[1].f, x0);/* x>=38.5318 or x==NaN */
    ...
    */
    ...
    dy0 = BBE_MOVN_2XF32T(_0, dy0, b_inf);
  ...
  for (n = 0; n<(N >> (LOG2_BBE_SIMD_WIDTH - 1)); n++)
  {
    ...
    b_max = BBE_OLEN_2XF32(x0, expfminmax[0].f);
    /*
    * Multiply by 1/ln(2)
    */
    c0 = log2_e[0].f;
    ...
    zout = BBE_MOVN_2XF32T(_0, zout, b_max);
    BBE_SVN_2XF32_IP(zout, pW, 2 * BBE_SIMD_WIDTH);
  }
  /* Process errors */
  {
