...
-------------------------------------------------------------------------*/
#if !HAVE_VFPU
DISCARD_FUN(void,vlognf,( float32_t * restrict y, const float32_t * restrict x, int N ))
#else
void vlognf ( float32_t * restrict y, const float32_t * restrict x, int N )
{
  int n;
  xb_vecN_2xf32 x0, y0, xabs, x2, x4;
  ...
  const xb_vecN_2xf32      * restrict ptbl = (const xb_vecN_2xf32      *)logf_tbl;
  ...
  for (n = 0; n<(N >> (LOG2_BBE_SIMD_WIDTH - 1)); n++)
  {
    ...
    xabs = BBE_SUBN_2XF32(fx, BBE_CONSTN_2XF32(1));
  ...
  for (n = 0; n<(N >> (LOG2_BBE_SIMD_WIDTH - 1)); n++)
  {
    ...
    //
    BBE_MULSN_2XF32(t1, t0, xabs);
    BBE_MULSN_2XF32(t3, t2, xabs);
    BBE_MULSN_2XF32(t5, t4, xabs);
    BBE_MULSN_2XF32(t7, t6, xabs);

    BBE_MULAN_2XF32(t3, t1, x2);
    BBE_MULAN_2XF32(t7, t5, x2);
    BBE_MULAN_2XF32(t7, t3, x4);
    //
    BBE_MULAN_2XF32(xabs, t7, x2);
  ...
  for (n = 0; n<(N >> (LOG2_BBE_SIMD_WIDTH - 1)); n++)
  {
    ...
    ft = BBE_MOVN_2XF32_FROMF32(ln2.f); ft = BBE_REPN_2XF32(ft, 0);
    BBE_MULAN_2XF32(y0, ef_reg, ft);
