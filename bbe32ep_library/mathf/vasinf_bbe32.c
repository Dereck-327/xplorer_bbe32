...
-------------------------------------------------------------------------*/
#if !HAVE_VFPU
DISCARD_FUN(void,vasinf, ( float32_t * restrict y, const float32_t * restrict x, int N ))
#else
void vasinf ( float32_t * restrict y, const float32_t * restrict x, int N )
{
  ...
  */
  ...
  const xb_vecN_2xf32  * restrict pT = (const xb_vecN_2xf32  *)asinftbl;
  ...
  for (n = 0; n<(N >> (LOG2_BBE_SIMD_WIDTH - 1)); n++)
  {
    ...
    */
...

    {
      ...
      /* Initial sqrt approximation with exponent range reduction */
