...
-------------------------------------------------------------------------*/
#if !HAVE_VFPU
DISCARD_FUN(void,vfastsinhf,( float32_t * restrict y, const float32_t * restrict x, int N ))
#else
void vfastsinhf ( float32_t * restrict y, const float32_t * restrict x, int N )
{
  ...
  for (n = 0; n<(N >> (LOG2_BBE_SIMD_WIDTH - 1)); n++)
  {
    ...
    */
    ...
    BBE_ADDEXPN_2XF32(zout,c1);
    BBE_ADDEXPN_2XF32(zout, c0);
