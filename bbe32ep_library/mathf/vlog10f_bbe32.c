...
-------------------------------------------------------------------------*/
#if !HAVE_VFPU
DISCARD_FUN(void,vlog10f,( float32_t * restrict y, const float32_t * restrict x, int N ))
#else
void vlog10f ( float32_t * restrict y, const float32_t * restrict x, int N )
{
  ...
  const xb_vecN_2xf32      * restrict ptbl = (const xb_vecN_2xf32      *)log10f_tbl;
  ...
  for (n = 0; n<(N >> (LOG2_BBE_SIMD_WIDTH - 1)); n++)
  {
    ...
    y0 = BBE_MULN_2XF32(xabs, t8);
    BBE_MULAN_2XF32(y0, t7, x2);
  ...
  for (n = 0; n<(N >> (LOG2_BBE_SIMD_WIDTH - 1)); n++)
  {
    ...
    ef_reg = BBE_FLOATN_2X32(e_reg, 2);
    y0 = BBE_ADDN_2XF32(y0, ef_reg);
    ft = BBE_MOVN_2XF32_FROMF32(_4log10_2minus1.f); ft = BBE_REPN_2XF32(ft, 0);
