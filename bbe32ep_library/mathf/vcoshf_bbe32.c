...
-------------------------------------------------------------------------*/
#if !HAVE_VFPU
DISCARD_FUN(void,vcoshf,( float32_t * restrict y, const float32_t * restrict x, int N ))
#else
void vcoshf ( float32_t * restrict y, const float32_t * restrict x, int N )
{
  int n;
  xb_vecN_2x32Uv flags, flags_n;
  xb_vecN_2xf32 x0, y0, dy0, zout, xmag;
  xb_vecN_2xf32 quarter;
  xb_vecN_2x32Uv temp0, temp1;
  vboolN_2 b_nan, b_snan, b_inf, b_outb, b_olt;
  __fenv_t fenv;
  ...
  for (n = 0; n<(N >> (LOG2_BBE_SIMD_WIDTH - 1)); n++)
  {
    ...
    */
    ...
    b_olt = BBE_OLTN_2XF32T(coshf_maxarg.f, xmag, BBE_NOTBN_2(b_inf));
    b_outb = BBE_ORBN_2(b_outb, b_olt); /* Exact result overflows */

    zout = BBE_MOVN_2XF32T(plusInff.f, zout, BBE_ORBN_2(b_inf, b_olt));
    BBE_SVN_2XF32_IP(zout, pZ, 2 * BBE_SIMD_WIDTH);
  }
  /* Process errors */
  {
