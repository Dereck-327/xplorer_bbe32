...
float32_t slog2f ( float32_t x )
{
  ...
  float32_t e1, x2, y, y1, y2, y3, y4, y5;
  ...
  x = XT_SUB_S(one, x);
  x2 = XT_MUL_S(x, x);
  y1 = log2f_tbl[1].f;   XT_MADD_S(y1, x, log2f_tbl[0].f);
  y2 = log2f_tbl[3].f;   XT_MADD_S(y2, x, log2f_tbl[2].f);
  y3 = log2f_tbl[5].f;   XT_MADD_S(y3, x, log2f_tbl[4].f);
  y4 = log2f_tbl[7].f;   XT_MADD_S(y4, x, log2f_tbl[6].f);
  y5 = log2f_tbl[9].f;   XT_MADD_S(y5, x, log2f_tbl[8].f);
...
#else
DISCARD_FUN(float32_t,slog2f,( float32_t x ))
#endif
