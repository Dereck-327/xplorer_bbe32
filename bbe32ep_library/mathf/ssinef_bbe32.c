...
{
  vbool1 b_nan, b_outl, b_cs, b_neg;
  xb_int32v sx, ji;
  ...
  sx = BBE_MOV32_FROMF32(x);
  sx = BBE_OPERATOR_AND32(sx, 0x80000000);
  x = XT_ABS_S(x);
  ...
  /* Determine the sign and sin/cos selector from the input range. */
  {
    ...
    b_cs = BBE_MOV1_FROMN(bsc);
    b_neg = BBE_MOV1_FROMN(bneg);
  }

  /* Select sine or cosine. */
  y = ys; XT_MOVT_S(y, yc, b_cs);
  /* Adjust the sign. */
  ys = XT_NEG_S(y); XT_MOVT_S(y, ys, b_neg);
  y = BBE_MOVF32_FROM32(BBE_OPERATOR_XOR32(BBE_MOV32_FROMF32(y), sx));
  BBE_MOVSCFV(SCF);
...
#else
DISCARD_FUN(float32_t,ssinef, ( float32_t x ))
#endif
