...
-------------------------------------------------------------------------*/
#if !HAVE_VFPU
DISCARD_FUN(void,vfastcosf,( float32_t * restrict y, const float32_t * restrict x, int N ))
#else
void vfastcosf ( float32_t * restrict y, const float32_t * restrict x, int N )
{
  ...
  for (n = 0; n<(N >> (LOG2_BBE_SIMD_WIDTH - 1)); n++)
  {
    ...
    */
    ...
    jf = BBE_MULN_2XF32(jf, pi2fc2);
      //  xabs = BBE_MOVN_2XF32T(xabs, xabs, b_outl);
      /*
      * Compute sine/cosine via minmax polynomial.
      */
      //cs0  = BBE_MOVN_2XF32_FROMNX16( BBE_MOVAV16(0x4000));
  ...
  if (N <= 0) return;
  /* Table of different constants used in computations */
  static const int16_t c_tbl[] =
  {
    (int16_t)0x4000,/* 1, Q14 */
  };
  /* Clear exception enable flags and exception status flags. */
