...
{
  float32_t one, two, zero;
  float32_t e1, x2, y, y1, y2, y3, y4, x4;
  ...
  if (BBE_MOVAB1(XT_ULE_S(x, zero)))
  {
    ...
    err = EDOM;
    XT_MOVNEZ(err, ERANGE, BBE_MOVAB1(bzero));
    errno = err;
 ...
 // BBE_MOVSCFV(XTENSA_SCF_SET_EXCEPTION_ENABLE(SCF, 0));  /* Clear FP flags */
  ...
  XT_MOVT_S(x, XT_MUL_S(x, two), b_small);
  XT_MOVNEZ(e, e - 1, BBE_MOVAB1(b_small));

  x = XT_SUB_S(one, x);
  x2 = XT_MUL_S(x, x);
  x4 = XT_MUL_S(x2, x2);
  y1 = log10f_tbl[1].f;   XT_MADD_S(y1, x, log10f_tbl[0].f);
  y2 = log10f_tbl[3].f;   XT_MADD_S(y2, x, log10f_tbl[2].f);
  y3 = log10f_tbl[5].f;   XT_MADD_S(y3, x, log10f_tbl[4].f);
  y4 = log10f_tbl[7].f;   XT_MADD_S(y4, x, log10f_tbl[6].f);

  XT_MADD_S(y2, y1, x2);
  XT_MADD_S(y4, y3, x2);
  XT_MADD_S(y4, y2, x4);
  y = XT_MUL_S(y4, x2);
  XT_MADD_S(y, x, log10f_tbl[8].f);
  ...
  */
  e1 = XT_FLOAT_S(e, 2);
  y = XT_ADD_S(y, e1);
  XT_MADD_S(y, e1, _4log10_2minus1.f);

  XT_MOVT_S(y, plusInff.f, b_xinf); /* set +Inf for x=+Inf */
...
#else
DISCARD_FUN(float32_t,slog10f,( float32_t x ))
#endif
