...
-------------------------------------------------------------------------*/
#if !HAVE_VFPU
DISCARD_FUN(void,vmax8f,( float32_t * restrict m, 
              ...
              float32_t * restrict x, int N )
{
  ...
  float32_t a, b;
  xb_vecN_2xf32 x0, x1;
  xb_vecN_2xf32 max0, max1;
  xb_vecN_2xf32 maxx, z1;
  vboolN_2 b0, b1;
  vboolN bx0, bx1, bxx;
  ...
  if (N <= 0)
  {
    for (k = 0; k<8; k++) { m[k] = minusInff.f; idx[k] = 0; }
    return;
  }
  id = BBE_SEQNX16();
  ...
  for (k = 0; k < 8; k += 2)
  {
    ...
    x[(int)q0] = minusInff.f;
    x[(int)q1] = minusInff.f;
  }
