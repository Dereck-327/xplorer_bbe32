...
-------------------------------------------------------------------------*/
#if !HAVE_VFPU
DISCARD_FUN(void,vminf,( float32_t * restrict m, 
       ...
       const float32_t * restrict x, int N )
{
  ...
  float32_t a, b;
  xb_vecN_2xf32 x0, x1;
  xb_vecN_2xf32 min0, min1, minx, z1;
  vboolN_2 b0, b1;
  vboolN bx0, bx1, bxx;
  ...
  if (N <= 0) { m[0] = m[1] = plusInff.f; idx[0] = idx[1] = 0; return; }
  id = BBE_SEQNX16();
  id = BBE_SHFLNX16I(id, BBE_SHFLI_MMC4X4X4X4_M2_STEP_1);
  _8 = 8; _8 = BBE_REPNX16(_8,0);
  id0 = id1 = 0;
  min0 = plusInff.f; min1 = plusInff.f;
  zm = (N - 1);              //  replicate (N-1)
  for (n = 0; n<(N >> (LOG2_BBE_SIMD_WIDTH-1)); n++)
  {
  #if 0
    BBE_LVN_2XF32_IP(x0, pX, 2 * BBE_SIMD_WIDTH);
    BBE_LVN_2XF32_IP(y0, pX, 2 * BBE_SIMD_WIDTH);
    b0 = BBE_OLTN_2XF32(x0, min0);
    bz = BBE_OLTN_2XF32(y0, tt0);
    tmp = BBE_MOVNX16T(BBE_MOVNX16_FROMN_2XF32(min0),BBE_MOVNX16_FROMN_2XF32(x0), BBE_MOVN_FROMN_2(b0));
    x1 = BBE_MOVN_2XF32_FROMNX16(tmp);
    tmp = BBE_MOVNX16T(BBE_MOVNX16_FROMN_2XF32(tt0),BBE_MOVNX16_FROMN_2XF32(y0), BBE_MOVN_FROMN_2(bz));
    y1 = BBE_MOVN_2XF32_FROMNX16(tmp);
    minx = BBE_MINN_2XF32(x0, min0);
    minx_ = BBE_MINN_2XF32(y0, tt0);

    bx0 = BBE_JOINB(b0, bz);
    tmp = BBE_MOVNX16T(id0, id, (bx0));
    id0 = BBE_MOVNX16T(id, id0, (bx0));

    b1 = BBE_OLTN_2XF32(x1, min1);
    bz = BBE_OLTN_2XF32(y1, tt1);
    min1 = BBE_MINN_2XF32(x1, min1);
    tt1 = BBE_MINN_2XF32(y1, tt1);
    bx1 = BBE_JOINB(b1, bz);
    ...
    bx0 = BBE_JOINB(b0, b0);
    //BBE_MOVIDXNX16F(tmp, id0, idn0, bx0);
    //BBE_MOVIDXNX16T(id0, id0, idn1, bx0);
    tmp = BBE_MOVNX16T(id0, id, (bx0));
    id0 = BBE_MOVNX16T(id, id0, (bx0));
  ...
  BBE_RBMINNUMN_2XF32(b0, a, min0); /* first minimum value */
  x0 = a;
  b0 = BBE_OEQN_2XF32(x0, min0);
  bx0 = BBE_JOINB(b0, b0);
  ...
  z1 = plusInff.f;
  tmp = BBE_MOVNX16T(BBE_MOVNX16_FROMN_2XF32(z1), BBE_MOVNX16_FROMN_2XF32(min0), BBE_MOVN_FROMN_2(b1));
  x1 = BBE_MOVN_2XF32_FROMNX16(tmp);
  b1 = BBE_OEQN_2XF32(x1, min1);            // flags: "min values is equivalent"
  ...
  min1 = BBE_MINN_2XF32(x1, min1);
  b = BBE_RMINNUMN_2XF32(min1); //second maximum value
  x1 = (b);
  b0 = BBE_OEQN_2XF32(x1, min1);
  bx0 = BBE_JOINB(b0, b0);
