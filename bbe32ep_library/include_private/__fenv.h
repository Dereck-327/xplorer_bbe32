...
/* Update floating-point exception enable bits: an exception is enabled iff it is both 1) enabled in scf and 2) indicated by excepts. */
#define XTENSA_FE_SET_EXCEPTION_ENABLE(scf, excepts) ( (scf) & (xb_int32v)( ( (excepts) << XTENSA_FE_EXCEPTION_ENABLE_OFFSET ) | ~XTENSA_FE_EXCEPTION_ENABLE_MASK ) )
...
inline_ int __feraiseexcept( int excepts )
{
  ...
  if ( 0 != ( excepts & ( FE_UNDERFLOW | FE_INEXACT | FE_INVALID | FE_OVERFLOW | FE_DIVBYZERO ) ) )
  {
    ...
    xtfloat unfl_tiny  = XT_CONST_S(1); XT_MOVT_S ( unfl_tiny, BBE_MOVF32_FROM32( 1 ), b_unfl ); //!!!!!!!!!!!not tested
    //!!!!!!!!!!!not tested
    xtfloat ovfl_bigf  = XT_CONST_S(1); XT_MOVT_S ( ovfl_bigf, BBE_MOVF32_FROM32( 0x7f7f7f7f ), b_ovfl ); /* 0x7f7f7f7f, 3.4e38f */
    int     inex_bigi  = XT_MOVI(1);    XT_MOVT   ( inex_bigi, 0x7f7f7f7f                                  , b_inex ); /* 0x7f7f7f7f */
    xtfloat inex_dum;
    xb_vecN_2x32v inex_bigvi = BBE_MOVN_2X32_FROM32(inex_bigi);
    ...
    __asm__ volatile ( "bbe_floatn_2x32 %0, %1, 0"      : "=v" (inex_dum) : "v" (inex_bigvi) );    /* (float)0x7f7f7f7f -> "inexact"         */
    __Pragma( "no_reorder" );
  }
#else
  unsigned long fsr;
...
/* Update floating-point exception enable bits: an exception is enabled iff it is both 1) enabled in scf and 2) indicated by excepts. */
#define XTENSA_FE_SET_EXCEPTION_ENABLE(scf, excepts) ( (scf) & (xb_int32v)( ( FENV_TO_XTENSA_MAP(excepts) << XTENSA_FE_EXCEPTION_ENABLE_OFFSET ) | ~XTENSA_FE_EXCEPTION_ENABLE_MASK ) )
