...
#define PACK1A(C,c_align,nbytes,y,pos,rsh)  \
{                                           \
    xb_vecNx16 one,y0;                      \
    one = BBE_MOVVA16(1);                   \
...
void lteprs_gen1 ( uint32_t * r, uint16_t * restrict c, int N )
{
    ...
    for (i = 0; i<(N >> 5); ++i)
    {
        ...
        c0 = BBE_BMUL32A(c0, 0);
        y = BBE_XORNX16(c0, c1);
        PACK1A(C, c_align, 32, y, 12, rsh);
        PACK1A(C, c_align, 32, y, 13, rsh);
        BBE_MOVBMULSTATEV(p11, p10, 0);
        BBE_MOVBMULSTATEV(p13, p12, 1);
        _c1 = BBE_EXTRNX16C(c1, BBE_SIMD_WIDTH / 2 - 1);
        BBE_MOVBMULACCA(_c1);
    ...
    N &= 31;
    {
        ...
        y = BBE_XORNX16(c0, c1);
        i = XT_MIN(M, 32); M -= i;
        PACK1A(C, c_align, i, y, 12, rsh);
        PACK1A(C, c_align, M, y, 13, rsh);
    }
