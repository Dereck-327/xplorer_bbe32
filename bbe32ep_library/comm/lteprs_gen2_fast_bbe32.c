...
#define PACK1(C,y,pos,rsh,intlv) \
{                                           \
    xb_vecNx16 three,y0;                    \
    three = BBE_MOVVA16(3);                 \
...
// pack 16x1-bit from pos@y to C: second variant!
#define PACK2(C,y,pos,rsh,intlv) \
{                                           \
    xb_vecNx40 threew,yw,y0w;               \
    xb_vecNx16 y0;                          \
    threew= BBE_MOVWA32(3);                 \
    yw  = BBE_UNPKUNX16(y);                 \
    y0w= BBE_REPNX40C(yw,pos);              \
    y0w= BBE_SRLNX40(y0w,rsh);              \
    y0w= BBE_ANDNX40 (y0w,threew);          \
...
void lteprs_gen2_fast ( uint32_t * r, uint16_t * restrict c, int N )
{
    ...
    BBE_MOVBMULACCA(_c1);
    N >>= 4;
    //Generates by 128 bits portions
    for (i = 0; i<(N >> 2); ++i)
    {
        BMUL32N(c1, 4);
        ...
        BMUL32N(c0, 4);
        y = BBE_XORNX16(c0, c1);
        PACK1(C, y, 3, rsh, intlv);
        PACK2(C, y, 4, rsh, intlv);
        PACK1(C, y, 5, rsh, intlv);
        PACK2(C, y, 6, rsh, intlv);
        BBE_MOVBMULSTATEV(p11, p10, 0);
        BBE_MOVBMULSTATEV(p13, p12, 1);
        _c1 = BBE_EXTRNX16C(c1, BBE_SIMD_WIDTH / 2 - 1);
        BBE_MOVBMULACCA(_c1);
