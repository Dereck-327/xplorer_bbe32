...
void lteprs_scramble6_fast ( uint32_t * r, uint16_t * restrict c, const uint16_t * b, int N )
{
    ...
    y0 = BBE_LVNX16_I((const xb_vecNx16*)lteprs6_tbl, 0 * 2 * BBE_SIMD_WIDTH);  sel0 = BBE_MOVVSELNX16(y0, 0);
    y0 = BBE_LVNX16_I((const xb_vecNx16*)lteprs6_tbl, 1 * 2 * BBE_SIMD_WIDTH);  sel1 = BBE_MOVVSELNX16(y0, 0);
    y0 = BBE_LVNX16_I((const xb_vecNx16*)lteprs6_tbl, 2 * 2 * BBE_SIMD_WIDTH);
    ...
    lteprs_scramble6(r, c, b, N);
#endif
} /* lteprs_scramble6_fast() */
