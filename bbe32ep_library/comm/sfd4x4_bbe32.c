        ...
        int N, int rsh )
{
    ...
    xb_vecNx16 *  restrict S2 = (xb_vecNx16  *)s2;
    xb_vecNx16 *  restrict S3 = (xb_vecNx16  *)s3;
    ...
    xb_vecNx16 *  restrict R4 = (xb_vecNx16  *)r4;
    xb_vecNx16 *  restrict R5 = (xb_vecNx16  *)r5;
    xb_vecNx16 *  restrict R6 = (xb_vecNx16  *)r6;
    xb_vecNx16 *  restrict R7 = (xb_vecNx16  *)r7;
    xb_vecNx16 *  restrict H0 = (xb_vecNx16  *)h;
    xb_vecNx16    y0, y1, x0, x1, x2, x3, h0, h1, h2, h3;
    xb_vecNx16    y2, y3, x4, x5, x6, x7, h4, h5, h6, h7;
    vsaN    shv;
    xb_vecNx40  A10, A32;
    xb_vecNx40  A54, A76;
    /*
    ...
    */
    ...
    NASSERT_ALIGN(s2, 2 * BBE_SIMD_WIDTH);
    NASSERT_ALIGN(s3, 2 * BBE_SIMD_WIDTH);
    ...
    NASSERT_ALIGN(r4, 2 * BBE_SIMD_WIDTH);
    NASSERT_ALIGN(r5, 2 * BBE_SIMD_WIDTH);
    NASSERT_ALIGN(r6, 2 * BBE_SIMD_WIDTH);
    NASSERT_ALIGN(r7, 2 * BBE_SIMD_WIDTH);
    ...
    for (n = 0; n<N / (BBE_SIMD_WIDTH / 2); n++)
    {
        BBE_LVNX16_IP(x4, R4, 2 * BBE_SIMD_WIDTH);
        BBE_LVNX16_IP(x5, R5, 2 * BBE_SIMD_WIDTH);
        BBE_LVNX16_IP(x6, R6, 2 * BBE_SIMD_WIDTH);
        BBE_LVNX16_IP(x7, R7, 2 * BBE_SIMD_WIDTH);
        BBE_LVNX16_XP(h4, H0, 4 * N);
        BBE_LVNX16_XP(h5, H0, 4 * N);
        BBE_LVNX16_XP(h6, H0, 4 * N);
        BBE_LVNX16_XP(h7, H0, 2 * (-6 * N + BBE_SIMD_WIDTH));

        A54 = BBE_MULRNX16J(x4, h4, shv);
        BBE_MULANX16J(A54, h5, x5);
        BBE_MULANX16J(A54, x6, h6);
        BBE_MULANX16J(A54, h7, x7);

        A76 = BBE_MULRNX16J(x4, h5, shv);
        BBE_MULSNX16J(A76, h4, x5);
        BBE_MULANX16J(A76, x6, h7);
        BBE_MULSNX16J(A76, h6, x7);

        y2 = BBE_PACKVNX40(A54, shv);
        y3 = BBE_PACKVNX40(A76, shv);
        BBE_SVNX16_IP(y2, S2, 2 * BBE_SIMD_WIDTH);
        BBE_SVNX16_IP(y3, S3, 2 * BBE_SIMD_WIDTH);
    }
} /* sfd4x4() */
