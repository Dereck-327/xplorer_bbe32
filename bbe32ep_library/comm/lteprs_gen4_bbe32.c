...
void lteprs_gen4 ( uint32_t * r, uint16_t * restrict c, int N )
{
    ...
    vselN         sel0, sel1;
    vsaN          shft;
    valign        r_align, c_align;
    xb_vecNx40    w, wmask;
    xb_vecNx16 y0, mask = BBE_MOVVA16(0x0F);
    const xb_vecNx16   * restrict P = (const xb_vecNx16 *)lteprs_poly;

    if (N < 0) return;

    c_align = BBE_ZALIGN();
    y = BBE_LVNX16_I((const xb_vecNx16*)lteprs4_tbl, 0 * BBE_SIMD_WIDTH);
    sel1 = sel0 = BBE_MOVVSELNX16(y, 0);
    BBE_SELUNX16(y, c0, c0, sel1, -4);
    y = BBE_LVNX16_I((const xb_vecNx16*)lteprs4_tbl, 2 * BBE_SIMD_WIDTH);
    shft = BBE_MOVVSV(y, 0);
    wmask = BBE_MOVWA32(0x0F); wmask = BBE_SLLNX40(wmask, shft);
    ...
    for (i = 0; i<(N >> 5); ++i)
    {
        ...
        BMUL32N(c0, 4);
        y = BBE_XORNX16(c0, c1);
        y0 = BBE_SELNX16(y, y, sel1);
        //        y0 = BBE_SRLNX16 (y0, shft );
        //        y0 = BBE_ANDNX16 (y0,mask);
        {
            w = BBE_UNPKSNX16(y0);
            w = BBE_ANDNX40(w, wmask);
            y0 = BBE_PACKVNX40(w, shft);
        }
    ...
    for (i = 0; i<N; ++i)
    {
        c[i] = (uint16_t)((rx1 & 15) ^ (rx2 & 15));
        rx1 = (rx1 >> 4) | ((((rx1 >> 3) & 30) ^ (rx1 & 30)) << 27);
        rx2 = (rx2 >> 4) | ((((rx2 >> 3) & 30) ^ ((rx2 >> 2) & 30) ^ ((rx2 >> 1) & 30) ^ (rx2 & 30)) << 27);
    }
