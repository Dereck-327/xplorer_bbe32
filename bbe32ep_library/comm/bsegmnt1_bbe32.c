...
void bsegmnt1 ( int16_t * restrict s, const uint8_t* b, int N )
{
    ...
    static const uint16_t ALIGN(32) rsh[16] = { 0x8000, 0x4000, 0x2000, 0x1000, 0x800, 0x400, 0x200, 0x100, 0x80, 0x40, 0x20, 0x10, 0x8, 0x4, 0x2, 0x1 };
    int i;
    xb_vecNx16  b0, r0, z0;
    valign v;
    ...
    z0 = BBE_LVNX16_I((const xb_vecNx16 *)rsh, 0);
    v = BBE_LA_PP(B);

    for (i = 0; i < (N >> LOG2_BBE_SIMD_WIDTH); ++i)
    {
        BBE_LAVNX16_XP(b0, v, B, 2);
        b0 = BBE_REPNX16(b0, 0);
        b0 = BBE_MULNX16PACKL(b0, z0);
        r0 = BBE_SRLINX16(b0, 15);
