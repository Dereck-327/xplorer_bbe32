                     ...
                     int N )
{
    ...
    for (n = 0; n<N / (BBE_SIMD_WIDTH / 2); n++)
    {
        ...
        /* Phase 0, demap lower BBE_SIMD_WIDTH/4 complex inputs to BBE_SIMD_WIDTH soft bits */
        y0 = BBE_SDMAP16QAMNX16C(scl, x, 0, 1, 1);
        /* Phase 1, demap higher BBE_SIMD_WIDTH/4 complex inputs to BBE_SIMD_WIDTH soft bits */
        y1 = BBE_SDMAP16QAMNX16C(scl, x, 1, 1, 1);
        /* Convert signed 8-bit values to 16-bit. */
        y0 = BBE_UNPKSNX2X8L(y0);
        y1 = BBE_UNPKSNX2X8L(y1);
        /* Save 2*BBE_SIMD_WIDTH soft bits */
        BBE_SVNX16_IP(y0, L, BBE_SIMD_WIDTH * 2);
        BBE_SVNX16_IP(y1, L, BBE_SIMD_WIDTH * 2);
    }
#else
          xb_vecNx16 * restrict L = (      xb_vecNx16 *)llr;
    const xb_vecNx16 * restrict S = (const xb_vecNx16 *)s;
    int n;
    xb_vecNx16 _1_256, x0, l1, l0;
    xb_vecNx16 _2Q10, _min, _max;
    ...
    for (n = 0; n < N / (BBE_SIMD_WIDTH / 2); n++)
    {
        ...
        l1 = BBE_ABSSNX16(x0);
        l1 = BBE_SUBSNX16(_2Q10, l1);
        BBE_DSELNX16I(l1, l0, l1, x0, BBE_DSELI_INTERLEAVE_2);
