...
inline_ int blk_last_stage_iDFT4_FP(complex_float *x, complex_float *y, int N, int L)
{
    ...
    for (i = 0; i<count / 2; i++)
    {
        /* pointer to the element x1 */
        p_src = (xb_vecN_4xcf32*)(x + 4 * i + stride/sizeof(*x));
        /* pointer to the element x1 */
        p_src2 = (xb_vecN_4xcf32*)(x + 4 * (i + count / 2) + stride / sizeof(*x));

        p_dst = (xb_vecN_4xcf32*)(y + 4 * i);
        p_dst2 = (xb_vecN_4xcf32*)(y + 4 * (i + count / 2));
        /*       #<swps>  11 cycles per pipeline stage in steady state with unroll=1
                #<swps>   3 pipeline stages
                #<swps>  36 real ops (excluding nop)
                #<swps> 
                #<swps>           10 cycles lower bound required by resources
                #<swps>      min   6 cycles required by recurrences
                #<swps>      min  10 cycles required by resources/recurrence
                #<swps>      min  22 cycles required for critical path
                #<swps>           28 cycles non-loop schedule length

                #<swps>    register file usage:
                #<swps>      'a' total 5 out of 16 [2-5,11]
                #<swps>      'v' total 16 out of 16 [0-15]
        */
        for (l = 0; l < L; l++)
        {
            ...
            */
            ...
            BBE_LVN_4XCF32_XP(_x3, p_src, -3 * stride               );
            BBE_LVN_4XCF32_XP(_x0, p_src, 2*stride                  );
            BBE_LVN_4XCF32_XP(_x2, p_src, -stride + N*sizeof(*x)     );
            ...
            BBE_LVN_4XCF32_XP(_x1, p_src2, 2 * stride);
            BBE_LVN_4XCF32_XP(_x3, p_src2, -3 * stride);
            BBE_LVN_4XCF32_XP(_x0, p_src2, 2 * stride);
            BBE_LVN_4XCF32_XP(_x2, p_src2, -stride + N*sizeof(*x));
...
-------------------------------------------------------------------------*/
#if !XCHAL_HAVE_BBEN_VECTORFFT || !HAVE_VFPU
DISCARD_FUN(void, bcifft64f, (complex_float * restrict y, complex_float * restrict x, int L) )
#else
void bcifft64f(complex_float * restrict y, complex_float * restrict x, int L)
{
    ...
    blk_last_stage_iDFT4_FP(x, y, N, L);
}
#endif //#if !XCHAL_HAVE_BBEN_VECTORFFT || ! HAVE_VFPU
