...
#if !(HAVE_FFT && 1)
DISCARD_FUN(int, tifft32768, (complex_fract16 * restrict y, complex_fract16 * restrict x, int bexp))
#else
/*-------------------------------------------------------------------------
Radix-2 inverse FFT on complex data with reduced twiddle table, auto scaling
...
int tifft32768 ( complex_fract16 * restrict y, complex_fract16 * restrict x, int bexp )
{
  ...
  scaling = R1_tDFT4_L64_16(tfft32768_tw1, (int16_t*)x, (int16_t*)y, 32768, bexp);
  scaling += R2_tDFT4xI4_U2(tfft32768_tw2, (int16_t*)y, (int16_t*)x, 32768, BBE_RRANGE());
  scaling += R3_DFT4xIv(tfft32768_tw3, (int16_t*)x, (int16_t*)y, 32768, 16, BBE_RRANGE());
  scaling += R3_DFT4xIv(tfft32768_tw4, (int16_t*)y, (int16_t*)x, 32768, 64, BBE_RRANGE());
  scaling += R3_DFT4xIv_2U(tfft32768_tw5, (int16_t*)x, (int16_t*)y, 32768, 256, BBE_RRANGE());
  scaling += R3_DFT4xIv_2U(tfft32768_tw6, (int16_t*)y, (int16_t*)x, 32768, 1024, BBE_RRANGE());
  {
    // Clip scaling to (1+log2(N)) 
    int total_scaling;
    bexp = BBE_RRANGE();
    total_scaling = (4 - bexp + scaling);
    bexp += (total_scaling > 16) ? total_scaling - 16 : 0;
  }
  scaling += R2_DFT8xIN_8((int16_t*)x, (int16_t*)y, 32768, bexp);
