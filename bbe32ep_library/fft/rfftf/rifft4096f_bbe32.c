...
DISCARD_FUN(int, rifft4096f, (float32_t * restrict y, complex_float * restrict x) )
#else
int rifft4096f(float32_t * restrict y, complex_float * restrict x)
{
    const complex_float *twiddle_table = (const complex_float*)rfftf4096_twd1;
    ...
    rifft_spec_conv_fp(x, twiddle_table, 4096);
    cifft2048f((complex_float*)y, x);
    return 0;
}
