...
DISCARD_FUN(int, rifft8192f, (float32_t * restrict y, complex_float * restrict x) )
#else
int rifft8192f(float32_t * restrict y, complex_float * restrict x)
{
    const complex_float *twiddle_table = (const complex_float*)rfftf8192_twd1;

    NASSERT((void*)x != y);
    NASSERT_ALIGN(x, BBE_SIMD_WIDTH * 2);
    NASSERT_ALIGN(y, BBE_SIMD_WIDTH * 2);

    rifft_spec_conv_fp(x, twiddle_table, 8192);
    cifft4096f((complex_float*)y, x);
    return 0;
}
