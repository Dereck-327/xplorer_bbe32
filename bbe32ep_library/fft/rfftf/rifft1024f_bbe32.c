...
DISCARD_FUN(int, rifft1024f, (float32_t * restrict y, complex_float * restrict x   ) )
#else
int rifft1024f(float32_t * restrict y, complex_float * restrict x   )
{
    const complex_float *twiddle_table = (const complex_float*)rfftf1024_twd1;
    NASSERT((void*)x != y);
    NASSERT_ALIGN(x, BBE_SIMD_WIDTH * 2);
    NASSERT_ALIGN(y, BBE_SIMD_WIDTH * 2);
    NASSERT_ALIGN(twiddle_table, BBE_SIMD_WIDTH * 2);

    rifft_spec_conv_fp(x, twiddle_table, 1024);
    cifft512f((complex_float*)y, x);
    return 0;
}
