...
DISCARD_FUN(int, rifft2048f, (float32_t * restrict y, complex_float * restrict x   ) )
#else
int rifft2048f(float32_t * restrict y, complex_float * restrict x   )
{
    const complex_float *twiddle_table = (const complex_float*)rfftf2048_twd1;
    ...
    rifft_spec_conv_fp(x, twiddle_table, 2048);
    cifft1024f((complex_float*)y, x);
    return 0;
}
