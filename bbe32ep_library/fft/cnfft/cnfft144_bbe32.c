...
#if !(HAVE_FFT && 1)
DISCARD_FUN(int, cnfft144, (void * restrict pScr,complex_fract16 * restrict y, complex_fract16 * restrict x, int bexp))
#else
/*-------------------------------------------------------------------------
Mixed radix forward FFT on complex data, auto scaling
...
int cnfft144 (void * restrict pScr,complex_fract16 * restrict y, complex_fract16 * restrict x, int bexp)
{
  ...
  scaling += R2_DFT3xIN_3((int16_t*)y, (int16_t*)y, 144, BBE_RRANGE());
  return scaling;
} /* cnfft144() */
#endif
