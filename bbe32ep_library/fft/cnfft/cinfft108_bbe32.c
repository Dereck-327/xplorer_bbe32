...
#if !(HAVE_FFT && 1)
DISCARD_FUN(int, cinfft108, (void * restrict pScr,complex_fract16 * restrict y, complex_fract16 * restrict x, int bexp))
#else
/*-------------------------------------------------------------------------
Mixed radix inverse FFT on complex data, auto scaling
...
int cinfft108 (void * restrict pScr,complex_fract16 * restrict y, complex_fract16 * restrict x, int bexp)
{
  ...
  scaling = R1_DFT4_L64_16_N4(fft108_tw1, (int16_t*)x, (int16_t*)pScr, 108, bexp);
  scaling += R2_DFT3xI4_expand(fft108_tw2, (int16_t*)pScr, (int16_t*)y, 108, BBE_RRANGE());
  scaling += R2_DFT9xIN_9((int16_t*)y, (int16_t*)x, 108 * 4 / 3, BBE_RRANGE());

  {
    ...
    valign v = BBE_ZALIGN();
    int i;
    int count = 108 * 4 / 3 / 16;
    for (i = 0; i<count; i++)
    {
      xb_vecNx16 tmp;
      BBE_LVNX16_IP(tmp, psrc, sizeof(*psrc));
      BBE_SAVNX16_XP(tmp, v, pdst, 32);
      BBE_LVNX16_IP(tmp, psrc, sizeof(*psrc));
      BBE_SAVNX16_XP(tmp, v, pdst, 16);
    }
    BBE_SAVNX16POS_FP(v, pdst);
  }
