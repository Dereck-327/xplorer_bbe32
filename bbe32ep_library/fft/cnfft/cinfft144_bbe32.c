...
#if !(HAVE_FFT && 1)
DISCARD_FUN(int, cinfft144, (void * restrict pScr,complex_fract16 * restrict y, complex_fract16 * restrict x, int bexp))
#else
/*-------------------------------------------------------------------------
Mixed radix inverse FFT on complex data, auto scaling
...
int cinfft144 (void * restrict pScr,complex_fract16 * restrict y, complex_fract16 * restrict x, int bexp)
{
  ...
  scaling = R1_DFT4_L64_16_N16(fft144_tw1, (int16_t*)x, (int16_t*)y, 144, bexp);
  scaling += R2_DFT4xI4_N16(fft144_tw2, (int16_t*)y, (int16_t*)x, 144, BBE_RRANGE());
  scaling += R3_DFT3xIv(fft144_tw3, (int16_t*)x, (int16_t*)y, 144, 16, BBE_RRANGE());
  scaling += R2_DFT3xIN_3((int16_t*)y, (int16_t*)x, 144, BBE_RRANGE());
  {
    xb_vecNx16 *psrc = (xb_vecNx16*)x;
    xb_vecNx16 *pdst = (xb_vecNx16*)y;
    int i;
    for (i = 0; i<((144 + 7)& (~7)); i += 8)
    {
      xb_vecNx16 tmp;
      BBE_LVNX16_IP(tmp, psrc, sizeof(*psrc));
      BBE_SVNX16_IP(tmp, pdst, sizeof(*pdst));
    }
  }
  return scaling;
} /* cinfft144() */
