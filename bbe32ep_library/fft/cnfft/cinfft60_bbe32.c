...
#if !(HAVE_FFT && 1)
DISCARD_FUN(int, cinfft60, (void * restrict pScr,complex_fract16 * restrict y, complex_fract16 * restrict x, int bexp))
#else
/*-------------------------------------------------------------------------
Mixed radix inverse FFT on complex data, auto scaling
...
int cinfft60 (void * restrict pScr,complex_fract16 * restrict y, complex_fract16 * restrict x, int bexp)
{
  ...
  scaling = R1_DFT4_L64_16_N4(fft60_tw1, (int16_t*)x, (int16_t*)y, 60, bexp);

  scaling += R2_DFT3xI4_expand(fft60_tw2, (int16_t*)y, (int16_t*)x, 60, BBE_RRANGE());
  scaling += R2_DFT5xIN_5_compress((int16_t*)x, (int16_t*)y, 60 * 4 / 3, BBE_RRANGE());
  return scaling;
} /* cinfft60() */
#endif
