...
#if !(HAVE_FFT && 1)
DISCARD_FUN(int, cinfft192, (void * restrict pScr,complex_fract16 * restrict y, complex_fract16 * restrict x, int bexp))
#else
/*-------------------------------------------------------------------------
Mixed radix inverse FFT on complex data, auto scaling
...
int cinfft192 (void * restrict pScr,complex_fract16 * restrict y, complex_fract16 * restrict x, int bexp)
{
  ...
  scaling = R1_DFT4_L64_16(fft192_tw1, (int16_t*)x, (int16_t*)y, 192, bexp);
  scaling += R2_DFT6xI4(fft192_tw2, (int16_t*)y, (int16_t*)x, 192, BBE_RRANGE());
  scaling += R2_DFT8xIN_8((int16_t*)x, (int16_t*)y, 192, BBE_RRANGE());
  return scaling;
} /* cinfft192() */
#endif
