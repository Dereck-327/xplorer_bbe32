...
#if !(HAVE_FFT && 1)
DISCARD_FUN(int, rfft8192, (complex_fract16 * restrict y, int16_t * restrict x, int bexp))
#else
/*-------------------------------------------------------------------------
Radix-2 forward FFT on real data, auto scaling
...
int rfft8192 ( complex_fract16 * restrict y, int16_t * restrict x, int bexp )
{
  ...
  bexp = cfftas4096(y, (complex_fract16*)x, bexp);

  bexp += rfft_spec_conv((int16_t*)y, fft_tw_tab_rfft_8192, 8192, BBE_RRANGE());
