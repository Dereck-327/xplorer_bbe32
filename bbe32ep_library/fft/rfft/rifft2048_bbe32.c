...
#if !(HAVE_FFT && 1)
DISCARD_FUN(int, rifft2048, (int16_t * restrict y, complex_fract16 * restrict x, int bexp))
#else
/*-------------------------------------------------------------------------
Radix-2 inverse FFT forming real data, auto scaling
...
int rifft2048 ( int16_t * restrict y, complex_fract16 * restrict x, int bexp )
{
  ...
  shift = rifft_spec_conv((int16_t*)x, fft_tw_tab_rfft_2048, 2048, &bexp);

  shift += cifftas1024_norm((complex_fract16*)y, x);
