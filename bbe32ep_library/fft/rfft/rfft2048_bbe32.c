...
#if !(HAVE_FFT && 1)
DISCARD_FUN(int, rfft2048, (complex_fract16 * restrict y, int16_t * restrict x, int bexp))
#else
/*-------------------------------------------------------------------------
Radix-2 forward FFT on real data, auto scaling
...
int rfft2048 ( complex_fract16 * restrict y, int16_t * restrict x, int bexp )
{
  ...
  bexp = cfftas1024(y, (complex_fract16*)x, bexp);

  bexp += rfft_spec_conv((int16_t*)y, fft_tw_tab_rfft_2048, 2048, BBE_RRANGE());
