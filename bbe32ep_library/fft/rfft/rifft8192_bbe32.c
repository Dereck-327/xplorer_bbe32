...
#if !(HAVE_FFT && 1)
DISCARD_FUN(int, rifft8192, (int16_t * restrict y, complex_fract16 * restrict x, int bexp))
#else
/*-------------------------------------------------------------------------
Radix-2 inverse FFT forming real data, auto scaling
...
int rifft8192 ( int16_t * restrict y, complex_fract16 * restrict x, int bexp )
{
  ...
  shift = rifft_spec_conv((int16_t*)x, fft_tw_tab_rfft_8192, 8192, &bexp);

  shift += cifftas4096_norm((complex_fract16*)y, x);
