...
#if !(HAVE_FFT && 1)
DISCARD_FUN(int, rifft32768, (int16_t * restrict y, complex_fract16 * restrict x, int bexp))
#else
/*-------------------------------------------------------------------------
Radix-2 inverse FFT forming real data, auto scaling
...
int rifft32768 ( int16_t * restrict y, complex_fract16 * restrict x, int bexp )
{
  ...
  shift = rifft_spec_conv((int16_t*)x, fft_tw_tab_rfft_32768, 32768, &bexp);

  shift += cifftas16384_norm((complex_fract16*)y, x);
