...
DISCARD_FUN(void, brfft32f, (complex_float * restrict y, float32_t * restrict x, int L) )
#else
void brfft32f(complex_float * restrict y, float32_t * restrict x, int L)
{

    complex_float *twiddle_table = (complex_float*)rfftf32_twd1;
    NASSERT_ALIGN(x, 2 * BBE_SIMD_WIDTH);
    NASSERT_ALIGN(y, 2 * BBE_SIMD_WIDTH);
    NASSERT((uintptr_t)x != (uintptr_t)y);
    NASSERT(L>0);

    bcfft16f(y, (complex_float*)x, L); 
    blkrfft_spec_conv_packed_fp(y, twiddle_table, 32, L);
}
