...
-------------------------------------------------------------------------*/
#if !(HAVE_FFT && 1)
DISCARD_FUN(int, cfftas16,  (complex_fract16 * restrict y, complex_fract16 * restrict x, int bexp))
#else
inline_ ATTRIBUTE_ALWAYS_INLINE int cfft16as_loc(int16_t * y,
  int16_t * x,
  int * bexp)
{
  xb_vecNx16 * Y;
...
int cfftas16 ( complex_fract16 * restrict y, complex_fract16 * restrict x, int bexp )
{
  ...
  return cfft16as_loc((int16_t*)y, (int16_t*)x, &bexp);
} /* cfftas16() */
#endif
