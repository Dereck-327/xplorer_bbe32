...
-------------------------------------------------------------------------*/
#if !(HAVE_FFT && 1)
DISCARD_FUN(int, cifftas16, ( complex_fract16 * restrict y, complex_fract16 * restrict x, int bexp ))
#else
inline_ ATTRIBUTE_ALWAYS_INLINE int cifftas16_loc(int16_t * y,
  int16_t * x,
  int * bexp)
{
  xb_vecNx16 * Y;
...
int cifftas16 ( complex_fract16 * restrict y, complex_fract16 * restrict x, int bexp )
{
  ...
  return cifftas16_loc((int16_t*)y, (int16_t*)x, &bexp);
} /* cifftas16() */
#endif 
