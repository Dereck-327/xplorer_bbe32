...
#if !(HAVE_FFT && 1)
DISCARD_FUN(int, cifftas16_norm, (complex_fract16 * restrict y, complex_fract16 * restrict x))
#else
/*-------------------------------------------------------------------------
Radix-2 inverse FFT on complex data, auto scaling
...
-------------------------------------------------------------------------*/
inline_ ATTRIBUTE_ALWAYS_INLINE int cifftas16_norm_loc( int16_t * y,
                                                   int16_t * x)
{
        xb_vecNx16 * Y;
  const xb_vecNx16 * X;
...

  {
    ...
    p0 = BBE_MULNX16JPACKQ( b0, tw0 );
    p1 = BBE_MULNX16JPACKQ( b1, tw1 );
  }
...
int cifftas16_norm ( complex_fract16 * restrict y, complex_fract16 * restrict x           )
{
  ...
  return cifftas16_norm_loc((int16_t*)y, (int16_t*)x);
} /* cifftas16_norm() */
#endif
