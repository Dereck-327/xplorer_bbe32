...
#if !(HAVE_FFT && 1)
DISCARD_FUN(int, cfftas32, (complex_fract16 * restrict y, complex_fract16 * restrict x, int bexp))
#else
inline_ ATTRIBUTE_ALWAYS_INLINE int cfftas32_loc(int16_t * y,
  const int16_t * x,
  int * bexp)
{
  //
...
int cfftas32 ( complex_fract16 * restrict y, complex_fract16 * restrict x, int bexp )
{
  NASSERT_ALIGN32(x);
  NASSERT_ALIGN32(y);
  return cfftas32_loc((int16_t*)y, (int16_t*)x, &bexp);
} /* cfftas32() */
#endif
