...
#if !(HAVE_FFT && 1)
DISCARD_FUN(int, cifftas32_norm, (complex_fract16 * restrict y, complex_fract16 * restrict x))
#else
/*-------------------------------------------------------------------------
Radix-2 inverse FFT on complex data, auto scaling
...
-------------------------------------------------------------------------*/
inline_ ATTRIBUTE_ALWAYS_INLINE int cifftas32_norm_loc( int16_t * y,
                                                  const int16_t * x
                                                  )
{
  //
...
int cifftas32_norm ( complex_fract16 * restrict y, complex_fract16 * restrict x           )
{
  ...
  return cifftas32_norm_loc((int16_t*)y, (int16_t*)x);
} /* cifftas32_norm() */
#endif
