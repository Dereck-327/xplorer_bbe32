...
#if !(HAVE_FFT && 1)
DISCARD_FUN(int, cfftas32_norm, (complex_fract16  * restrict y, complex_fract16 * restrict x))
#else
/*-------------------------------------------------------------------------
Radix-2 forward FFT on complex data, auto scaling
...
-------------------------------------------------------------------------*/
inline_ ATTRIBUTE_ALWAYS_INLINE int cfftas32_norm_loc( int16_t * y,
                                            const int16_t * x)
{
  //
  // DFT32 decomposition:
...
int cfftas32_norm ( complex_fract16 * restrict y, complex_fract16 * restrict x           )
{
  NASSERT_ALIGN32(x);
  NASSERT_ALIGN32(y);
  return cfftas32_norm_loc((int16_t*)y, (int16_t*)x);
} /* cfftas32_norm() */
#endif
