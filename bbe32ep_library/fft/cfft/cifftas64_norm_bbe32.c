...
#if !(HAVE_FFT && 1)
DISCARD_FUN(int, cifftas64_norm, (complex_fract16 * restrict y, complex_fract16 * restrict x))
#else
/*-------------------------------------------------------------------------
Radix-2 inverse FFT on complex data, auto scaling
...
int cifftas64_norm ( complex_fract16 * restrict y, complex_fract16 * restrict x           )
{
  ...
  NASSERT_ALIGN32(y);
  NASSERT_ALIGN32(x);
  scaling = R1_DFT4_L64_16_norm(fft64_tw1, (int16_t*)x, (int16_t*)y, 64);
  scaling += R2_DFT4xI4(fft64_tw2, (int16_t*)y, (int16_t*)x, 64, BBE_RRANGE());
  scaling += R2_DFT4xIN_4((int16_t*)x, (int16_t*)y, 64, BBE_RRANGE());
  return scaling;
} /* cifftas64_norm() */
#endif
