...
#if !(HAVE_FFT && 1)
DISCARD_FUN(int, cifftas128, ( complex_fract16 * restrict y, complex_fract16 * restrict x, int bexp ))
#else
/*-------------------------------------------------------------------------
Radix-2 inverse FFT on complex data, auto scaling
...
int cifftas128 ( complex_fract16 * restrict y, complex_fract16 * restrict x, int bexp )
{
  ...
  NASSERT_ALIGN32(y);
  NASSERT_ALIGN32(x);
  scaling = R1_DFT4_L64_16(fft128_tw1, (int16_t*)x, (int16_t*)y, 128, bexp);
  scaling += R2_DFT4xI4_U2(fft128_tw2, (int16_t*)y, (int16_t*)x, 128, BBE_RRANGE());
  scaling += R2_DFT8xIN_8((int16_t*)x, (int16_t*)y, 128, BBE_RRANGE());
  return scaling;
} /* cifftas128() */
#endif
