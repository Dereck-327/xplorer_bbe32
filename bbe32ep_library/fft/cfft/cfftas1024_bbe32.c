...
#if !(HAVE_FFT && 1)
DISCARD_FUN(int, cfftas1024, (complex_fract16 * restrict y, complex_fract16 * restrict x, int bexp))
#else
/*-------------------------------------------------------------------------
Radix-2 forward FFT on complex data, auto scaling
...
int cfftas1024 ( complex_fract16 * restrict y, complex_fract16 * restrict x, int bexp )
{
  ...
  NASSERT_ALIGN32(x);
#if 0 
  scaling = R1_DFT4_L64_16_fs(fft1024_tw1, (int16_t*)x, (int16_t*)y, 1024, bexp);
#else
  scaling = R1_DFT4_L64_16(fft1024_tw1, (int16_t*)x, (int16_t*)y, 1024, bexp);
#endif

#if 1
  scaling += R2_DFT4xI4_U2(fft1024_tw2, (int16_t*)y, (int16_t*)x, 1024, BBE_RRANGE());
#else
  scaling += R2_DFT4xI4_wSEL(fft1024_tw2, (int16_t*)y, (int16_t*)x, 1024, BBE_RRANGE());
#endif

  scaling += R3_DFT4xIv(fft1024_tw3, (int16_t*)x, (int16_t*)y, 1024, 16, BBE_RRANGE());
#if 1
  scaling += R3_DFT4xIv(fft1024_tw4, (int16_t*)y, (int16_t*)x, 1024, 64, BBE_RRANGE());
#else
  scaling += R3_DFT4xIv(fft1024_tw4, (int16_t*)y, (int16_t*)x, 1024, 64, BBE_RRANGE());
#endif
  scaling += R2_DFT4xIN_4((int16_t*)x, (int16_t*)y, 1024, BBE_RRANGE());
  return scaling;
} /* cfftas1024() */
#endif
